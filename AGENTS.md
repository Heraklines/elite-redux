...
- The external gate is the default checkpoint gate: Lane A and Lane P each use one GitHub-hosted runner, Lane B uses eight historically weighted shards, and Lane C uses three shards. Each shard runs one Vitest controller with `--pool=forks --isolate --no-file-parallelism`: files remain sequential and isolated while the controller reuses Vite transforms. Do not restore one full Vitest CLI per file; green run `29177743451` measured 5.4-6.5 minute B shards because repeated startup consumed roughly 20-25 seconds per file, while an isolated-fork canary of the same shard took about 183 seconds. Keep the slowest shard near four minutes by rebalancing from green-run evidence.
