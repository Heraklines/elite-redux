...
SELECTED M4 ENCOUNTER POLICY (G12 correction): ordinary species-pool and low-level move/ability candidates from vanilla balance tables are not authoritative because production initialization overwrites movesets and active abilities. The wave-11 parity encounter is supported only as a complete post-initialization oracle-captured vector. Route generation may expose a biome, but no species, ability, moveset, IV/nature, level, or command is inferred from that biome alone.
