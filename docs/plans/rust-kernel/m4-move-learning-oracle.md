...
The Bulbasaur 16→17 lane above remains valid isolated oracle evidence but is not reachable in the default Classic wave-9 segment because its level cap is 10. The frozen composed parity segment therefore selects Squirtle `SpeciesId=7`, level 8→9, Medium Slow growth, and the post-initialization vanilla level table entry `[9, MoveId.RAPID_SPIN]` from `src/data/balance/pokemon-level-moves.ts:111-123`. `MoveId.RAPID_SPIN=229` is confirmed by `src/data/elite-redux/er-moves.ts` and the enum registry.

The exact full-moveset decision is `[33 TACKLE, 39 TAIL_WHIP, 55 WATER_GUN, 110 WITHDRAW]` plus candidate `229 RAPID_SPIN`; raw input replaces slot 0. Expected result: `[229,39,55,110]`. The exporter must still capture the exact UI graph, owner, operation identity, control/menu IDs, and canonical before/after state. No Bulbasaur level-17 claim is part of the wave-9-to-11 parity fixture.
