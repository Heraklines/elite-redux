...
Player party material lives in persistent `PlayerPokemon` objects stored by BattleScene. Shared core fields include numeric Pokémon id, species/form, level, EXP, HP/stats/IVs/nature, moveset, status, friendship, switch/field/transient state, and tera state (`Pokemon` around lines 341-480; `PlayerPokemon` at 8176; `EnemyPokemon` at 8912). The original extraction candidate used Bulbasaur 16→17, but that is unreachable under the default Classic wave-9 level cap and is not the composed parity contract. The frozen segment instead uses Squirtle species ID 7 crossing 8→9 and learning Rapid Spin move ID 229 from the post-initialization table.
...
- Progression: use an oracle-exported wave-9 source battle where Squirtle numeric species ID 7 crosses level 8 to 9 under the default Classic cap 10 and opens the move-learning child for Rapid Spin move ID 229. The full moveset is `[33,39,55,110]`; the raw-key choice replaces slot 0, producing `[229,39,55,110]`. Exact EXP, IVs, nature, stats, ownership, participation, and mutation evidence remain exporter-owned inputs.
