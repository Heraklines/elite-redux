...
All rows below refer to current source pin399d5d368f0b5642ebf8f45bd8a5e73350fa4de7 and the single current-phase integration branch. The remote command actually used is `python3 scripts/ci/m9e_current_phase_diagnostic.py`; it builds and executes exact whole targets. Additional named acceptance targets are required work, not claimed executions. Latest completed integrated candidate2f124b45/run35800914716 compiled and built the selected targets;20 whole targets passed144 tests, while phase2/4 and native host failed. Its 1780-second shared deadline ended after22 short targets, before either natural campaign target started. Clippy passed, JavaScript19/19 passed, and one1356-byte formatter page was required and has since been applied. Focused evidence on product-equivalent branches isolates the host and phase defects, and a separate source-equivalent run passed the complete natural co-op wave-200 test. The separate current-entry focused run35798529907 passed13/13 CLI entry, batch and native-capture witnesses on a harness-only child of6c871966. These are not aggregate M9 acceptance.

| Row | State | Actual evidence and required test/command | Next failing condition | Owner |
|---|---|---|---|---|
| A — shared current game/agent entry | PARTIAL_REMOTE_PASS | Focused run35798529907 passed all13 whole current CLI entry, batch and native-capture tests on a harness-only child of6c871966; current repro, worker/Wasm and same-candidate aggregate remain unverified. | Prove every normal entry and typed external event at the final product SHA without legacy fallback. | root |
| B — continuous natural solo and supported rules | FAILING | On2f, natural solo campaign1/1, defender2/2 and target11/11 passed; phase2/4 failed. Focused successor76d5024fa passed3/4 after repairing Title READ collision, then Flash advance rejected a successor material. Source reward rows remain selected-context evidence only. | Repair Flash material validation, source-owned damage/stat effects, full reward descendants and next encounter, then complete finite-run corpus and supported branch fixtures. | root |
| C — two-human co-op/liveness | PARTIAL_REMOTE_PASS | Source-equivalent harness-only branch2d86bea93/run35803370863 passed the complete natural independent-seat co-op wave-200 target1/1 in1404.6 test seconds at the ordinary default-stack profile. It remains unproven at the final product SHA with Worker/WebRTC. | Same-SHA natural startup and campaign, multiple retention windows and real Worker/WebRTC platform witness on final candidate. | root |
| D — persistence and causal replay | PARTIAL_REMOTE_PASS | Focused successor76d5024fa passed native browser host14/14 on the default stack after isolating capsule import. A frame-split replay successor bb73f2f6/run35804902669 passed the complete natural replay target1/1 on unchanged product source under the ordinary default stack; same-SHA integrated replay, CLI/Wasm eventwise equality and storage seams remain open. | Prove the promoted replay frame on final product SHA, then normal CLI/Wasm eventwise equality, storage uncertainty/conflict/retry and pending operation restoration. | root |
| E — current reload/performance/tools | IMPLEMENTED_UNVERIFIED | `m9e_current_reload`, `m9e_current_rulechange_reload`, and current cost probes exist but have no result on this candidate. Material retention3/3 and2/2 on086 are debug correctness results, not optimized benchmark acceptance. | Current native/browser worker rebuild and incompatibility evidence, quarantined effects, measured execution/hash/copy costs and optimized benchmarks. | root |
| F — browser semantics and integrated acceptance | IMPLEMENTED_UNVERIFIED | On2f, `python3 scripts/ci/m9e_current_phase_js_diagnostic.py` passed19 named checks, including human prompt lifetime and disposal. Focused successor76d5024fa passed native host14/14 after the import-stack repair; the full browser platform matrix remains unverified. | Actual browser matrix and keys/Worker/storage/network seams, complete control producer/continuation and semantic payload evidence, then all six rows at one SHA. | root |
