...
All rows below refer to current source pin399d5d368f0b5642ebf8f45bd8a5e73350fa4de7 and the single current-phase integration branch. The remote command actually used is `python3 scripts/ci/m9e_current_phase_diagnostic.py`; it builds and executes exact whole targets. Additional named acceptance targets are required work, not claimed executions. Latest completed candidate0e9dcc94f/run34705926187 passed compilation and both builds, plus15 whole targets/124 tests andJavaScript18. Phase0/4, target9/11, host stack, Clippy, formatting and replay deadline remain red. Priorb25471985/run34704877639 stopped at compilation;086613d4a/run34702704401 had target8/11 and the same15 whole-target/124 count. Local successors remain unqualified. Earlier b66337a5d/run34700330243 evidence is retained separately and does not qualify the successor.
...
| B — continuous natural solo and supported rules | FAILING | On0e9, defender2/2 passed; phase0/4 and target9/11 failed after bootstrap; Struggle now passes. Natural solo campaign never started. Actual command above retains the full campaign target. Source reward34700286214 and counter/score34700591441 are enabling observations only. | Source-owned damage/stat effects, full reward descendants and next encounter, then complete finite-run corpus and supported branch fixtures. | root; diagnostic_repair damage/stat; xp_repair reward/encounter |
| C — two-human co-op/liveness | IMPLEMENTED_UNVERIFIED | `m9e_coop_v7`4/4 on086 with default stack. `m9e_natural_coop_campaign_v7` never started. | Independent-seat natural startup, continuing shared material, multiple retention windows and real Worker/WebRTC platform witness on final candidate. | root |
| D — persistence and causal replay | FAILING | `m9e_natural_campaign_replay` interrupted555583ms on0e9 at1600 captured/replayed bootstrap events under the remaining shared budget; host transport still stack overflowed. Recording180737ms and roundtrip344326ms dominate; actual apply4890ms. Smaller setup frames and exact empty-checkpoint proof reuse await execution. | Complete contiguous campaign replay plus normal CLI/Wasm eventwise equality, storage uncertainty/conflict/retry and pending operation restoration. | root; stack_audit replay/host diagnostics |
...
| F — browser semantics and integrated acceptance | IMPLEMENTED_UNVERIFIED | `python3 scripts/ci/m9e_current_phase_js_diagnostic.py` passed18 whole-file checks on0e9; native host target compiled but its network/transport test stack overflowed. | Actual browser matrix and keys/Worker/storage/network seams, complete control producer/continuation and semantic payload evidence, then all six rows at one SHA. | root |
...
Full Town constructor source v7 onad5d633/run34708837073 measured a57350-byte complete species output against the unchanged32768-byte cap and failed without truncation. The source-only successor339ede32 packs repeated full rows and canonical digests losslessly, reconstructs and validates the complete graph, and is under remote diagnostic. It changes no compiled game bundle or runtime acceptance.
