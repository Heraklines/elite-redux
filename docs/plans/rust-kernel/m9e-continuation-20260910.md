...
All rows below refer to current source pin399d5d368f0b5642ebf8f45bd8a5e73350fa4de7 and the single current-phase integration branch. The remote command actually used is `python3 scripts/ci/m9e_current_phase_diagnostic.py`; it builds and executes exact whole targets. Additional named acceptance targets are required work, not claimed executions. Latest completed candidateb25471985/run34704877639 stopped at compilation before native tests; JavaScript18 passed. Prior086613d4a/run34702704401 passed compilation and15 whole targets/124 tests, but failed Clippy, three targets and the replay deadline. Local successors remain unqualified. Earlier b66337a5d/run34700330243 evidence is retained separately and does not qualify the successor.
...
| B — continuous natural solo and supported rules | FAILING | On086, defender2/2 passed; phase0/4 and target8/11 failed after bootstrap. Natural solo campaign never started. Actual command above retains the full campaign target. Source reward34700286214 and counter/score34700591441 are enabling observations only. | Source-owned damage/stat effects, full reward descendants and next encounter, then complete finite-run corpus and supported branch fixtures. | root; diagnostic_repair damage/stat; xp_repair reward/encounter |
...
| D — persistence and causal replay | FAILING | `m9e_natural_campaign_replay` interrupted534044ms on086 at1472 captured/replayed bootstrap events under the remaining shared budget; host transport still stack overflowed. Timing instrumentation and deeper frame split await execution. | Complete contiguous campaign replay plus normal CLI/Wasm eventwise equality, storage uncertainty/conflict/retry and pending operation restoration. | root; stack_audit replay/host diagnostics |
...
| F — browser semantics and integrated acceptance | IMPLEMENTED_UNVERIFIED | `python3 scripts/ci/m9e_current_phase_js_diagnostic.py` passed18 whole-file checks on086; native host target compiled but its network/transport test stack overflowed. | Actual browser matrix and keys/Worker/storage/network seams, complete control producer/continuation and semantic payload evidence, then all six rows at one SHA. | root |
