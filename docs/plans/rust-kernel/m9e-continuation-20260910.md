...
All rows below refer to current source pin399d5d368f0b5642ebf8f45bd8a5e73350fa4de7 and the single current-phase integration branch. The remote command actually used is `python3 scripts/ci/m9e_current_phase_diagnostic.py`; it builds and executes exact whole targets. Additional named acceptance targets are required work, not claimed executions. Latest completed candidate6c871966/run35797665319 compiled and built the selected targets;20 whole targets passed144 tests. Phase2/4 failed at the missing Title storage option, the native browser host overflowed inside capsule-import processing, and natural co-op was interrupted after404 seconds at the unchanged shared deadline before replay started. The current-target target passed11/11. JavaScript19/19 passed. One formatter page and one Clippy `print_stderr` diagnostic remain. The separate current-entry focused run35798529907 passed13/13 CLI entry, batch and native-capture witnesses on a branch whose only delta from6c871966 is its diagnostic harness. Neither run is aggregate M9 acceptance.
...
| B — continuous natural solo and supported rules | FAILING | On6c, natural solo campaign1/1, defender2/2 and target11/11 passed; phase2/4 failed at Title READ fixture setup. Source reward rows remain selected-context evidence only. | Source-owned damage/stat effects, full reward descendants and next encounter, then complete finite-run corpus and supported branch fixtures. | root |
| C — two-human co-op/liveness | FAILING | `m9e_coop_v7`4/4 passed on6c with default stack. `m9e_natural_coop_campaign_v7` was interrupted after404 seconds at the unchanged shared deadline, before a direct bootstrap receipt. | Independent-seat natural startup, continuing shared material, multiple retention windows and real Worker/WebRTC platform witness on final candidate. | root |
| D — persistence and causal replay | FAILING | On6c, host export and canonical decode passed, then capsule import overflowed within `BrowserKernelHostV2::process_bytes`; natural campaign replay never started after co-op consumed the shared deadline. | Complete contiguous campaign replay plus normal CLI/Wasm eventwise equality, storage uncertainty/conflict/retry and pending operation restoration. | root |
...
| F — browser semantics and integrated acceptance | IMPLEMENTED_UNVERIFIED | `python3 scripts/ci/m9e_current_phase_js_diagnostic.py` passed19 named checks on6c, including human prompt lifetime and disposal. Native host capsule import still overflowed; full browser platform matrix is unverified. | Actual browser matrix and keys/Worker/storage/network seams, complete control producer/continuation and semantic payload evidence, then all six rows at one SHA. | root |
