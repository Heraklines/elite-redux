...
All rows below refer to current source pin399d5d368f0b5642ebf8f45bd8a5e73350fa4de7 and the single current-phase integration branch. The remote command actually used is `python3 scripts/ci/m9e_current_phase_diagnostic.py`; it builds and executes exact whole targets. Additional named acceptance targets are required work, not claimed executions. Latest completed candidate8717e253/run35795173493 passed compilation, formatting and both builds;16 whole targets passed125 tests, phase2/4 and targeting10/11 remain red, host import overflowed, replay lost its intended base at the third bootstrap attempt, and natural co-op was interrupted at its unchanged600-second command limit. Twenty targets completed; four later targets did not. JavaScript19/19 passed. Clippy found two nested conditionals. Earlier partial results remain in the run ledger and do not qualify this successor.

| Row | State | Actual evidence and required test/command | Next failing condition | Owner |
|---|---|---|---|---|
| A — shared current game/agent entry | IMPLEMENTED_UNVERIFIED | Selected `m9e_fresh_friendship_profile`:4/4 on086. Public-process `m9e_current_entry`, `m9e_current_batch`, `m9e_current_native_capture`, and `m9e_current_repro` still need same-candidate execution. | Bootstrap now passes; public commands must prove current identity and all typed external events without legacy fallback. | root |
| B — continuous natural solo and supported rules | FAILING | On871, natural solo campaign1/1 passed, defender2/2 passed, phase2/4 and target10/11 failed; selected source reward rows and exact ownership rejection remain under repair. Source observations are enabling evidence only. | Source-owned damage/stat effects, full reward descendants and next encounter, then complete finite-run corpus and supported branch fixtures. | root |
| C — two-human co-op/liveness | FAILING | `m9e_coop_v7`4/4 passed on871 with default stack. `m9e_natural_coop_campaign_v7` started but exceeded its unchanged600-second command limit before any wave receipt. | Independent-seat natural startup, continuing shared material, multiple retention windows and real Worker/WebRTC platform witness on final candidate. | root |
| D — persistence and causal replay | FAILING | On871, four-input bootstrap segments exceeded the unchanged2MiB capsule bound at event8, so strict contiguous-base assertion failed. The host transport test overflowed during current capsule import after successful export. | Complete contiguous campaign replay plus normal CLI/Wasm eventwise equality, storage uncertainty/conflict/retry and pending operation restoration. | root |
