...
class CandidateSetTransformerTest(unittest.TestCase):
    ...
    def test_zero_length_history_uses_the_current_candidate_set(self) -> None:
        model = CandidateSetTransformer(
            CandidateTransformerConfig(
                feature_count=3,
                token_vocabulary_size=8,
        ...
        token_ids = torch.ones((2, 3, 5, 1), dtype=torch.long)
        token_mask = torch.ones_like(token_ids, dtype=torch.bool)
        empty_history_features = torch.zeros((2, 0, 1, 3))
        empty_history_candidate_mask = torch.zeros((2, 0, 1), dtype=torch.bool)
        empty_history_token_ids = torch.zeros((2, 0, 1, 5, 1), dtype=torch.long)
        empty_history_token_mask = torch.zeros_like(empty_history_token_ids, dtype=torch.bool)
        empty_history_chosen = torch.zeros((2, 0), dtype=torch.long)
        empty_history_step_mask = torch.zeros((2, 0), dtype=torch.bool)
        with torch.inference_mode():
            without_history = model(features, candidate_mask, token_ids, token_mask)
            explicit_empty = model(
                features,
                candidate_mask,
                token_ids,
                token_mask,
                empty_history_features,
                empty_history_candidate_mask,
                empty_history_token_ids,
                empty_history_token_mask,
                empty_history_chosen,
                empty_history_step_mask,
            )
        torch.testing.assert_close(explicit_empty[0], without_history[0])
        torch.testing.assert_close(explicit_empty[1], without_history[1])
