...
fn knockout_bonus_uses_raw_damage_and_weights_accuracy() {
    assert_eq!(
        standard_attack_score(60, 100, 50, 50).to_bits(),
        522.5_f64.to_bits()
    );
    assert_eq!(
        standard_attack_score(49, 100, 50, 100).to_bits(),
        36.75_f64.to_bits()
    );
    assert_eq!(
        standard_attack_score(50, 100, 50, 50).to_bits(),
        518.75_f64.to_bits()
    );
}
...
fn zero_damage_and_always_hit_preserve_source_boundaries() {
    for hp in [0, 1, u32::MAX] {
        for accuracy in [i16::MIN, 0, 1, 50, 100, i16::MAX] {
            assert_eq!(
                standard_attack_score(0, 0, hp, accuracy).to_bits(),
                0.0_f64.to_bits()
            );
        }
    }
    for accuracy in [i16::MIN, -1, 0, 100, 101, i16::MAX] {
        assert_eq!(
            standard_attack_score(1, 0, 1, accuracy).to_bits(),
            1075.0_f64.to_bits()
        );
    }
}
...
fn damage_not_base_power_controls_non_knockout_ranking() {
    assert_eq!(
        standard_attack_score(46, 400, 400, 100).to_bits(),
        8.625_f64.to_bits()
    );
    assert_eq!(
        standard_attack_score(13, 400, 400, 100).to_bits(),
        2.4375_f64.to_bits()
    );
...
fn all_remote_pinned_javascript_cases_match_binary64_bits() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    for line in raw.lines() {
        ...
        assert_eq!(
            standard_attack_score(damage, max_hp, hp, accuracy).to_bits(),
            expected,
            "{line}"
        );
        count += 1;
