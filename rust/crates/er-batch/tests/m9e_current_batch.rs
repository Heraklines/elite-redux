...
use er_batch::current::{
    CURRENT_BATCH_MAXIMUM_ENVIRONMENTS, CURRENT_BATCH_MAXIMUM_EVENTS,
    CURRENT_BATCH_MAXIMUM_RESULT_BYTES, CurrentBatch, CurrentBatchEnvironmentId, CurrentBatchError,
    CurrentBatchEvent, CurrentBatchLimits, CurrentBatchResult,
};
use er_env::current::{CurrentExternalEvent, CurrentGameSession, CurrentSessionError};
use er_game::m9e_content_v2::{GameContentBundleV2, PreparedGameContentV2};
use er_kernel::game_kernel_v7::{
    GameKernelEffectV7, GameKernelRoleV7, GameKernelV7Error, KernelPresentationOutcomeV2,
};
...
fn safe(value: u64) -> SafeU53 {
    SafeU53::new(value).expect("safe fixture value")
}
fn id(value: u64) -> CurrentBatchEnvironmentId {
    CurrentBatchEnvironmentId(safe(value))
}
fn seat() -> SeatId {
    SeatId::new(safe(1))
}
...
fn create_fixture() -> TestResult<Fixture> {
    ...
    assert_eq!(
        session.observe()?.control.ok_or("battle control")?.kind,
...
fn reference_results(
    ...
    events: &[CurrentBatchEvent],
) -> TestResult<Vec<CurrentBatchResult>> {
    let mut results = Vec::new();
    for (ordinal, operation) in events.iter().enumerate() {
        let session = references
            .get_mut(&operation.environment)
            .ok_or("reference session missing")?;
        let step = session.apply(operation.event.clone())?;
        results.push(CurrentBatchResult {
            ordinal,
...
fn press_pair(
    ...
    let _ = compare(
        ...
        references,
        vec![
            operation(1, key(code.clone(), true)),
            operation(1, key(code, false)),
        ],
    )?;
    Ok(())
}
...
fn current_batch_natural_controls_and_held_time_return_ordered_typed_results() -> TestResult {
    ...
    let mut batch = CurrentBatch::from_sessions(
        Arc::clone(&content),
        CurrentBatchLimits::default(),
    ...
    assert_eq!(
        initial.control.ok_or("title")?.kind,
        GameControlKindV2::Title
    );
    for _ in 0..3 {
        press_pair(&mut batch, &mut references, PhysicalKey::Space)?;
    }
    let bound = batch
        .observe(id(1))?
        .control
        ...
        + 1;
    for _ in 0..bound {
        if selected(batch.session(id(1))?)? == "bootstrap/starter/confirm" {
            break;
        }
        press_pair(&mut batch, &mut references, PhysicalKey::ArrowDown)?;
    }
    assert_eq!(
        selected(batch.session(id(1))?)?,
    ...
    assert!(
        !pending.is_empty(),
        "batch must leave actual platform work pending"
    );
    ...
    assert_eq!(
        batch.observe(id(1))?.control.ok_or("battle")?.kind,
        GameControlKindV2::BattleCommand
    );
    let results = compare(
        &mut batch,
        &mut references,
        vec![
            operation(1, key(PhysicalKey::ArrowDown, true)),
            operation(1, time(249)),
            operation(1, time(1)),
            operation(1, key(PhysicalKey::ArrowDown, false)),
            operation(1, time(500)),
        ],
    )?;
    let selection = |result: &CurrentBatchResult| {
        result
            .observation
            .control
            .as_ref()
    ...
    assert_eq!(
        results.iter().map(selection).collect::<Vec<_>>(),
        ["party", "party", "fight", "fight", "fight"]
            .map(|value| Some(format!("battle/command/{value}")))
    );
...
fn rejected_event_rolls_back_every_affected_environment_and_allows_continuation() -> TestResult {
    ...
    let result = batch.execute_with(
        events.clone(),
        |_, results| -> Result<_, CurrentBatchError> {
            completion_called = true;
            Ok(results)
        },
    );
    assert!(
        matches!(result, Err(CurrentBatchError::Event { ordinal: 2, environment,
...
enum CompletionError {
    #[error(transparent)]
    Batch(#[from] CurrentBatchError),
    #[error(transparent)]
    Encoding(#[from] serde_json::Error),
    #[error("complete adapter response exceeds its byte bound")]
    ResponseCapacity,
}
...
fn aggregate_completion_rejection_preserves_all_snapshots_and_effect_retry() -> TestResult {
    ...
    let rejected = batch.execute_with(
        events.clone(),
        |candidate, results| -> Result<Vec<u8>, CompletionError> {
            called = true;
            ...
            assert!(Arc::ptr_eq(
                candidate.session(id(1))?.content(),
                &fixture().expect("prepared fixture").content
            ));
            ...
            if bytes.len() > 1 {
                return Err(CompletionError::ResponseCapacity);
            }
            Ok(bytes)
        },
    );
    ...
    let mut references =
        BTreeMap::from([(id(1), active()?), (id(2), active()?), (id(3), active()?)]);
...
fn aggregate_result_byte_limit_is_exact_atomic_and_resets_each_call() -> TestResult {
    ...
    assert!(
        matches!(result, Err(CurrentBatchError::ResultCapacity { maximum }) if maximum == bytes - 1)
    );
    assert!(
        !called,
        "aggregate completion must not receive an over-budget retained array"
    );
    assert_eq!(
        [rejected.snapshot(id(1))?, rejected.snapshot(id(2))?],
        before
    );
    let mut exact = batch(CurrentBatchLimits {
        maximum_result_bytes: bytes,
        ..limits
    })?;
    assert_eq!(exact.execute(events.clone())?, expected);
    for event in events {
        let _ = rejected.execute(vec![event])?;
    }
    assert_eq!(
        [rejected.snapshot(id(1))?, rejected.snapshot(id(2))?],
    ...
    for _ in 0..3 {
        let _ = exact.execute(vec![operation(2, time(1))])?;
    }
    Ok(())
}
...
fn equal_prepared_content_normalizes_and_forks_without_changing_private_state() -> TestResult {
    let fixture = fixture()?;
    let separate = Arc::new(PreparedGameContentV2::prepare(Arc::clone(
        fixture.content.bundle(),
    ))?);
    ...
    let _ = compare(
        ...
        &mut references,
        vec![
            operation(8, key(PhysicalKey::ArrowDown, true)),
            operation(8, time(250)),
        ],
    )?;
    assert_eq!(batch.snapshot(id(1))?, before);
...
fn limits_missing_duplicate_disposed_and_wrong_content_are_atomic() -> TestResult {
    ...
    for limits in [
        CurrentBatchLimits {
            maximum_environments: 0,
            ..CurrentBatchLimits::default()
        },
        CurrentBatchLimits {
            maximum_environments: CURRENT_BATCH_MAXIMUM_ENVIRONMENTS + 1,
            ..CurrentBatchLimits::default()
        },
        CurrentBatchLimits {
            maximum_events: 0,
        ...
        CurrentBatchLimits {
            maximum_events: CURRENT_BATCH_MAXIMUM_EVENTS + 1,
            ..CurrentBatchLimits::default()
        },
        CurrentBatchLimits {
        ...
        CurrentBatchLimits {
            maximum_result_bytes: CURRENT_BATCH_MAXIMUM_RESULT_BYTES + 1,
            ..CurrentBatchLimits::default()
        },
    ] {
        assert!(matches!(
            CurrentBatch::new(Arc::clone(&content), limits),
            Err(CurrentBatchError::InvalidLimits)
        ));
    }
    ...
    let before = [batch.snapshot(id(1))?, batch.snapshot(id(2))?];
    assert!(matches!(
        batch.insert(id(1), active()?),
        Err(CurrentBatchError::DuplicateEnvironment { .. })
    ));
    assert!(matches!(
        batch.fork(id(1), id(3)),
        Err(CurrentBatchError::EnvironmentCapacity { maximum: 2 })
    ));
    assert!(
        matches!(batch.execute(vec![operation(1, time(1)), operation(9, time(1))]),
        Err(CurrentBatchError::MissingEnvironment { environment }) if environment == id(9))
    );
    assert!(matches!(
        batch.execute(vec![operation(1, time(1)); 3]),
        Err(CurrentBatchError::EventCapacity { maximum: 2 })
    ));
    assert_eq!([batch.snapshot(id(1))?, batch.snapshot(id(2))?], before);
    batch.remove(id(2))?;
    ...
    disposed.dispose();
    assert!(matches!(
        batch.insert(id(2), disposed),
        Err(CurrentBatchError::Session {
            source: CurrentSessionError::Disposed,
            ..
        })
    ));
    assert!(matches!(
        batch.fork(id(9), id(2)),
        Err(CurrentBatchError::MissingEnvironment { .. })
    ));
    ...
    assert!(
        matches!(batch.insert(id(2), natural(wrong)?), Err(CurrentBatchError::ContentMismatch { environment }) if environment == id(2))
    );
    ...
    assert!(matches!(
        batch.snapshot(id(1)),
        Err(CurrentBatchError::Disposed)
    ));
    assert!(matches!(
        batch.execute(Vec::new()),
        Err(CurrentBatchError::Disposed)
    ));
    assert!(matches!(
        batch.insert(id(1), active()?),
