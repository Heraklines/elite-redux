...
impl<'a> CurrentTargetExecution<'a> {
    pub fn from_state(state: &'a GameStateV6) -> Result<Self, CurrentTargetExecutionError> {
        let value = Self {
            owner: state.current_targeting.as_ref().ok_or(CurrentTargetExecutionError)?,
            profile: state.current_friendship_profile.as_ref().ok_or(CurrentTargetExecutionError)?,
        };
        ...
        value.validate_run(state.active_run.as_ref().ok_or(CurrentTargetExecutionError)?)?;
        Ok(value)
    }
    ...
    pub fn validate_run(&self, run: &RunStateV3) -> Result<(), CurrentTargetExecutionError> {
        ...
        if battle.weather.kind != WeatherKind::None || battle.terrain.kind != TerrainKind::None
            || !battle.arena_conditions.is_empty() || battle.global_ability_suppression.ignore_abilities || battle.global_ability_suppression.source.is_some()
            || !run.modifiers.is_empty()
        {
            return Err(CurrentTargetExecutionError);
        }
        for pokemon in run.party.iter().chain(&battle.enemy_party) {
            if pokemon.fusion.is_some() || !pokemon.held_items.is_empty() || pokemon.abilities.active_suppressed
                || pokemon.abilities.passive_suppressed.iter().any(|value| *value)
            {
                return Err(CurrentTargetExecutionError);
            }
    ...
    fn passive_attr(&self, pokemon: &PokemonStateV5) -> Result<u8, CurrentTargetExecutionError> {
        ...
        self.profile.accounts.binary_search_by_key(&pokemon.species_id, |row| row.species)
            .ok().map(|index| self.profile.accounts[index].passive_attr)
            .filter(|mask| *mask <= 63).ok_or(CurrentTargetExecutionError)
    }
    ...
    pub fn ability_sources(&self, run: &RunStateV3, pokemon: &PokemonStateV5)
        -> Result<Vec<BehaviorSourceId>, CurrentTargetExecutionError>
    {
        self.validate_run(run)?;
        let mask = if pokemon.owner_seat.is_some() { self.passive_attr(pokemon)? } else { 0 };
        let enemy_limit = if pokemon.level >= 24 { 3 } else if pokemon.level >= 15 { 2 } else { 1 };
        ...
        for (slot, candidate) in pokemon.abilities.passives.iter().enumerate() {
            let Some(ability) = *candidate else { continue; };
            ...
            if !admitted || ids.iter().any(|(seen, _)| *seen == ability)
        ...
        Ok(ids.into_iter().map(|(ability, passive)| {
    ...
    pub fn execution_targets(&self, run: &RunStateV3, actor: PokemonId,
        definition: &MoveDefinitionV3, retained: &[FieldSlot])
        -> Result<Vec<FieldSlot>, CurrentTargetExecutionError>
    {
        self.validate_run(run)?;
        let battle = run.battle.as_ref().ok_or(CurrentTargetExecutionError)?;
        let user_slot = battle.field.slots.iter().find(|row| row.occupant == Some(actor))
            .map(|row| row.slot).ok_or(CurrentTargetExecutionError)?;
        ...
        if result.len() == 1 && matches!(definition.target, MoveTarget::NearOther | MoveTarget::Other)
            && definition.move_type == PokemonType::Poison
        {
            let mut active = [false; 6];
            for row in &battle.field.slots {
                active[flat(row.slot)?] = row.occupant.and_then(|id| find_pokemon(run, id))
                    .is_some_and(|pokemon| pokemon.hp > 0 && !pokemon.fainted);
            }
            let user_index = flat(user_slot)?;
            let opponent_count = active.iter().enumerate()
                .filter(|(index, value)| **value && *index / 3 != user_index / 3).count();
            // Ask the already qualified source arrangement core for reachability.
            let adjacent = crate::current_move_targets::resolve_move_targets(&crate::current_move_targets::TargetContext {
                ...
                target: MoveTarget::AllNearOthers, replacement: None,
                variable: vec![None; opponent_count], spread_flag: false, multi_hit: false,
                ghost: false, fog: false, fog_suppressed: false,
                flying: definition.move_type == PokemonType::Flying,
                pulse: definition.flags.contains(&MoveFlag::Pulse), arrangement: true, random_index: None,
            }).map_err(|_| CurrentTargetExecutionError)?;
            let mut redirect = None;
            for row in &battle.field.slots {
                let Some(id) = row.occupant else { continue; };
                if id == actor || !adjacent.targets.contains(&i8::try_from(flat(row.slot)?)
                    .map_err(|_| CurrentTargetExecutionError)?) { continue; }
                ...
                let applies = self.ability_sources(run, holder)?.iter().any(|source|
                    ability_numeric_id(source).and_then(|id| source_ability(id).ok())
                ...
                if applies {
                    ...
                    if redirect.replace(row.slot).is_some() { return Err(CurrentTargetExecutionError); }
                }
            }
            if let Some(slot) = redirect { result[0] = slot; }
        }
        result.retain(|slot| battle.field.slots.iter().find(|row| row.slot == *slot)
            .and_then(|row| row.occupant).and_then(|id| find_pokemon(run, id))
            .is_some_and(|pokemon| pokemon.hp > 0 && !pokemon.fainted));
        Ok(result)
    }
    pub fn plan(&self, run: &RunStateV3, actor: PokemonId, definition: &MoveDefinitionV3)
        -> Result<CurrentTargetPlan, CurrentTargetExecutionError>
    {
        self.validate_run(run)?;
        let battle = run.battle.as_ref().ok_or(CurrentTargetExecutionError)?;
        let field = battle.field.slots.iter().find(|row| row.occupant == Some(actor))
            .ok_or(CurrentTargetExecutionError)?;
        let user = find_pokemon(run, actor).ok_or(CurrentTargetExecutionError)?;
        if user.hp == 0 || user.fainted { return Err(CurrentTargetExecutionError); }
        let (source_target, multi_hit) = source_move(definition.id.get().get())?;
        if source_target != definition.target { return Err(CurrentTargetExecutionError); }
        ...
        if matches!(source_target, MoveTarget::RandomNearEnemy | MoveTarget::Attacker | MoveTarget::Curse) {
            return Err(CurrentTargetExecutionError);
        }
        ...
        let opponent_count = allowed.iter().enumerate()
            .filter(|(index, allowed)| **allowed && *index / 3 != user_index / 3).count();
        let source_ids = self.ability_sources(run, user)?;
        let spread = source_ids.iter().any(|source| ability_numeric_id(source)
            .and_then(|id| source_ability(id).ok()).is_some_and(|ability| ability.sound_spread))
            && definition.flags.contains(&MoveFlag::SoundBased);
        let result = crate::current_move_targets::resolve_move_targets(&crate::current_move_targets::TargetContext {
            capacities: [battle.format.player_capacity, battle.format.enemy_capacity],
            allowed,
            ...
            active: allowed,
            user: u8::try_from(user_index).map_err(|_| CurrentTargetExecutionError)?,
            target: source_target, replacement: None, variable: vec![None; opponent_count],
            spread_flag: spread, multi_hit,
            ghost: false, fog: false, fog_suppressed: false,
            flying: definition.move_type == PokemonType::Flying,
            pulse: definition.flags.contains(&MoveFlag::Pulse),
            arrangement: true, random_index: None,
        }).map_err(|_| CurrentTargetExecutionError)?;
        let ordered = result.targets.into_iter().map(unflat)
            .collect::<Result<Vec<_>, _>>()?;
        Ok(CurrentTargetPlan { ordered, multiple: result.multiple })
    }
}

impl CurrentTargetPlan {
    pub fn selections(&self) -> Result<Vec<BattleTargetSelection>, CurrentTargetExecutionError> {
        if self.ordered.is_empty() { return Ok(Vec::new()); }
        if self.multiple || self.ordered.len() == 1 {
            Ok(vec![BattleTargetSelection::implicit()])
        } else {
            self.ordered.iter().map(|slot| BattleTargetSelection::selected(vec![*slot])
                .map_err(|_| CurrentTargetExecutionError)).collect()
        }
    }

    pub fn retain(&self, selection: &BattleTargetSelection) -> Result<Vec<FieldSlot>, CurrentTargetExecutionError> {
        match selection {
            BattleTargetSelection::Implicit if self.multiple || self.ordered.len() == 1 => Ok(self.ordered.clone()),
            BattleTargetSelection::Selected(selected) if !self.multiple && self.ordered.len() > 1 && selected.len() == 1
                && self.ordered.contains(&selected[0]) => Ok(selected.clone()),
...
fn source_ability(id: u64) -> Result<SourceAbility, CurrentTargetExecutionError> {
    if !matches!(id, 0 | 18 | 41 | 43 | 47 | 49 | 51 | 62 | 65 | 66 | 67 | 75 | 82 | 94 | 113 | 172 | 192 | 257 | 268 | 5006 | 5033 | 5082 | 5097 | 5115) {
        return Err(CurrentTargetExecutionError);
    }
    ...
    Ok(SourceAbility { bypass_faint: matches!(id, 49 | 268), sound_spread: id == 5115,
        poison_redirect: id == 5082, sun_condition: id == 94 })
}

fn source_move(id: u64) -> Result<(MoveTarget, bool), CurrentTargetExecutionError> {
    use MoveTarget::*;
    let target = match id {
        10 | 33 | 40 | 61 | 64 | 78 | 79 | 98 | 103 | 310 | 331 | 448 | 458 | 497 | 541 => NearOther,
...
fn ability_numeric_id(source: &BehaviorSourceId) -> Option<u64> {
    match source {
        BehaviorSourceId::ActiveAbility { numeric_id } | BehaviorSourceId::PassiveAbility { numeric_id } => Some(numeric_id.get()),
        _ => None,
    }
}

pub fn find_pokemon(run: &RunStateV3, id: PokemonId) -> Option<&PokemonStateV5> {
    run.party.iter().chain(run.battle.iter().flat_map(|battle| &battle.enemy_party)).find(|pokemon| pokemon.id == id)
}

fn flat(slot: FieldSlot) -> Result<usize, CurrentTargetExecutionError> {
    if slot.position >= 3 { return Err(CurrentTargetExecutionError); }
    Ok(usize::from(slot.position) + if slot.side == BattleSide::Player { 0 } else { 3 })
}

fn unflat(index: i8) -> Result<FieldSlot, CurrentTargetExecutionError> {
    if !(0..6).contains(&index) { return Err(CurrentTargetExecutionError); }
    Ok(FieldSlot { side: if index < 3 { BattleSide::Player } else { BattleSide::Enemy },
