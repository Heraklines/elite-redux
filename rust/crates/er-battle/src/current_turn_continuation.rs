...
use er_state::current_turn_execution::{
    CurrentFaintMoveSourceV1, CurrentTurnExecutionV1, CurrentTurnFaintV1, CurrentTurnStageV1,
};
...
pub fn begin_current_turn(
    ...
    let owner = CurrentTurnExecutionV1 {
        ...
        wave: battle.wave,
        turn: battle.turn,
        authority: authority.authority_seat,
...
fn advance_current_turn(
        ...
        || finalize != (usize::from(owner.next_action) == owner.actions.len())
    {
        return Err(BattleV5Error::State(
            "current turn has an unresolved interlude or wrong action frontier".to_owned(),
        ));
    }
    let battle = run.battle.as_ref().ok_or(BattleV5Error::NoBattle)?;
    let mut rng = RngRuntime::from_states_at_sequence(
    ...
    for mutation in &mutations {
        ...
        } = mutation
        {
            ...
            let slot = battle
                ...
                .slots
                .iter()
                .find(|row| row.occupant == Some(*pokemon))
                .map(|row| row.slot)
            ...
            faints.push(CurrentTurnFaintV1 {
                id: next.next_faint_sequence,
                pokemon: *pokemon,
                slot,
                source_move,
            });
            next.next_faint_sequence = SafeU53::new(
                next.next_faint_sequence
                    .get()
                    .checked_add(1)
                    .ok_or(BattleV5Error::Overflow)?,
            )
...
fn make_chunk(
    ...
    let next_control =
        if owner.stage != CurrentTurnStageV1::Complete || outcome != BattleOutcome::Ongoing {
    ...
    Ok(BattleTransitionV5 {
        ...
        after_digest: mechanical_digest(&after)?,
        after_state: after,
        accepted_commands: owner.accepted_commands.clone(),
