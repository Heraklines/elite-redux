...
mod tests {
    ...
    fn tera_admission_consumes_side_budget_and_persists_through_switch() {
        ...
        assert_eq!(
            admit_tera(&tera.state, BattleSide::Player, &scope, 10).expect_err("tera_admission_consumes_side_budget_and_persists_through_switch: expected rejection"),
        ...
        assert_eq!(
            admit_tera(&cleaned.state, BattleSide::Player, &scope, 10).expect_err("tera_admission_consumes_side_budget_and_persists_through_switch: expected rejection"),
            FormsTransitionError::TeraBudgetExhausted {
                side: BattleSide::Player
            }
        );
    ...
    fn resolve_cluster_fixture() -> Result<std::path::PathBuf, String> {
