...
impl CurrentDispatcher {
    ...
    fn session(&self, params: &Value) -> Result<&CurrentGameSession, AgentDispatchErrorV1> {
        self.sessions.get(self.session_id(params)?)
    ...
    fn reserve_id(&self, id: &str) -> Result<(), AgentDispatchErrorV1> {
        if id.is_empty() || id.len() > 128 || self.sessions.contains_key(id) {
            return Err(invalid("session identity is empty, too long, or already exists"));
        }
    ...
    fn create(&mut self, params: &Value) -> Result<Value, AgentDispatchErrorV1> {
        let request: CreateRequest = serde_json::from_value(params.clone()).map_err(invalid_error)?;
        self.reserve_id(&request.session)?;
        let session = match request.start {
            CurrentStart::Natural { profile, seed, owner_seat, save_slots, local_is_host } => {
                CurrentGameSession::natural_start(
                    profile, seed, owner_seat, save_slots, local_is_host,
                    Arc::clone(&self.content), None,
                ).map_err(backend)?
            }
            CurrentStart::Snapshot { snapshot, owner_seat, role } => {
                CurrentGameSession::from_snapshot(snapshot, owner_seat, role, Arc::clone(&self.content))
                    .map_err(backend)?
            }
        };
    ...
    fn apply(&mut self, params: &Value, event: CurrentExternalEvent) -> Result<Value, AgentDispatchErrorV1> {
...
impl AgentDispatcherV1 for CurrentDispatcher {
    fn dispatch(&mut self, method: &str, params: &Value) -> Result<Value, AgentDispatchErrorV1> {
        match method {
            ...
            "session.raw_input" => self.apply(params, CurrentExternalEvent::RawInput {
                input: required::<RawInputEvent>(params, "input")?,
            }),
            "session.advance_time" => self.apply(params, CurrentExternalEvent::AdvanceTime {
                milliseconds: required::<SafeU53>(params, "milliseconds")?,
            }),
            "session.network_frame" => self.apply(params, CurrentExternalEvent::NetworkFrame {
                generation: required(params, "generation")?, bytes: required(params, "bytes")?,
            }),
            "session.transport_changed" => self.apply(params, CurrentExternalEvent::TransportChanged {
                generation: required(params, "generation")?, connected: required(params, "connected")?,
            }),
            "session.presentation_settled" => self.apply(params, CurrentExternalEvent::PresentationOutcome {
            ...
            "session.storage_result" => self.apply(params, CurrentExternalEvent::StorageResult {
                request_id: required(params, "request_id")?, result: required(params, "result")?,
            }),
            "platform.event" => self.apply(params, required(params, "event")?),
            "session.restore" => {
                let id = self.session_id(params)?.to_owned();
                let mut candidate = self.session(params)?.fork().map_err(backend)?;
                candidate.restore(required(params, "snapshot")?).map_err(backend)?;
                self.sessions.insert(id, candidate);
            ...
            "content.inspect" => Ok(json!({"kernel_version": 7, "content_identity": self.content.identity()})),
...
pub fn run(options: &BTreeMap<String, String>) -> Result<(), Box<dyn Error>> {
    ...
    let bundle: GameContentBundleV2 = crate::decode_file(&path).map_err(|error| {
        format!("current agent requires V2 content; use agent-v6 for historical V1 content: {error}")
    })?;
    let content = Arc::new(PreparedGameContentV2::prepare(Arc::new(bundle))?);
    let maximum_sessions = options.get("maximum-sessions")
    ...
    let mut dispatcher = CurrentDispatcher { content, sessions: BTreeMap::new(), maximum_sessions };
    if let Some(path) = options.get("snapshot") {
        let snapshot: CoreGameKernelSnapshotV7 = crate::decode_file(std::path::Path::new(path))?;
        let owner = options.get("seat").map_or(Ok(1), |value| value.parse::<u64>())?;
        let role = match options.get("role").map(String::as_str).unwrap_or("AUTHORITY") {
        ...
        let session = CurrentGameSession::from_snapshot(
            snapshot, SeatId::new(SafeU53::new(owner)?), role, Arc::clone(&dispatcher.content),
        )?;
        dispatcher.sessions.insert("current".to_owned(), session);
    }
    let mut server = AgentJsonlServerV1::new(dispatcher, AgentProtocolLimitsV1 {
    ...
    loop {
        let response = match read_bounded_jsonl_line_v1(&mut reader, &mut line, MAXIMUM_MESSAGE_BYTES)? {
...
fn bounded(value: Value) -> Result<Value, AgentDispatchErrorV1> {
    if serde_json::to_vec(&value).map_err(backend)?.len() > MAXIMUM_MESSAGE_BYTES {
        return Err(backend("current response exceeds the bounded inline protocol limit"));
    }
    Ok(value)
}

fn required<T: serde::de::DeserializeOwned>(params: &Value, name: &str) -> Result<T, AgentDispatchErrorV1> {
    let value = params.get(name).cloned().ok_or_else(|| invalid(&format!("missing {name}")))?;
    serde_json::from_value(value).map_err(invalid_error)
}

fn invalid(message: &str) -> AgentDispatchErrorV1 {
    AgentDispatchErrorV1 { code: AgentErrorCodeV1::InvalidRequest, message: message.to_owned() }
}
...
fn backend(error: impl ToString) -> AgentDispatchErrorV1 {
    AgentDispatchErrorV1 { code: AgentErrorCodeV1::BackendError, message: error.to_string() }
}
