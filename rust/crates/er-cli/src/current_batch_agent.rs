...
use er_batch::current::{CurrentBatch, CurrentBatchEnvironmentId, CurrentBatchError, CurrentBatchEvent, CurrentBatchLimits};
...
enum CapsuleTag { #[serde(rename = "CAPSULE")] Capsule }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct CapsuleStart { kind: CapsuleTag, capsule: Box<CurrentReproCapsuleV1> }

#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum BatchStart { Current(CurrentStart), Capsule(CapsuleStart) }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct EnvironmentStart { environment: CurrentBatchEnvironmentId, start: BatchStart }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct CreateRequest {
    batch: String, environments: Vec<EnvironmentStart>,
    #[serde(default)] limits: CurrentBatchLimits,
}

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct ResetRequest { batch: String, environments: Vec<EnvironmentStart> }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct BatchRequest { batch: String }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct ReadRequest { batch: String, environments: Vec<CurrentBatchEnvironmentId> }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct EventsRequest { batch: String, events: Vec<CurrentBatchEvent> }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct RawInput { environment: CurrentBatchEnvironmentId, input: RawInputEvent }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct RawRequest { batch: String, inputs: Vec<RawInput> }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct Advance { environment: CurrentBatchEnvironmentId, milliseconds: SafeU53 }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct AdvanceRequest { batch: String, advances: Vec<Advance> }

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct ForkRequest {
    batch: String, source_environment: CurrentBatchEnvironmentId,
...
impl From<CurrentBatchError> for BatchCompletionError {
    fn from(error: CurrentBatchError) -> Self { Self(backend(error)) }
}

#[derive(Debug)]
pub(crate) struct CurrentBatches {
    content: Arc<PreparedGameContentV2>, entries: BTreeMap<String, CurrentBatch>,
}

impl CurrentBatches {
    pub(crate) fn new(content: Arc<PreparedGameContentV2>) -> Self {
        Self { content, entries: BTreeMap::new() }
    }
    pub(crate) fn len(&self) -> usize { self.entries.len() }
    pub(crate) fn environment_count(&self) -> usize { self.entries.values().map(CurrentBatch::len).sum() }

    fn batch(&self, id: &str) -> Result<&CurrentBatch, AgentDispatchErrorV1> {
        self.entries.get(id).ok_or_else(|| backend("current batch missing or closed"))
    }
    fn batch_mut(&mut self, id: &str) -> Result<&mut CurrentBatch, AgentDispatchErrorV1> {
        self.entries.get_mut(id).ok_or_else(|| backend("current batch missing or closed"))
    }
    fn capacity(&self, standalone: usize, removed: usize, added: usize, maximum: usize)
        -> Result<(), AgentDispatchErrorV1>
    {
        let retained = self.environment_count().checked_sub(removed)
    ...
    fn prepare(&self, starts: Vec<EnvironmentStart>, limits: CurrentBatchLimits)
        -> Result<CurrentBatch, AgentDispatchErrorV1>
    {
        ...
        for entry in starts {
            let session = match entry.start {
                ...
                BatchStart::Capsule(CapsuleStart { kind: CapsuleTag::Capsule, capsule }) => {
                    // Validate and replay privately. Recorded effects are evidence,
                    // never output during environment construction.
                    replay_current_capsule_v1(&capsule, Arc::clone(&self.content), CurrentReproLimitsV1::default())
                        .map_err(backend)?
                }
            };
    ...
    fn execute(&mut self, id: &str, events: Vec<CurrentBatchEvent>, context: AgentResponseContextV1<'_>) -> Result<Value, AgentDispatchErrorV1> {
        self.batch_mut(id)?.execute_with(events, |_, results| {
            ...
            admit(context, json!({"batch": id, "kernel_version": 7, "results": results}))
                .map_err(BatchCompletionError)
        }).map_err(|error: BatchCompletionError| error.0)
    }

    pub(crate) fn dispatch(&mut self, method: &str, params: &Value, standalone: usize, maximum: usize,
        ...
        -> Result<Value, AgentDispatchErrorV1>
    {
        match method {
            "batch.create" => {
                ...
                if request.batch.is_empty() || request.batch.len() > 128 || self.entries.contains_key(&request.batch) {
                    return Err(invalid("batch identity is empty, too long, or already exists"));
                }
                if self.entries.len() >= maximum { return Err(backend("current batch handle capacity reached")); }
                ...
                let response = admit(context, json!({"batch": request.batch, "kernel_version": 7,
            ...
            "batch.reset" => {
                ...
                self.capacity(standalone, original.len(), request.environments.len(), maximum)?;
                let candidate = self.prepare(request.environments, limits)?;
                let response = admit(context, json!({"batch": request.batch, "kernel_version": 7,
                    "environments": candidate.environment_ids()}))?;
                if let Some(mut previous) = self.entries.insert(request.batch, candidate) { previous.dispose(); }
                Ok(response)
            }
            "batch.events" => {
            ...
            "batch.raw_input" => {
                ...
                self.execute(&request.batch, request.inputs.into_iter().map(|item| CurrentBatchEvent {
                    environment: item.environment, event: CurrentExternalEvent::RawInput { input: item.input },
                }).collect(), context)
            }
            "batch.advance_time" => {
                let request: AdvanceRequest = parse(params)?;
                self.execute(&request.batch, request.advances.into_iter().map(|item| CurrentBatchEvent {
                    environment: item.environment, event: CurrentExternalEvent::AdvanceTime { milliseconds: item.milliseconds },
                }).collect(), context)
            }
            "batch.observe" | "batch.snapshot" => {
                ...
                let mut response_bytes = serde_json::to_vec(&json!({"batch": request.batch,
                    "kernel_version": 7, "results": []})).map_err(backend)?.len();
                for environment in request.environments {
                    ...
                    response_bytes = response_bytes.saturating_add(
                        serde_json::to_vec(&value).map_err(backend)?.len() + usize::from(!results.is_empty()));
                    if response_bytes > MAXIMUM_MESSAGE_BYTES {
                        return Err(backend("current batch read exceeds inline response capacity"));
                    }
                    results.push(value);
                }
                admit(context, json!({"batch": request.batch, "kernel_version": 7, "results": results}))
            }
            "batch.fork" => {
                ...
                self.batch_mut(&request.batch)?.fork(request.source_environment, request.target_environment).map_err(backend)?;
                Ok(response)
            }
            "batch.close" => {
                ...
                if let Some(mut batch) = self.entries.remove(&request.batch) { batch.dispose(); }
                Ok(response)
            }
...
impl Drop for CurrentBatches {
    fn drop(&mut self) { for batch in self.entries.values_mut() { batch.dispose(); } }
}
