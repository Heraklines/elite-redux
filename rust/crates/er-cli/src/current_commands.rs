...
pub(crate) fn read_json<T: DeserializeOwned>(path: &Path, maximum: usize) -> Result<T, Box<dyn Error>> {
...
fn seat(options: &Options) -> Result<SeatId, Box<dyn Error>> {
    let value = options.get("seat").map_or(Ok(1), |value| value.parse::<u64>())?;
...
fn restored(options: &Options) -> Result<CurrentGameSession, Box<dyn Error>> {
    ...
    Ok(CurrentGameSession::from_snapshot(snapshot, seat(options)?, role, content)?)
}
...
pub fn new_run(options: &Options) -> Result<(), Box<dyn Error>> {
    ...
    let seed = options.get("seed").ok_or("new-run requires --seed")?.clone();
    let save_slot = options.get("save-slot").filter(|slot| !slot.is_empty())
        .ok_or("new-run requires a nonempty --save-slot")?.clone();
    play(CurrentGameSession::natural_start(
        profile, seed, seat(options)?, vec![save_slot], true, content, None,
    )?)
}
...
pub fn simulate(options: &Options) -> Result<(), Box<dyn Error>> {
    ...
    let steps = options.get("steps").map_or(Ok(1), |value| value.parse::<u64>())?;
    if steps > 100_000 {
        return Err("simulate permits at most 100000 virtual-time steps per invocation".into());
    }
    let milliseconds = SafeU53::new(options.get("milliseconds")
    ...
    for position in 1..=steps {
        ...
        crate::write_line(&serde_json::to_string(&json!({"position": position, "step": step}))?)?;
    }
...
fn play(mut session: CurrentGameSession) -> Result<(), Box<dyn Error>> {
    ...
    let mut bytes = Vec::new();
    loop {
        crate::write_line(&serde_json::to_string(&json!({"observation": session.observe()?}))?)?;
        ...
        if line == "snapshot" {
            crate::write_line(&serde_json::to_string(&json!({"snapshot": session.snapshot()?}))?)?;
            continue;
        }
        let events = if line.trim_start().starts_with('{') {
            ...
            vec![
                CurrentExternalEvent::RawInput { input: RawInputEvent::KeyDown {
                    printable: matches!(key, PhysicalKey::Space), code: key.clone(),
                    browser_repeat: false, focus: InputFocus::Game,
                } },
                CurrentExternalEvent::RawInput { input: RawInputEvent::KeyUp { code: key } },
            ]
        };
        for event in events {
