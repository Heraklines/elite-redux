...
impl Default for CaptureLimits {
    fn default() -> Self {
        ...
        Self { maximum_events: limits.maximum_events, maximum_bytes: limits.maximum_bytes }
    }
}

impl CaptureLimits {
    pub(crate) fn checked(self) -> Result<CurrentReproLimitsV1, AgentDispatchErrorV1> {
        let limits = CurrentReproLimitsV1 { maximum_events: self.maximum_events, maximum_bytes: self.maximum_bytes };
...
impl From<CurrentSessionError> for ApplyError {
    fn from(value: CurrentSessionError) -> Self { Self::Kernel(value) }
}

impl NativeCapture {
    pub(crate) fn checkpoint(session: &CurrentGameSession, limits: CaptureLimits, position: u64) -> Result<Self, AgentDispatchErrorV1> {
        let (seat, role) = session.session_context().map_err(backend)?;
        let recorder = CurrentReproRecorderV1::new_at_position(
            session.snapshot().map_err(backend)?, seat, role, Arc::clone(session.content()), limits.checked()?, position,
        ).map_err(backend)?;
        Ok(Self { recorder, limits })
    }

    pub(crate) fn imported(capsule: CurrentReproCapsuleV1, content: Arc<er_game::m9e_content_v2::PreparedGameContentV2>, limits: CaptureLimits)
        ...
        let (recorder, session) = CurrentReproRecorderV1::from_capsule(capsule, content, limits.checked()?).map_err(backend)?;
        let capture = if browser_origin { Self::checkpoint(&session, limits, position)? }
            else { Self { recorder, limits } };
        Ok((capture, session))
    }
    ...
    pub(crate) fn gap(&mut self, reason: &str) { let _ = self.recorder.invalidate_attempt(reason); }
    ...
    pub(crate) fn reset(&self, session: &CurrentGameSession) -> Result<Self, AgentDispatchErrorV1> {
        ...
        let Some(position) = position.checked_add(1).filter(|position| *position <= er_repro::current::MAXIMUM_CURRENT_REPRO_POSITION_V1) else {
    ...
    pub(crate) fn apply(&mut self, session: &mut CurrentGameSession, event: CurrentExternalEvent, origin: &str, context: AgentResponseContextV1<'_>)
        -> Result<Value, AgentDispatchErrorV1> {
        let before = match session.snapshot() {
            Ok(snapshot) => snapshot,
            Err(error) => { self.gap("native pre-event snapshot unavailable"); return Err(backend(error)); }
        };
        let result = session.apply_with(event.clone(), |candidate, step| {
            let observation = candidate.observe().map_err(|error| ApplyError::Adapter(backend(error)))?;
            let after = candidate.snapshot().map_err(|error| ApplyError::Adapter(backend(error)))?;
        ...
        match result {
            Ok((response, step, observation, after)) => {
                let _ = self.recorder.record_with_origin(&before, event, Ok(&step), &after, &observation, Some(origin));
                Ok(response)
            }
            Err(ApplyError::Kernel(error)) => {
                match session.observe() {
                    Ok(observation) => { let _ = self.recorder.record_with_origin(&before, event, Err(&error), &before, &observation, Some(origin)); }
