...
use er_kernel_worker::{
    KernelGenerationIdentityV2, KernelSessionIdV1, KernelWorkerInitializationV2,
};
use er_lab::kernel_reload::{
    ChildKernelGenerationV2, CurrentKernelSupervisorV2, CurrentReloadTicketV2, CurrentTailLimitsV2,
    KernelWorkerDeadlinesV2, VerifiedKernelExecutableV2,
};
use er_types::SeatId;
use serde_json::{Value, json};

use crate::current_agent::{
    CurrentCompletionError, MAXIMUM_MESSAGE_BYTES, backend, bounded, invalid, required,
};
...
impl WorkerConfiguration {
    pub(crate) fn from_options(
        ...
        if keys.iter().all(|key| !options.contains_key(*key)) {
            return Ok(None);
        }
        if keys.iter().any(|key| !options.contains_key(*key)) {
            return Err(
                "worker backend requires --worker-executable, --worker-root and --worker-identity"
                    .into(),
            );
        }
        let mut bytes = Vec::new();
        File::open(&options["worker-identity"])?
            .take(65_537)
            .read_to_end(&mut bytes)?;
        if bytes.len() > 65_536 {
            return Err("worker identity exceeds 64 KiB".into());
        }
        let identity: KernelGenerationIdentityV2 = serde_json::from_slice(&bytes)?;
        if &identity.content_identity != content.identity() {
            return Err("worker content identity differs".into());
        }
        Ok(Some(Self {
            artifact: VerifiedKernelExecutableV2::verify(
                &options["worker-root"],
                &options["worker-executable"],
                identity,
            )?,
        }))
    }
    ...
    pub(crate) fn adopt(
    ...
    ) -> Result<CurrentBackend, AgentDispatchErrorV1> {
        ...
        let artifact = VerifiedKernelExecutableV2::verify(
            self.artifact.allowed_root(),
            self.artifact.executable(),
            identity,
        )
        .map_err(backend)?;
        Ok(CurrentBackend::Worker(Box::new(
            WorkerSession::from_snapshot(
                artifact,
...
impl CurrentBackend {
    pub(crate) fn observe(&mut self) -> Result<CurrentGameObservation, AgentDispatchErrorV1> {
        match self {
            Self::Native(session) => session.observe().map_err(backend),
            Self::Worker(session) => session.supervisor.observe().map_err(backend),
        }
    }
    pub(crate) fn snapshot(&mut self) -> Result<CoreGameKernelSnapshotV7, AgentDispatchErrorV1> {
        match self {
            Self::Native(session) => session.snapshot().map_err(backend),
            Self::Worker(session) => session.supervisor.snapshot().map_err(backend),
        }
    }
        ...
        match self {
            ...
            Self::Worker(session) => {
                ...
                CurrentGameSession::from_snapshot(
                    session.supervisor.snapshot().map_err(backend)?,
                    seat,
                    role,
                    Arc::clone(&session.content),
                )
        ...
        match self {
            Self::Native(session) => session
                .apply_with(event, |candidate, step| {
                    bounded(json!({"step": step, "observation": candidate.observe()?}))
            ...
            Self::Worker(session) => {
                ...
                Ok(
                    json!({"step": accepted.evidence.step, "observation": accepted.evidence.observation}),
                )
            }
        }
    }
    pub(crate) fn restore(
        ...
        match self {
            ...
            Self::Worker(session) => {
                ...
                identity.generation.0 = identity
                    .generation
                    .0
                    .checked_add(1)
                    .ok_or_else(|| backend("worker generation exhausted"))?;
                let artifact = VerifiedKernelExecutableV2::verify(
                    session.artifact.allowed_root(),
                    session.artifact.executable(),
                    identity,
                )
                .map_err(backend)?;
                let candidate = WorkerSession::from_snapshot(
                    artifact,
                    snapshot,
                    seat,
                    role,
                ...
                let retirement_issue = previous
                    .supervisor
                    .dispose()
                    .err()
                    .map(|error| error.to_string().chars().take(1_024).collect::<String>());
                Ok(
                    json!({"restored": true, "kernel_version": 7, "retirement_issue": retirement_issue}),
                )
            }
        }
    }
    pub(crate) fn fork(&mut self, id: &str) -> Result<Self, AgentDispatchErrorV1> {
        match self {
            ...
            Self::Worker(session) => {
                ...
                .map_err(backend)?;
                Ok(Self::Worker(Box::new(WorkerSession::from_snapshot(
                    artifact,
                    session.supervisor.snapshot().map_err(backend)?,
                    seat,
                    role,
    ...
    pub(crate) fn dispose(&mut self) -> Option<String> {
        match self {
            ...
            Self::Worker(session) => session
                ...
                .dispose()
                .err()
                .map(|error| error.to_string().chars().take(1_024).collect()),
        }
    }
    pub(crate) fn reload(
        &mut self,
        ...
        match self {
            Self::Native(_) => Err(invalid(
                "session.reload requires the configured worker backend",
            )),
...
impl WorkerSession {
    fn from_snapshot(
        ...
        let mut endpoint = ChildKernelGenerationV2::spawn_with_limits(
            &artifact,
        ...
        endpoint
            .initialize(
                content.bundle().as_ref().clone(),
                KernelWorkerInitializationV2::Snapshot {
                    snapshot_bytes: serde_json::to_vec(&snapshot).map_err(backend)?,
                    local_seat: seat,
                    role,
                },
            )
            .map_err(backend)?;
        Ok(Self {
            supervisor: CurrentKernelSupervisorV2::new(endpoint, CurrentTailLimitsV2::default())
                .map_err(backend)?,
            artifact,
            content,
            ticket: None,
    ...
    fn context(&self) -> Result<(SeatId, GameKernelRoleV7), AgentDispatchErrorV1> {
        self.supervisor
            .session_context()
            .ok_or_else(|| backend("worker context unavailable"))
    }
    fn reload(&mut self, params: &Value, begin_id: u64) -> Result<Value, AgentDispatchErrorV1> {
        match required::<String>(params, "action")?.as_str() {
            "begin" => {
                ...
                let result = bounded(
                    json!({"ticket": begin_id, "frontier": ticket.frontier(), "identity": ticket.identity()}),
                )?;
            ...
            "activate" => {
                ...
                let (id, ticket) = self
                    .ticket
                    .as_ref()
                    .ok_or_else(|| invalid("no pending reload ticket"))?;
                if *id != expected {
                    return Err(invalid("stale reload ticket"));
                }
                ...
                let prepared = self
                    ...
                    .prepare_reload(
                        ticket.clone(),
                        &artifact,
                        self.content.bundle().as_ref().clone(),
                    )
                    .map_err(backend)?;
                ...
                Ok(
                    json!({"previous_identity": accepted.previous_identity, "active_identity": accepted.active_identity,
