...
fn snapshots(cli: &mut Cli, batch: &str, environments: &[u64]) -> TestResult<Value> {
    cli.result(
        "batch.snapshot",
        json!({"batch": batch, "environments": environments}),
    )
}

fn events(
    cli: &mut Cli,
    references: &mut [CurrentGameSession],
    input: Vec<(u64, CurrentExternalEvent)>,
) -> TestResult {
    let mut expected = Vec::new();
    let mut wire = Vec::new();
    for (ordinal, (environment, event)) in input.into_iter().enumerate() {
        let session = references
            .get_mut(usize::try_from(environment - 1)?)
            .ok_or("reference environment")?;
    ...
    same(
        &cli.result("batch.events", json!({"batch": "live", "events": wire}))?,
...
fn press(cli: &mut Cli, references: &mut [CurrentGameSession], code: PhysicalKey) -> TestResult {
    events(
        cli,
        references,
        vec![(1, key(code.clone(), true)), (1, key(code, false))],
    )
}
...
fn actual_current_batch_preserves_order_timers_rollback_and_fork() -> TestResult {
    ...
    same(
        &cli.result(
            "batch.create",
            json!({"batch": "live", "environments": [
        {"environment": 2, "start": start()}, {"environment": 1, "start": start()}]}),
        )?,
        &json!({"batch": "live", "kernel_version": 7, "environments": [1, 2]}),
    )?;
    assert_eq!(
        references[0].observe()?.control.ok_or("title")?.kind,
        GameControlKindV2::Title
    );
    for _ in 0..3 {
        press(&mut cli, &mut references, PhysicalKey::Space)?;
    }
    let bound = references[0]
        .observe()?
        .control
        .ok_or("starter")?
        .menu
        ...
        + 1;
    for _ in 0..bound {
        if selected(&references[0])? == "bootstrap/starter/confirm" {
            break;
        }
    ...
    assert_eq!(
        references[0].observe()?.control.ok_or("battle")?.kind,
    ...
    let empty_request =
        json!({"protocol_version": 1, "id": "", "method": "batch.events", "params": parameters});
    ...
    assert!(
        ...
        "admission witness must stage actual effects"
    );
    assert_ne!(
        er_canonical::content_digest(&candidate.snapshot()?)?,
        er_canonical::content_digest(&references[0].snapshot()?)?,
        "admission witness must stage a real state change"
    );
    ...
    assert!(
        serde_json::to_vec(&success)?.len() + 1 > 4 << 20,
        "fixture must overflow the success envelope"
    );
    drop(success);
    drop(candidate);
    ...
    assert!(
        rejected["result"].is_null(),
        "oversized success published effects"
    );
    assert_eq!(rejected["error"]["code"], "BACKEND_ERROR");
    assert!(
        rejected["error"]["message"]
            .as_str()
            .is_some_and(|text| text.contains("success response JSONL"))
    );
    drop(rejected);
    drop(long_id);
    assert_eq!(
        er_canonical::content_digest(&snapshots(&mut cli, "live", &[1, 2])?)?,
        before_envelope
    );
    events(
        &mut cli,
        &mut references,
        vec![(1, key(PhysicalKey::ArrowDown, true))],
    )?;
    assert_eq!(selected(&references[0])?, "battle/command/party");
    events(
        &mut cli,
        &mut references,
        vec![(2, time(7)), (1, time(249))],
    )?;
    assert_eq!(selected(&references[0])?, "battle/command/party");
    events(
        &mut cli,
        &mut references,
        vec![(2, time(11)), (1, time(1)), (2, time(13))],
    )?;
    assert_eq!(selected(&references[0])?, "battle/command/fight");
    ...
    cli.rejects(
        "batch.events",
        json!({"batch": "live", "events": [
        {"environment": 1, "event": time(250)},
        {"environment": 2, "event": {"kind": "AUTHORITY_MATERIAL", "bytes": [255]}}]}),
        "event 1",
    )?;
    assert_eq!(
        er_canonical::content_digest(&snapshots(&mut cli, "live", &[2, 1, 2])?)?,
        before
    );
    events(
        &mut cli,
        &mut references,
        vec![(1, time(250)), (2, time(17))],
    )?;
    assert_eq!(selected(&references[0])?, "battle/command/party");
    events(
        &mut cli,
        &mut references,
        vec![(1, time(500)), (1, key(PhysicalKey::ArrowDown, false))],
    )?;
    assert_eq!(selected(&references[0])?, "battle/command/party");
    events(&mut cli, &mut references, vec![(1, time(500))])?;
    assert_eq!(selected(&references[0])?, "battle/command/party");
    same(
        &snapshots(&mut cli, "live", &[2, 1, 2])?,
        &json!({"batch": "live", "kernel_version": 7,
    ...
    cli.result(
        "batch.fork",
        json!({"batch": "live", "source_environment": 1, "target_environment": 3}),
    )?;
    let fork = references[0].snapshot()?;
    press(&mut cli, &mut references, PhysicalKey::ArrowDown)?;
    same(
        &snapshots(&mut cli, "live", &[3])?,
        &json!({"batch": "live", "kernel_version": 7,
    ...
    cli.rejects(
        "batch.snapshot",
        json!({"batch": "live", "environments": [1]}),
        "missing or closed",
    )?;
    assert_eq!(
        cli.result("lab.resources", json!({}))?["total_environments"],
        0
    );
    cli.finish()
}
...
fn actual_current_batch_import_reset_limits_and_global_quota_are_atomic() -> TestResult {
    ...
    cli.rejects(
        "batch.create",
        json!({"batch": "imports", "environments": [
        {"environment": 1, "start": snapshot_start},
        {"environment": 2, "start": {"kind": "CAPSULE", "capsule": bad}}]}),
        "content_identity",
    )?;
    assert_eq!(
        cli.result("lab.resources", json!({}))?["total_environments"],
    ...
    cli.rejects(
        ...
        {"environment": 10, "start": {"kind": "CAPSULE", "capsule": bad}}]}),
        "content_identity",
    )?;
    same(&snapshots(&mut cli, "imports", &[1, 2])?, &before)?;
    cli.result(
        "session.create",
        json!({"session": "standalone", "start": snapshot_start}),
    )?;
    cli.rejects(
        "batch.fork",
        json!({"batch": "imports", "source_environment": 1, "target_environment": 3}),
        "capacity",
    )?;
    cli.rejects(
        "session.create",
        json!({"session": "overflow", "start": start()}),
        "capacity",
    )?;
    assert_eq!(
        cli.result("lab.resources", json!({}))?["total_environments"],
    ...
    same(
        &cli.result(
            "batch.advance_time",
            json!({"batch": "imports", "advances": [
        {"environment": 7, "milliseconds": 5}]}),
        )?,
        &json!({"batch": "imports", "kernel_version": 7,
        "results": [{"ordinal": 0, "environment": 7, "step": next_step, "observation": reference.observe()?}]}),
    )?;
    same(
        &cli.result(
            "batch.observe",
            json!({"batch": "imports", "environments": [7, 7]}),
        )?,
    ...
    let input = RawInputEvent::KeyDown {
    ...
    cli.rejects(
        "batch.raw_input",
        json!({"batch": "tiny", "inputs": [
        {"environment": 1, "input": input}]}),
        "results exceed 2 JSON bytes",
    )?;
    same(&snapshots(&mut cli, "tiny", &[1])?, &tiny_before)?;
    cli.result(
        "batch.reset",
        json!({"batch": "tiny", "environments": [
        {"environment": 1, "start": snapshot_start}]}),
    )?;
    cli.rejects(
        "batch.raw_input",
    ...
    let mut corrected = CurrentGameSession::from_snapshot(
        initial,
        SeatId::new(safe(1)),
        GameKernelRoleV7::Authority,
        Arc::clone(reference.content()),
    )?;
    let step = corrected.apply(CurrentExternalEvent::RawInput {
        input: input.clone(),
    })?;
    assert_ne!(
        er_canonical::content_digest(&corrected.snapshot()?)?,
