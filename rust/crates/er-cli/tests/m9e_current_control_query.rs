...
fn exercise_queries(worker: bool) -> TestResult {
    ...
    for (field, value) in [
        ("maximum_events", json!(0)),
        ("maximum_events", json!(4097)),
        ("maximum_events", json!(1.5)),
        ("submit", json!("true")),
        ("target", json!("")),
        ("target", json!("é".repeat(129))),
        ("expected_control_digest", json!("bad")),
        ("expected_menu_instance", json!(-1)),
        ("unknown", json!(true)),
    ] {
        let mut bad = title_params.clone();
        bad[field] = value;
        reject(
            &cli.request("control.plan_navigation", bad)?,
            "INVALID_REQUEST",
            "",
        );
    }
    ...
    missing
        .as_object_mut()
        .ok_or("parameters")?
        .remove("expected_control_digest");
    reject(
        &cli.request("control.plan_navigation", missing)?,
        "INVALID_REQUEST",
        "missing field",
    );
    reject(
    ...
    reject(
        &cli.request("control.describe", json!({"session": "x".repeat(129)}))?,
        "INVALID_REQUEST",
        "session",
    );
    ...
    reject(
        &cli.request("control.plan_navigation", stale)?,
        "BACKEND_ERROR",
        "digest is stale",
    );
    reject(
        &cli.request(
            ...
            parameters(&title, "unknown-target", false, 4096),
        )?,
        "BACKEND_ERROR",
        "hidden, disabled, or unknown",
    );
    reject(
        &cli.request(
            "control.plan_navigation",
            parameters(&title, "bootstrap/title/new-game", true, 1),
        )?,
        "BACKEND_ERROR",
        "event bound",
    );
    ...
    for input in inputs {
        raw(&mut cli, &mut reference, input)?;
    }
    assert_eq!(
        reference.observe()?.control.ok_or("mode control")?.kind,
        GameControlKindV2::ModeSelect
    );
    let mode = describe(&mut cli, &reference)?;
    assert_ne!(
        mode["description"]["menu_instance"],
        title["description"]["menu_instance"]
    );
    let mode_before = checkpoint(&mut cli, &reference, !worker)?;
    reject(
        &cli.request("control.plan_navigation", title_params)?,
        "BACKEND_ERROR",
        "digest is stale",
    );
    let target = mode["description"]["selected_option"]
        .as_str()
        .ok_or("selected mode")?
        .to_owned();
    ...
    reject(
        &cli.request("control.plan_navigation", stale_instance)?,
        "BACKEND_ERROR",
        "menu instance is stale",
    );
    ...
    assert_eq!(
        moved["description"]["menu_instance"],
        mode["description"]["menu_instance"]
    );
    assert_ne!(
        moved["description"]["selected_option"],
        mode["description"]["selected_option"]
    );
    ...
    reject(
        &cli.request(
            "control.plan_navigation",
            parameters(&mode, &target, false, 4096),
        )?,
        "BACKEND_ERROR",
        "digest is stale",
    );
    let back = cli.result(
        "control.plan_navigation",
        parameters(&moved, &target, false, 4096),
    )?;
    assert_eq!(
        back["plan"]["expected_path"]
            .as_array()
            .ok_or("path")?
            .last(),
        Some(&json!(target))
    ...
    assert_eq!(
        reference.observe()?.control.ok_or("mode")?.kind,
        GameControlKindV2::ModeSelect
    );
    assert!(
        reference.snapshot()?.pending_presentations.is_empty(),
        "short fixture has no presentation ownership claim"
    );
    ...
    for (method, params, duplicate_id) in [
        ...
        (
            "control.plan_navigation",
            parameters(&returned, &target, false, 4096),
            "same-plan",
        ),
    ] {
        ...
        reject(
            &cli.request_id(method, params.clone(), duplicate_id)?,
            "DUPLICATE_REQUEST",
            "",
        );
        if !worker {
            let empty =
                json!({"protocol_version": 1, "id": "", "method": method, "params": params});
            let id = "q".repeat(LINE_BOUND - serde_json::to_vec(&empty)?.len() - 2);
            reject(
                &cli.request_id(method, params, &id)?,
                "BACKEND_ERROR",
    ...
    if !worker {
        ...
        reject(
            &cli.request("control.unimplemented", json!({"session": SESSION}))?,
            "METHOD_NOT_FOUND",
