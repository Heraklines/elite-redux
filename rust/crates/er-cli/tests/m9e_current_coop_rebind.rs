...
use er_repro::current::{
    CurrentReproCapsuleV1, CurrentReproLimitsV1, CurrentReproOutcomeV1, replay_current_capsule_v1,
};
...
fn one_frame(values: &[Vec<u8>]) -> TestResult<Vec<u8>> {
    let [bytes] = values else {
        return Err("exactly one actual frame required".into());
    };
    Ok(bytes.clone())
}
...
impl Endpoint {
    fn new(host: bool) -> TestResult<Self> {
        ...
        let config = if host {
            authority_protocol(authority, guest, generation(1))?
        } else {
            replica_protocol(authority, guest, generation(1))?
        };
        ...
        cli.result(
            "batch.create",
            json!({"batch":"natural-startup","environments":[{"environment":1,"start":{
    ...
    fn ordinary(&mut self, event: CurrentExternalEvent) -> TestResult<GameKernelStepV7> {
        ...
        let response = self
            .cli
            .result("platform.event", json!({"session":SESSION,"event":event}))?;
        assert_eq!(
            response,
            json!({"step":step,"observation":self.session.observe()?})
        );
        Ok(step)
    }
    fn rebind(
        ...
        let response = self.cli.result(
            "session.coop.rebind",
            json!({"session":SESSION,"control":control}),
        )?;
        assert_eq!(
            response,
            json!({"rebind":output,"observation":self.session.observe()?})
        );
        self.checkpoint()?;
        Ok(output)
    }
    fn reject_control(&mut self, control: CurrentCoopRebindEventV1) -> TestResult {
        ...
        let error = self
            .session
            .apply_rebind(control.clone())
            .expect_err("wrong-generation control must fail");
        assert_eq!(self.session.snapshot()?, before);
        let response = self.cli.request(
            "session.coop.rebind",
            json!({"session":SESSION,"control":control}),
        )?;
        ...
        let last = capsule
            .attempts
            .last()
            .ok_or("recorded rejection missing")?;
    ...
    fn press(&mut self) -> TestResult<Vec<Vec<u8>>> {
        ...
        for input in [
            ...
            wire.extend(frames(
                &self.ordinary(CurrentExternalEvent::RawInput { input })?,
            ));
        }
        Ok(wire)
    }
    ...
    fn starters(&mut self) -> TestResult<Vec<Vec<u8>>> {
        ...
        if mode.challenge_selection && self.host {
            wire.extend(self.choose("bootstrap/challenge/done")?);
        }
        let GameKernelLifecycleSnapshotV7::Bootstrap(before) = self.checkpoint()?.lifecycle else {
            return Err("starter stage".into());
        };
        ...
        if self.host {
            for _ in 0..4 {
                ...
                ) || matches!(&snapshot.lifecycle, GameKernelLifecycleSnapshotV7::Bootstrap(value) if value.stage == RunBootstrapStageV1::Complete)
                {
                    break;
                }
                wire.extend(self.press()?);
        ...
        self.checkpoint()?;
        self.begin_capture_after_bootstrap()?;
        Ok(wire)
    }
    fn begin_capture_after_bootstrap(&mut self) -> TestResult {
        assert!(self.bootstrap_batch);
    ...
    fn restore_same(&mut self) -> TestResult {
        ...
        self.cli.result(
            "session.restore",
            json!({"session":SESSION,"snapshot":snapshot}),
        )?;
        assert_eq!(self.checkpoint()?, snapshot);
        Ok(())
    }
    ...
    fn next_frame(&mut self) -> TestResult<Vec<u8>> {
        for _ in 0..8 {
            self.settle()?;
            let values = self.press()?;
            if !values.is_empty() {
                return one_frame(&values);
            }
        }
...
fn reject_actual_control_response(
    peer: &mut Endpoint,
    control: CurrentCoopRebindEventV1,
) -> TestResult {
    ...
    assert_eq!(
        accepted,
        json!({"rebind":expected,"observation":candidate.observe()?})
    );
    assert_eq!(
        peer.cli
            .result("session.snapshot", json!({"session":"admission-probe"}))?,
        serde_json::to_value(candidate.snapshot()?)?
    );
    let suffix: CurrentReproCapsuleV1 = serde_json::from_value(
        peer.cli.result(
    ...
    peer.cli
        .result("session.close", json!({"session":"admission-probe"}))?;
    Ok(())
}
...
fn actual_native_cli_rebind_preserves_capture_admission_restore_and_gameplay() -> TestResult {
    ...
    guest.reject_control(CurrentCoopRebindEventV1::Receive {
        generation: generation(1),
        bytes: wire.clone(),
    })?;
    ...
    for index in 0usize..8 {
        ...
        if index == 2 {
            ...
            peer.cli.result(
                ...
                json!({"session":"restored-midphase","capsule":imported}),
            )?;
            assert_eq!(
                peer.cli
                    .result("session.snapshot", json!({"session":"restored-midphase"}))?,
                serde_json::to_value(&before)?
            );
            let response = peer.cli.result(
                "session.coop.rebind",
                json!({"session":"restored-midphase","control":CurrentCoopRebindEventV1::Retry}),
            )?;
            assert_eq!(
                response,
                json!({"rebind":retried,"observation":peer.session.observe()?})
            );
            peer.cli
    ...
    for peer in [&mut host, &mut guest] {
        ...
        assert!(
            capsule
                ...
                .all(
                    |attempt| attempt.origin.as_deref() == Some("session.coop.rebind")
                        && matches!(
                            &attempt.outcome,
        ...
        let accepted = receives
            .iter()
            .copied()
            .filter(|index| {
                matches!(
                    &capsule.attempts[*index].outcome,
                    CurrentReproOutcomeV1::RebindApplied { .. }
                )
            })
            .collect::<Vec<_>>();
        ...
        missing.attempts.remove(accepted[0]);
        for (index, attempt) in missing.attempts.iter_mut().enumerate() {
            attempt.position = missing.base_position + index as u64 + 1;
        }
        missing.final_position = missing.base_position + missing.attempts.len() as u64;
        assert!(
            replay_current_capsule_v1(&missing, content()?, CurrentReproLimitsV1::default())
                .is_err()
        );
        let denied = peer.cli.request(
            "session.from_capsule",
            json!({"session":"missing-control","capsule":missing}),
        )?;
        assert_eq!(denied["error"]["code"], "BACKEND_ERROR");
    ...
    assert_eq!(
        one_frame(&frames(&host.ordinary(
            CurrentExternalEvent::NetworkFrame {
                generation: generation(2),
                bytes: proposal
            }
    ...
    for (method, code) in [
        ...
        assert_eq!(
            host.cli.request(
                method,
                json!({"session":SESSION,"control":{"kind":"RETRY"}})
            )?["error"]["code"],
            code
        );
        assert_eq!(host.checkpoint()?, before);
