...
use er_types::{ConnectionGeneration, FrameContext, GameControlPlanV2, InputFocus, MembershipRevision, PhysicalKey, RawInputEvent, RunId, SafeU53, SeatId, SessionId, StarterSelectionV1, TimeClass};
use er_web::contracts_v2::{BrowserEffectV2, BrowserRequestEnvelopeV2, BrowserRequestV2, BrowserResponseEnvelopeV2, BrowserResponseV2, BrowserSessionContextV2, BrowserSessionInitializationV2};
...
fn safe(value: u64) -> SafeU53 { SafeU53::new(value).expect("bounded test value") }
fn profile() -> TestResult<ProfileStateV1> {
    Ok(ProfileStateV1 { schema_version: PROFILE_STATE_SCHEMA_VERSION_V1,
        ...
        statistics: ProfileStatistics { runs_started: SafeU53::ZERO, runs_won: SafeU53::ZERO,
            ...
            highest_wave: WaveIndex::new(safe(1))? }, dex: DexState::default() })
}
...
impl Endpoint {
    fn new(content: Arc<PreparedGameContentV2>, worker: bool, host: bool) -> TestResult<Self> {
        ...
        let config = if host { authority_protocol(authority, guest, generation)? } else { replica_protocol(authority, guest, generation)? };
        ...
        let mut endpoint = Self { cli, browser: BrowserKernelHostV2::from_content(content), browser_sequence: 0, host };
        endpoint.browser_request(BrowserRequestV2::Initialize { initialization: Box::new(BrowserSessionInitializationV2::NaturalCoop {
            context: BrowserSessionContextV2 { local_seat: local, role: if host { GameKernelRoleV7::Authority } else { GameKernelRoleV7::Replica },
                scheduler: KernelSchedulerSnapshotV2 { next_timer_id: Some(SafeU53::ZERO), timers: Vec::new(), pauses: Vec::new(), disposed: false },
                protocol: Some(protocol) }, profile: profile()?, seed: SEED.to_owned(), save_slots: vec!["entry-slot".to_owned()], local_is_host: host,
        }) })?;
        endpoint.checkpoint()?;
        Ok(endpoint)
    }
    fn browser_request(&mut self, request: BrowserRequestV2) -> TestResult<BrowserResponseV2> {
        ...
        let bytes = self.browser.process_bytes(&er_canonical::canonical_bytes(&envelope)?)?;
        ...
        assert!(!matches!(response.response, BrowserResponseV2::Fault { .. }), "browser rejected valid entry request");
        self.browser_sequence += 1;
        Ok(response.response)
    }
    fn checkpoint(&mut self) -> TestResult<CoreGameKernelSnapshotV7> {
        let snapshot: CoreGameKernelSnapshotV7 = serde_json::from_value(self.cli.result("session.snapshot", json!({"session":SESSION}))?)?;
        let BrowserResponseV2::Snapshot { snapshot: browser } = self.browser_request(BrowserRequestV2::Snapshot)? else { return Err("browser snapshot absent".into()); };
        assert_eq!(&snapshot, browser.as_ref(), "complete browser-host and actual CLI snapshot diverged");
        Ok(snapshot)
    }
    ...
    fn event(&mut self, event: CurrentExternalEvent) -> TestResult<GameKernelStepV7> {
        let (method, params, request) = match event {
            CurrentExternalEvent::RawInput { input } => ("session.raw_input", json!({"session":SESSION,"input":input}), BrowserRequestV2::RawInput { event: input }),
            CurrentExternalEvent::NetworkFrame { generation, bytes } => ("session.network_frame", json!({"session":SESSION,"generation":generation,"bytes":bytes}), BrowserRequestV2::NetworkFrame { generation: generation.get(), bytes }),
            CurrentExternalEvent::RetryCoopSetup => ("session.coop.retry", json!({"session":SESSION}), BrowserRequestV2::RetryCoopSetup),
        ...
        let BrowserResponseV2::Effects { batch } = self.browser_request(request)? else { return Err("browser effect response absent".into()); };
        let frames = network_frames(&step);
        let browser_frames = batch.effects.iter().filter_map(|effect| match effect {
            BrowserEffectV2::SendNetworkFrame { generation, bytes } => { assert_eq!(*generation, safe(1)); Some(bytes.clone()) }, _ => None,
        }).collect::<Vec<_>>();
        assert_eq!(frames, browser_frames, "actual outgoing startup frames differ by entry");
        let native_presentation = step.effects.iter().filter_map(|effect| match effect { GameKernelEffectV7::Presentation(effect) => Some(effect), _ => None }).collect::<Vec<_>>();
        let browser_presentation = batch.effects.iter().filter_map(|effect| match effect { BrowserEffectV2::Presentation { effect } => Some(effect), _ => None }).collect::<Vec<_>>();
        assert_eq!(native_presentation, browser_presentation);
        if !self.host {
            assert!(step.effects.iter().all(|effect| !matches!(effect, GameKernelEffectV7::Platform(_))));
            assert!(batch.effects.iter().all(|effect| !matches!(effect, BrowserEffectV2::StorageRequest { .. })));
        }
        Ok(step)
    }
    fn press(&mut self, key: PhysicalKey) -> TestResult<Vec<Vec<u8>>> {
        let mut frames = Vec::new();
        for input in [RawInputEvent::KeyDown { code: key.clone(), printable: false, browser_repeat: false, focus: InputFocus::Game }, RawInputEvent::KeyUp { code: key }] {
            frames.extend(network_frames(&self.event(CurrentExternalEvent::RawInput { input })?));
        }
        Ok(frames)
    }
    fn navigate(&mut self, target: &str) -> TestResult {
        ...
        for _ in 0..bound {
            ...
            if menu.selected_option_id.as_str() == target { return Ok(()); }
            let current = menu.options.iter().position(|option| option.option_id == menu.selected_option_id).ok_or("selected option absent")?;
            let wanted = menu.options.iter().position(|option| option.option_id.as_str() == target).ok_or("target absent")?;
            let down = (wanted + menu.options.len() - current) % menu.options.len();
            let up = (current + menu.options.len() - wanted) % menu.options.len();
            assert!(self.press(if up < down { PhysicalKey::ArrowUp } else { PhysicalKey::ArrowDown })?.is_empty());
        }
        Err("raw target unreachable".into())
    }
    fn choose(&mut self, content: &PreparedGameContentV2) -> TestResult<ChoicePublication> {
        let mode = content.bundle().bootstrap.modes.iter().find(|mode| mode.cooperative && mode.supported).ok_or("cooperative mode absent")?;
        ...
        if mode.challenge_selection && self.host { self.navigate("bootstrap/challenge/done")?; frames.extend(self.press(PhysicalKey::Space)?); }
        let GameKernelLifecycleSnapshotV7::Bootstrap(bootstrap) = self.checkpoint()?.lifecycle else { return Err("raw starter menu absent".into()); };
        ...
        for starter in bootstrap.catalog.starters.iter().skip(if self.host { 0 } else { 2 }) {
            if starter.cost <= budget { budget -= starter.cost; chosen.push(starter.clone()); if chosen.len() == count { break; } }
        }
        assert_eq!(chosen.len(), count);
        for starter in &chosen { self.navigate(&format!("bootstrap/starter/{}",starter.pokemon_id.get()))?; frames.extend(self.press(PhysicalKey::Space)?); }
        ...
        if self.host {
            for _ in 0..4 {
                ...
                if matches!(snapshot.lifecycle, GameKernelLifecycleSnapshotV7::Bootstrap(ref setup) if setup.stage == RunBootstrapStageV1::Complete) { break; }
                frames.extend(self.press(PhysicalKey::Space)?);
            }
        }
        Ok((chosen, frames))
    ...
    fn replay_capture(&mut self, content: Arc<PreparedGameContentV2>, native: bool) -> TestResult {
        ...
        if native {
            let exported = self.cli.result("session.capsule.export", json!({"session":SESSION}))?;
            let capsule: CurrentReproCapsuleV1 = serde_json::from_value(exported["capsule"].clone())?;
            assert!(capsule.attempts.iter().any(|attempt| matches!(attempt.event, CurrentExternalEvent::RetryCoopSetup)));
            assert!(capsule.attempts.iter().any(|attempt| matches!(attempt.event, CurrentExternalEvent::NetworkFrame { .. })));
            assert_eq!(replay_current_capsule_v1(&capsule, content.clone(), CurrentReproLimitsV1::default())?.snapshot()?, expected);
        }
        let BrowserResponseV2::Effects { batch } = self.browser_request(BrowserRequestV2::ExportRepro)? else { return Err("browser capsule effect absent".into()); };
        let bytes = batch.effects.iter().find_map(|effect| match effect { BrowserEffectV2::CurrentReproReady { capsule_bytes } => Some(capsule_bytes), _ => None }).ok_or("browser capsule absent")?;
        let capsule: CurrentReproCapsuleV1 = serde_json::from_slice(bytes)?;
        assert!(capsule.attempts.iter().any(|attempt| matches!(attempt.event, CurrentExternalEvent::RetryCoopSetup)));
        assert_eq!(replay_current_capsule_v1(&capsule, content, CurrentReproLimitsV1::default())?.snapshot()?, expected);
        Ok(())
    }
    fn finish(mut self) -> TestResult {
        self.cli.result("session.close", json!({"session":SESSION}))?;
        self.browser_request(BrowserRequestV2::Dispose)?;
        self.cli.finish()
    }
}
fn network_frames(step: &GameKernelStepV7) -> Vec<Vec<u8>> {
    step.effects.iter().filter_map(|effect| match effect { GameKernelEffectV7::ProposalReady { bytes, .. } | GameKernelEffectV7::AuthorityMaterial { bytes, .. } => Some(bytes.clone()), _ => None }).collect()
}
fn exercise(worker: bool) -> TestResult {
    ...
    assert!(matches!(host.checkpoint()?.lifecycle, GameKernelLifecycleSnapshotV7::Bootstrap(ref setup) if setup.stage == RunBootstrapStageV1::Complete));
    ...
    let admitted = host.event(CurrentExternalEvent::NetworkFrame { generation: ConnectionGeneration::new(safe(1)), bytes: choices.clone() })?;
    ...
    let duplicate = host.event(CurrentExternalEvent::NetworkFrame { generation: ConnectionGeneration::new(safe(1)), bytes: choices.clone() })?;
    assert_eq!(network_frames(&duplicate), replies);
    assert_eq!(duplicate.effects.len(), 1);
    let applied = guest.event(CurrentExternalEvent::NetworkFrame { generation: ConnectionGeneration::new(safe(1)), bytes: replies[0].clone() })?;
    ...
    assert_eq!(host_before.pending_presentations, guest_before.pending_presentations);
    let GameKernelLifecycleSnapshotV7::Active(state) = &guest_before.lifecycle else { return Err("co-op did not activate".into()); };
    ...
    assert!(guest.event(CurrentExternalEvent::NetworkFrame { generation: ConnectionGeneration::new(safe(1)), bytes: replies[0].clone() })?.effects.is_empty());
...
#[test]
fn current_native_cli_owned_coop_retry_replay_matches_browser_host() -> TestResult { exercise(false) }
#[test]
fn current_process_worker_owned_coop_retry_replay_matches_browser_host() -> TestResult { exercise(true) }
fn frame(
    sender: SeatId,
    authority: SeatId,
