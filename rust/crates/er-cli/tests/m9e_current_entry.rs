...
impl CommandFiles {
    ...
    fn write(&self, name: &str, value: &impl serde::Serialize) -> Result<PathBuf, Box<dyn Error>> {
...
#[test]
fn normal_new_run_resume_and_simulate_use_current_session_events() -> Result<(), Box<dyn Error>> {
...
fn normal_commands_report_v2_content_and_reject_historical_state_injection() -> Result<(), Box<dyn Error>> {
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("new-run-v6"));
    Ok(())
}
