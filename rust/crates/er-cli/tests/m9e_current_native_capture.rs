...
use er_repro::current::{
    CurrentReproCapsuleV1, CurrentReproLimitsV1, CurrentReproRecorderV1, replay_current_capsule_v1,
};
...
fn active(content: Arc<PreparedGameContentV2>) -> TestResult<CurrentGameSession> {
    ...
    let snapshot = CHECKPOINT.get_or_init(|| {
        build_active(Arc::clone(&content))
            .expect("real natural route")
            .snapshot()
...
fn build_active(content: Arc<PreparedGameContentV2>) -> TestResult<CurrentGameSession> {
    ...
    assert_eq!(
        session.observe()?.control.ok_or("battle")?.kind,
...
fn event(
    ...
    let (method, params) = match &event {
        CurrentExternalEvent::RawInput { input } => {
            ("session.raw_input", json!({"session": id, "input": input}))
        }
        CurrentExternalEvent::AdvanceTime { milliseconds } => (
            "session.advance_time",
            json!({"session": id, "milliseconds": milliseconds}),
        ),
        _ => ("platform.event", json!({"session": id, "event": event})),
    };
    same(
        &cli.result(method, params)?,
        &json!({"step": step, "observation": reference.observe()?}),
    )?;
    same(
        &cli.result("session.snapshot", json!({"session": id}))?,
...
fn actual_native_capture_replays_natural_events_rejections_and_imported_history() -> TestResult {
    ...
    assert_eq!(
        cli.result("protocol.hello", json!({}))?["capture"],
        json!({"supported": true, "scope": "STANDALONE_NATIVE", "methods": ["session.capsule.export", "session.capsule.status"]})
    );
    cli.result(
        "session.create",
        json!({"session": "natural", "start": start()}),
    )?;
    assert_eq!(
        reference.observe()?.control.ok_or("title")?.kind,
    ...
    event(
        ...
        "natural",
        &mut reference,
        key(PhysicalKey::Space, true),
    )?;
    event(
        &mut cli,
        "natural",
        &mut reference,
        key(PhysicalKey::Space, false),
    )?;
    event(&mut cli, "natural", &mut reference, time(249))?;
    let invalid = CurrentExternalEvent::PresentationOutcome {
        event_id: er_types::PresentationEventId::new(safe(999_999)),
        outcome: KernelPresentationOutcomeV2::Settled,
    };
    assert!(reference.apply(invalid.clone()).is_err());
    cli.rejects(
        "platform.event",
        json!({"session": "natural", "event": invalid}),
        "invalid",
    )?;
    ...
    assert!(matches!(
        capsule.attempts.last().ok_or("rejection")?.outcome,
        er_repro::current::CurrentReproOutcomeV1::KernelRejected { .. }
    ));
    cli.result(
        "session.from_capsule",
        json!({"session": "import", "capsule": capsule}),
    )?;
    ...
    assert_eq!(
        continued.attempts.last().ok_or("time")?.origin.as_deref(),
        Some("session.advance_time")
    );
...
fn actual_native_capture_rotation_fork_restore_and_byte_gaps_are_explicit() -> TestResult {
    ...
    assert_eq!(
        (
            rotated.base_position,
            rotated.final_position,
            rotated.attempts.len()
        ),
        (2, 3, 1)
    );
    ...
    cli.result(
        "session.fork",
        json!({"session": "active", "target_session": "fork"}),
    )?;
    ...
    assert!(
        quiet.apply(time(500))?.effects.is_empty(),
        "released navigation must not repeat"
    );
    event(&mut cli, "fork", &mut fork, time(500))?;
    assert_eq!(selected(&fork)?, "battle/command/fight");
    ...
    cli.result(
        "session.restore",
        json!({"session": "fork", "snapshot": original}),
    )?;
    ...
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "tiny"}))?["status"]["kind"],
        "UNAVAILABLE"
    );
    let mut tiny = restored.fork()?;
    event(&mut cli, "tiny", &mut tiny, time(1))?;
    assert!(
        cli.request("session.capsule.export", json!({"session": "tiny"}))?["error"].is_object()
    );
    cli.result("session.close", json!({"session": "fork"}))?;
    assert!(
        cli.request("session.capsule.status", json!({"session": "fork"}))?["error"].is_object()
    );
    ...
    let exhausted = CurrentReproRecorderV1::new_at_position(
        restored.snapshot()?,
        SeatId::new(safe(1)),
        GameKernelRoleV7::Authority,
        Arc::clone(restored.content()),
    ...
    cli.result(
        "session.from_capsule",
        json!({"session": "maximum", "capsule": exhausted}),
    )?;
    assert_ne!(
        restored.snapshot()?,
        reference.snapshot()?,
        "restore must change actual gameplay state"
    );
    cli.result(
        "session.restore",
        json!({"session": "maximum", "snapshot": reference.snapshot()?}),
    )?;
    same(
        &cli.result("session.snapshot", json!({"session": "maximum"}))?,
        &serde_json::to_value(reference.snapshot()?)?,
    )?;
    ...
    assert!(
        cli.request("session.capsule.export", json!({"session": "maximum"}))?["error"].is_object()
    );
    let mut maximum_reference = reference.fork()?;
    event(&mut cli, "maximum", &mut maximum_reference, time(500))?;
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "maximum"}))?["status"]["position"],
        maximum
    );
    cli.finish()
}
    ...
    fn terminal(mut self) -> TestResult<ReplayOutput> {
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                return Err("terminal exit deadline".into());
            }
        ...
        while !self.stderr.as_ref().ok_or("terminal stderr")?.is_finished() {
            if Instant::now() >= deadline {
                return Err("terminal stderr deadline".into());
            }
            std::thread::sleep(Duration::from_millis(10));
        }
        let stderr = self
            .stderr
            .take()
            .ok_or("terminal stderr")?
            .join()
            .map_err(|_| "terminal stderr reader panicked")?;
...
fn actual_native_capture_late_response_and_rejected_ingress_preserve_gameplay() -> TestResult {
    ...
    let empty =
        json!({"protocol_version": 1, "id": "", "method": "platform.event", "params": parameters});
    ...
    drop(long_id);
    same(
        &cli.result("session.snapshot", json!({"session": "active"}))?,
        &serde_json::to_value(&before)?,
    )?;
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "active"}))?["status"]["kind"],
        "UNAVAILABLE"
    );
    ...
    assert_eq!(
        (
            recovered.base_position,
            recovered.final_position,
    ...
    let rejected_export = cli.request_id(
        "session.capsule.export",
        json!({"session": "active"}),
        &export_id,
    )?;
    ...
    assert_eq!(
        cli.request_id("session.advance_time", params, "typed-id")?["error"]["code"],
        "DUPLICATE_REQUEST"
    );
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "active"}))?["status"]["kind"],
    ...
    let other_reference = CurrentGameSession::from_snapshot(
        before.clone(),
        SeatId::new(safe(1)),
        GameKernelRoleV7::Authority,
        Arc::clone(reference.content()),
    ...
    let bad = cli.request(
        "session.raw_input",
        json!({"session": "active", "input": {"invalid": true}}),
    )?;
    ...
    assert_eq!(
        cli.request(
            "session.advance_time",
            json!({"session": "", "milliseconds": 1})
        )?["error"]["code"],
        "INVALID_REQUEST"
    );
    for id in ["active", "other"] {
        assert_eq!(
            cli.result("session.capsule.status", json!({"session": id}))?["status"]["kind"],
            "UNAVAILABLE"
        );
    }
    event(&mut cli, "active", &mut reference, time(1))?;
    assert_eq!(
        cli.request(
            "session.advance_time",
            json!({"session": "x".repeat(129), "milliseconds": 1})
        )?["error"]["code"],
        "INVALID_REQUEST"
    );
    assert_eq!(
    ...
    event(&mut cli, "active", &mut reference, time(1))?;
    assert_eq!(
        cli.raw(&vec![b'x'; (4 << 20) + 1])?["error"]["code"],
        "REQUEST_TOO_LARGE"
    );
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "active"}))?["status"]["kind"],
    ...
    same(
        &cli.result("session.snapshot", json!({"session": "other"}))?,
        &serde_json::to_value(before)?,
    )?;
    event(&mut cli, "active", &mut reference, time(1))?;
    ...
    assert_eq!(
        final_capsule.final_position,
        final_capsule.base_position + 1
    );
    cli.finish()
}
...
fn actual_native_capture_browser_import_declares_native_suffix_at_original_frontier() -> TestResult
{
    ...
    let mut browser =
        er_web::host_v2::BrowserKernelHostV2::from_content(Arc::clone(reference.content()));
    let mut send = |sequence: u64, request: BrowserRequestV2| -> TestResult<BrowserResponseV2> {
        ...
        let response: BrowserResponseEnvelopeV2 = serde_json::from_slice(
            &browser.process_bytes(&er_canonical::canonical_bytes(&envelope)?)?,
        )?;
        ...
        BrowserRequestV2::Initialize {
            initialization: Box::new(BrowserSessionInitializationV2::Snapshot {
                context: BrowserSessionContextV2 {
                    ...
                    scheduler: initial.scheduler.clone(),
                    protocol: initial.protocol.clone(),
                },
                snapshot: initial,
            }),
        },
    ...
    let BrowserResponseV2::Effects { batch } = send(2, BrowserRequestV2::ExportRepro)? else {
        return Err("browser export effects".into());
    };
    let bytes = batch
        .effects
        .into_iter()
    ...
    cli.result(
        "session.from_capsule",
        json!({"session": "native", "capsule": capsule}),
    )?;
    let suffix = export(&mut cli, "native", &reference)?;
    assert_eq!(
        (suffix.base_position, suffix.final_position),
        (capsule.final_position, capsule.final_position)
    );
    ...
    assert!(
        continued
            .attempts
            .iter()
            .all(|attempt| attempt.browser_transport.is_none())
    );
    cli.finish()
}
...
impl ReplayFiles {
    ...
    fn run(&self, command: &str, bytes: &[u8]) -> TestResult<ReplayOutput> {
        assert!(
            bytes.len() < 4 << 20,
            "fixture must pass the independent file-size bound"
        );
...
fn normal_replay_accepts_extended_native_capture(cli: &mut Cli) -> TestResult {
    ...
    assert_eq!(
        limits.maximum_events, 256,
        "recorder default must remain unchanged"
    );
    ...
    cli.result(
        "session.create",
        json!({"session": "wide", "start": start(),
        "capture_limits": {"maximum_events": maximum, "maximum_bytes": limits.maximum_bytes}}),
    )?;
    assert_eq!(
        cli.result("session.capsule.status", json!({"session": "wide"}))?["limits"],
        json!({"maximum_events": 4096, "maximum_bytes": 2 << 20})
    );
    cli.rejects(
        "session.create",
        json!({"session": "too-wide", "start": start(),
        "capture_limits": {"maximum_events": maximum + 1, "maximum_bytes": limits.maximum_bytes}}),
        "limits",
    )?;
    let mut session = reference(content()?)?;
    for _ in 0..=limits.maximum_events {
        event(cli, "wide", &mut session, time(0))?;
    }
    ...
    assert!(
        ...
        "real exported capsule must fit the unchanged byte cap"
    );
    ...
    for command in ["replay", "capsule-validate"] {
        ...
        assert!(
            succeeded,
            "{command} rejected a supported native capture: {diagnostic}"
        );
    ...
    oversized
        .attempts
        .first_mut()
        .ok_or("oversize attempt")?
        .origin = Some("x".repeat(limits.maximum_bytes));
    ...
    for command in ["replay", "capsule-validate"] {
        ...
        assert!(
            diagnostic.contains("capsule bounds"),
            "wrong replay rejection: {diagnostic}"
        );
    }
