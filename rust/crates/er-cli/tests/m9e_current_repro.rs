...
fn safe(value: u64) -> SafeU53 { SafeU53::new(value).expect("safe fixture integer") }
...
struct Browser { host: BrowserKernelHostV2, sequence: u64 }
impl Browser {
    fn send(&mut self, request: BrowserRequestV2) -> TestResult<BrowserResponseV2> {
        let request = BrowserRequestEnvelopeV2 {
            version: BROWSER_WORKER_PROTOCOL_VERSION_V2, request_id: safe(self.sequence + 1),
            sequence: safe(self.sequence), request,
        };
        let bytes = self.host.process_bytes(&er_canonical::canonical_bytes(&request)?)?;
    ...
    fn press(&mut self, code: PhysicalKey) -> TestResult {
        self.send(BrowserRequestV2::RawInput { event: key(code.clone(), true) })?;
        self.send(BrowserRequestV2::RawInput { event: key(code, false) })?;
        Ok(())
    }
    fn selected(&self) -> TestResult<String> {
        Ok(self.host.kernel_ref().ok_or("kernel missing")?.current_control()
            .ok_or("control missing")?.menu.as_ref().ok_or("menu missing")?
            .selected_option_id.as_str().to_owned())
    }
}

struct Fixture {
    content: Arc<PreparedGameContentV2>, capsule: CurrentReproCapsuleV1,
    snapshot: CoreGameKernelSnapshotV7,
}
impl Fixture {
    fn reference(&self) -> TestResult<CurrentGameSession> {
        Ok(CurrentGameSession::from_snapshot(self.snapshot.clone(), self.capsule.local_seat,
            self.capsule.role, Arc::clone(&self.content))?)
    }
    fn summary(&self) -> TestResult<Value> {
        Ok(json!({"kernel_version": 7, "processed_attempts": self.capsule.attempts.len(),
...
fn create_fixture() -> TestResult<Fixture> {
    ...
    let mut browser = Browser { host: BrowserKernelHostV2::from_bundle_bytes(&bytes)?, sequence: 0 };
    ...
    assert!(matches!(browser.send(BrowserRequestV2::Initialize {
        initialization: Box::new(BrowserSessionInitializationV2::NaturalStart {
            context: BrowserSessionContextV2 {
                local_seat: SeatId::new(safe(1)), role: GameKernelRoleV7::Authority,
                scheduler: KernelSchedulerSnapshotV2 { next_timer_id: Some(SafeU53::ZERO),
                    timers: Vec::new(), pauses: Vec::new(), disposed: false }, protocol: None,
            }, profile, seed: "current-browser-cli-repro".to_owned(),
            save_slots: vec!["preview-slot".to_owned()], local_is_host: true,
        }),
    })?, BrowserResponseV2::Ready));
    assert_eq!(browser.host.kernel_ref().ok_or("kernel")?.current_control().ok_or("title")?.kind,
        GameControlKindV2::Title);
    for _ in 0..3 { browser.press(PhysicalKey::Space)?; }
    let bound = browser.host.kernel_ref().ok_or("kernel")?.current_control().ok_or("starter")?
        .menu.as_ref().ok_or("starter menu")?.options.len() + 1;
    for _ in 0..bound {
        if browser.selected()? == "bootstrap/starter/confirm" { break; }
        browser.press(PhysicalKey::ArrowDown)?;
    }
    assert_eq!(browser.selected()?, "bootstrap/starter/confirm");
    for _ in 0..4 { browser.press(PhysicalKey::Space)?; }
    let pending = browser.host.kernel_ref().ok_or("kernel")?.snapshot()?.pending_presentations;
    assert!(!pending.is_empty(), "natural battle emits presentations to acknowledge");
    ...
    assert_eq!(browser.host.kernel_ref().ok_or("kernel")?.current_control().ok_or("battle")?.kind,
        GameControlKindV2::BattleCommand);
    assert_eq!(browser.selected()?, "battle/command/fight");
    browser.send(BrowserRequestV2::RawInput { event: key(PhysicalKey::ArrowDown, true) })?;
    assert_eq!(browser.selected()?, "battle/command/party");
    browser.send(BrowserRequestV2::AdvanceTime { milliseconds: safe(249) })?;
    assert_eq!(browser.selected()?, "battle/command/party");
    browser.send(BrowserRequestV2::AdvanceTime { milliseconds: safe(1) })?;
    assert_eq!(browser.selected()?, "battle/command/fight");
    browser.send(BrowserRequestV2::RawInput { event: key(PhysicalKey::ArrowDown, false) })?;
    for event in [BrowserLifecycleEventV2::Hidden, BrowserLifecycleEventV2::Visible] {
        browser.send(BrowserRequestV2::Lifecycle { event })?;
    }
    browser.send(BrowserRequestV2::AdvanceTime { milliseconds: safe(500) })?;
    assert_eq!(browser.selected()?, "battle/command/fight");
    ...
    assert_eq!(capsule.final_position, capsule.base_position + capsule.attempts.len() as u64);
    for milliseconds in [249, 1, 500] {
        assert!(capsule.attempts.iter().any(|attempt|
            attempt.event == CurrentExternalEvent::AdvanceTime { milliseconds: safe(milliseconds) }));
    }
    for origin in ["browser.lifecycle.HIDDEN", "browser.lifecycle.VISIBLE"] {
        assert!(capsule.attempts.iter().any(|attempt| attempt.origin.as_deref() == Some(origin)));
    }
    assert!(capsule.attempts.iter().any(|attempt| matches!(attempt.event,
        CurrentExternalEvent::PresentationOutcome { .. })));
    let transport = capsule.browser_transport.as_ref().ok_or("browser transport missing")?;
    assert_eq!((transport.base_generation, transport.final_generation), (safe(1), safe(1)));
    assert!(matches!(browser.send(BrowserRequestV2::Dispose)?, BrowserResponseV2::Disposed));
    Ok(Fixture { content, capsule, snapshot })
}
...
impl TempDirectory {
    fn new() -> TestResult<Self> {
        ...
        let path = std::env::temp_dir().join(format!("m9e-current-repro-{}-{nonce}", std::process::id()));
...
struct Cli {
    child: Child, input: Option<ChildStdin>, output: BufReader<ChildStdout>, next_id: u64,
...
impl Cli {
    fn spawn(mut command: Command) -> TestResult<Self> {
        ...
        Ok(Self { child, input, output, next_id: 0, stderr: Some(stderr) })
    }
    fn read(&mut self) -> TestResult<Value> {
        let mut line = Vec::new();
        self.output.by_ref().take(MAX_LINE_BYTES + 1).read_until(b'\n', &mut line)?;
    ...
    fn request(&mut self, method: &str, params: Value) -> TestResult<Value> {
        ...
        serde_json::to_writer(&mut *input, &json!({"protocol_version": 1, "id": id, "method": method, "params": params}))?;
        input.write_all(b"\n")?;
        input.flush()?;
        let response = self.read()?;
        assert_eq!(response["protocol_version"], 1);
        assert_eq!(response["id"], id, "unsolicited output or response ordering changed");
        Ok(response)
    }
    fn result(&mut self, method: &str, params: Value) -> TestResult<Value> {
        let mut response = self.request(method, params)?;
        assert!(response["error"].is_null(), "{method} rejected: {}", response["error"]);
        Ok(response["result"].take())
    }
    fn finish(mut self) -> TestResult {
        drop(self.input.take());
        let mut extra = [0_u8; 1];
        assert_eq!(self.output.read(&mut extra)?, 0, "unexpected extra output, including replayed effects");
        let status = self.child.wait()?;
        let stderr = self.stderr.take().ok_or("stderr reader")?.join().map_err(|_| "stderr reader panicked")??;
        assert!(status.success(), "CLI failed (bounded stderr prefix): {}", String::from_utf8_lossy(&stderr));
        Ok(())
    }
}
impl Drop for Cli {
...
fn import(cli: &mut Cli, fixture: &Fixture) -> TestResult {
    let actual = cli.result("session.from_capsule", json!({"session": SESSION, "capsule": fixture.capsule}))?;
    ...
    assert_json(&cli.result("session.snapshot", json!({"session": SESSION}))?, &serde_json::to_value(&fixture.snapshot)?)?;
    assert_json(&cli.result("session.observe", json!({"session": SESSION}))?, &serde_json::to_value(fixture.reference()?.observe()?)?)
}
fn continue_session(cli: &mut Cli, reference: &mut CurrentGameSession) -> TestResult {
    for event in [
        CurrentExternalEvent::RawInput { input: key(PhysicalKey::ArrowDown, true) },
        CurrentExternalEvent::AdvanceTime { milliseconds: safe(250) },
        CurrentExternalEvent::RawInput { input: key(PhysicalKey::ArrowDown, false) },
    ] {
        let step = reference.apply(event.clone())?;
        assert_json(&cli.result("platform.event", json!({"session": SESSION, "event": event}))?,
            &json!({"step": step, "observation": reference.observe()?}))?;
        assert_json(&cli.result("session.snapshot", json!({"session": SESSION}))?,
            &serde_json::to_value(reference.snapshot()?)?)?;
    }
    assert_eq!(reference.observe()?.control.ok_or("control")?.menu.ok_or("menu")?
        .selected_option_id.as_str(), "battle/command/fight");
...
fn native_browser_capsule_replays_through_actual_cli_and_continues() -> TestResult {
    ...
    let mut command = Command::new(env!("CARGO_BIN_EXE_er-cli"));
    command.args(["replay", "--capsule"]).arg(&path).arg("--content").arg(content_path());
...
fn actual_worker_capsule_import_rejects_tampering_without_claiming_session_id() -> TestResult {
    ...
    let identity = KernelGenerationIdentityV2 {
        ...
        worker_abi_version: KERNEL_WORKER_ABI_VERSION_V2, minimum_snapshot_schema: 7, maximum_snapshot_schema: 7,
    ...
    command.arg("--worker-executable").arg(artifact.executable()).arg("--worker-root")
        .arg(artifact.allowed_root()).arg("--worker-identity").arg(identity_path);
    let mut cli = Cli::spawn(command)?;
    assert_eq!(cli.result("protocol.hello", json!({}))?["backend"], "WORKER_V2");
    for tamper_time in [false, true] {
        let mut bad = fixture.capsule.clone();
        let message = if tamper_time {
            let attempt = bad.attempts.iter_mut().find(|attempt|
                attempt.event == CurrentExternalEvent::AdvanceTime { milliseconds: safe(249) })
                .ok_or("249ms attempt missing")?;
            attempt.event = CurrentExternalEvent::AdvanceTime { milliseconds: safe(248) };
            "current replay diverged at attempt"
        } else {
            bad.content_identity.bundle_hash.push('0');
            "content_identity"
        };
        let response = cli.request("session.from_capsule", json!({"session": SESSION, "capsule": bad}))?;
        ...
        assert!(missing["error"]["message"].as_str().is_some_and(|text| text.contains("missing or closed")));
    }
    import(&mut cli, fixture)?;
    assert_json(&cli.result("session.fork", json!({"session": SESSION, "target_session": "isolated"}))?,
        &json!({"session": "isolated", "kernel_version": 7}))?;
    continue_session(&mut cli, &mut fixture.reference()?)?;
    assert_json(&cli.result("session.snapshot", json!({"session": "isolated"}))?, &serde_json::to_value(&fixture.snapshot)?)?;
    for session in ["isolated", SESSION] {
        assert_json(&cli.result("session.close", json!({"session": session}))?,
