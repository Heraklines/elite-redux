...
impl Fixture {
    fn variant(&self) -> Result<Self, Box<dyn Error>> {
        ...
        assert_eq!(
            std::env::var("ER_M9E_RULE_WORKER_PARENT_SHA")?,
...
impl Script {
    ...
    fn double_navigation_tick(&mut self, milliseconds: u64) -> Result<(), Box<dyn Error>> {
        ...
        expected
            ...
            .as_mut()
            .ok_or("private control owner")?
            .return_control = party_control.clone();
        assert_eq!(expected.scheduler.timers.len(), 1);
        assert_eq!(expected.scheduler.timers[0].remaining_active_ms, safe(250));
        assert_eq!(
            expected.replay_sequence,
            safe(before.replay_sequence.get() + 1)
        );
...
-> Result<(), Box<dyn Error>> {
    ...
    script.push(
        "session.reload",
        json!({"session": SESSION, "action": "activate", "ticket": 3,
        "executable": fixture.executable, "identity": divergent}),
        Expected::Error {
            code: "BACKEND_ERROR",
            message: format!(
                "exact-preservation mismatch at event position {}: ordered effects or observation",
                script.frontier
            ),
        },
    );
