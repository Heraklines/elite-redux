...
use er_types::{GameContentIdentity, GameControlKindV2, GameControlPlanV2, InputFocus, PhysicalKey,
...
fn reject(response: &Value, code: &str, message: &str) {
    ...
    assert!(response["error"]["message"].as_str().is_some_and(|text| text.contains(message)),
        "wrong error category: {}", response["error"]);
}
...
fn expected_fields(snapshot: &CoreGameKernelSnapshotV7) -> TestResult<Vec<(Value, Vec<u8>)>> {
    let (profile, run, control) = match &snapshot.lifecycle {
        GameKernelLifecycleSnapshotV7::Bootstrap(value) => (&value.profile, None, Some(&value.control)),
        GameKernelLifecycleSnapshotV7::Active(state) =>
            (&state.profile, state.active_run.as_ref(), state.active_run.as_ref().map(|run| &run.control)),
        GameKernelLifecycleSnapshotV7::Terminal { state, control, .. } =>
            (&state.profile, state.active_run.as_ref(), Some(control)),
    };
    let mut fields = vec![
        (json!({"kind": "PROFILE"}), er_canonical::canonical_bytes(profile)?),
        (json!({"kind": "RUN"}), er_canonical::canonical_bytes(&run)?),
    ];
    if let Some(control) = control {
        fields.push((json!({"kind": "CONTROL"}), er_canonical::canonical_bytes(control)?));
    }
    if let Some(run) = run {
        let pokemon = run.party.first().ok_or("natural party missing")?;
        fields.extend([
            (json!({"kind": "PARTY"}), er_canonical::canonical_bytes(&run.party)?),
            (json!({"kind": "POKEMON", "value": pokemon.id}), er_canonical::canonical_bytes(pokemon)?),
            (json!({"kind": "BATTLE"}), er_canonical::canonical_bytes(&run.battle)?),
            (json!({"kind": "WORLD"}), er_canonical::canonical_bytes(&run.world)?),
            (json!({"kind": "PROGRESSION"}), er_canonical::canonical_bytes(&run.progression_queue)?),
            (json!({"kind": "SCENARIO"}), er_canonical::canonical_bytes(&run.scenario)?),
        ]);
    }
    Ok(fields)
}
...
fn query_batch(cli: &mut Cli, reference: &CurrentGameSession, native: bool) -> TestResult {
    ...
    if let Some(capture) = &before.1 {
        assert_eq!(capture["status"]["kind"], "AVAILABLE", "query conservation needs an available capture");
    }
    ...
    for method in ["state.query", "state.inspect"] {
        for (query, bytes) in &fields {
            ...
            same(&cli.result(method, query_params(query.clone(), bytes.len()))?, &expected)?;
            reject(&cli.request(method, query_params(query.clone(), bytes.len() - 1))?, "BACKEND_ERROR", "bound is invalid");
        }
        fields.reverse();
    }
    if fields.len() == 3 {
        ...
        for method in ["state.query", "state.inspect"] {
            for query in [json!({"kind":"PARTY"}), json!({"kind":"POKEMON","value":1}),
                json!({"kind":"BATTLE"}), json!({"kind":"WORLD"}), json!({"kind":"PROGRESSION"}), json!({"kind":"SCENARIO"})] {
                reject(&cli.request(method, query_params(query, QUERY_BOUND))?, "BACKEND_ERROR", "state query path does not exist");
            }
        }
    }
...
fn drain_presentations(cli: &mut Cli, reference: &mut CurrentGameSession, native: bool,
    settlements: &mut usize) -> TestResult {
    let pending = reference.snapshot()?.pending_presentations;
    if !pending.is_empty() && *settlements == 0 {
        assert!(matches!(reference.snapshot()?.lifecycle, GameKernelLifecycleSnapshotV7::Active(_)), "positive ownership witness must be current Active");
    ...
    for _ in 0..32 {
        let Some(pending) = reference.snapshot()?.pending_presentations.first().cloned() else { return Ok(()); };
        ...
        same(&cli.result("session.presentation_settled", json!({"session": SESSION, "event_id": pending.event_id,
                "outcome": "SETTLED"}))?, &json!({"step": step, "observation": reference.observe()?}))?;
        *settlements += 1;
        same(&cli.result("session.snapshot", json!({"session": SESSION}))?, &serde_json::to_value(reference.snapshot()?)?)?;
    }
...
fn press(cli: &mut Cli, reference: &mut CurrentGameSession, code: PhysicalKey, native: bool,
    settlements: &mut usize) -> TestResult {
...
fn natural_active(cli: &mut Cli, reference: &mut CurrentGameSession, native: bool) -> TestResult {
    ...
    for _ in 0..3 { press(cli, reference, PhysicalKey::Space, native, &mut settlements)?; }
    ...
    let control = observation.control.as_ref().ok_or("starter control missing")?;
    let menu = control.menu.as_ref().ok_or("starter menu missing")?;
    let before_plan = checkpoint(cli, reference, native)?;
    let planned = cli.result("control.plan_navigation", json!({"session":SESSION,
    ...
    assert_eq!(reference.observe()?.control.as_ref().and_then(|control| control.menu.as_ref())
        .map(|menu| menu.selected_option_id.as_str()), Some("bootstrap/starter/confirm"),
        "actual raw setup did not select starter confirmation");
    for _ in 0..4 { press(cli, reference, PhysicalKey::Space, native, &mut settlements)?; }
    assert!(settlements > 0, "natural fixture did not exercise real presentation callbacks");
    assert!(reference.snapshot()?.pending_presentations.is_empty());
    assert!(matches!(reference.snapshot()?.lifecycle, GameKernelLifecycleSnapshotV7::Active(_)));
    assert_eq!(reference.observe()?.control.ok_or("battle control")?.kind, GameControlKindV2::BattleCommand);
    Ok(())
}
...
fn rejection_batch(cli: &mut Cli, reference: &CurrentGameSession, native: bool) -> TestResult {
    ...
    for method in ["state.query", "state.inspect"] {
        ...
        for (field, value) in [("maximum_bytes", json!(0)), ("maximum_bytes", json!(-1)),
            ("maximum_bytes", json!(1.5)), ("maximum_bytes", json!(true)), ("maximum_bytes", json!(QUERY_BOUND+1)),
            ("maximum_bytes", json!(u64::MAX)), ("session", json!("")), ("session", json!("é".repeat(65))),
            ("query", json!({"kind":"PROFILE","ignored":true})), ("query", json!({"kind":"PROFILE","value":1})),
            ("query", json!({"kind":"POKEMON","value":true})), ("query", json!({"kind":"POKEMON","value":1,"ignored":1})),
            ("query", json!({"kind":"UNKNOWN"})), ("query", json!({"kind":"POKEMON","value":-1})),
            ("query", json!({"kind":"POKEMON","value":9007199254740992_u64})), ("ignored", json!(true))] {
        ...
        for field in ["query", "maximum_bytes", "session"] {
            let mut bad = valid.clone(); bad.as_object_mut().ok_or("params")?.remove(field);
            reject(&cli.request(method, bad)?, "INVALID_REQUEST", "missing field");
        }
        reject(&cli.request(method, query_params(json!({"kind":"POKEMON","value":9007199254740991_u64}), QUERY_BOUND))?,
            "BACKEND_ERROR", "state query path does not exist");
        let duplicate = format!("duplicate-{method}");
        assert!(cli.request_id(method, valid.clone(), &duplicate)?["error"].is_null());
        reject(&cli.request_id(method, valid.clone(), &duplicate)?, "DUPLICATE_REQUEST", "");
        let empty = json!({"protocol_version":1,"id":"","method":method,"params":valid});
        let id = "s".repeat(LINE_BOUND - serde_json::to_vec(&empty)?.len() - 2);
        reject(&cli.request_id(method, valid.clone(), &id)?, "BACKEND_ERROR", "success response JSONL");
    }
    unchanged(cli, &before)
}

fn historical_selector_parity(reference: &CurrentGameSession, content: &PreparedGameContentV2) -> TestResult {
    let snapshot = reference.snapshot()?;
    let GameKernelLifecycleSnapshotV7::Active(state) = &snapshot.lifecycle else { return Err("active fixture required".into()); };
    ...
    let mut historical = GameStateV5 {
        schema_version: 5,
        content_identity: GameContentIdentity { oracle_sha: identity.oracle_sha.clone(), content_hash: identity.bundle_hash.clone(),
    ...
    for (selector, bytes) in expected_fields(&snapshot)? {
        ...
        assert_eq!(result.digest, digest(&serde_json::from_slice::<Value>(&bytes)?)?);
        assert_eq!(result.query, query);
        assert_eq!(er_lab::query_state_v1(&historical, query.clone(), 0), Err(er_lab::LabQueryErrorV1::Invalid));
        assert_eq!(er_lab::query_state_v1(&historical, query, bytes.len()-1), Err(er_lab::LabQueryErrorV1::Invalid));
    }
    historical.active_run = None;
    for selector in [json!({"kind":"PARTY"}), json!({"kind":"POKEMON","value":1}), json!({"kind":"BATTLE"}),
        json!({"kind":"CONTROL"}), json!({"kind":"WORLD"}), json!({"kind":"PROGRESSION"}), json!({"kind":"SCENARIO"})] {
        let query: er_lab::StateQueryV1 = serde_json::from_value(selector)?;
        assert_eq!(er_lab::query_state_v1(&historical, query.clone(), 0), Err(er_lab::LabQueryErrorV1::Invalid));
        assert_eq!(er_lab::query_state_v1(&historical, query, QUERY_BOUND), Err(er_lab::LabQueryErrorV1::StatePath));
    }
    assert_eq!(er_lab::query_state_v1(&historical, er_lab::StateQueryV1::Run, 4)?.canonical_bytes, b"null");
    Ok(())
}

fn controlled_terminal(reference: &CurrentGameSession, content: Arc<PreparedGameContentV2>) -> TestResult<CurrentGameSession> {
    let snapshot = reference.snapshot()?;
    assert!(snapshot.pending_presentations.is_empty() && snapshot.pending_platform.is_empty());
    let GameKernelLifecycleSnapshotV7::Active(mut state) = snapshot.lifecycle else { return Err("active fixture required".into()); };
    ...
    let fresh = CurrentGameSession::from_active(state, SafeU53::new(1)?, SeatId::new(SafeU53::new(1)?),
        GameKernelRoleV7::Authority, Arc::clone(&content), snapshot.input_router, snapshot.scheduler, None)?;
    let mut terminal = fresh.snapshot()?;
    assert!(terminal.material_ledger.records.is_empty());
    let GameKernelLifecycleSnapshotV7::Active(state) = terminal.lifecycle.clone() else { return Err("fresh active fixture required".into()); };
    terminal.lifecycle = GameKernelLifecycleSnapshotV7::Terminal {
        state,
        control: GameControlPlanV2 { schema_version: er_types::GAME_CONTROL_PLAN_SCHEMA_VERSION_V2,
            revision: terminal.material_ledger.next_authority_revision, kind: GameControlKindV2::Complete,
            owner_seat: None, action_context: None, menu: None, actionable: false },
        terminal: TerminalState { terminal_id: "controlled-query-terminal".to_owned(),
            reason: "CONTROLLED_QUERY_FIXTURE_WITHOUT_RUN".to_owned() },
    };
    terminal.validate(content.as_ref())?;
    let restored = CurrentGameSession::from_snapshot(terminal.clone(), SeatId::new(SafeU53::new(1)?),
        GameKernelRoleV7::Authority, content)?;
    assert_eq!(restored.snapshot()?, terminal, "controlled terminal restore changed the source image");
    assert!(restored.observe()?.control.is_none(), "snapshot control is distinct from absent run observation");
    Ok(restored)
}

fn exercise_queries(worker: bool) -> TestResult {
    let content = content();
    let mut reference = CurrentGameSession::natural_start(serde_json::from_value::<ProfileStateV1>(profile())?,
        SEED.to_owned(), SeatId::new(SafeU53::new(1)?), vec!["query-slot".to_owned()], true, Arc::clone(&content), None)?;
    ...
    reject(&cli.request("state.delta", query_params(json!({"kind":"RUN"}), QUERY_BOUND))?, "BACKEND_ERROR", "method state.delta is not implemented by the current V7 adapter");
    assert_eq!(digest(&cli.result("session.snapshot", json!({"session":SESSION}))?)?, active_before.0);
    if !worker {
        assert_eq!(active_before.1.as_ref().ok_or("native capture")?["status"]["kind"], "AVAILABLE");
    ...
    cli.result("session.create", json!({"session":SESSION,"start":{"kind":"SNAPSHOT","snapshot":terminal.snapshot()?,
    ...
    reject(&cli.request("state.unimplemented", json!({"session":SESSION}))?, "METHOD_NOT_FOUND", "unknown agent protocol method");
    assert_eq!(digest(&cli.result("session.snapshot", json!({"session":SESSION}))?)?, terminal_before.0);
    if !worker {
        assert_eq!(terminal_before.1.as_ref().ok_or("native capture")?["status"]["kind"], "AVAILABLE");
        assert_eq!(cli.result("session.capsule.status", json!({"session":SESSION}))?["status"]["kind"], "UNAVAILABLE");
    }
    cli.result("session.close", json!({"session":SESSION}))?;
    cli.finish()
...
#[test]
fn current_state_queries_preserve_natural_and_controlled_terminal_snapshots_and_capture() -> TestResult {
