...
fn source_save(content: Arc<PreparedGameContentV2>) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    for _ in 0..3 {
        for event in keys(PhysicalKey::Space) {
            source.apply(event)?;
        }
    }
    let bound = source
    ...
    for _ in 0..bound {
        ...
        for event in keys(PhysicalKey::ArrowDown) {
            source.apply(event)?;
        }
    }
    for _ in 0..4 {
    ...
    let GameKernelLifecycleSnapshotV7::Active(state) = source.snapshot()?.lifecycle else {
        return Err("actual natural source inactive".into());
    };
...
fn record(
    ...
    expected: &mut Vec<(usize, CoreGameKernelSnapshotV7)>,
    ...
    requests.push(request(
        &format!("event-{id}"),
        "platform.event",
        json!({"session":"current", "event":event}),
    ));
    let id = requests.len();
    requests.push(request(
        &format!("snapshot-{id}"),
...
#[test]
fn current_cli_opt_in_title_list_cancel_read_matches_every_full_native_snapshot()
-> Result<(), Box<dyn Error>> {
    let content = content()?;
    ...
    for code in [PhysicalKey::ArrowDown, PhysicalKey::Space] {
        for event in keys(code) {
            record(&mut session, &mut requests, &mut expected, event)?;
        }
    }
    let cancelled_id = session.snapshot()?.pending_platform[0].request_id;
    for event in keys(PhysicalKey::Escape) {
        record(&mut session, &mut requests, &mut expected, event)?;
    }
    ...
    requests.push(request(
        ...
        json!({"session":"current", "request_id":cancelled_id,
    ...
    record(
        ...
        CurrentExternalEvent::StorageResult {
            ...
            result: KernelStorageResultV2::Slots {
                slots: vec!["stored-actual".to_owned()],
            },
        },
    )?;
    for event in keys(PhysicalKey::Space) {
    ...
    assert_eq!(
        session.observe()?.control.ok_or("control absent")?.kind,
    ...
    for (index, expected) in expected {
        assert_eq!(snapshot(&responses[index])?, expected, "request {index}");
    }
...
#[test]
fn current_cli_absent_flag_preserves_old_title_and_rejects_non_host_opt_in()
-> Result<(), Box<dyn Error>> {
    ...
    let requests = vec![
        old,
        request("old", "session.snapshot", json!({"session":"current"})),
        denied,
        request(
            "old-after",
            "session.snapshot",
