...
use er_repro::current::{
    CurrentCaptureStatusV1, CurrentReproCapsuleV1, CurrentReproErrorV1, CurrentReproLimitsV1,
...
use er_types::{
    GameContentIdentity, GameControlKindV2, InputFocus, PhysicalKey, RawInputEvent, SafeU53, SeatId,
};
...
fn build_fixture() -> TestResult<Fixture> {
    ...
    let mut session = CurrentGameSession::natural_start(
        ...
        "current-validator-fixture".to_owned(),
        seat,
    ...
    assert_eq!(
        session.observe()?.control.ok_or("title")?.kind,
...
fn read_output(path: &Path) -> TestResult<Vec<u8>> {
    ...
    if bytes.len() > 8 << 20 {
        return Err("bounded CLI output exceeded".into());
    }
    Ok(bytes)
}

fn run(files: &Files, command: &str, options: &[(&str, &Path)]) -> TestResult<ProcessOutput> {
    ...
    for (name, path) in options {
        child.arg(format!("--{name}")).arg(path);
    }
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            return Err("validator subprocess deadline".into());
        }
...
fn rejected(output: ProcessOutput) {
    ...
    assert!(
        output.1.is_empty(),
        "failed validation must not publish success"
    );
...
fn legacy_capsule(fixture: &Fixture) -> TestResult<Vec<u8>> {
    ...
    let state = er_state::m7_state::GameStateV5 {
        ...
        content_identity: legacy_identity(fixture),
        profile: fixture.save.state.profile.clone(),
        active_run: None,
    };
    state.validate()?;
    let capsule = er_repro::ReproCapsuleV1::new(
        er_repro::CapsuleModeV1::SelfContained,
        identity,
        er_repro::FailureOracleV1::InvariantViolation("compatibility container witness".to_owned()),
        &er_canonical::canonical_bytes(&state)?,
        b"[]",
        Vec::new(),
...
fn ordinary_validate_save_accepts_v2_and_rejects_legacy_or_wrong_content() -> TestResult {
    ...
    assert_eq!(
        result["validation"],
        "CANONICAL_SAVE_AND_CURRENT_CONTENT_STATE"
    );
    assert_eq!(
        result["checksum"],
        serde_json::to_value(&fixture.save.checksum)?
    );
    assert_eq!(
        result["content_identity"],
        serde_json::to_value(fixture.content.identity())?
    );
    ...
    state.content_identity.bundle_hash =
        er_types::GameContentBundleHash::parse(format!("blake3-v1:{}", "f".repeat(64)))?;
    let other = GameSaveV2::new(
        state.content_identity.clone(),
        fixture.save.generation,
        state,
    )?;
    other.validate()?;
    ...
    assert_eq!(
        decoded.state.validate_with(fixture.content.as_ref()),
        Err(GameStateV6Error::Content)
    );
    let absent = files.write("absent-species.json", &bytes)?;
    rejected_with_error(
        run(&files, "validate-save", &[("save", &absent)])?,
        "Error: Content",
    );
    let legacy = er_save::GameSaveV1::new(
        &legacy_identity(fixture),
        fixture.save.state.profile.clone(),
        None,
    )?;
    ...
    rejected_with_error(
        run(&files, "validate-save", &[("save", &oversized)])?,
...
fn ordinary_capsule_validation_replays_current_and_rejects_tampered_or_legacy_input() -> TestResult
{
    let fixture = fixture()?;
    let files = Files::new()?;
    let valid = files.write(
        "current-capsule.json",
        &er_canonical::canonical_bytes(&fixture.capsule)?,
    )?;
    ...
    assert_eq!(
        ...
        fixture.capsule.final_snapshot_digest
    );
    assert_eq!(
        serde_json::from_value::<CoreGameKernelSnapshotV7>(result["snapshot"].clone())?,
        fixture.replayed
    );
    ...
    tampered.attempts[2].event = CurrentExternalEvent::AdvanceTime {
    ...
    rejected_with_error(
        run(&files, "capsule-validate", &[("capsule", &tampered)])?,
        &format!(
            "Error: {:?}",
            CurrentReproErrorV1::Divergence {
                position: 3,
    ...
    rejected_with_error(
        run(&files, "capsule-validate", &[("capsule", &omitted)])?,
        &format!(
            "Error: {:?}",
            CurrentReproErrorV1::Divergence {
                position: 2,
                field: "final_snapshot_digest"
            }
        ),
    );
    let mut wrong = fixture.capsule.clone();
    wrong.content_identity.bundle_hash =
        er_types::GameContentBundleHash::parse(format!("blake3-v1:{}", "f".repeat(64)))?;
    let wrong = files.write(
        "wrong-content.json",
        &er_canonical::canonical_bytes(&wrong)?,
    )?;
    ...
    let compatibility = run(
        ...
        "capsule-validate-v1",
        &[
            ("artifact-root", &files.0),
            ("capsule", Path::new("legacy.cap")),
        ],
    )?;
    assert!(
        compatibility.0.success(),
        "{}",
        String::from_utf8_lossy(&compatibility.2)
    );
    ...
    rejected_with_error(
        run(&files, "capsule-validate", &[("capsule", &oversized)])?,
        "Error: \"current command input exceeds its byte limit\"",
    );
    ...
    let output = run(
        &files,
        "capsule-validate-v1",
        &[("artifact-root", &files.0), ("capsule", escape)],
    )?;
    let error = String::from_utf8_lossy(&output.2);
    assert!(
        error.contains("Invalid")
            || error.contains("invalid")
            || error.contains("PathEscape")
