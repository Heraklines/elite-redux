//! Explicit M9 admissions for source-extracted static ordinary binary-exact recoil attributes.
...
pub(crate) fn admit_static_move_recoil(
    ...
    for (index, unit) in catalog.behavior_units().iter().enumerate() {
        if unit.semantic.resolution != CatalogResolution::BespokeGap
            ...
            || unit.semantic.effect.attribute.as_deref() != Some("RecoilAttr")
        {
            continue;
        }
        ...
        if !matches!(
            ...
            return Err(FullContentBuildErrorV1::Semantic(
                "static recoil admission produced a different operation".to_owned(),
            ));
        }
        let classification = output.classifications.0.get_mut(index).ok_or_else(|| {
            FullContentBuildErrorV1::Semantic("missing recoil classification".to_owned())
        })?;
        let record = output.units.get_mut(index).ok_or_else(|| {
            FullContentBuildErrorV1::Semantic("missing recoil compile record".to_owned())
        })?;
        ...
        {
            return Err(FullContentBuildErrorV1::Semantic(
                "recoil admission does not match frozen classification".to_owned(),
            ));
        }
        let next = output
            .programs
    ...
    output.report.bespoke_unit_count = output
        ...
        .ok_or_else(|| {
            FullContentBuildErrorV1::Semantic("recoil closure count underflow".to_owned())
        })?;
