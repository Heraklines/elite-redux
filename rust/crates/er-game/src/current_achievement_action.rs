...
use er_state::current_battle_source_events::{CurrentBattleSourceEventV1, CurrentHitCheckV1, CurrentMoveUseModeV1, CurrentResolvedHitCheckV1};
...
const INITIAL_TOWN_SPECIES: [u64; 67] = [
    10, 13, 16, 19, 21, 23, 29, 32, 43, 46, 48, 52, 63, 69, 132, 133, 161, 163,
    165, 167, 172, 173, 174, 175, 187, 191, 261, 263, 265, 266, 268, 270, 273,
    276, 280, 283, 285, 290, 293, 300, 396, 399, 401, 415, 420, 440, 446, 447,
    504, 506, 509, 519, 543, 546, 570, 572, 661, 664, 734, 819, 821, 824, 831,
    915, 921, 924, 926,
];
...
pub(crate) fn fold_current_achievement_action(
    before: &GameStateV6, after: &GameStateV6, content: &PreparedGameContentV2,
    events: &[CurrentBattleSourceEventV1], cues: &[BattlePresentationCueV5],
    ...
    for state in [before, after] {
        ...
        if run.world.biome.get().get() != 0 || source.initial_enemy.form_index != 0
        ...
        let ids: BTreeSet<_> = biome.pokemon_pools.iter().flat_map(|pool| &pool.species)
            .map(|id| id.get().get()).collect();
        if biome.key != "TOWN" || biome.trainer_chance_denominator != 0
    ...
    for event in events {
        if let CurrentBattleSourceEventV1::MoveResolution { user, move_id, .. } = event {
            ...
            let definition = content.battle.move_definition(*move_id).map_err(|_| failure())?;
...
fn fold_source_admitted_action(
    ...
    let tracker = before.current_achievement_tracker.as_ref().ok_or_else(failure)?;
    if !tracker.valid(run) || after.current_achievement_tracker.as_ref() != Some(tracker)
        || before.content_identity != after.content_identity
        || before.content_identity != *content.identity()
        || run.party.len() != next_run.party.len()
        || run.party.iter().zip(&next_run.party).any(|(old, new)|
            old.id != new.id || old.stat_stages != new.stat_stages)
    { return Err(failure()); }
    ...
    let mut fold = ActionFold {
        tracker: tracker.clone(), keys: Vec::new(), wave: run.wave, turn: battle.turn,
    };
    match (&before.current_turn_execution, &after.current_turn_execution) {
        (None, Some(next)) => {
            next.validate(next_run).map_err(|_| failure())?;
            if next.next_action != 0 || next.finalization_done
                || next.stage != CurrentTurnStageV1::ReadyForMove
                || next.turn != battle.turn || !events.is_empty() || !cues.is_empty()
            { return Err(failure()); }
            ...
            for row in &battle.field.slots {
                if row.slot.side != BattleSide::Player { continue; }
        ...
        (Some(previous), Some(next)) => {
            ...
            if previous.finalization_done
                ...
                || previous.run != next.run || previous.battle != next.battle
                || previous.wave != next.wave || previous.turn != next.turn
            ...
            if next.finalization_done {
                if usize::from(previous.next_action) != previous.actions.len()
                    || previous.next_action != next.next_action || !events.is_empty()
                { return Err(failure()); }
            } else {
                if previous.next_action.checked_add(1) != Some(next.next_action) { return Err(failure()); }
                let selected = previous.actions.get(usize::from(previous.next_action)).ok_or_else(failure)?;
                let selected_move = match &selected.command {
                    er_types::battle_command::BattleCommand::Fight { actor, move_slot, .. } => Some(
                        er_battle::m7_resolver::effective_move_definition_v5(
                            &content.battle, member(run, *actor)?, *move_slot,
                        ).map_err(|_| failure())?.0.id,
                    ),
                    er_types::battle_command::BattleCommand::Switch { .. } => None,
                };
                if events.iter().any(|event| match event {
                    CurrentBattleSourceEventV1::MoveResolution { user, source_slot, move_id, .. }
                    | CurrentBattleSourceEventV1::MoveDamage { user, source_slot, move_id, .. } =>
                        *user != selected.command.actor() || *source_slot != selected.source_slot
                            || Some(*move_id) != selected_move,
                }) { return Err(failure()); }
            }
            if fold.tracker.battle.as_ref().is_none_or(|state| state.wave_index != run.wave) {
                return Err(failure());
            }
        }
        (Some(previous), None) => {
            previous.validate(run).map_err(|_| failure())?;
            if previous.finalization_done || previous.stage != CurrentTurnStageV1::ReadyForMove
    ...
    if !fold.tracker.valid(next_run) { return Err(failure()); }
...
impl ActionFold {
    fn action(
        &mut self, before: &RunStateV3, after: &RunStateV3, content: &PreparedGameContentV2,
        events: &[CurrentBattleSourceEventV1], cues: &[BattlePresentationCueV5],
        ...
        if before.run_id != after.run_id || before.wave != after.wave
            || battle.battle_id != next_battle.battle_id || events.len() > 7
            || battle.enemy_party.len() != next_battle.enemy_party.len()
        { return Err(failure()); }
        let mut hp: BTreeMap<_, _> = before.party.iter().chain(&battle.enemy_party)
            .map(|pokemon| (pokemon.id, pokemon.hp)).collect();
        ...
        for event in events {
            match event {
                CurrentBattleSourceEventV1::MoveResolution {
                    user, source_slot, move_id, use_mode, first_hit, targets,
                } => {
                    if resolution.is_some() || !*first_hit || *use_mode != CurrentMoveUseModeV1::Direct
                        || targets.is_empty() || targets.len() > 6 || !damage_cues.is_empty()
                    { return Err(failure()); }
                    let actor = at_slot(before, *source_slot, *user)?;
                    if actor.fainted || actor.hp == 0 || actor.hp > actor.max_hp
                        || !actor.moves.iter().flatten().any(|slot| slot.move_id == *move_id) { return Err(failure()); }
                    let definition = content.battle.move_definition(*move_id).map_err(|_| failure())?;
                    let mut slots = BTreeSet::new();
                    for row in targets {
                        let target = at_slot(before, row.slot, row.target)?;
                        if target.fainted || target.hp == 0 || target.hp > target.max_hp { return Err(failure()); }
                        if !slots.insert(row.slot) || !matches!(row.result,
                            CurrentHitCheckV1::Hit | CurrentHitCheckV1::Miss
                                | CurrentHitCheckV1::NoEffect | CurrentHitCheckV1::NoEffectNoMessage)
                        { return Err(failure()); }
                    }
                    self.move_resolution(*user, source_slot.side == BattleSide::Player, *move_id,
                        definition.category, battle.format.player_capacity == 2, targets)?;
                    resolution = Some((*user, *source_slot, *move_id, targets));
                }
                CurrentBattleSourceEventV1::MoveDamage {
                    user, source_slot, target, target_slot, move_id, use_mode, damage,
                    target_hp_before, target_hp_after, target_max_hp, super_effective,
                    hit_count, hits_left, ..
                } => {
                    ...
                    if (resolved_user, resolved_slot, resolved_move) != (*user, *source_slot, *move_id)
                        || *use_mode != CurrentMoveUseModeV1::Direct || (*hit_count, *hits_left) != (1, 1)
                        || !damaged.insert(*target) || *damage == 0
                        || hp.get(target) != Some(target_hp_before) || holder.max_hp != *target_max_hp
                        || target_hp_before.checked_sub(*damage) != Some(*target_hp_after)
                        || !targets.iter().any(|row| row.target == *target && row.slot == *target_slot
                    ...
                    if source_slot.side == BattleSide::Enemy && target_slot.side == BattleSide::Player && *super_effective {
                        self.enemy_super_effective_survival(*user, *target, *target_hp_before, *target_hp_after);
                    }
                    if source_slot.side == BattleSide::Player && target_slot.side == BattleSide::Enemy {
                        let definition = content.battle.move_definition(*move_id).map_err(|_| failure())?;
                        if definition.category != MoveCategory::Status { self.state().player_dealt_direct_damage = true; }
                        if *target_hp_after == 0 { self.player_direct_ko(*user, *source_slot, *target, *move_id)?; }
                    }
                }
            }
        }
        ...
        for cue in cues {
            match cue {
                BattlePresentationCueV5::MoveUsed { pokemon, move_id } => {
                    ...
                    if user != *pokemon || resolved_move != *move_id { return Err(failure()); }
                    move_cues += 1;
                }
                BattlePresentationCueV5::HpChanged { pokemon, before, after } => {
                    // Unobserved recoil/residual/drain hooks need their own
                    // source event. Never invent a damage source from this cue.
                    if healing_started || damage_cues.get(damage_cursor) != Some(&(*pokemon, *before, *after)) {
                        return Err(failure());
                    }
                    damage_cursor += 1;
                }
                BattlePresentationCueV5::AbilityHeal { pokemon, before: old, after: new, .. } => {
                    healing_started = true;
                    let holder = member(before, *pokemon)?;
                    if damage_cursor != damage_cues.len() || hp.get(pokemon) != Some(old)
                        || new <= old || *new > holder.max_hp
                    { return Err(failure()); }
                    hp.insert(*pokemon, *new);
                    if before.party.iter().any(|row| row.id == *pokemon) { self.player_heal(*pokemon); }
                }
                BattlePresentationCueV5::Switched { slot, pokemon } => {
                    if !events.is_empty() || switched { return Err(failure()); }
                    let holder = at_slot(after, *slot, *pokemon)?;
                    if hp.get(pokemon) != Some(&holder.hp) { return Err(failure()); }
                    switched = true;
                    if slot.side == BattleSide::Player { self.player_switch_in(*pokemon, holder.hp, holder.max_hp)?; }
                }
                _ => {}
            }
        }
        if damage_cursor != damage_cues.len() || move_cues != usize::from(resolution.is_some())
            || after.party.iter().chain(&next_battle.enemy_party).any(|row| hp.get(&row.id) != Some(&row.hp))
        { return Err(failure()); }
        Ok(())
    }

    fn state(&mut self) -> &mut CurrentAchievementBattleV1 {
        if self.tracker.battle.as_ref().is_none_or(|state| state.wave_index != self.wave) {
        ...
        self.tracker.battle.get_or_insert_with(|| CurrentAchievementBattleV1::fresh(self.wave, self.turn))
    }

    fn key(&mut self, key: &str) { self.keys.push(key.to_owned()); }
    ...
    fn note_turn_effect(&mut self, key: String) {
        ...
        effects.insert(key);
        if effects.len() >= 10 { self.key("THE_LONGEST_TURN"); }
    }
    ...
    fn turn_start_player(&mut self, pokemon: PokemonId, hp: u32, max_hp: u32) -> Result<(), GameRuntimeV6Error> {
        if hp == 0 || hp > max_hp { return Err(failure()); }
        self.state().ko_stints.entry(pokemon).or_insert(CurrentAchievementKoStintV1 {
            kos: 0, had_plus_six: false,
            entered_low_hp: u64::from(hp) * 4 <= u64::from(max_hp), healed: false,
        });
        Ok(())
    }

    fn player_heal(&mut self, pokemon: PokemonId) {
        if let Some(stint) = self.state().ko_stints.get_mut(&pokemon) { stint.healed = true; }
        self.note_turn_effect(format!("heal:{}", pokemon.get().get()));
    }

    fn player_switch_in(&mut self, pokemon: PokemonId, hp: u32, max_hp: u32) -> Result<(), GameRuntimeV6Error> {
        if hp == 0 || hp > max_hp { return Err(failure()); }
        let state = self.state();
        state.switched_in_player_ids.insert(pokemon);
        state.ko_stints.insert(pokemon, CurrentAchievementKoStintV1 {
            kos: 0, had_plus_six: false,
            entered_low_hp: u64::from(hp) * 4 <= u64::from(max_hp), healed: false,
        });
    ...
    fn move_resolution(
        &mut self, user: PokemonId, player: bool, move_id: MoveId,
        category: MoveCategory, double: bool, checks: &[CurrentResolvedHitCheckV1],
    ) -> Result<(), GameRuntimeV6Error> {
        ...
        if player {
            ...
            if state.player_acted_this_turn != Some(true) { state.flash_failed = true; }
            if state.no_sell_token.as_ref().is_some_and(|token| token.attacker_id == user) {
        ...
        if player {
            ...
            if move_id.get().get() == 57 && double {
                self.state().last_spread_move = Some(CurrentAchievementSpreadMoveV1 { move_id, user_id: user, turn });
            }
            return Ok(());
        }
        if category == MoveCategory::Status { return Ok(()); }
        let player_checks: Vec<_> = checks.iter().filter(|row| row.slot.side == er_types::battle_ids::BattleSide::Player).collect();
        if player_checks.is_empty() { return Ok(()); }
        // Source tests exact MISS. Neither immunity result is an evasion streak.
        if player_checks.iter().all(|row| row.result == CurrentHitCheckV1::Miss) {
            let state = self.state();
            state.weave_streak = state.weave_streak.checked_add(1).ok_or_else(failure)?;
            if state.weave_streak >= 3 { self.key("WEAVE_NATION_CERTIFIED"); }
        } else { self.state().weave_streak = 0; }
    ...
    fn damage_source(&mut self, target: PokemonId, user: PokemonId) {
        ...
        if sources.len() >= 4 { self.key("CHAIN_REACTION"); }
    }
    ...
    fn player_direct_ko(&mut self, user: PokemonId, source_slot: FieldSlot, target: PokemonId, move_id: MoveId)
        -> Result<(), GameRuntimeV6Error>
    {
        let turn = self.turn;
        let key = format!("{}:{}:{}", user.get().get(), move_id.get().get(), turn.get().get());
        let state = self.state();
        if state.spread_ko_key.as_ref() != Some(&key) {
            state.spread_ko_key = Some(key);
            state.spread_ko_count = Some(0);
        }
        state.spread_ko_count = Some(state.spread_ko_count.ok_or_else(failure)?.checked_add(1).ok_or_else(failure)?);
        ...
        if setup { self.key("SETUP_PAYOFF"); }
        if zero_to_hero { self.key("ZERO_TO_HERO"); }
        self.tracker.persistent.parallel_play_ko_ids.insert(user);
        if self.state().charge_low_hp_user_ids.contains(&user) { self.key("CHARGE_IT_TO_THE_GAME"); }
        if self.tracker.persistent.learned_move_stamps.get(&move_id)
            .is_some_and(|wave| wave.get().get().checked_add(1) == Some(self.wave.get().get()))
        { self.key("TECHNICAL_DIFFICULTIES"); }
        self.state().enemy_ko_killers.insert(target, CurrentAchievementKillerV1 {
            user_id: user, field_index: source_slot.position,
        });
        Ok(())
    }

    fn enemy_super_effective_survival(&mut self, user: PokemonId, target: PokemonId, before: u32, after: u32) {
        if before > 1 && after == 1 {
            self.state().no_sell_token = Some(CurrentAchievementNoSellV1 { attacker_id: user, survivor_id: target });
        }
    }
}
...
fn at_slot(run: &RunStateV3, slot: FieldSlot, id: PokemonId) -> Result<&PokemonStateV5, GameRuntimeV6Error> {
    let battle = run.battle.as_ref().ok_or_else(failure)?;
    if !battle.field.slots.iter().any(|row| row.slot == slot && row.occupant == Some(id))
