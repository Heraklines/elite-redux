...
pub(super) fn project_current_battle_cues(
    ...
    for (index, cue) in cues.iter().enumerate() {
        ...
        let (family, payload) = match cue {
            Cue::AbilityShown { pokemon, ability, innate_slot } => {
                if ability.get().get() != 5082 { return Err(GameRuntimeV6Error::Invalid); }
                observations.push(CurrentDefenderObservationV1::Applied {
                    ordinal, holder: *pokemon, innate_slot: *innate_slot,
                });
                (PresentationCueFamilyV1::Ability, Some(Payload::AbilityShown {
                    holder: *pokemon, ability: *ability, innate_slot: *innate_slot,
                }))
            }
            Cue::AbilityHeal { pokemon, before, after, requested_heal } => (
                PresentationCueFamilyV1::Hp, Some(Payload::HpRestored {
                    holder: *pokemon, before: *before, after: *after, requested_heal: *requested_heal,
                }),
            ),
            Cue::AbilityHidden { pokemon, ability, innate_slot } => (
                PresentationCueFamilyV1::Ability, Some(Payload::AbilityHidden {
                    holder: *pokemon, ability: *ability, innate_slot: *innate_slot,
                }),
            ),
            Cue::MoveNoEffect { pokemon, move_id } => (
                PresentationCueFamilyV1::Move, Some(Payload::MoveNoEffect {
                    holder: *pokemon, move_id: *move_id,
                }),
            ),
            Cue::Switched { pokemon, .. } => {
                observations.push(CurrentDefenderObservationV1::Summoned { ordinal, holder: *pokemon });
        ...
        if let Some(payload) = &payload {
            payload.validate(semantic).map_err(|_| GameRuntimeV6Error::Invalid)?;
        }
        effects.push(GamePresentationEffectV2 {
            event_id: PresentationEventId::new(revision), semantic,
            blocking: mapping.blocking, skip: mapping.skip, payload,
        });
    }
    candidate.current_defender_dispatch = CurrentDefenderDispatchV1::observe(
        ...
        candidate.active_run.as_ref().ok_or(GameRuntimeV6Error::Invalid)?,
        operation_id, revision, &observations,
