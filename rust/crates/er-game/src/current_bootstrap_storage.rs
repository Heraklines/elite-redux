...
pub enum BootstrapStorageKindV1 {
    List,
    Read { slot: String },
}
...
pub(super) fn control_entries(
    ...
    let Some(storage) = storage else {
        return if storage_stage(stage) {
            Err(RunBootstrapErrorV1::Invalid)
        } else {
            Ok(None)
        };
    };
    if stage == RunBootstrapStageV1::Title {
        return Ok(Some((
            ...
            vec![
                ...
                (
                    "bootstrap/title/existing-saves".to_owned(),
                    true,
                    BootstrapActionV1::OpenExistingSaves,
                ),
            ],
            None,
        )));
    }
    if !storage_stage(stage) {
        return Ok(None);
    }
    ...
    if stage == RunBootstrapStageV1::ExistingSaveSelect {
        rows.extend(storage.slots.iter().enumerate().map(|(index, slot)| {
            (
                format!("bootstrap/existing/{index:04}"),
                true,
                BootstrapActionV1::SelectExistingSave(slot.clone()),
            )
        }));
    }
    rows.push((
        "bootstrap/existing/cancel".to_owned(),
        true,
...
impl RunBootstrapMachineV1 {
    pub fn enable_current_storage(&mut self) -> Result<(), RunBootstrapErrorV1> {
        ...
        candidate.current_storage = Some(CurrentBootstrapStorageV1 {
            owner_seat: owner,
        ...
        candidate.control = build_control(
            candidate.stage,
    ...
    pub fn current_storage_effect(&self) -> Option<GamePlatformEffectV2> {
        ...
        Some(match &pending.kind {
            BootstrapStorageKindV1::List => GamePlatformEffectV2::StorageList {
                request: pending.request_id,
            },
            BootstrapStorageKindV1::Read { slot } => GamePlatformEffectV2::StorageRead {
                request: pending.request_id,
                slot: slot.clone(),
            },
        })
    ...
    pub(super) fn apply_current_storage_action(
        &mut self,
        action: &BootstrapActionV1,
    ) -> Result<bool, RunBootstrapErrorV1> {
        let kind = match (self.stage, action) {
            (RunBootstrapStageV1::Title, BootstrapActionV1::OpenExistingSaves) => {
                BootstrapStorageKindV1::List
            }
            (
                RunBootstrapStageV1::ExistingSaveSelect,
                BootstrapActionV1::SelectExistingSave(slot),
            ) => {
                if !self
                    .current_storage
                    .as_ref()
                    .is_some_and(|storage| storage.slots.contains(slot))
                {
                    return Err(RunBootstrapErrorV1::IllegalAction);
                }
        ...
        self.stage = match kind {
            BootstrapStorageKindV1::List => RunBootstrapStageV1::ExistingSaveListing,
            BootstrapStorageKindV1::Read { .. } => RunBootstrapStageV1::ExistingSaveLoading,
        };
        storage.pending = Some(PendingBootstrapStorageV1 {
            request_id,
            kind,
            source_menu: self.menu_instance_high_water,
            source_revision: self.control.revision,
            waiting_menu: next_menu(self.menu_instance_high_water)?,
            waiting_revision: next_safe(self.control.revision)?,
        });
        Ok(true)
    }

    pub fn accept_current_slots(
        &mut self,
        request: PlatformRequestId,
        slots: Vec<String>,
    ) -> Result<(), RunBootstrapErrorV1> {
        self.validate()?;
        if !slots_valid(&slots)
            || self.current_storage_effect() != Some(GamePlatformEffectV2::StorageList { request })
        {
            return Err(RunBootstrapErrorV1::IllegalAction);
        }
        let mut candidate = self.clone();
    ...
    pub fn accept_current_missing(
        &mut self,
        request: PlatformRequestId,
    ) -> Result<(), RunBootstrapErrorV1> {
        self.validate()?;
        let Some(GamePlatformEffectV2::StorageRead {
            request: owned,
            slot,
        }) = self.current_storage_effect()
        else {
            return Err(RunBootstrapErrorV1::IllegalAction);
        };
        if owned != request {
    ...
    pub(super) fn validate_current_storage(&self) -> Result<(), RunBootstrapErrorV1> {
        let Some(storage) = &self.current_storage else {
            return if storage_stage(self.stage) {
                Err(RunBootstrapErrorV1::Invalid)
            } else {
                Ok(())
            };
        };
        if storage.next_platform_request_id == SafeU53::ZERO
            || self.control.revision == SafeU53::ZERO
            || !self.catalog.local_is_host
            || !slots_valid(&storage.slots)
            || self
                .catalog
                .starters
                .iter()
                .any(|starter| starter.owner_seat != storage.owner_seat)
            ...
            || storage
                .missing_slot
                .as_ref()
                .is_some_and(|slot| !slot_valid(slot) || storage.slots.contains(slot))
            || (storage.missing_slot.is_some()
                && self.stage != RunBootstrapStageV1::ExistingSaveSelect)
            || (!storage_stage(self.stage)
                && (!storage.slots.is_empty() || storage.pending.is_some()))
        ...
        match (&storage.pending, self.stage) {
            (Some(pending), stage) => {
                let kind_valid = match (&pending.kind, stage) {
                    ...
                    (
                        BootstrapStorageKindV1::Read { slot },
                        RunBootstrapStageV1::ExistingSaveLoading,
                    ) => storage.slots.contains(slot),
                    _ => false,
                };
                if !kind_valid
                    || pending.request_id.get() == SafeU53::ZERO
                    || pending.source_menu == MenuInstanceId::ZERO
                    || pending.source_revision == SafeU53::ZERO
                    ...
                    || pending.waiting_menu != self.menu_instance_high_water
                    || pending.waiting_revision != self.control.revision
                {
                    return Err(RunBootstrapErrorV1::Invalid);
                }
            }
            (
                None,
                RunBootstrapStageV1::ExistingSaveListing | RunBootstrapStageV1::ExistingSaveLoading,
            ) => return Err(RunBootstrapErrorV1::Invalid),
            _ => {}
        }
        ...
        if let (Some(expected_menu), Some(actual_menu)) = (&mut expected.menu, &self.control.menu) {
            if !expected_menu
                .options
                .iter()
                .any(|option| option.option_id == actual_menu.selected_option_id)
            {
                return Err(RunBootstrapErrorV1::Invalid);
            }
