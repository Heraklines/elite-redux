...
use er_state::current_experience_owner::{
    CurrentExperienceCapPolicyV1, CurrentExperienceEncounterV1, CurrentExperienceExecutionOriginV1,
    CurrentPendingExperienceV1,
};
use er_state::current_experience_settlement::{
    CurrentExperienceAwardV1, CurrentExperiencePhaseV1, CurrentLearnMoveAssignmentV1,
...
fn context<'a>(
    ...
    if owner.execution_origin != Some(CurrentExperienceExecutionOriginV1::FreshNormalClassic)
        ...
        || owner.content_identity != before.content_identity
    ...
    let pending = owner
        .pending
        .iter()
        .find(|value| value.id == pending_id)
        .ok_or_else(failure)?;
    let metadata = content
        .progression
        .experience_for_compiled_form(pending.source.species, pending.source.compiled_form)
        .map_err(|_| failure())?;
...
pub(crate) fn plan_current_victory_experience(
    ...
    let form = match pending.source.source_sprite_key.as_str() {
        "mega" => ExperienceForm::Mega,
        "mega-x" => ExperienceForm::MegaX,
        "mega-y" => ExperienceForm::MegaY,
        "primal" => ExperienceForm::Primal,
        "gigantamax" => ExperienceForm::Gigantamax,
        "eternamax" => ExperienceForm::Eternamax,
        _ => ExperienceForm::Other,
    };
    let value = defeated_experience_value(DefeatedExperienceSource {
        base_experience: pending.source.unadjusted_base_exp,
        level: pending.defeated_level,
        form,
        battle: OrdinaryExperienceBattle::Wild,
    })
    .map_err(|_| failure())?;
    let party =
        run.party
            .iter()
            .map(|pokemon| {
                Ok(UnboostedPartyExperienceMember {
                    hp: pokemon.hp,
                    level: pokemon.level,
                    participated: pending.participants.iter().any(|row| {
                        row.pokemon == pokemon.id && Some(row.owner) == pokemon.owner_seat
                    }),
                    ...
                    on_field: battle
                        ...
                        .slots
                        .iter()
                        .any(|row| row.occupant == Some(pokemon.id)),
                })
            })
    ...
    let plan = plan_unboosted_party_experience(&UnboostedPartyExperienceInput {
        ...
        exp_balance_stacks: None,
        multiple_participant_stacks: None,
        multiplier_override: None,
        party,
    })
    .map_err(|_| failure())?;
    plan.phase_insertions
        .into_iter()
        .map(|row| {
            Ok(CurrentExperiencePhaseV1 {
                pending_id,
                pokemon: run.party.get(row.party_index).ok_or_else(failure)?.id,
                party_index: u8::try_from(row.party_index).map_err(|_| failure())?,
                on_field: row.on_field,
                phase_argument: represented_experience(row.experience)?,
            })
        })
        .collect()
}
...
pub(crate) fn prepare_current_experience_phase(
    before: &GameStateV6,
    content: &PreparedGameContentV2,
    phase: &CurrentExperiencePhaseV1,
    ...
    Ok(CurrentExperienceAwardV1 {
        phase: phase.clone(),
        experience: phase.phase_argument,
        last_level: pokemon.level,
        last_experience: pokemon.experience,
    })
}
...
pub(crate) fn apply_current_experience_award(
    ...
    let target = candidate
        ...
        .ok_or_else(failure)?
        .party
        .get_mut(usize::from(award.phase.party_index))
        .ok_or_else(failure)?;
...
pub(crate) fn apply_current_level_up(
    ...
    if pokemon.level != level_up.new_level
        || level_up.previous_level >= level_up.new_level
    ...
    let hp = current_hp_after_stat_calculation(pokemon.hp, pokemon.max_hp, stats.hp)
        .map_err(|_| failure())?;
    let mut candidate = before.clone();
    let target = candidate
        .active_run
        ...
        .ok_or_else(failure)?
        .party
        .get_mut(usize::from(level_up.award.phase.party_index))
        .ok_or_else(failure)?;
    target.stats = stats;
...
fn phase_pokemon<'a>(
    ...
    let pokemon = run
        .party
        .get(usize::from(phase.party_index))
        .ok_or_else(failure)?;
    if pokemon.id != phase.pokemon {
        return Err(failure());
    }
    Ok(pokemon)
}
...
pub(crate) fn plan_current_level_up_children(
    ...
    if level_up.previous_level < 100 {
        let mut moves = definition
            .level_moves
            .iter()
            .filter(|row| {
                row.level > 0
                    && i32::from(row.level) > i32::from(level_up.previous_level)
                    && i32::from(row.level) <= i32::from(pokemon.level)
            })
            .collect::<Vec<_>>();
        ...
        for row in moves {
            if !learn_move_candidates.contains(&row.move_id) {
                learn_move_candidates.push(row.move_id);
            }
        }
    }
    ...
    if !pokemon.pause_evolutions {
        for id in &definition.evolutions {
            ...
            if evolution.consume_item.is_some()
                || evolution
                    .source_form
                    .is_some_and(|form| form != pokemon.form_index)
            {
                continue;
            }
    ...
    Ok(CurrentLevelUpChildrenV1 {
        parent: parent.clone(),
        learn_move_candidates,
        evolution_candidates,
    })
}

fn ordinary_evolution_condition(
    ...
    match condition {
        ...
        EvolutionConditionV2::KnownMove(move_id) => Ok(pokemon
            .moves
            .iter()
            .flatten()
        ...
        EvolutionConditionV2::All(conditions) => {
            for condition in conditions {
                if !ordinary_evolution_condition(condition, pokemon)? {
                    return Ok(false);
                }
            }
            Ok(true)
...
pub(crate) fn begin_current_learn_move_batch(
    ...
    for move_id in &children.learn_move_candidates {
        if move_id.get() != SafeU53::ZERO
            && !offered.contains(move_id)
            && !pokemon
                .moves
                .iter()
                .flatten()
    ...
    Ok(CurrentLearnMoveBatchV1 {
        ...
        complete: offered.is_empty(),
        offered,
        original_moves: pokemon.moves,
        assignments: Vec::new(),
        pending_move: None,
        list_cursor: 0,
...
pub(crate) fn apply_current_learn_move_batch(
    ...
    let (run, _) = context(before, content, phase.pending_id)?;
    let pokemon = phase_pokemon(run, phase)?;
    if batch.complete || batch.offered.is_empty() || batch.assignments.len() >= batch.offered.len()
    {
        return Err(failure());
    }
    let mut expected_offered = Vec::new();
    for id in &batch.children.learn_move_candidates {
        if id.get() != SafeU53::ZERO
            && !expected_offered.contains(id)
            && !batch
                .original_moves
                .iter()
                .flatten()
                .any(|slot| slot.move_id == *id)
        {
            expected_offered.push(*id);
        }
    }
    if batch.offered != expected_offered
        || batch
            .original_moves
            .windows(2)
    ...
    for assignment in &batch.assignments {
        ...
        *expected_moves
            .get_mut(usize::from(assignment.slot.get()))
            .ok_or_else(failure)? = Some(new_learned_move(assignment.move_id));
    }
    if batch
        .pending_move
        .is_some_and(|id| !batch.offered.contains(&id) || seen.contains(&id))
    {
        return Err(failure());
    }
    if pokemon.moves != expected_moves {
        return Err(failure());
    }
    ...
    let target = candidate
        ...
        .ok_or_else(failure)?
        .party
        .get_mut(usize::from(phase.party_index))
        .ok_or_else(failure)?;
    if batch.cancel_confirmation
        != matches!(action, CurrentLearnMoveBatchActionV1::ConfirmCancel { .. })
    {
        return Err(failure());
    }
    if batch.pending_move.is_some()
        != matches!(
            action,
            CurrentLearnMoveBatchActionV1::Assign { .. }
    ...
    let learned = match action {
        CurrentLearnMoveBatchActionV1::SelectMove { move_id } => {
            if !batch.offered.contains(move_id) || seen.contains(move_id) {
                return Err(failure());
            }
            content
                .battle
                ...
                .map_err(|_| failure())?;
            result.list_cursor = u16::try_from(
                batch
                    .offered
                    .iter()
            ...
            if let Some(index) = target.moves.iter().position(Option::is_none) {
                let slot = er_types::battle_ids::MoveSlotIndex::new(
                    u8::try_from(index).map_err(|_| failure())?,
                )
                .map_err(|_| failure())?;
                target.moves[index] = Some(new_learned_move(*move_id));
                result.assignments.push(CurrentLearnMoveAssignmentV1 {
                    move_id: *move_id,
                    slot,
                });
        ...
        CurrentLearnMoveBatchActionV1::Assign { move_id, slot } => {
            if batch.pending_move != Some(*move_id)
                || !batch.offered.contains(move_id)
                || seen.contains(move_id)
                || target.moves.iter().any(Option::is_none)
            {
                return Err(failure());
            }
            content
            ...
            *target
                ...
                .ok_or_else(failure)? = Some(new_learned_move(*move_id));
            result.assignments.push(CurrentLearnMoveAssignmentV1 {
                move_id: *move_id,
                slot: *slot,
            });
        ...
        CurrentLearnMoveBatchActionV1::Done => {
            result.list_cursor = u16::try_from(
                batch.offered.len() - batch.assignments.len()
                    + usize::from(!batch.assignments.is_empty()),
            )
            .map_err(|_| failure())?;
            if result.assignments.is_empty() {
                result.cancel_confirmation = true;
            } else {
...
fn clamp_batch_cursor(batch: &mut CurrentLearnMoveBatchV1) -> Result<(), GameRuntimeV6Error> {
    let remaining = u16::try_from(
        batch
            .offered
            .len()
            .checked_sub(batch.assignments.len())
            .ok_or_else(failure)?,
    )
    .map_err(|_| failure())?;
