...
use er_progression::current_experience::{add_normal_classic_experience, ExperiencePosition};
...
fn current_progress(descendant: &D) -> Option<AwardProgress<'_>> {
    let (award, xp_applied, stats_applied, level_up) = match descendant {
        ...
        D::PartyAwardPresentation { award, level_up, .. } => (award, true, false, level_up.as_ref()),
        D::LevelUpStart { level_up } => (&level_up.award, true, false, Some(level_up)),
        D::LevelUpPresentation { end, .. } => (&end.level_up.award, true, true, Some(&end.level_up)),
        D::LevelUpChildren { children } | D::Evolution { children } => (&children.parent.level_up.award, true, true, Some(&children.parent.level_up)),
        D::LearnMoveBatch { batch } => (&batch.children.parent.level_up.award, true, true, Some(&batch.children.parent.level_up)),
        D::HidePartyBar { award } | D::HidePartyBarPresentation { award, .. } => (award, true, true, None),
    };
    Some(AwardProgress { award, xp_applied, stats_applied, level_up,
...
pub(crate) fn validate_current_experience_progress(state: &GameStateV6, content: &PreparedGameContentV2) -> Result<(), GameRuntimeV6Error> {
    let Some(owner) = state.current_battle_participation.as_ref().and_then(|p| p.experience.as_ref()) else { return Ok(()); };
    if owner.source_progression.is_none() { return Ok(()); }
    ...
    let Some(pending) = owner.pending.first() else { return Ok(()); };
    let run = state.active_run.as_ref().ok_or_else(failure)?;
    if pending.recipients.len() != run.party.len() { return Err(failure()); }
    ...
    for (pokemon, recipient) in pre_run.party.iter_mut().zip(&pending.recipients) {
        if pokemon.id != recipient.pokemon || pokemon.owner_seat != Some(recipient.owner)
            || pokemon.pokerus != recipient.pokerus
        { return Err(failure()); }
        let captured = recipient.stats.as_ref().ok_or_else(failure)?;
        if captured.max_hp != captured.stats.hp || recipient.hp > captured.max_hp { return Err(failure()); }
    ...
    if let Some(victory) = &pending.victory {
        ...
        { return Err(failure()); }
        progress.extend(victory.completed.iter().map(|award| AwardProgress { award, xp_applied: true, stats_applied: true, level_up: None, check_optional_level_up: false }));
    ...
    for (index, (pokemon, recipient)) in run.party.iter().zip(&pending.recipients).enumerate() {
        ...
        let awards = progress.iter().filter(|p| usize::from(p.award.phase.party_index) == index).collect::<Vec<_>>();
        if awards.len() > 1 { return Err(failure()); }
        if let Some(progress) = awards.first() {
            let award = progress.award;
            if award.phase.pokemon != pokemon.id || award.phase.pending_id != pending.id
                || award.last_level != recipient.level || award.last_experience != recipient.experience
            ...
            let position = add_normal_classic_experience(growth, ExperiencePosition { level: recipient.level, total: recipient.experience }, award.experience, wave).map_err(|_| failure())?;
            if let Some(level_up) = progress.level_up {
                if level_up.award != *award || level_up.previous_level != recipient.level
                    || level_up.new_level != position.level || position.level <= recipient.level
                    || level_up.previous_stats != captured.stats
                { return Err(failure()); }
            }
            if progress.check_optional_level_up && (progress.level_up.is_some() != (position.level > recipient.level))
            { return Err(failure()); }
            if progress.xp_applied { expected_level = position.level; expected_xp = position.total; }
            if progress.stats_applied && position.level > recipient.level {
                ...
                let form_id = er_types::FormId::parse(format!("{}:{}", pokemon.species_id.get().get(), pokemon.form_index)).map_err(|_| failure())?;
                let form = content.battle.form(&form_id).map_err(|_| failure())?;
                if form.species != pokemon.species_id { return Err(failure()); }
                let nature = content.progression.pack().natures.iter().find(|n| n.id == pokemon.effective_nature).ok_or_else(failure)?;
                expected_stats = calculate_current_unmodified_stats(&leveled, form.stat_override.unwrap_or(species.base_stats), nature).map_err(|_| failure())?;
                expected_hp = current_hp_after_stat_calculation(recipient.hp, captured.max_hp, expected_stats.hp).map_err(|_| failure())?;
            }
        }
        if pokemon.level != expected_level || pokemon.experience != expected_xp
            || pokemon.stats != expected_stats || pokemon.max_hp != expected_stats.hp
            || pokemon.hp != expected_hp || pokemon.fainted != (expected_hp == 0)
        { return Err(failure()); }
    }
    Ok(())
}
