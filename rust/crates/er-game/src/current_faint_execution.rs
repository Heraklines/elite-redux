...
use er_state::current_faint_execution::{
    CurrentEnemyFaintHistoryV1, CurrentFaintAddressV1, CurrentFaintPhaseV1,
};
...
fn faint_score(
    ...
    cap: u16,
    iv_sum: u32,
) -> Result<SafeU53, GameRuntimeV6Error> {
    if cap == 0 || level == 0 || iv_sum > 186 {
        return Err(failure());
    }
    let score = ((base_exp.get() as f64)
        * (f64::from(level) / f64::from(cap))
        * ((f64::from(iv_sum) / 93.0) * 0.2 + 0.8))
        .ceil();
    if !score.is_finite() || !(0.0..=9_007_199_254_740_991.0).contains(&score) {
        return Err(failure());
    }
...
fn address(
    ...
    if faints.len() != 1 || turn.finalization_done || turn.next_action == 0 {
        return Err(failure());
    }
    let faint = &faints[0];
    let enemy = battle.enemy_party.first().ok_or_else(failure)?;
    if faint.pokemon != source.initial_enemy.pokemon
        || enemy.id != faint.pokemon
        || faint.slot.side != BattleSide::Enemy
    ...
    let observed = observation
        .faints
        .iter()
        .find(|f| f.occurrence == pending.observation)
        .ok_or_else(failure)?;
    if observed.pokemon != faint.pokemon
        || observed.slot != faint.slot
        || observed.current_faint != Some(faint.id)
        || observed.current_action != Some(turn.next_action)
        || observed.before_hp == 0
        || pending.source.pokemon != faint.pokemon
        || observed.resolved_turn != turn.turn
    ...
    let selected = turn
        .actions
        .get(usize::from(turn.next_action - 1))
        .ok_or_else(failure)?;
    ...
    if !run.party.iter().any(|p| p.id == *actor && p.hp > 0) {
        return Err(failure());
    }
    if selected.source_slot.side != BattleSide::Player
        || !battle
            ...
            .slots
            .iter()
            .any(|s| s.slot == selected.source_slot && s.occupant == Some(*actor))
        || !selected
            .current_targets
            .as_ref()
            .is_some_and(|targets| targets.contains(&faint.slot))
    {
        return Err(failure());
    }
    ...
    match &resolved.struggle_pp_before {
        None => {
            if !live_actor.moves[usize::from(move_slot.get())]
                .as_ref()
                .is_some_and(|slot| slot.move_id == move_id)
            {
                return Err(failure());
            }
            content
        ...
        Some(slots) => {
            if &live_actor.moves != slots || move_id.get().get() != 165 {
                return Err(failure());
            }
            let mut pre_action = live_actor.clone();
            pre_action.moves = *slots;
            let (definition, fallback) = er_battle::m7_resolver::effective_move_definition_v5(
                &content.battle,
                &pre_action,
                *move_slot,
            )
            .map_err(|_| failure())?;
            if !fallback || definition.id != move_id {
                return Err(failure());
            }
        }
    }
    ...
    let killer = tracker_battle
        .enemy_ko_killers
        .get(&faint.pokemon)
        .ok_or_else(failure)?;
    if tracker.run_id != run.run_id
        || tracker_battle.wave_index != run.wave
        || killer.user_id != *actor
    ...
    for slot in battle
        ...
        .slots
        .iter()
        .filter(|s| s.slot.side == BattleSide::Player)
    {
        if let Some(id) = slot.occupant {
            ...
            if !pending
                .participants
                .iter()
                .any(|p| p.pokemon == id && p.owner == seat)
                || !observation
                    .participants
                    .iter()
                    .any(|p| p.pokemon == id && p.owner == seat)
    ...
    let metadata = content
        .progression
        .experience_for_compiled_form(enemy.species_id, enemy.form_index)
        .map_err(|_| failure())?;
...
pub(crate) fn prepare_current_enemy_faint(
    before: &GameStateV6,
    content: &PreparedGameContentV2,
    pending_id: SafeU53,
) -> Result<CurrentFaintPrelude, GameRuntimeV6Error> {
    let address = address(before, content, pending_id)?;
    let source = current_source_progression(before, content)?;
    if !source.initial_faint.valid(source.initial_enemy.pokemon)
        || source.initial_faint.phase.is_some()
    {
        return Err(failure());
    }
    let pending = before
    ...
    if pending.victory.is_some()
        || pending.friendship.as_ref() != Some(&CurrentFriendshipPhaseV1::default())
    {
        return Err(failure());
    }
    ...
    if !run
        ...
        .slots
        .iter()
        .any(|s| s.slot == address.slot && s.occupant == Some(address.pokemon))
    {
        return Err(failure());
    }
    ...
    let tracker = candidate
        .current_achievement_tracker
        .as_mut()
        .and_then(|t| t.battle.as_mut())
        .ok_or_else(failure)?;
    if tracker.enemy_ko_turns.contains_key(&address.pokemon) {
        return Err(failure());
    }
    ...
    let effects = tracker
        .longest_turn_effects
        .get_or_insert_with(Default::default);
    effects.insert(format!("faint:{}", address.pokemon.get()));
    let mut achievements = Vec::new();
    if effects.len() >= 10 {
        achievements.push("THE_LONGEST_TURN".to_owned());
    }
    if tracker
        .no_sell_token
        .as_ref()
        .is_some_and(|token| token.attacker_id == address.pokemon)
    {
    ...
    source
        .initial_faint
        .history
        .push(CurrentEnemyFaintHistoryV1 {
            pokemon: address.pokemon,
            turn: address.turn,
        });
    Ok(CurrentFaintPrelude {
        state: candidate,
        address,
        achievements,
    })
...
pub(crate) fn complete_current_faint_animation(
    before: &GameStateV6,
    content: &PreparedGameContentV2,
    pending_id: SafeU53,
    ...
    let source = current_source_progression(before, content)?;
    if !matches!(&source.initial_faint.phase, Some(CurrentFaintPhaseV1::Animation { address, event_id: retained }) if address == &expected && *retained == event_id)
    {
        return Err(failure());
    }
    let mut candidate = before.clone();
    ...
    let slot = battle
        ...
        .slots
        .iter_mut()
        .find(|s| s.slot == expected.slot)
        .ok_or_else(failure)?;
    if slot.occupant != Some(expected.pokemon) {
        return Err(failure());
    }
...
pub(crate) fn complete_current_faint_message(
    before: &GameStateV6,
    content: &PreparedGameContentV2,
    pending_id: SafeU53,
    event_id: PresentationEventId,
    ...
    let expected = address(before, content, pending_id)?;
    let source = current_source_progression(before, content)?;
    if !matches!(&source.initial_faint.phase, Some(CurrentFaintPhaseV1::Message { address, event_id: retained }) if address == &expected && *retained == event_id)
    {
        return Err(failure());
    }
    let battle = before
    ...
    if battle
        ...
        .slots
        .iter()
        .any(|slot| slot.occupant == Some(expected.pokemon))
    {
        return Err(failure());
    }
    let mut candidate = before.clone();
...
fn validate_current_initial_faint_at_frontier(
    ...
    if !matches!(
        source.initial_faint.phase,
        Some(CurrentFaintPhaseV1::ReadyForVictory { .. })
    ) {
        let owner = state
            .current_battle_participation
            .as_ref()
            .and_then(|p| p.experience.as_ref())
            .ok_or_else(failure)?;
        if owner.pending.iter().any(|pending| {
    ...
    let Some(phase) = &source.initial_faint.phase else {
        return Ok(());
    };
    let expected = address(state, content, phase.address().pending_id)?;
    if phase.address() != &expected {
        return Err(failure());
    }
    let battle = state
        .active_run
    ...
    let tracker = state
        ...
        .ok_or_else(failure)?;
    if tracker.faint_ledger_turn != Some(expected.turn)
        || !tracker.enemy_field_faints.contains(&expected.pokemon)
        || tracker.enemy_ko_turns.get(&expected.pokemon) != Some(&expected.turn)
        || tracker.longest_turn_number != Some(expected.turn)
        || !tracker
            .longest_turn_effects
            .as_ref()
            .is_some_and(|effects| effects.contains(&format!("faint:{}", expected.pokemon.get())))
    {
        return Err(failure());
    }
    Ok(())
...
const TOWN_NONBOSS_SPECIES: &[u64] = &[
...
mod tests {
    ...
    fn faint_score_matches_five_actual_source_method_observations() {
        ...
        for (level, ivs, expected) in [
            ...
            assert_eq!(
                faint_score(base_exp, level, 10, ivs.iter().sum())
                    .expect("observed score")
                    .get(),
                expected
            );
        }
