...
pub(super) fn transition(
    ...
    if operation_id.as_str().is_empty() || revision == SafeU53::ZERO
        ...
        || run.control.actionable || turn.authority != authority_seat
        || before.current_presentation.is_none()
    { return Err(failure()); }
    let (mut candidate, request) = match &phase {
        GameOwnedPhaseV1::FaintBegin { .. } => {
            let prelude = crate::current_faint_execution::prepare_current_enemy_faint(before, content, pending)?;
            if !prelude.achievements.is_empty() {
                return Err(GameRuntimeV6Error::Domain(format!(
                    "Faint achievement requests require owned reward dispatch: {}", prelude.achievements.join(",")
                )));
            }
            let payload = GamePresentationPayloadV1::FaintAnimation { holder: prelude.address.pokemon, tween_milliseconds: 500 };
            (prelude.state, Some((prelude.address, payload, true)))
        }
        GameOwnedPhaseV1::FaintPresentation { event_id, animation: true, .. } => {
            let candidate = crate::current_faint_execution::complete_current_faint_animation(before, content, pending, *event_id)?;
            let source = candidate.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref())
                .and_then(|owner| owner.source_progression.as_ref()).ok_or_else(failure)?;
            let Some(F::MessageReady { address }) = &source.initial_faint.phase else { return Err(failure()); };
            let address = address.clone();
            let payload = GamePresentationPayloadV1::FaintMessage { holder: address.pokemon };
            (candidate, Some((address, payload, false)))
        }
        GameOwnedPhaseV1::FaintPresentation { event_id, animation: false, .. } => (
            crate::current_faint_execution::complete_current_faint_message(before, content, pending, *event_id)?, None,
        ),
        _ => return Err(failure()),
    };
    ...
    if let Some((address, payload, animation)) = request {
        ...
        let source = candidate.current_battle_participation.as_mut().and_then(|owner| owner.experience.as_mut())
            .and_then(|owner| owner.source_progression.as_mut()).ok_or_else(failure)?;
        source.initial_faint.phase = Some(if animation { F::Animation { address, event_id } } else { F::Message { address, event_id } });
    }
