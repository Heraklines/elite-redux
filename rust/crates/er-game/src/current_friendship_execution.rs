...
use er_progression::current_friendship::{
    ExistingStarterAccount, FriendshipSourceProvenance, add_resolved_starter_candy,
...
use er_state::current_experience_owner::{
    CurrentExperienceExecutionOriginV1, CurrentFriendshipAwardV1,
    CurrentFriendshipClockRequestV1, CurrentFriendshipPhaseV1,
    CurrentFriendshipClockPurposeV1, CurrentFriendshipHeadV1,
};
use er_state::current_friendship_profile::{CURRENT_FRIENDSHIP_RIBBON_V1, CurrentFriendshipRibbonV1};
...
fn event_rate(utc: i64) -> Result<(u8, bool), GameRuntimeV6Error> {
    ...
    Ok(EVENTS.iter().find(|(start, end, _, _)| *start < utc && utc < *end)
...
fn candy_rate(difficulty: RunDifficultyV1, wave: u64) -> Result<u8, GameRuntimeV6Error> {
    ...
    rates.get(usize::try_from((wave - 1) / 20).map_err(|_| GameRuntimeV6Error::Invalid)?)
        .copied().ok_or(GameRuntimeV6Error::Invalid)
}
...
pub(crate) fn prepare_clock_result(
    ...
    before.validate_with(content).map_err(|_| GameRuntimeV6Error::Invalid)?;
    ...
    let profile = before.current_friendship_profile.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let difficulty = before.current_run_difficulty.ok_or(GameRuntimeV6Error::Action)?;
    if owner.execution_origin != Some(CurrentExperienceExecutionOriginV1::FreshNormalClassic)
        || profile.owner_seat != owner.authority || difficulty.run_id != run.run_id
        || !content.supports_current_experience_mode(run.mode)
        || !run.modifiers.is_empty()
    {
        return Err(GameRuntimeV6Error::Action);
    }
    let pending_index = owner.pending.iter().position(|pending| {
        pending.friendship.as_ref().is_some_and(|phase| !phase.complete)
    }).ok_or(GameRuntimeV6Error::Action)?;
    let pending = &owner.pending[pending_index];
    let phase = pending.friendship.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    if pending.id != request.pending || phase.clock.as_ref() != Some(request) {
        return Err(GameRuntimeV6Error::Action);
    }
    phase.validate(pending).map_err(|_| GameRuntimeV6Error::Invalid)?;
    let head = phase.head.as_ref().ok_or(GameRuntimeV6Error::Invalid)?;
    let pokemon_index = run.party.iter().position(|pokemon| pokemon.id == request.recipient)
        .ok_or(GameRuntimeV6Error::Action)?;
    ...
    if pokemon.owner_seat != Some(profile.owner_seat) || pokemon.hp == 0
        || pokemon.form_index != 0 || pokemon.fusion.is_some()
    ...
    let account_index = profile.accounts.binary_search_by_key(&pokemon.species_id, |row| row.species)
        .map_err(|_| GameRuntimeV6Error::Action)?;
    let account = &profile.accounts[account_index];
    let source_id = u32::try_from(pokemon.species_id.get().get())
        .map_err(|_| GameRuntimeV6Error::Action)?;
    let cost = crate::current_friendship_profile::PINNED_STARTER_COSTS.iter()
        .find(|(species, _)| *species == source_id)
        .map(|(_, cost)| *cost).ok_or(GameRuntimeV6Error::Action)?;
    ...
    let provenance = er_canonical::fixture_digest(&(
        &before.content_identity, run.run_id, run.wave, difficulty, request, utc,
        pokemon.species_id, cost, multiplier, rate,
    ...
    let resolved = ResolvedPositiveFriendship {
        ...
        capped: false,
        fun_debug: false,
        mode: ResolvedFriendshipMode::Classic { candy_multiplier: f64::from(multiplier) },
    ...
    let arithmetic_head = prepare_phase_head(f64::from(head.before), 3.0, Some(3.0), false, false)
        .map_err(|_| GameRuntimeV6Error::Action)?;
    let tail = finish_phase_tail(&arithmetic_head,
        &[ExistingStarterAccount {
            species_id: source_id,
            friendship_progress: account.friendship_progress.get(),
            candy_count: i64::try_from(account.candy_count.get()).map_err(|_| GameRuntimeV6Error::Invalid)?,
        }], &resolved).map_err(|_| GameRuntimeV6Error::Action)?;
    let plan = tail.plan;
    let result = plan.accounts.first().filter(|_| plan.accounts.len() == 1)
    ...
    let award = CurrentFriendshipAwardV1 {
        request: request.clone(), utc_milliseconds: utc,
        source_root: pokemon.species_id, candy_root: pokemon.species_id,
        ...
        progress_after: SafeU53::new(result.friendship_progress).map_err(|_| GameRuntimeV6Error::Invalid)?,
        candy_before: account.candy_count,
        candy_after: SafeU53::new(u64::try_from(result.candy_count).map_err(|_| GameRuntimeV6Error::Invalid)?)
            .map_err(|_| GameRuntimeV6Error::Invalid)?,
        max_clock: head.max_clock.clone(),
        max_utc_milliseconds: head.max_utc_milliseconds,
    };
    let mut candidate = before.clone();
    candidate.active_run.as_mut().ok_or(GameRuntimeV6Error::Invalid)?.party[pokemon_index]
        .friendship = after_friendship;
    let updated = candidate.current_friendship_profile.as_mut()
        .ok_or(GameRuntimeV6Error::Invalid)?.accounts.get_mut(account_index)
        .ok_or(GameRuntimeV6Error::Invalid)?;
    updated.friendship_progress = award.progress_after;
    updated.candy_count = award.candy_after;
    let updated_pending = candidate.current_battle_participation.as_mut()
    ...
    let updated_phase = updated_pending.friendship.as_mut().ok_or(GameRuntimeV6Error::Invalid)?;
    ...
    let payloads = plan.intents.into_iter().map(|intent| match intent {
        er_progression::current_friendship::FriendshipIntent::ShowStarterCandy {
            root, scaled_count, before_candy, after_candy,
        } => Ok(GamePresentationPayloadV1::StarterCandy {
            root: er_types::battle_ids::SpeciesId::new(safe(u64::from(root))?),
            scaled_count,
            before_candy: safe(u64::try_from(before_candy).map_err(|_| GameRuntimeV6Error::Invalid)?)?,
            after_candy: safe(u64::try_from(after_candy).map_err(|_| GameRuntimeV6Error::Invalid)?)?,
        }),
...
pub(crate) fn prepare_next_friendship(
    ...
    before.validate_with(content).map_err(|_| GameRuntimeV6Error::Invalid)?;
    let observation = before.current_battle_participation.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let owner = observation.experience.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let run = before.active_run.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let profile = before.current_friendship_profile.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let rewards = profile.rewards.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    if owner.execution_origin != Some(CurrentExperienceExecutionOriginV1::FreshNormalClassic)
        || before.current_targeting.is_none() || !run.modifiers.is_empty()
        || profile.owner_seat != owner.authority
    { return Err(GameRuntimeV6Error::Action); }
    let index = owner.pending.iter().position(|pending| pending.friendship.as_ref()
        .is_some_and(|phase| !phase.complete)).ok_or(GameRuntimeV6Error::Action)?;
    let pending = &owner.pending[index];
    let phase = pending.friendship.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    if phase.clock.is_some() || phase.head.is_some() { return Err(GameRuntimeV6Error::Action); }
    ...
    let pokemon = run.party.iter().find(|pokemon| pokemon.id == recipient)
        .ok_or(GameRuntimeV6Error::Action)?;
    if pokemon.hp == 0 || pokemon.owner_seat != Some(profile.owner_seat)
        || pokemon.form_index != 0 || pokemon.fusion.is_some()
        || !matches!(pokemon.species_id.get().get(), 1 | 4 | 7)
        || !pokemon.held_items.is_empty()
    { return Err(GameRuntimeV6Error::Action); }
    ...
    } else { CurrentFriendshipClockPurposeV1::TimedEvent };
    let mut candidate = before.clone();
    let request = CurrentFriendshipClockRequestV1 {
        request: candidate.identities.allocate_platform_request_id()
            .map_err(|_| GameRuntimeV6Error::Invalid)?,
        pending: pending.id, recipient, purpose,
    };
    candidate.active_run.as_mut().ok_or(GameRuntimeV6Error::Invalid)?.party.iter_mut()
        .find(|value| value.id == recipient).ok_or(GameRuntimeV6Error::Invalid)?.friendship = after;
    ...
    phase.head = Some(CurrentFriendshipHeadV1 {
        recipient, before: pokemon.friendship, after, max_clock: None, max_utc_milliseconds: None,
    });
...
fn phase_mut(state: &mut GameStateV6, index: usize)
    -> Result<&mut CurrentFriendshipPhaseV1, GameRuntimeV6Error>
{
    state.current_battle_participation.as_mut().and_then(|value| value.experience.as_mut())
        .and_then(|owner| owner.pending.get_mut(index)).and_then(|value| value.friendship.as_mut())
        .ok_or(GameRuntimeV6Error::Invalid)
}

fn award_ribbon(state: &mut GameStateV6, species: er_types::battle_ids::SpeciesId)
    -> Result<(), GameRuntimeV6Error>
{
    ...
    if !matches!(species.get().get(), 1 | 4 | 7) { return Err(GameRuntimeV6Error::Action); }
    let rewards = state.current_friendship_profile.as_mut().and_then(|value| value.rewards.as_mut())
        .ok_or(GameRuntimeV6Error::Action)?;
    match rewards.ribbons.binary_search_by_key(&species, |row| row.species) {
        Ok(_) => {},
        Err(index) => rewards.ribbons.insert(index, CurrentFriendshipRibbonV1 {
            species, bits: safe(CURRENT_FRIENDSHIP_RIBBON_V1)?,
        }),
    }
    Ok(())
}
...
fn settle_max_clock(
    before: &GameStateV6, content: &PreparedGameContentV2, index: usize,
    request: &CurrentFriendshipClockRequestV1, utc: i64,
) -> Result<(GameStateV6, Vec<GamePresentationPayloadV1>), GameRuntimeV6Error> {
    if !(-8_640_000_000_000_000..=8_640_000_000_000_000).contains(&utc) {
        return Err(GameRuntimeV6Error::Action);
    }
    let run = before.active_run.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    let profile = before.current_friendship_profile.as_ref().ok_or(GameRuntimeV6Error::Action)?;
    if profile.rewards.as_ref().ok_or(GameRuntimeV6Error::Action)?.max_is_unlocked()
        || run.party.is_empty() || run.party.len() > 6
    { return Err(GameRuntimeV6Error::Action); }
    let difficulty = before.current_run_difficulty.ok_or(GameRuntimeV6Error::Action)?;
    let count = match difficulty.difficulty {
        RunDifficultyV1::Youngster => 10, RunDifficultyV1::Ace => 15,
        RunDifficultyV1::Elite => 20, RunDifficultyV1::Hell => 30,
        RunDifficultyV1::Mystery => return Err(GameRuntimeV6Error::Action),
    };
    let rate = candy_rate(difficulty.difficulty, run.wave.get().get())?;
    let species = run.party.iter().find(|pokemon| pokemon.id == request.recipient)
        .ok_or(GameRuntimeV6Error::Action)?.species_id;
    let mut candidate = before.clone();
    let mut payloads = vec![GamePresentationPayloadV1::AchievementUnlocked {
        achievement: GamePresentationAchievementV1::MaxFriendship, utc_milliseconds: utc,
    }];
    let updated_profile = candidate.current_friendship_profile.as_mut().ok_or(GameRuntimeV6Error::Invalid)?;
    updated_profile.rewards.as_mut().ok_or(GameRuntimeV6Error::Invalid)?.max_friendship_unlocked_at = Some(utc);
    ...
    for pokemon in &run.party {
        if pokemon.owner_seat != Some(profile.owner_seat) || pokemon.form_index != 0
            || pokemon.fusion.is_some() || !matches!(pokemon.species_id.get().get(), 1 | 4 | 7)
        { return Err(GameRuntimeV6Error::Action); }
        let account = updated_profile.accounts.binary_search_by_key(&pokemon.species_id, |row| row.species)
            .map_err(|_| GameRuntimeV6Error::Action)?;
        ...
        let result = add_resolved_starter_candy(
            ...
            count, true, true, false, rate,
        ).map_err(|_| GameRuntimeV6Error::Action)?;
        row.candy_count = safe(u64::try_from(result.candy_count).map_err(|_| GameRuntimeV6Error::Invalid)?)?;
        if let Some(scaled_count) = result.candy_bar_count {
            payloads.push(GamePresentationPayloadV1::StarterCandy {
                root: row.species, scaled_count, before_candy: previous, after_candy: row.candy_count,
            });
        }
    }
    updated_profile.rewards.as_mut().ok_or(GameRuntimeV6Error::Invalid)?.cosmetic_bits = vec![0, 0, 0, 0, 128, 1];
    award_ribbon(&mut candidate, species)?;
    let next_request = candidate.identities.allocate_platform_request_id().map_err(|_| GameRuntimeV6Error::Invalid)?;
    let phase = phase_mut(&mut candidate, index)?;
    ...
    phase.clock = Some(CurrentFriendshipClockRequestV1 {
        request: next_request, pending: request.pending, recipient: request.recipient,
