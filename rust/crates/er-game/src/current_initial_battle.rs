...
pub(crate) fn construct_current_initial_battle(
    ...
    if run_seed.is_empty()
        || !wave_slope.is_finite() || wave_slope <= 0.0
        || !quadratic_divisor.is_finite() || quadratic_divisor <= 0.0
    {
        return Err(failure());
    }
    ...
    for _ in 0..16 {
        let index = rng.integer_in_range(SafeU53::ZERO, SafeU53::new(61).map_err(|_| failure())?)
            .map_err(|_| failure())?.get() as usize;
    ...
    Ok(CurrentInitialBattleConstructionV1 {
        wave_seed, battle_seed, enemy_level: rounded as u16,
        #[cfg(test)]
        level_rng_before,
        #[cfg(test)]
...
mod tests {
    ...
    fn initial_classic_constructor_matches_actual_source_seed_and_level() {
        ...
        assert_eq!(constructed.level_rng_before.state_string,
            "!rnd,696234,0.050779115641489625,0.7305614135693759,0.37434818921610713");
        assert_eq!(constructed.level_rng_after.state_string,
        ...
        let battle = runtime.initialize_battle(&constructed.wave_seed,
            er_types::battle_ids::WaveIndex::new(SafeU53::new(1).expect("safe wave")).expect("positive wave"))
            .expect("current battle seed admission");
        assert_eq!(battle.battle_seed, constructed.battle_seed);
        assert_eq!(runtime.run_state(), before, "isolated constructor preserves encounter RNG");
    }
}
