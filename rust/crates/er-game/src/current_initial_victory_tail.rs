...
pub(crate) fn projection_for_validation(
    ...
    if let Some(reward) = tail.reward.as_ref() {
        ...
        if let Some(candy) = reward.candy.as_ref() {
            projected.current_friendship_profile = Some((*candy.profile_before).clone());
        }
    }
    ...
    if let Some(tm) = tail.reward.as_ref().and_then(|reward| reward.tm.as_ref()) {
        crate::current_reward_tm::set_history(
            &mut projected,
            tm.holder,
            tm.history_before.clone(),
        )?;
        projected.current_achievement_tracker = Some(*tm.tracker_before.clone());
    }
    Ok(projected)
}
