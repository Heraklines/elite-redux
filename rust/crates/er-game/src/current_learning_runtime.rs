...
pub(super) fn execute(
    ...
    let owner = before
        ...
        .ok_or_else(failure)?;
    let (pending_id, batch) = owner
        .pending
        .iter()
        .find_map(|pending| {
            if let D::LearnMoveBatch { batch } = &pending.victory.as_ref()?.descendant {
                Some((pending.id, batch))
            } else {
                None
            }
        })
        .ok_or_else(failure)?;
    let canonical =
        crate::m7_progression_control::current_learn_move_batch_control(&context.action, batch)
            .map_err(|_| failure())?;
    let accepted = GameActionV1::CurrentLearnMoveBatch {
        action: action.clone(),
    };
    ...
    if run.control != canonical
        || !context.authority
        || !menu
            .options
            .iter()
            .any(|row| row.enabled && row.visible && row.action == accepted)
            && !back
    {
        return Err(failure());
    }
    let (mut candidate, next, learned) =
    ...
    next_context.operation_id = OperationId::new(format!(
        "current/learn/{}/{}/{}",
        run.run_id.get().get(),
        pending_id.get(),
        next_context.authority_revision.get()
    ))
    .map_err(|_| failure())?;
    let next_control = if next.complete {
        GameControlPlanV2 {
            schema_version: er_types::GAME_CONTROL_PLAN_SCHEMA_VERSION_V2,
            revision: next_context.authority_revision,
        ...
        crate::m7_progression_control::current_learn_move_batch_control(&next_context, &next)
            .map_err(|_| failure())?
    };
    candidate
        .current_battle_participation
        ...
        .ok_or_else(failure)?
        .descendant = D::LearnMoveBatch { batch: next };
...
pub(crate) fn validate_transition(
    ...
    let failure = || GameRuntimeV6Error::Action;
    let Some(GameActionV1::CurrentLearnMoveBatch { action }) = &transition.accepted_action else {
        return Err(failure());
    };
    let context = before
        .active_run
    ...
    let input = GameActionDispatchContextV1 {
        action: context,
        authority: true,
    ...
    let mut presentation = vec![GamePresentationEffectV2 {
        event_id: PresentationEventId::new(transition.authority_revision),
        semantic,
    ...
    let next_control = normalize_next_control(
        &mut candidate,
        safe_increment(transition.authority_revision)?,
    )?;
    ...
    let expected = GameTransitionMaterialV6 {
        ...
        domain: GameActionDomainV2::MoveLearning,
        ...
        accepted_action: transition.accepted_action.clone(),
        owned_phase: None,
