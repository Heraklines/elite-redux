...
impl CurrentLevelAchievement {
    pub(crate) fn source_key(self) -> &'static str {
        match self {
            Self::Level100 => "LV_100",
            Self::Level250 => "LV_250",
            Self::Level1000 => "LV_1000",
        }
    }
...
pub(crate) fn prepare_current_level_account(
    ...
    let retained = owner
        .pending
        .iter()
        .find(|p| p.id == level_up.award.phase.pending_id)
    ...
    let pokemon = before
        .active_run
        .as_ref()
        .and_then(|run| run.party.get(usize::from(level_up.award.phase.party_index)))
        .ok_or_else(failure)?;
    if pokemon.id != level_up.award.phase.pokemon
        || pokemon.level != level_up.new_level
    ...
    let achievements = [
        CurrentLevelAchievement::Level100,
        CurrentLevelAchievement::Level250,
        CurrentLevelAchievement::Level1000,
    ]
    .into_iter()
    .filter(|achievement| level_up.new_level >= achievement.threshold())
    .collect();
    Ok(CurrentLevelAccountCandidate {
        state: candidate,
        achievements,
    })
}
