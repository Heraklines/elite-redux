...
use er_state::current_friendship_profile::{
    CURRENT_FRIENDSHIP_RIBBON_V1, CurrentFriendshipProfileV1, CurrentFriendshipRibbonV1,
};
...
fn safe(value: u64) -> Result<SafeU53, E> {
    SafeU53::new(value).map_err(|_| E::Action)
}
...
pub(crate) fn replay(
    ...
    if candy.max_utc.is_some() && !needs_max {
        return Err(E::Action);
    }
    if needs_max {
        let Some(utc) = candy.max_utc else {
            if candy.event_utc.is_some() {
                return Err(E::Action);
            }
            return Ok((profile, effects));
        };
        crate::current_friendship_execution::candy_rates(
            candy,
            utc,
            difficulty,
            run.wave.get().get(),
        ...
        let (mut updated, team) =
            crate::current_achievement_rewards::prepare_achievement_team_candy(
        ...
        effects.push(P::AchievementUnlocked {
            achievement: A::MaxFriendship,
    ...
    let Some(utc) = candy.event_utc else {
        return Ok((profile, effects));
    };
    let (multiplier, fusions, rate) = crate::current_friendship_execution::candy_rates(
        candy,
        utc,
        difficulty,
        run.wave.get().get(),
    ...
    let resolution = er_canonical::fixture_digest(&(
        identity,
        candy.pending,
        candy.offer,
        candy.holder,
    ...
    let resolved = ResolvedPositiveFriendship {
        provenance: FriendshipSourceProvenance {
            oracle_sha: identity.oracle_sha.as_str().to_owned(),
    ...
    let head = prepare_phase_head(f64::from(pokemon.friendship), 6.0, Some(6.0), true, false)
        .map_err(|_| E::Action)?;
    let tail = finish_phase_tail(
        ...
        &[ExistingStarterAccount {
            ...
            candy_count: i64::try_from(account.candy_count.get()).map_err(|_| E::Action)?,
        }],
        &resolved,
    )
    .map_err(|_| E::Action)?;
    if tail.plan.friendship != f64::from(after) || tail.plan.accounts.len() != 1 {
        return Err(E::Action);
    }
    let result = &tail.plan.accounts[0];
    profile.accounts[index].friendship_progress = safe(result.friendship_progress)?;
    profile.accounts[index].candy_count =
        safe(u64::try_from(result.candy_count).map_err(|_| E::Action)?)?;
    for intent in tail.plan.intents {
        let er_progression::current_friendship::FriendshipIntent::ShowStarterCandy {
            root,
            scaled_count,
            before_candy,
            after_candy,
        ...
        effects.push(P::StarterCandy {
            root: er_types::battle_ids::SpeciesId::new(safe(u64::from(root))?),
            scaled_count,
            before_candy: safe(u64::try_from(before_candy).map_err(|_| E::Action)?)?,
            after_candy: safe(u64::try_from(after_candy).map_err(|_| E::Action)?)?,
        });
    }
    Ok((profile, effects))
}
