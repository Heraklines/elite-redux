...
pub(crate) fn friendship(before: u16) -> Result<u16, E> {
    ...
    Ok(if before.saturating_add(6) > 200 {
        before.max(200)
    } else {
        before + 6
    })
...
pub(crate) fn apply_level(
    ...
    if !matches!(before.species_id.get().get(), 1 | 4 | 7)
        || before.form_index != 0
        || !(1..=10).contains(&before.level)
        || before.fusion.is_some()
        || !before.held_items.is_empty()
        || before.hp == 0
        || before.fainted
    ...
    after.experience =
        er_progression::progression::current_growth_experience_for_level(growth, after.level)
            .map_err(|_| E::Action)?;
    ...
    let form = content
        .battle
        .form(
            &er_types::FormId::parse(format!("{}:0", after.species_id.get().get()))
                .map_err(|_| E::Action)?,
        )
        .map_err(|_| E::Action)?;
    if form.species != after.species_id {
        return Err(E::Action);
    }
    let nature = content
        .progression
        ...
        .natures
        .iter()
        .find(|n| n.id == after.effective_nature)
        .ok_or(E::Action)?;
    ...
    .map_err(|_| E::Action)?;
    after.hp = er_progression::current_stats::current_hp_after_stat_calculation(
        after.hp,
        after.max_hp,
        stats.hp,
    )
    .map_err(|_| E::Action)?;
    after.max_hp = stats.hp;
    after.stats = stats;
    Ok(after)
}

pub(crate) fn empty_children(
    ...
    let definition = content
        ...
        .ok_or(E::Action)?;
    if previous < 100
        && definition.level_moves.iter().any(|row| {
            row.level > 0
                && i32::from(row.level) > i32::from(previous)
                && i32::from(row.level) <= i32::from(pokemon.level)
        })
    {
        return Err(E::Domain(
            "Rare Candy retains unresolved LearnMoveBatch children".into(),
        ));
    }
    ...
    for id in &definition.evolutions {
        ...
        if evolution.consume_item.is_some() {
            return Err(E::Domain(
                "Rare Candy item evolution condition ordering is unresolved".into(),
            ));
        }
        if condition(&evolution.condition, pokemon)? {
            return Err(E::Domain(
                "Rare Candy retains unresolved Evolution children".into(),
            ));
        }
    }
    Ok(())
...
fn condition(
    value: &er_progression::content_v2::EvolutionConditionV2,
    pokemon: &PokemonStateV5,
) -> Result<bool, E> {
    use er_progression::content_v2::EvolutionConditionV2 as C;
    match value {
        C::Always => Ok(true),
        C::MinimumLevel(level) => Ok(pokemon.level >= *level),
        C::MinimumFriendship(value) => Ok(pokemon.friendship >= *value),
        C::KnownMove(id) => Ok(pokemon
            .moves
            .iter()
            .flatten()
        ...
        _ => Err(E::Domain(
            "Rare Candy evolution predicate is unresolved".into(),
        )),
    }
}
