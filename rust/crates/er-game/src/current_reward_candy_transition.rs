...
pub(super) fn transition(
    ...
    let pending = match &phase {
        GameOwnedPhaseV1::RewardCandy { pending, .. } => *pending,
        GameOwnedPhaseV1::FriendshipClock { request, .. } => request.pending,
    ...
    let (mut candidate, payloads, requested) = match &phase {
        GameOwnedPhaseV1::FriendshipClock {
            ...
            let (next, payloads) = crate::current_reward_candy::settle_clock(
                before,
                content,
                request,
                *utc_milliseconds,
        ...
        GameOwnedPhaseV1::RewardCandy { callback, .. }
            if matches!(candy.phase, C::Queued) && callback.is_none() =>
        {
            (
                crate::current_reward_candy::begin(before, content, pending)?,
                Vec::new(),
                None,
            )
        }
        GameOwnedPhaseV1::RewardCandy { callback, .. } => {
            let (next, request) =
                crate::current_reward_candy::advance(before, content, pending, *callback)?;
    ...
    if let Some(stats) = requested {
        presentation.push(GamePresentationEffectV2 {
            ...
            blocking: mapping.blocking,
            skip: mapping.skip,
            payload: Some(crate::current_reward_candy::presentation(&candy, stats)?),
        });
    }
    ...
    if let Some(stats) = requested {
        ...
        if stats {
            owner.phase = C::Stats { event_id };
            owner.stats_event = Some(event_id);
        } else {
            owner.phase = C::Message { event_id };
            owner.message_event = Some(event_id);
        }
    ...
    let next_clock = crate::current_reward_candy::clock(crate::current_reward_candy::owner(
        &candidate, pending,
    )?);
    let platform_effects = next_clock
        .filter(|clock| Some(*clock) != prior_clock)
        .map(|request| {
