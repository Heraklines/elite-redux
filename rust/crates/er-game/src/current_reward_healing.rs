...
pub(crate) enum HealingItem { Potion, SuperPotion, HyperPotion, MaxPotion }
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum EffectError { Invalid, UnresolvedSource }
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum HealingContext { SourceNeutralMultiplierOne }

impl HealingItem {
    #[cfg(test)]
#[allow(dead_code)]
    pub(crate) fn source_id(self) -> &'static str {
        match self { Self::Potion => "POTION", Self::SuperPotion => "SUPER_POTION",
            Self::HyperPotion => "HYPER_POTION", Self::MaxPotion => "MAX_POTION" }
    }
    fn amount(self, max_hp: u32) -> Result<u32, EffectError> {
        let (points, percent) = match self {
            Self::Potion => (20_u64,10_u64), Self::SuperPotion => (50,25),
            Self::HyperPotion => (200,50), Self::MaxPotion => (0,100),
        };
...
pub(crate) fn heal(
    ...
    if before.mechanics != MechanicStateStoreV2::default()
        || before.status.kind != StatusKind::None {
        return Err(EffectError::UnresolvedSource);
    }
    let mut after = before.clone();
    after.hp = u32::try_from((u64::from(before.hp) + u64::from(item.amount(before.max_hp)?))
        .min(u64::from(before.max_hp))).map_err(|_| EffectError::Invalid)?;
...
#[allow(dead_code)]
impl HealingReceipt {
    pub(crate) fn apply(before: &PokemonStateV5, item: HealingItem, context: HealingContext)
        -> Result<(Self, PokemonStateV5), EffectError> {
        let after = heal(before, item, context)?;
        Ok((Self { item, context, before: Box::new(before.clone()) }, after))
    }
