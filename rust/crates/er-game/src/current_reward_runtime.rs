...
pub(super) fn execute(
    ...
    let applied = candidate
        ...
        .is_some_and(|reward| {
            ...
            ) && reward.tm.as_ref().is_some_and(|tm| !tm.phase.actionable()))
        });
    ...
    if applied {
        ...
        crate::current_reward_selection::install_control(
            ...
            pending.id,
            context.menu_instance,
            safe_increment(context.authority_revision)?,
            context.authority_seat,
        )?;
    }
...
pub(crate) fn validate_transition(
    ...
    let context = before
        .active_run
        .as_ref()
        .and_then(|run| run.control.action_context.as_ref())
        .ok_or_else(failure)?;
    if context.operation_id != transition.operation_id
        ...
        || transition.domain != GameActionDomainV2::Reward
    {
        return Err(failure());
    }
