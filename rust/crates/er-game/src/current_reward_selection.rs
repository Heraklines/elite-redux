...
pub(crate) fn validate_live(state: &GameStateV6, id: SafeU53) -> Result<(), Error> {
    ...
    for declined in &selection.declined_tms {
        if !matches!(
            declined.phase,
            er_state::current_reward_tm::CurrentRewardTmPhaseV1::Declined { .. }
        ) {
            return Err(invalid());
        }
        let replay = crate::current_reward_tm::replay(state, selection, declined)?;
        let tracker = selection
            .tm
            .as_ref()
            .map(|tm| tm.tracker_before.as_ref())
            .or(state.current_achievement_tracker.as_ref())
            .ok_or_else(invalid)?;
        if replay.party != selection.party_before
            || replay.history != er_state::current_reward_tm::CurrentUsedTmsV1::Undefined
            || &replay.tracker != tracker
        {
            return Err(invalid());
        }
    }
    ...
    if let Some(tm) = selection.tm.as_deref() {
        ...
        if matches!(tm.phase, T::Declined { .. }) && matches!(selection.stage, Stage::Choice) {
            ...
            if replay.party != run.party
                ...
                || state.current_achievement_tracker.as_ref() != Some(&replay.tracker)
                || owned_run(state)? != &selection.run_before
            {
                return Err(invalid());
            }
            return Ok(());
        ...
        let (offer, holder) = match selection.stage {
            Stage::TmPending { offer, holder }
                if !matches!(tm.phase, T::Complete { .. } | T::Declined { .. }) =>
            {
                (offer, holder)
            }
            Stage::Applied {
                offer,
                holder: Some(holder),
            } if matches!(tm.phase, T::Complete { .. }) => (offer, holder),
        ...
        if holder != tm.holder
            || selection
                .offers
                .get(usize::from(offer))
                .is_none_or(|o| o.source_id != "TM_CASE" || o.args.is_some())
        {
            return Err(invalid());
        }
        let replay = crate::current_reward_tm::replay(state, selection, tm)?;
        if replay.party != run.party
            || replay.history != *crate::current_reward_tm::history(state, holder)?
            || state.current_achievement_tracker.as_ref() != Some(&replay.tracker)
            || owned_run(state)? != &selection.run_before
        {
            return Err(invalid());
    ...
    let (party, inventory) = match selection.stage {
        ...
        Stage::TmMove { offer, holder } => {
            if selection
                ...
                .get(usize::from(offer))
                .is_none_or(|o| o.source_id != "TM_CASE" || o.args.is_some())
                || !selection.party_before.iter().any(|p| p.id == holder)
            {
                return Err(invalid());
            }
            (selection.party_before.clone(), selection.run_before.clone())
...
pub(crate) fn validate(
    ...
    for tm in selection
        .declined_tms
        .iter()
        .chain(selection.tm.iter().map(Box::as_ref))
    {
        ...
        if let Some(first) = tm.messages.first() {
            if last_attempt_event.is_some_and(|previous| previous >= first.event_id) {
                return Err(invalid());
            }
        ...
        for message in &tm.messages {
            ...
            let effect = crate::m9e_material_v6::GamePresentationEffectV2 {
                ...
                skip: mapping.skip,
                payload: Some(crate::current_reward_tm::payload(
                    selection,
                    tm,
                    message.kind,
                )?),
            ...
            if !state.current_presentation.as_ref().is_some_and(|owner| {
                owner
                    .receipts
                    .iter()
                    .any(|r| r.event_id == message.event_id && r.effect_sha256 == hash)
            }) {
                return Err(invalid());
            }
        }
        ...
        if event.is_some_and(|event| tm.messages.last().is_none_or(|m| m.event_id != event)) {
            return Err(invalid());
        }
    }
...
pub(crate) fn select(
    ...
    let mut selection = tail(before, id)?
        ...
        .ok_or_else(invalid)?
        .clone();
    if matches!(selection.stage, Stage::Choice)
        && selection.tm.as_ref().is_some_and(|tm| {
            matches!(
                tm.phase,
                er_state::current_reward_tm::CurrentRewardTmPhaseV1::Declined { .. }
            )
    ...
    {
        if selection.declined_tms.len() >= 4096 {
            return Err(invalid());
        }
        selection
            .declined_tms
    ...
    let (offer, holder) = match selection.stage {
        ...
        Stage::Holder { offer } => {
            ...
            if selection.offers[usize::from(offer)].source_id == "TM_CASE" {
                if crate::current_reward_tm::available(p)?.is_empty() {
                    return Err(invalid());
                }
                selection.stage = Stage::TmMove {
                    offer,
                    holder: p.id,
                };
                let mut state = before.clone();
        ...
        Stage::TmMove { offer, holder } => {
            let tm =
                crate::current_reward_tm::prepare(before, content, &selection, holder, ordinal)?;
            selection.tm = Some(Box::new(tm));
            selection.stage = Stage::TmPending { offer, holder };
            let mut state = before.clone();
            set_selection(&mut state, id, selection)?;
            validate(&state, content, id)?;
            return Ok(state);
        ...
        Stage::TmPending { .. } => {
            ...
            tm.phase = match (&tm.phase, ordinal) {
                (T::Replace, 0) => T::ForgetQueued,
                (T::Replace, 1) => T::Stop,
                (T::ChooseSlot, 0..=3) => {
                    tm.slot = ordinal as u8;
                    T::LearningQueued
                }
                (T::ChooseSlot, 4) => T::Stop,
                (T::Stop, 0) => T::DeclineQueued,
                (T::Stop, 1) => {
                    tm.slot = 4;
                    T::Queued
                }
...
pub(crate) fn cancel_holder(
    ...
    selection.stage = match selection.stage {
        ...
        Stage::TmPending { offer, holder } => {
            ...
            if !matches!(
                tm.phase,
                er_state::current_reward_tm::CurrentRewardTmPhaseV1::ChooseSlot
            ) {
                return Err(invalid());
            }
            tm.phase = er_state::current_reward_tm::CurrentRewardTmPhaseV1::Stop;
            Stage::TmPending { offer, holder }
        }
        _ => return Err(invalid()),
    };
    ...
    let (entries, cancel) = match selection.stage {
        ...
        Stage::TmMove { holder, .. } => {
            let p = selection
                ...
                .ok_or_else(invalid)?;
            (
                crate::current_reward_tm::available(p)?
                    .iter()
                    .enumerate()
                    .map(|(i, m)| {
                        (
                            format!("reward/{}/tm/{}", id.get(), m.get().get()),
                            action(i as u32),
                        )
                    })
                    .collect::<Vec<_>>(),
        ...
        Stage::TmPending { holder, .. } => {
            ...
            let keys = match tm.phase {
                ...
                T::ChooseSlot => {
                    ...
                    let mut keys = p
                        ...
                        .map(|(i, slot)| {
                            slot.as_ref()
                                .map(|m| format!("forget/{i}/{}", m.move_id.get().get()))
                                .ok_or_else(invalid)
                        })
                        .collect::<Result<Vec<_>, _>>()?;
                    keys.push("do-not-learn".into());
                    keys
                }
                _ => return Err(invalid()),
            };
            let cancel = if matches!(tm.phase, T::ChooseSlot) {
                GameMenuCancelV2::Back {
                    action: Box::new(GameActionV1::Reward {
                        action: RewardActionV1::Decline,
                    }),
            ...
            (
                keys.into_iter()
                    .enumerate()
                    .map(|(i, key)| (format!("reward/{}/tm/{key}", id.get()), action(i as u32)))
                    .collect::<Vec<_>>(),
                cancel,
            )
        }
