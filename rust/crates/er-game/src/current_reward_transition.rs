...
pub(super) fn transition(
    ...
    let pending=match &phase {
        GameOwnedPhaseV1::RewardBegin{pending,..}|GameOwnedPhaseV1::RewardTmLearn{pending}
        |GameOwnedPhaseV1::RewardTmPresentation{pending,..}=>*pending,_=>return Err(failure()),
    };
    ...
    if operation_id.as_str().is_empty() || revision == SafeU53::ZERO
        ...
        || run.control.actionable || battle.authority_seat != authority_seat
        || before.current_turn_execution.is_some() || before.current_presentation.is_none() {
        return Err(failure());
    }
    ...
    let (candidate,rng_audit)=match &phase {
        GameOwnedPhaseV1::RewardBegin{menu_instance,..}=>{
            ...
            crate::current_reward_selection::install_control(&mut candidate,pending,*menu_instance,safe_increment(revision)?,authority_seat)?;
            (candidate,audit)
        },
        GameOwnedPhaseV1::RewardTmLearn{..}|GameOwnedPhaseV1::RewardTmPresentation{..}=>{
            ...
            let mut selection=before.current_battle_participation.as_ref().and_then(|o|o.experience.as_ref())
                .and_then(|o|o.pending.iter().find(|p|p.id==pending)).and_then(|p|p.victory_tail.as_ref())
                .and_then(|t|t.reward.as_deref()).ok_or_else(failure)?.clone();
            let S::TmPending{offer,holder}=selection.stage else{return Err(failure());};
            let mut candidate=before.clone();
            use er_state::current_reward_tm::{CurrentRewardTmMessageKindV1 as M,CurrentRewardTmMessageV1};
            let tm=selection.tm.as_ref().ok_or_else(failure)?;
            let message=match &phase {
                GameOwnedPhaseV1::RewardTmLearn{..}=>Some(match tm.phase {
                    T::Queued if tm.slot==4=>M::Intro,T::Queued=>M::Learned,
                    T::ForgetQueued=>M::ForgetQuestion,T::LearningQueued=>M::Forgotten,
                    T::DeclineQueued=>M::NotLearned,_=>return Err(failure()),
                }),
                GameOwnedPhaseV1::RewardTmPresentation{event_id,..}=>{
                    if tm.phase.event()!=Some(*event_id){return Err(failure());}
                    match tm.phase{T::Forgotten{..}=>Some(M::Learned),_=>None}
            ...
            if let Some(kind)=message {
                ...
                if tm.messages.len()>=4096{return Err(failure());}
                tm.messages.push(CurrentRewardTmMessageV1{kind,event_id});
                tm.phase=match kind{M::Intro=>T::Intro{event_id},M::ForgetQuestion=>T::ForgetPrompt{event_id},
                    M::Forgotten=>T::Forgotten{event_id},M::Learned=>T::Present{event_id},M::NotLearned=>T::DeclinePresent{event_id}};
            }else{
                let tm=selection.tm.as_mut().ok_or_else(failure)?;
                tm.phase=match tm.phase {
                    T::Intro{..}=>T::Replace,T::ForgetPrompt{..}=>T::ChooseSlot,
                    T::Present{event_id}=>{selection.stage=S::Applied{offer,holder:Some(holder)};T::Complete{event_id}},
                    T::DeclinePresent{event_id}=>{selection.stage=S::Choice;T::Declined{event_id}},
                    _=>return Err(failure()),
                };
            }
            ...
            if actionable {
                crate::current_reward_selection::install_control(&mut candidate,pending,menu_instance,safe_increment(revision)?,authority_seat)?;
            }else{install_waiting(&mut candidate,revision)?;}
            (candidate,Vec::new())
        },
        _=>return Err(failure()),
    };
    ...
    Ok(GameTransitionMaterialV6 {
        ...
        operation_id, authority_seat, authority_revision: revision,
        content_identity: content.identity().clone(), accepted_action: None, owned_phase: Some(phase),
        ...
        before_digest, after_digest, after_state: candidate, next_control,
        presentation, rng_audit, platform_effects: Vec::new(),
    })
}
