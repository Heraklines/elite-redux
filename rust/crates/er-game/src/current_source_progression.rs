...
pub(crate) fn current_source_progression<'a>(
    ...
    if !source.valid(run)
        || !source
            .turn_progress
            .as_ref()
            .is_some_and(|progress| progress.valid(run) && progress.turn == battle.turn)
    ...
    for (row, pokemon) in source.party.iter().zip(&run.party) {
        let index = usize::try_from(row.selection.pokemon_id.get().get())
            .ok()
        ...
        if entry.species_id.get() != row.selection.species_id
            ...
            || entry.ability_index != row.selection.ability_index
            || entry.cost != row.selection.cost
            || pokemon.species_id.get() != row.selection.species_id
            || pokemon.form_index != row.selection.form_index
            || row.ability_index != 0
        {
            return Err(failure());
        }
