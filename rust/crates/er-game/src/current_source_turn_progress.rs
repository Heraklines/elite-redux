...
pub(crate) fn progress(state: &GameStateV6) -> Result<&CurrentSourceTurnProgressV1, GameRuntimeV6Error> {
    state.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref())
        ...
        .and_then(|source| source.turn_progress.as_ref()).ok_or_else(failure)
}
pub(crate) fn install(state: &mut GameStateV6, value: CurrentSourceTurnProgressV1) -> Result<(), GameRuntimeV6Error> {
    state.current_battle_participation.as_mut().and_then(|owner| owner.experience.as_mut())
        .and_then(|owner| owner.source_progression.as_mut()).ok_or_else(failure)?.turn_progress = Some(value);
    Ok(())
}
...
pub(crate) fn fold(
    ...
    if before.current_turn_execution.is_none() && after.current_turn_execution.is_some() {
        if !events.is_empty() || !cues.is_empty() || next_battle.turn != battle.turn { return Err(failure()); }
        // Actual TurnInit resets only the current active field, not benched history.
        for row in &mut next.pokemon {
            if battle.field.slots.iter().any(|slot| slot.occupant == Some(row.pokemon)) {
                row.damage_taken = SafeU53::ZERO;
                row.last_reset_turn = battle.turn;
            }
        }
    } else if next_battle.turn != battle.turn {
        if !events.is_empty() || next_battle.turn.get().get() != battle.turn.get().get().checked_add(1).ok_or_else(failure)? {
            return Err(failure());
        }
        ...
        for event in events {
            let (pokemon, amount) = match event {
                ...
                CurrentBattleSourceEventV1::StruggleRecoilDamage { user, requested_damage, .. } => (*user, *requested_damage),
            };
            let row = next.pokemon.iter_mut().find(|row| row.pokemon == pokemon).ok_or_else(failure)?;
            if row.last_reset_turn != battle.turn { return Err(failure()); }
            row.damage_taken = add(row.damage_taken, u64::from(amount))?;
        }
        for cue in cues {
            if let BattlePresentationCueV5::Switched { slot, pokemon } = cue {
                if !events.is_empty() { return Err(failure()); }
                let outgoing = battle.field.slots.iter().find(|row| row.slot == *slot)
                    .and_then(|row| row.occupant).ok_or_else(failure)?;
                let one = SafeU53::new(1).map_err(|_| failure())?;
                let old = next.pokemon.iter_mut().find(|row| row.pokemon == outgoing).ok_or_else(failure)?;
                old.turn_count = one;
                ...
                let incoming = next.pokemon.iter_mut().find(|row| row.pokemon == *pokemon).ok_or_else(failure)?;
    ...
    if !next.valid(next_run) || next.turn != next_battle.turn { return Err(failure()); }
    install(after, next)
}

pub(crate) fn increment_field(before: &GameStateV6, next: &mut CurrentSourceTurnProgressV1) -> Result<(), GameRuntimeV6Error> {
    let run = before.active_run.as_ref().ok_or_else(failure)?;
    let battle = run.battle.as_ref().ok_or_else(failure)?;
    for slot in &battle.field.slots {
        let Some(id) = slot.occupant else { continue; };
        let pokemon = run.party.iter().chain(&battle.enemy_party).find(|pokemon| pokemon.id == id).ok_or_else(failure)?;
        if pokemon.fainted || pokemon.hp == 0 { continue; }
        let row = next.pokemon.iter_mut().find(|row| row.pokemon == id).ok_or_else(failure)?;
...
pub(crate) fn settle_tail(before: &GameStateV6, after: &mut GameStateV6) -> Result<(), GameRuntimeV6Error> {
    let mut next = progress(before)?.clone();
    let run = before.active_run.as_ref().ok_or_else(failure)?;
    if !next.valid(run) || next.turn != run.battle.as_ref().ok_or_else(failure)?.turn { return Err(failure()); }
    increment_field(before, &mut next)?;
    next.turn = after.active_run.as_ref().and_then(|run| run.battle.as_ref()).ok_or_else(failure)?.turn;
    if next.turn.get().get() != progress(before)?.turn.get().get().checked_add(1).ok_or_else(failure)? { return Err(failure()); }
    install(after, next)
}
