...
use er_state::current_turn_execution::{
    CurrentStatStageChildPhaseV1 as Phase, CurrentStatStageChildV1,
};
...
pub(crate) fn validate<'a>(
    ...
    let row = progress
        .pokemon
        .iter()
        .find(|row| row.pokemon == child.target)
        .ok_or_else(failure)?;
    ...
    if row.stat_stages_decreased
        != (matches!(child.phase, Phase::Message { .. }) && child.before > -6)
    {
        return Err(failure());
    }
    let targeting = er_battle::current_target_execution::CurrentTargetExecution::from_state(state)
        .map_err(|_| failure())?;
    let target = run
        .party
        .iter()
        .chain(&battle.enemy_party)
        .find(|p| p.id == child.target)
        .ok_or_else(failure)?;
...
pub(crate) fn advance(
    ...
    let (commit_stage, next) = match (&child.phase, callback) {
        ...
        (Phase::Animation { event_id }, Some((received, true))) if *event_id == received => {
            (true, Some(false))
        }
        (Phase::Message { event_id }, Some((received, false))) if *event_id == received => {
            (false, None)
        }
    ...
    if commit_stage {
        ...
        let target = run
            ...
            .find(|p| p.id == child.target)
            .ok_or_else(failure)?;
        if target.stat_stages.attack != child.before {
            return Err(failure());
        }
        ...
        let progress = candidate
            ...
            .and_then(|p| p.source_progression.as_mut())
            .and_then(|p| p.turn_progress.as_mut())
            .ok_or_else(failure)?;
        let row = progress
            .pokemon
            .iter_mut()
    ...
    if next.is_none() {
        er_battle::m7_resolver::complete_current_stat_tail(before, &content.battle)
            .map_err(|_| failure())?;
        candidate
...
pub(crate) fn validate_all(
    ...
    if state
        .current_turn_execution
        .as_ref()
        .is_none_or(|turn| turn.stat_child.is_none())
    {
        return Ok(());
    }
    ...
    if mapping.blocking != er_types::battle_ui::PresentationBlockingPolicy::BlocksHumanInput
        || state.current_presentation.as_ref().is_none_or(|owner| {
            !owner
                .receipts
                .iter()
                .any(|receipt| receipt.event_id == event_id && receipt.effect_sha256 == hash)
        })
    {
        return Err(failure());
    }
