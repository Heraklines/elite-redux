...
pub(super) fn transition(
    ...
    let callback = match &phase {
        ...
        GameOwnedPhaseV1::StatStagePresentation {
            event_id,
            animation,
        } => Some((*event_id, *animation)),
        _ => return Err(failure()),
    };
    let (mut candidate, request) =
        crate::current_stat_stage_execution::advance(before, content, callback)?;
    let mut presentation = Vec::new();
    if let Some(animation) = request {
        let payload = if animation {
            GamePresentationPayloadV1::StatStageAnimation {
                holder: child.target,
                stat: child.stat,
                before: child.before,
                after: child.after(),
            ...
            GamePresentationPayloadV1::StatStageMessage {
                holder: child.target,
                stat: child.stat,
                before: child.before,
                after: child.after(),
