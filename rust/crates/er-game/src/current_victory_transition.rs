...
use crate::current_victory_pump::{self, CurrentVictoryPresentation as P, CurrentVictoryPump as Pump};
...
pub(super) fn transition(
    ...
    let pending_id = match &phase {
        GameOwnedPhaseV1::Victory { pending, .. } | GameOwnedPhaseV1::VictoryPresentation { pending, .. } => *pending,
        _ => return Err(failure()),
    };
    let pending = before.current_battle_participation.as_ref()
        .and_then(|owner| owner.experience.as_ref())
        .and_then(|owner| owner.pending.iter().find(|pending| pending.id == pending_id)).ok_or_else(failure)?;
    if operation_id.as_str().is_empty() || revision == SafeU53::ZERO
        ...
        || !pending.friendship.as_ref().is_some_and(|friendship| friendship.complete)
    ...
    let result = match &phase {
        GameOwnedPhaseV1::VictoryPresentation { event_id, .. } => Pump::Advanced(
            current_victory_pump::resume_current_victory_presentation(before, content, pending_id, *event_id)?),
        ...
        GameOwnedPhaseV1::Victory { .. } => current_victory_pump::pump_current_victory(before, content, pending_id)?,
        _ => return Err(failure()),
    };
    let result = match result {
        Pump::LevelUpAccount(level_up) => {
            let account = crate::current_level_account::prepare_current_level_account(before, content, &level_up)?;
            if let Some(unresolved) = account.achievements.first() {
                return Err(GameRuntimeV6Error::Domain(format!("unresolved LevelAchv reward: {}", unresolved.source_key())));
            }
            let (candidate, end) = crate::current_experience_settlement::apply_current_level_up(&account.state, content, &level_up)?;
    ...
    let (mut candidate, request) = match result {
        ...
        Pump::AwaitingPresentation(event_id) => return Err(GameRuntimeV6Error::Domain(
            format!("presentation {} is still awaiting its actual callback", event_id.get()))),
        Pump::LearnMoves(batch) => return Err(GameRuntimeV6Error::Domain(format!(
            "retained learning for Pokemon {} requires its owned raw menu", batch.children.parent.level_up.award.phase.pokemon.get()))),
        Pump::Evolution(children) => return Err(GameRuntimeV6Error::Domain(format!(
            "retained evolution for Pokemon {} has unresolved descendants", children.parent.level_up.award.phase.pokemon.get()))),
    ...
    if let Some(request) = request {
        let payload = match &request {
            ...
            P::LevelUp(end) => {
                ...
                let pokemon = candidate.active_run.as_ref()
                    .and_then(|run| run.party.get(usize::from(level.award.phase.party_index))).ok_or_else(failure)?;
                GamePresentationPayloadV1::LevelStats { holder: pokemon.id, previous_level: level.previous_level,
                    level: level.new_level, previous_stats: level.previous_stats, stats: pokemon.stats }
            }
            P::HidePartyBar(award) => GamePresentationPayloadV1::HidePartyExperience { holder: award.phase.pokemon },
        };
        ...
        presentation.push(GamePresentationEffectV2 { event_id: PresentationEventId::new(revision),
            semantic, blocking: mapping.blocking, skip: mapping.skip, payload: Some(payload) });
        ...
        let descendant = match request {
            ...
            P::PartyAward { award, level_up } => D::PartyAwardPresentation { award, level_up, event_id },
        ...
        candidate.current_battle_participation.as_mut().and_then(|owner| owner.experience.as_mut())
            ...
            .and_then(|pending| pending.victory.as_mut()).ok_or_else(failure)?.descendant = descendant;
    }
    let batch = candidate.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref())
        .and_then(|owner| owner.pending.iter().find(|pending| pending.id == pending_id))
        .and_then(|pending| pending.victory.as_ref()).and_then(|victory| match &victory.descendant {
            D::LearnMoveBatch { batch } if !batch.complete => Some(batch),
            _ => None,
        });
    if let Some(batch) = batch {
        let GameOwnedPhaseV1::Victory { menu_instance, .. } = &phase else { return Err(failure()); };
        ...
        if *menu_instance == MenuInstanceId::ZERO { return Err(failure()); }
        let next_revision = safe_increment(revision)?;
        let context = GameActionContextV1 {
            operation_id: OperationId::new(format!("current/learn/{}/{}/{}",
                run.run_id.get().get(), pending_id.get(), next_revision.get())).map_err(|_| failure())?,
            authority_seat, authority_revision: next_revision, menu_instance: *menu_instance,
        };
        let control = crate::m7_progression_control::current_learn_move_batch_control(&context, batch)
            .map_err(|_| failure())?;
    ...
    Ok(GameTransitionMaterialV6 {
        ...
        mutations: vec![GameMutationEvidenceV2 { ordinal: 0, domain: GameActionDomainV2::Progression,
