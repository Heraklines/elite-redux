...
pub use current_storage::{
    BootstrapStorageKindV1, CurrentBootstrapStorageV1, PendingBootstrapStorageV1,
};
...
impl RunBootstrapMachineV1 {
    ...
    fn apply_game_action_transaction(
        &mut self,
        action: GameActionV1,
    ...
    fn apply_action(&mut self, action: BootstrapActionV1) -> Result<(), RunBootstrapErrorV1> {
        if self.apply_current_storage_action(&action)? {
            return Ok(());
        }
...
type BootstrapControlEntries = (
    GameControlKindV2,
    Vec<(String, bool, BootstrapActionV1)>,
    Option<BootstrapActionV1>,
);

fn build_menu_control(
    stage: RunBootstrapStageV1,
    owner: SeatId,
    revision: SafeU53,
    instance: MenuInstanceId,
    (kind, entries, cancel): BootstrapControlEntries,
) -> Result<GameControlPlanV2, RunBootstrapErrorV1> {
