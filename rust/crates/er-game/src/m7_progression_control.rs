...
pub fn current_learn_move_batch_control(
    ...
    if batch.cancel_confirmation {
        ...
        let yes = menu
            .options
            .iter()
            .find(|row| row.option_id != no)
            .ok_or(ProgressionControlError::Empty)?
            .option_id
            .clone();
        let edges = vec![
    ...
    if let Some(move_id) = batch.pending_move {
        for index in 0..4 {
            ...
            options.push((
                format!("current/learn-batch/{}/slot/{index}", move_id.get()),
                GameActionV1::CurrentLearnMoveBatch {
                    action: CurrentLearnMoveBatchActionV1::Assign { move_id, slot },
                },
            ));
        }
        return current_batch_vertical_control(
            context,
            "current/learn-batch/slot",
            &options,
            0,
            GameMenuCancelV2::Back {
                action: Box::new(GameActionV1::CurrentLearnMoveBatch {
    ...
    for move_id in &batch.offered {
        if batch.assignments.iter().any(|row| row.move_id == *move_id) {
            continue;
        }
        options.push((
            format!("current/learn-batch/move/{}", move_id.get()),
            GameActionV1::CurrentLearnMoveBatch {
    ...
    let undo = GameActionV1::CurrentLearnMoveBatch {
        action: CurrentLearnMoveBatchActionV1::Undo,
    };
    if !batch.assignments.is_empty() {
        options.push(("current/learn-batch/undo".to_owned(), undo));
    }
    options.push(("current/learn-batch/done".to_owned(), done.clone()));
    current_batch_vertical_control(
        context,
        "current/learn-batch",
        &options,
        usize::from(batch.list_cursor),
        GameMenuCancelV2::Back {
            action: Box::new(done),
        },
    )
...
fn current_batch_vertical_control(
    ...
    let selected_id = &entries
        ...
        .ok_or(ProgressionControlError::Empty)?
        .0;
    menu.selected_option_id = menu
        .options
        .iter()
        .find(|row| row.option_id.as_str() == selected_id)
        .ok_or(ProgressionControlError::Empty)?
        .option_id
        .clone();
    // Actual batch UP/DOWN stops at the first/last row; no wrap is installed.
    menu.navigation.retain(|edge| {
        !((edge.from.as_str() == first && edge.direction == NavigationDirection::Up)
