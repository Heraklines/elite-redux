...
impl GameStateV6ContentContext for PreparedGameContentV2 {
    fn current_random_target_commands_match(&self, state: &er_state::m9e_state_v6::GameStateV6) -> bool {
        crate::current_random_target_admission::validate_live_pending_commands(state, self).is_ok()
    }
    fn current_initial_victory_tail_matches(
        &self,
    ...
    fn current_experience_progress_matches(
        ...
        crate::current_experience_validation::validate_current_experience_progress(state, self)
            .is_ok()
    }
        ...
        !state
            ...
            .is_some_and(|source| {
                matches!(
                    ...
                    Some(
                        er_state::current_faint_execution::CurrentFaintPhaseV1::MessageReady { .. }
                    )
                )
            })
    ...
    fn current_targeting_form_matches(&self, species: SpeciesId, form: u16) -> bool {
        matches!(species.get().get(), 1 | 4 | 7)
            && form == 0
            && self
                .progression
                .experience_for_compiled_form(species, form)
                .is_ok_and(|metadata| {
                    metadata.source_form_count == 0
                        && matches!(
                            metadata.source_form,
                            er_progression::content_v2::ExperienceSourceFormV2::Species
                        )
                })
    }
        ...
        self.identity.oracle_sha.as_str() == "399d5d368f0b5642ebf8f45bd8a5e73350fa4de7"
            && self.world.mode(mode).is_some_and(|definition| {
                definition.key == "CLASSIC"
                    ...
                    && !definition.challenge_selection
            })
    }
    ...
    fn current_experience_source_matches(
        ...
        self.progression
            .experience_for_compiled_form(source.species, source.compiled_form)
            .is_ok_and(|metadata| {
                ...
                source.source_form == source_form
                    && source.unadjusted_base_exp == metadata.base_exp
