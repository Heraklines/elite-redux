...
use er_game::current_starter_pokerus::{daily_starter_species, DATE_TIME_CLIP_MILLISECONDS};
...
use er_types::{BootstrapActionV1, GameButton, PlatformRequestId, RunDifficultyV1, SafeU53, SeatId, StarterSelectionV1};
...
fn request(value: u64) -> Result<PlatformRequestId> { Ok(PlatformRequestId::new(safe(value)?)) }
...
fn bootstrap(content: &PreparedGameContentV2, current: bool) -> Result<RunBootstrapMachineV1> {
    ...
    let profile = ProfileStateV1 {
        schema_version: PROFILE_STATE_SCHEMA_VERSION_V1, unlocks: vec![], achievements: vec![],
        challenges: vec![], flags: Default::default(), dex: DexState::default(),
    ...
    let starters = bundle.bootstrap.starters.iter().enumerate().map(|(index, starter)| Ok(StarterSelectionV1 {
        pokemon_id: PokemonId::new(safe(u64::try_from(index)? + 1)?), species_id: starter.species_id.get(),
        form_index: starter.form_index, ability_index: starter.ability_index,
        cost: starter.cost, owner_seat: owner,
    })).collect::<Result<Vec<_>>>()?;
    let catalog = BootstrapCatalogV1 {
        modes: bundle.bootstrap.modes.iter().map(|mode| BootstrapModePolicyV1 {
            mode: mode.mode, challenge_selection: mode.challenge_selection,
            cooperative: mode.cooperative, supported: mode.supported,
        }).collect(),
        challenges: bundle.bootstrap.choices.iter().flat_map(|choice| choice.values.iter().cloned()
            .map(|value| (choice.id.clone(), value))).collect(),
        starters, save_slots: vec!["daily-owned".to_owned()], automatic_coop_save_slot: None,
        maximum_starter_cost: bundle.bootstrap.maximum_starter_cost,
        maximum_starters: bundle.bootstrap.maximum_starters, local_is_host: true, developer_mode: false,
    };
    let mut bootstrap = RunBootstrapMachineV1::new(profile, "daily-owned-natural".to_owned(), owner, catalog)?;
    if current {
        bootstrap.current_friendship_profile = Some(er_game::current_friendship_profile::fresh_profile(content, owner)?);
...
fn choose(bootstrap: &mut RunBootstrapMachineV1, action: BootstrapActionV1) -> Result<()> {
    if action == BootstrapActionV1::Cancel { bootstrap.button(GameButton::Cancel)?; return Ok(()) }
    let wanted = GameActionV1::Bootstrap { action };
    let count = bootstrap.control.menu.as_ref().ok_or("real menu required")?.options.len();
    for _ in 0..count { if !bootstrap.button(GameButton::Up)? { break } }
    for _ in 0..count {
        if bootstrap.control.menu.as_ref().and_then(|menu| menu.selected_action()) == Some(&wanted) {
...
fn enter(bootstrap: &mut RunBootstrapMachineV1, content: &PreparedGameContentV2) -> Result<()> {
    ...
    let mode = content.bundle().bootstrap.modes.iter().find(|mode|
        content.world.mode(mode.mode).is_some_and(|definition| definition.key == "CLASSIC"))
        .ok_or("actual Classic mode required")?.mode;
    choose(bootstrap, BootstrapActionV1::SelectMode(mode))?;
    if bootstrap.stage == RunBootstrapStageV1::ChallengeSelect { choose(bootstrap, BootstrapActionV1::Confirm)?; }
...
fn pick(bootstrap: &mut RunBootstrapMachineV1, species: u32) -> Result<()> {
    let starter = bootstrap.catalog.starters.iter().find(|starter| starter.species_id.get() == u64::from(species))
        .ok_or("actual content starter required")?.clone();
...
fn finish(bootstrap: &mut RunBootstrapMachineV1) -> Result<()> {
    ...
    choose(bootstrap, BootstrapActionV1::SelectDifficulty(RunDifficultyV1::Youngster))?;
    choose(bootstrap, BootstrapActionV1::SelectSaveSlot("daily-owned".to_owned()))?;
...
fn sample(bootstrap: &mut RunBootstrapMachineV1, id: u64, milliseconds: i64) -> Result<()> {
    ...
    assert_eq!(context.menu_instance, before.menu.as_ref().ok_or("waiting menu")?.instance_id);
...
fn daily_selection_matches_all_actual_source_dates() -> Result<()> {
    ...
    assert!(daily_starter_species(-DATE_TIME_CLIP_MILLISECONDS - 1).is_err());
    assert_eq!(daily_starter_species(0)?, daily_starter_species(86_399_999)?);
    assert_eq!(daily_starter_species(-1)?, daily_starter_species(-86_400_000)?);
    Ok(())
}
...
fn pending_clock_cancel_restore_and_request_floor_are_owned() -> Result<()> {
    ...
    assert_eq!(state.control.menu.as_ref().ok_or("waiting menu")?.options.len(), 1);
    let before = state.clone();
    let starter = state.catalog.starters[0].clone();
    assert!(state.apply_game_action(GameActionV1::Bootstrap { action: BootstrapActionV1::SelectStarter(starter) }).is_err());
    ...
    for (id, milliseconds, owner) in [(8, 0, seat()?), (7, DATE_TIME_CLIP_MILLISECONDS+1, seat()?), (7, 0, SeatId::new(safe(2)?))] {
        assert!(restored.accept_starter_pokerus_clock(request(id)?, milliseconds, owner).is_err());
        assert_eq!(restored, state);
    }
    choose(&mut state, BootstrapActionV1::Cancel)?;
    assert_eq!(state.stage, RunBootstrapStageV1::Title);
    assert_eq!(state.current_starter_pokerus.as_ref().ok_or("owner")?.next_platform_request_id, safe(8)?);
    let cancelled = state.clone();
    assert!(state.accept_starter_pokerus_clock(request(7)?, 0, seat()?).is_err());
    assert_eq!(state, cancelled);
    enter(&mut state, &content)?;
    let before = state.clone();
    assert!(state.begin_starter_pokerus_clock(request(7)?, seat()?).is_err());
    assert_eq!(state, before);
    sample(&mut state, 8, -1)?;
    let after = state.clone();
    assert!(state.accept_starter_pokerus_clock(request(8)?, -1, seat()?).is_err());
...
fn picks_retain_their_source_day_through_reentry_and_natural_construction() -> Result<()> {
    ...
    pick(&mut state, 4)?;
    finish(&mut state)?;
    let bindings = state.current_starter_pokerus_selections()?.ok_or("owned selections")?;
    assert_eq!(bindings.iter().map(|row| (row.selection.species_id.get(),row.pokerus)).collect::<Vec<_>>(), vec![(1,true),(4,true)]);
    ...
    let party = &natural.active_run.as_ref().ok_or("actual natural run")?.party;
    assert_eq!(party.iter().map(|pokemon| (pokemon.species_id.get().get(),pokemon.pokerus)).collect::<Vec<_>>(), vec![(1,Some(true)),(4,Some(true))]);
    Ok(())
}
...
fn clock_counter_overflow_and_forged_restore_fail_atomically() -> Result<()> {
    ...
    enter(&mut state, &content)?;
    let before = state.clone();
    assert!(state.begin_starter_pokerus_clock(request(9_007_199_254_740_991)?, seat()?).is_err());
    assert_eq!(state, before);
    ...
    for field in ["request_id", "menu_revision", "local_seat"] {
        ...
        if field == "request_id" { forged["current_starter_pokerus"]["display"]["value"][field] = serde_json::json!(99); }
        else { forged["current_starter_pokerus"]["display"]["value"]["context"][field] = serde_json::json!(99); }
    ...
    assert!(overflow.accept_starter_pokerus_clock(request(1)?, 0, seat()?).is_err());
...
fn historical_absence_remains_unknown_and_requires_actual_fresh_profile() -> Result<()> {
    ...
    assert_eq!(natural.active_run.as_ref().ok_or("natural run")?.party[0].pokerus, None);
    Ok(())
}
...
fn removed_and_corrupted_picks_cannot_reuse_an_unrelated_daily_receipt() -> Result<()> {
    ...
    pick(&mut state, 1)?;
    finish(&mut state)?;
    assert!(!state.current_starter_pokerus_selections()?.ok_or("bindings")?[0].pokerus);
    let original = serde_json::to_value(&state)?;
    for property in ["selection", "receipt"] {
        let mut forged = original.clone();
        if property == "selection" { forged["current_starter_pokerus"]["selected"][0][property]["species_id"] = serde_json::json!(4); }
        else { forged["current_starter_pokerus"]["selected"][0][property]["species"] = serde_json::json!([1]); }
        let restored: RunBootstrapMachineV1 = serde_json::from_value(forged)?;
        assert!(restored.validate().is_err());
...
const SOURCE_DATES: &[(i64, &[u32])] = &[
    ...
    (8640000000000000, &[632, 8194, 850, 4144, 848, 10766, 627, 629]),
    (-8640000000000000, &[441, 222, 4554, 493, 10843, 10797, 878, 228]),
    (0, &[806, 203, 672, 932, 190, 621, 10710, 211]),
