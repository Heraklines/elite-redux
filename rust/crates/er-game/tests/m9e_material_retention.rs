...
use er_game::m9e_material_v6::{
    AppliedMaterialRetentionV1, GameMaterialApplyOutcomeV6, GameMaterialV6, GameMaterialV6Error,
    GamePlatformEffectV2, MAX_APPLIED_MATERIAL_RECORDS_V1, apply_game_material_v6,
...
use er_types::{
    GameActionContextV1, GameActionV1, OperationId, PlatformRequestId, PresentationEventId,
    SafeU53, SaveActionV1, SeatId,
};
...
fn conflicting(bytes: &[u8]) -> TestResult<Vec<u8>> {
    ...
    let GameMaterialV6::GameAction(transition) = &mut material else {
        return Err("save material variant".into());
    };
    transition.accepted_action = Some(GameActionV1::Save {
        action: SaveActionV1::Delete {
            slot: "conflicting-slot".to_owned(),
        },
    });
...
{
    let content = content()?;
    let retention = AppliedMaterialRetentionV1::BoundedSuffix {
        maximum_records: MAX_APPLIED_MATERIAL_RECORDS_V1,
    };
    let initial = state(&content)?;
    let mut authority = GameRuntimeV6::new_with_retention(
        Some(initial.clone()),
        Arc::clone(&content),
        safe(1),
        retention,
    ...
    for revision in 1..=count {
        ...
        assert_eq!(
            prepared.platform_effects,
            vec![GamePlatformEffectV2::StorageDelete {
                request: PlatformRequestId::new(safe(revision)),
                slot: "retention-slot".to_owned(),
            }]
        );
        assert_eq!(prepared.presentation.len(), 1);
        assert_eq!(
            prepared.presentation[0].event_id,
        ...
        assert_eq!(
            ledger.records.len(),
            usize::try_from(revision)?.min(MAX_APPLIED_MATERIAL_RECORDS_V1)
        );
        assert_eq!(
            ledger
                .records
            ...
            safe(
                revision
                    .saturating_sub(MAX_APPLIED_MATERIAL_RECORDS_V1 as u64 - 1)
                    .max(1)
            )
        );
        if revision.is_multiple_of(MAX_APPLIED_MATERIAL_RECORDS_V1 as u64) {
            ...
            let preview = GameActionDispatcherV1::prepare_with_retention(
                authority.state(),
                &content,
                authority.material_ledger(),
                action(),
                context(safe(revision + 1))?,
                retention,
            )?;
            assert_eq!(authority.snapshot(), before);
            let mut resumed = GameRuntimeV6::from_snapshot_with_retention(
                serde_json::from_slice(&serde_json::to_vec(&before)?)?,
                Arc::clone(&content),
                retention,
            )?;
            ...
            let mut historical =
                GameRuntimeV6::from_snapshot(before.clone(), Arc::clone(&content))?;
            assert_eq!(
                historical.material_retention(),
                AppliedMaterialRetentionV1::HistoricalHardStop
            );
            assert_eq!(
                historical.execute(action(), context(safe(revision + 1))?),
                Err(GameRuntimeV6Error::Material(
                    GameMaterialV6Error::Ledger.to_string()
                ))
            );
            ...
            assert_eq!(
                apply_game_material_v6(
                    &mut legacy_apply.state,
                    &mut legacy_apply.material_ledger,
                    &content,
                    &preview.material_bytes
                ),
                Err(GameMaterialV6Error::Ledger)
            );
        ...
        if revision == count {
            ...
            assert_rejected_unchanged(
                ...
                &first,
                retention,
                GameMaterialV6Error::StaleUnverifiable,
            );
            assert_rejected_unchanged(
                &before,
                &content,
...
#[test]
fn small_suffix_retained_conflicts_late_invalid_and_stale_material_preserve_full_frontier()
-> TestResult {
    let content = content()?;
    ...
    assert_eq!(
        before
            ...
            .collect::<Vec<_>>(),
        vec![safe(8), safe(9), safe(10)]
    );
    for bytes in &materials[7..] {
        assert_eq!(
            runtime.apply_material_bytes(bytes)?,
            GameMaterialApplyOutcomeV6::DuplicateApplied
        );
        ...
        assert_rejected_unchanged(
            ...
            &conflicting(bytes)?,
            retention,
            GameMaterialV6Error::ConflictingDuplicate,
        );
    }
    for bytes in &materials[..7] {
        assert_rejected_unchanged(
    ...
    let GameMaterialV6::GameAction(transition) = &mut invalid else {
        return Err("save material variant".into());
    };
    ...
    assert_rejected_unchanged(
        ...
        &bytes,
        retention,
        GameMaterialV6Error::Frontier,
    );
    assert!(runtime.apply_material_bytes(&bytes).is_err());
    assert_eq!(runtime.snapshot(), before);
    assert_eq!(
        runtime.apply_material_bytes(&next.material_bytes)?,
        GameMaterialApplyOutcomeV6::Applied
    );
...
fn retention_policy_restore_and_revision_exhaustion_reject_without_retirement() -> TestResult {
    ...
    for maximum_records in [0, MAX_APPLIED_MATERIAL_RECORDS_V1 + 1] {
        assert_eq!(
            AppliedMaterialRetentionV1::BoundedSuffix { maximum_records }.validate(),
            Err(GameMaterialV6Error::Ledger)
        );
    }
    ...
    let GameMaterialV6::GameAction(transition) = &mut overflow else {
        return Err("save material variant".into());
    };
    ...
    assert_rejected_unchanged(
        ...
        &bytes,
        retention,
        GameMaterialV6Error::Revision,
    );
    assert_eq!(
        runtime.apply_material_bytes(&last.material_bytes)?,
        GameMaterialApplyOutcomeV6::DuplicateApplied
    );
    assert_eq!(runtime.snapshot(), before);
    ...
    assert_eq!(
        gap.material_ledger.validate_with_retention(retention),
        Err(GameMaterialV6Error::Ledger)
    );
    assert!(
        GameRuntimeV6::from_snapshot_with_retention(gap, Arc::clone(&content), retention).is_err()
    );
    assert!(
        GameRuntimeV6::from_snapshot_with_retention(
            before.clone(),
            Arc::clone(&content),
            AppliedMaterialRetentionV1::BoundedSuffix { maximum_records: 1 }
        )
        .is_err()
    );
    assert_eq!(runtime.snapshot(), before);
