...
use crate::current_proposal_v7::{
    CurrentProposalMaterialReceiptV2, MAX_CURRENT_RECEIPT_MATERIAL_BYTES_V1, decode_current_hex_v1,
};
...
fn decode(bytes: &[u8]) -> Result<CurrentCoopFrameV1> {
    ...
    if encode(&frame, MAX_OWNER_BYTES)? != bytes {
        return Err(GameKernelV7Error::Invalid);
    }
    Ok(frame)
}
...
fn validate_starters(
    content: &PreparedGameContentV2,
    seat: SeatId,
    starters: &[StarterSelectionV1],
) -> Result<()> {
...
fn validate_choices(
    choices: &CurrentCoopChoicesV1,
    owner: &CurrentCoopSetupSnapshotV1,
    content: &PreparedGameContentV2,
) -> Result<()> {
    let guest = if owner.local.sender_seat_id == owner.local.authority_seat_id {
        &owner.peer
    } else {
        &owner.local
    };
    if &choices.context != guest
        || choices.content != owner.content
        || choices.seed != owner.seed
        || !content
            .bundle()
            .bootstrap
            .modes
            .iter()
            .any(|mode| mode.mode == choices.mode && mode.cooperative && mode.supported)
    {
        return Err(GameKernelV7Error::Invalid);
    }
...
fn validate_started(
    frame: &CurrentCoopFrameV1,
    owner: &CurrentCoopSetupSnapshotV1,
    content: &PreparedGameContentV2,
) -> Result<Vec<u8>> {
    let CurrentCoopFrameV1::CurrentCoopStarted {
        authority,
        choices,
        host,
        material_hex,
    ...
    let expected = if owner.local.sender_seat_id == owner.local.authority_seat_id {
        &owner.local
    } else {
        &owner.peer
    };
    ...
    if authority != expected
        || owner.choices.as_ref() != Some(choices)
        || host.mode != Some(choices.mode)
        || !host
            .difficulty
            .is_some_and(|difficulty| difficulty.production())
        || !host
            .save_slot
            .as_ref()
            .is_some_and(|slot| !slot.is_empty() && slot.len() <= 128)
        || host.choices.iter().any(|(id, value)| {
            !content
                .bundle()
                .bootstrap
                .choices
                .iter()
                .any(|entry| &entry.id == id && entry.values.contains(value))
        })
    {
        return Err(GameKernelV7Error::Invalid);
    }
    validate_starters(content, authority.sender_seat_id, &host.starters)?;
    let bytes = decode_current_hex_v1(material_hex, MAX_CURRENT_RECEIPT_MATERIAL_BYTES_V1)
        .map_err(|_| GameKernelV7Error::Invalid)?;
    let material = GameMaterialV6::decode(&bytes).map_err(|_| GameKernelV7Error::Invalid)?;
    let GameMaterialV6::NewRun(transition) = &material else {
        return Err(GameKernelV7Error::Invalid);
    };
    let run = transition
        .after_state
    ...
    if transition.authority_seat != authority.sender_seat_id
        || transition.authority_revision != safe_one()
        || transition.operation_id.as_str() != OPERATION
        ...
        || run.seed != owner.seed
        || run.mode != choices.mode
        || run.party.len() != host.starters.len() + choices.starters.len()
    {
        return Err(GameKernelV7Error::Invalid);
    }
    for (pokemon, chosen) in run
        .party
        .iter()
        .zip(host.starters.iter().chain(&choices.starters))
    {
        if pokemon.owner_seat != Some(chosen.owner_seat)
            || pokemon.species_id.get() != chosen.species_id
            || pokemon.form_index != chosen.form_index
        {
            return Err(GameKernelV7Error::Invalid);
        }
    }
...
pub(crate) fn validate_snapshot(
    ...
    let Some(owner) = &snapshot.current_coop_setup else {
        return Ok(());
    };
    ...
    if owner.schema_version != 1
        || owner.content != *content.identity()
        || owner.seed.is_empty()
        || owner.seed.len() > 1024
        || owner.local != protocol.frame_context.context
        || Some(&owner.peer) != protocol.peer_identity.peer.as_ref()
    {
        return Err(GameKernelV7Error::Invalid);
    }
    ...
    let cached = match (&owner.last_reply, &owner.last_reply_v2) {
        ...
        (Some(reply), None) => {
            reply
                ...
                .map_err(|_| GameKernelV7Error::Invalid)?;
            Some((
                reply.evidence().map_err(|_| GameKernelV7Error::Invalid)?,
                &reply.authority_context,
                &reply.material_fingerprint,
                &owner.local,
                &owner.peer,
            ))
        }
        (None, Some(reply)) => {
            ...
            if evidence.material.transition().authority_revision
            ...
            Some((
                ...
                &reply.authority_context,
                &reply.material_fingerprint,
                &epoch.binding.authority_target,
                &epoch.binding.guest_target,
            ))
        }
        (None, None) => None,
    };
    ...
    match (&snapshot.lifecycle, &owner.started) {
        (GameKernelLifecycleSnapshotV7::Bootstrap(bootstrap), None) => {
            if bootstrap.seed != owner.seed
                || bootstrap.current_storage.is_some()
                || bootstrap.catalog.local_is_host != (protocol.role == EndpointRole::Authority)
                || snapshot.current_proposal.is_some()
                || !snapshot.pending_platform.is_empty()
                || !snapshot.pending_presentations.is_empty()
                || snapshot.scheduler.disposed
            {
                return Err(GameKernelV7Error::Invalid);
            }
                ...
                match (&owner.choices, bootstrap.stage) {
                    (Some(choices), RunBootstrapStageV1::WaitingForPartner)
                        if bootstrap.selections.mode == Some(choices.mode)
                            && bootstrap.selections.starters == choices.starters => {}
                    (None, stage)
                        if stage != RunBootstrapStageV1::WaitingForPartner
                            && stage != RunBootstrapStageV1::Complete => {}
                    _ => return Err(GameKernelV7Error::Invalid),
                }
...
impl GameKernelV7 {
    ...
    pub fn enable_current_coop_setup(&mut self) -> Result<()> {
        ...
        if self.current_coop_setup.is_some()
            ...
            || bootstrap.current_storage.is_some()
            || bootstrap.stage != RunBootstrapStageV1::Title
            || self.replay_sequence != SafeU53::ZERO
            || !bootstrap.pressed_keys.is_empty()
            || bootstrap.catalog.local_is_host != (self.role == GameKernelRoleV7::Authority)
        ...
        candidate.current_coop_setup = Some(Box::new(CurrentCoopSetupSnapshotV1 {
            ...
            peer: protocol
                ...
                .clone()
                .ok_or(GameKernelV7Error::Invalid)?,
            content: self.content.identity().clone(),
    ...
    pub fn retry_current_coop_setup(&self) -> Result<GameKernelStepV7> {
        if self.has_current_coop_rebind() {
            ...
            let Some(CurrentProposalOwnerSnapshotV1::Pending { retained }) = &self.current_proposal
            else {
                return Err(GameKernelV7Error::Invalid);
            };
        ...
        let effect = if self.role == GameKernelRoleV7::Authority {
            owner
                .started
                .as_ref()
                .map(|frame| encode(frame, MAX_OWNER_BYTES))
                .transpose()?
                .map(|bytes| GameKernelEffectV7::AuthorityMaterial {
            ...
            owner
                ...
                .map(|choices| {
                    encode(
                        &CurrentCoopFrameV1::CurrentCoopChoices {
                ...
                .transpose()?
                .map(|bytes| GameKernelEffectV7::ProposalReady {
                    operation_id: OperationId::new(OPERATION).expect("constant operation"),
                    bytes,
                })
    ...
    pub(super) fn advance_coop_bootstrap(&mut self) -> Result<GameKernelStepV7> {
        ...
        {
            let choices = CurrentCoopChoicesV1 {
                context: owner.local.clone(),
                content: owner.content.clone(),
                seed: owner.seed.clone(),
                mode: bootstrap
                    .selections
                    .mode
                    .ok_or(GameKernelV7Error::Invalid)?,
                starters: bootstrap.selections.starters.clone(),
            };
            ...
            && owner.choices.is_some()
        {
            ...
            for effect in &mut step.effects {
                ...
                {
                    if operation_id.as_str() != OPERATION {
                        return Err(GameKernelV7Error::Invalid);
                    }
    ...
    pub(super) fn ingest_coop_setup(
        &mut self,
        ...
        let step = match (&self.role, &frame) {
            ...
            (GameKernelRoleV7::Replica, CurrentCoopFrameV1::CurrentCoopStarted { .. }) => {
                ...
                candidate.lifecycle = GameKernelLifecycleV7::Active(
                    GameRuntimeV6::new_with_retention(
                        None,
