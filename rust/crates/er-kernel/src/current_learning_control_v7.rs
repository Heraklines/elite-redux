...
pub(crate) fn validate_private_learning_control(
    state: &GameStateV6,
    private: Option<&GameControlPlanV2>,
    local_seat: SeatId,
    ...
    let context = canonical
        ...
        .ok_or(GameKernelV7Error::Invalid)?;
    let expected =
        er_game::m7_progression_control::current_learn_move_batch_control(context, batch)
            .map_err(|_| GameKernelV7Error::Invalid)?;
    if *canonical != expected
        || canonical.kind != GameControlKindV2::MoveLearn
        || canonical.owner_seat != Some(local_seat)
        || context.authority_seat != local_seat
        || state.current_presentation.is_none()
    ...
    if private_menu.selected_action().is_none() {
        return Err(GameKernelV7Error::Invalid);
    }
    ...
    normalized
        ...
        .as_mut()
        .ok_or(GameKernelV7Error::Invalid)?
        .selected_option_id = canonical_menu.selected_option_id.clone();
...
impl GameKernelV7 {
    pub(super) fn navigate_current_learning_control(
        ...
        if current_batch(state).is_none() {
            return Ok(None);
        }
        ...
        validate_private_learning_control(
            state,
