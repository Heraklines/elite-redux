...
use super::*;
use crate::snapshot_v7::{PendingCurrentPhaseAckV1, CurrentPhasePresentationKindV1 as K};
...
pub(crate) fn expected_presentations(state: &GameStateV6) -> Vec<PendingCurrentPhaseAckV1> {
    ...
    let Some(owner) = state.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref()) else {
        return Vec::new();
    };
    let mut expected = Vec::new();
    if let Some(source) = &owner.source_progression {
        let faint = match &source.initial_faint.phase {
            Some(F::Animation { address, event_id }) => Some((address.pending_id, *event_id, K::FaintAnimation)),
            Some(F::Message { address, event_id }) => Some((address.pending_id, *event_id, K::FaintMessage)),
            _ => None,
        };
    ...
    for pending in &owner.pending {
        let Some(victory) = &pending.victory else { continue; };
        ...
        expected.push(PendingCurrentPhaseAckV1 { pending: pending.id, event_id, kind: K::Victory });
    }
    expected
}

pub(crate) fn expected_presentation(state: &GameStateV6, event_id: PresentationEventId) -> Option<PendingCurrentPhaseAckV1> {
    let matches: Vec<_> = expected_presentations(state).into_iter().filter(|expected| expected.event_id == event_id).collect();
    if matches.len() == 1 { matches.first().copied() } else { None }
}

pub(crate) fn receipt_matches(state: &GameStateV6, content: &PreparedGameContentV2, ack: PendingCurrentPhaseAckV1) -> bool {
    if expected_presentation(state, ack.event_id) != Some(ack) { return false; }
    if ack.kind == K::Victory {
        let victory = crate::snapshot_v7::PendingVictoryAckV1 { pending: ack.pending, event_id: ack.event_id };
    ...
    let Some(phase) = state.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref())
        .and_then(|owner| owner.source_progression.as_ref()).and_then(|source| source.initial_faint.phase.as_ref()) else { return false; };
    let holder = phase.address().pokemon;
    let payload = match ack.kind {
        K::FaintAnimation => P::FaintAnimation { holder, tween_milliseconds: 500 },
