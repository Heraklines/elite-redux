...
fn retained_effect(
    ...
    let (family, payload) = match ack.kind {
        ...
        K::RewardTm => (
            ...
            current_phase_receipt_v7::reward_tm_payload(state, ack.event_id)
                .ok_or(GameKernelV7Error::Invalid)?,
        ),
