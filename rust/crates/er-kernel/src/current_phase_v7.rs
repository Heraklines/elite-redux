...
pub(super) fn bootstrap_clock_effect(bootstrap: &RunBootstrapMachineV1) -> Option<GamePlatformEffectV2> {
    bootstrap.current_starter_pokerus_pending().map(|pending| GamePlatformEffectV2::StarterPokerusClock {
...
impl GameKernelV7 {
    pub fn apply_current_utc_clock_result(
        &mut self, request_id: PlatformRequestId, utc_milliseconds: i64,
        ...
        let pending = candidate.pending_platform.get(&request_id).ok_or(GameKernelV7Error::Invalid)?;
        if let GamePlatformEffectV2::StarterPokerusClock { request, context } = &pending.effect {
            ...
            if *request != request_id || context.local_seat != candidate.local_seat
                || bootstrap_clock_effect(bootstrap).as_ref() != Some(&pending.effect)
            { return Err(GameKernelV7Error::Invalid); }
            bootstrap.accept_starter_pokerus_clock(request_id, utc_milliseconds, candidate.local_seat)
        ...
        if request.request != request_id { return Err(GameKernelV7Error::Invalid); }
        let phase = GameOwnedPhaseV1::FriendshipClock { request: request.clone(), utc_milliseconds };
    ...
    pub(super) fn resume_owned_phase(&mut self, output: &mut GameKernelStepV7)
        -> Result<(), GameKernelV7Error>
    {
        if self.role != GameKernelRoleV7::Authority
            || self.pending_presentations.values().any(|pending| pending.blocking
                == er_types::battle_ui::PresentationBlockingPolicy::BlocksHumanInput)
        { return Ok(()); }
        let GameKernelLifecycleV7::Active(runtime) = &self.lifecycle else { return Ok(()); };
        let Some(state) = runtime.state() else { return Ok(()); };
        let Some(turn) = &state.current_turn_execution else { return Ok(()); };
        let phase = match &turn.stage {
            CurrentTurnStageV1::ReadyForMove => {
                if usize::from(turn.next_action) < turn.actions.len() { GameOwnedPhaseV1::TurnStep }
                else { GameOwnedPhaseV1::TurnFinish }
            },
    ...
    fn execute_owned_phase(&mut self, phase: GameOwnedPhaseV1)
        -> Result<GameKernelStepV7, GameKernelV7Error>
    {
        ...
        let authority_seat = run.battle.as_ref().ok_or(GameKernelV7Error::Invalid)?.authority_seat;
        let operation_id = OperationId::new(format!("current/phase/{}/{}/{}",
            run.run_id.get().get(), run.wave.get().get(), runtime.next_authority_revision().get()))
            .map_err(|_| GameKernelV7Error::Invalid)?;
        let mut staged = GameRuntimeV6::from_snapshot_with_retention(
            runtime.snapshot(), self.content.clone(), MATERIAL_RETENTION_V7,
        ).map_err(runtime_error)?;
        let mut step = execute_current_transaction(&mut staged, GameInternalEventV2::OwnedPhaseRequested {
            operation_id, authority_seat, phase,
        })?;
