...
use crate::current_proposal_v7::{
    ...
    CurrentProposalOwnerSnapshotV1, DecodedCurrentReceiptV1, MAX_CURRENT_RECEIPT_BYTES_V1,
...
impl GameKernelV7 {
    pub fn natural_start_with_fresh_friendship(
        ...
        if profile.schema_version != er_state::m7_state::PROFILE_STATE_SCHEMA_VERSION_V1
            ...
            || !profile.challenges.is_empty()
            || !profile.flags.is_empty()
            || !profile.dex.entries.is_empty()
        ...
        let owner =
            er_game::current_friendship_profile::fresh_profile(&start.content, start.local_seat)
                .map_err(|_| GameKernelV7Error::Invalid)?;
        let mut value = Self::natural_start(
            start.profile,
            start.seed,
            start.local_seat,
            start.save_slots,
    ...
    fn validate_current_network_pair(
        ...
        if self.has_current_coop_rebind() {
            current_coop_rebind_v7::validate_open_pair(
                &self.snapshot()?,
                self.local_seat,
                role,
                true,
    ...
    fn ingest_rebound_current_frame(
        ...
        match self.role {
            GameKernelRoleV7::Authority => {
                let proposal =
                    decode_current_proposal_v1(bytes).map_err(|_| GameKernelV7Error::Invalid)?;
                if proposal.connection_generation != generation
                    || proposal.proposal.context.authority_revision < floor
                {
                    return Err(GameKernelV7Error::Invalid);
                }
                if let Some(reply) = self
                    .current_coop_setup
                    .as_ref()
                    .and_then(|setup| setup.last_reply_v2.as_ref())
                    && reply.proposal_hex == current_bytes_hex_v1(bytes)
                {
                    return Ok(GameKernelStepV7 {
                        effects: vec![GameKernelEffectV7::AuthorityMaterial {
                ...
                for effect in &mut step.effects {
                    ...
                    {
                        if *operation_id != proposal.proposal.context.operation_id
                            || *material != evidence.material_bytes
                        {
                            return Err(GameKernelV7Error::Invalid);
                        }
                        *material = reply
            ...
            GameKernelRoleV7::Replica => {
                if let Ok(receipt) = CurrentProposalMaterialReceiptV2::decode(bytes) {
                    ...
                    self.apply_current_receipt_evidence(
                        evidence,
                        receipt.authority_context,
                        receipt.proposal_hex,
    ...
    pub fn transport_changed(
        ...
        if self.has_current_coop_rebind() {
            return self.current_rebind_transport_changed(generation, connected);
        }
        let mut candidate = self.clone();
    ...
    fn admit_current_game_proposal(
            ...
            && owner.started.is_some()
        {
            ...
            if let Some((context, transaction)) = rebound_context {
                owner.last_reply_v2 = Some(Box::new(
                    CurrentProposalMaterialReceiptV2::from_admission(
                        bytes,
                        material,
                        context,
                        transaction,
    ...
    pub fn validate(&self) -> Result<(), GameKernelV7Error> {
        ...
        let profile = match &self.lifecycle {
            ...
            | GameKernelLifecycleV7::Terminal { runtime, .. } => runtime
                .state()
                .and_then(|state| state.current_friendship_profile.as_ref()),
        };
    ...
    fn bootstrap_input(
        ...
        let current_storage =
            bootstrap.current_storage.is_some() || bootstrap.current_starter_pokerus.is_some();
        ...
        if bootstrap.needs_starter_pokerus_clock() {
            ...
            bootstrap
                .begin_starter_pokerus_clock(request, self.local_seat)
                .map_err(|error| GameKernelV7Error::Bootstrap(error.to_string()))?;
        ...
        .run_to_quiescence(|event| match event {
            GameInternalEventV2::OwnedPhaseRequested {
                ...
                let prepared = runtime
                    .execute_owned_phase(operation_id, authority_seat, phase)
