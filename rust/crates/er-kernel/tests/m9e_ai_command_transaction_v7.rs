...
fn natural(content: Arc<PreparedGameContentV2>, cooperative: bool) -> Result<GameKernelV7, Box<dyn Error>> {
    let mode = content.bundle().bootstrap.modes.iter()
        .find(|mode| mode.supported && mode.cooperative == cooperative).ok_or("supported mode missing")?;
    let mode_option = format!("bootstrap/mode/{}", mode.mode.get());
    let mut kernel = GameKernelV7::natural_start(
        profile()?, "m9e-ai-command-transaction".to_owned(), SeatId::new(safe(1)),
        vec!["ai-command-slot".to_owned()], true, content,
    ...
    let battle = state.active_run.as_ref().and_then(|run| run.battle.as_ref()).ok_or("battle missing")?;
    let actors = battle.field.slots.iter().filter(|slot| slot.slot.side == BattleSide::Enemy && slot.occupant.is_some()).count();
...
fn restore(snapshot: CoreGameKernelSnapshotV7, content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7, Box<dyn Error>> {
    ...
    let kernel = GameKernelV7::from_snapshot(
        serde_json::from_slice(&serde_json::to_vec(&snapshot)?)?, SeatId::new(safe(1)), GameKernelRoleV7::Authority, content,
    )?;
...
fn later_actor_rejection_preserves_the_complete_ai_command_owner() -> Result<(), Box<dyn Error>> {
    ...
    let battle = state.active_run.as_mut().and_then(|run| run.battle.as_mut()).ok_or("battle missing")?;
    let actors = battle.field.slots.iter().filter(|slot| slot.slot.side == BattleSide::Enemy)
        .filter_map(|slot| slot.occupant).collect::<Vec<_>>();
    assert_eq!(actors.len(), 2);
    assert_eq!(battle.enemy_party.len(), 2);
    let first = battle.enemy_party.iter().find(|pokemon| pokemon.id == actors[0]).ok_or("first actor missing")?;
    ...
    battle.enemy_party.iter_mut().find(|pokemon| pokemon.id == actors[1]).ok_or("second actor missing")?.moves = [None; 4];
    state.validate_with(content.as_ref())?;
    let mut kernel = GameKernelV7::from_active(
        state, original.material_ledger.next_authority_revision, SeatId::new(safe(1)), GameKernelRoleV7::Authority,
        content.clone(), original.input_router, original.scheduler, original.protocol,
    )?;
    ...
    assert!(kernel.snapshot()? == before, "later-actor failure changed the full snapshot");
    assert!(kernel.prepare_authority_ai_commands().is_err());
    assert!(kernel.snapshot()? == before, "repeated failure changed the full snapshot");
    Ok(())
}

#[test]
fn command_cursor_rejection_preserves_ai_sequence_and_all_other_owners() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    ...
    boundary.authority_ai.as_mut().ok_or("AI owner missing")?.decision_sequence = maximum;
    let mut successful = restore(boundary, content.clone())?;
    assert_eq!(successful.prepare_authority_ai_commands()?.len(), 1);
    assert_eq!(successful.snapshot()?.authority_ai.ok_or("AI owner missing")?.decision_sequence, maximum + 1);
    let mut exhausted = original;
    exhausted.authority_ai.as_mut().ok_or("AI owner missing")?.decision_sequence = maximum + 1;
    let mut kernel = restore(exhausted.clone(), content)?;
    assert!(kernel.prepare_authority_ai_commands().is_err());
    assert!(kernel.snapshot()? == exhausted, "cursor failure changed the full snapshot");
    Ok(())
}

#[test]
fn complete_two_actor_preparation_commits_once_and_replays_identical_commands() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    ...
    assert_eq!(after.authority_ai.as_ref().ok_or("AI owner missing")?.decision_sequence,
               before.authority_ai.as_ref().ok_or("AI owner missing")?.decision_sequence + 2);
