...
fn owned_title_with_capacity(
    content: Arc<PreparedGameContentV2>, host: bool, capacity: SafeU53,
) -> Result<GameKernelV7, Box<dyn Error>> {
    ...
    if let BattleProtocolRoleConfig::Authority { proposal_capacity, .. } = &mut config.role {
...
fn pair_with_capacity(capacity: SafeU53) -> TestResult<(GameKernelV7, GameKernelV7)> {
    let mut host = if capacity == safe(64) { owned_title(content()?, true)? }
        else { owned_title_with_capacity(content()?, true, capacity)? };
    let mut guest = if capacity == safe(64) { owned_title(content()?, false)? }
        else { owned_title_with_capacity(content()?, false, capacity)? };
...
fn actual_guest_proposal(host: &mut GameKernelV7, guest: &mut GameKernelV7, generation: u64) -> TestResult<(Vec<u8>, Vec<u8>)> {
    ...
    assert_eq!(decoded.connection_generation, ConnectionGeneration::new(safe(generation)));
    let pending = guest.snapshot()?;
    let retained = pending.current_proposal.as_ref().ok_or("actual pending guest proposal")?.retained();
    assert_eq!(retained.publication_context.connection_generation, decoded.connection_generation);
    assert_eq!(er_kernel::current_proposal_v7::decode_current_hex_v1(&retained.proposal_hex, 16_384)?, proposal);
...
fn open_rebind_executes_actual_owned_gameplay_and_retries_strict_v2_receipt() -> TestResult {
    use er_kernel::current_proposal_v7::{CurrentProposalMaterialReceiptV1, CurrentProposalMaterialReceiptV2};
    let (mut host, mut guest) = pair()?;
    let offer = begin(&mut host, &mut guest)?;
    handshake(&mut host, &mut guest, offer)?;
    let transcript = owner(&host.snapshot()?)?.transcript.clone();
    ...
    assert_eq!(decoded.authority_context.connection_generation, ConnectionGeneration::new(safe(2)));
    let host_after = host.snapshot()?;
    assert!(host_after.current_coop_setup.as_ref().ok_or("setup")?.last_reply.is_none());
    assert_eq!(wire(&host.ingest_network_frame(ConnectionGeneration::new(safe(2)), &proposal)?)?, reply);
    ...
    assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(2)), &reply).is_err());
    ...
    assert!(step.effects.iter().all(|effect| matches!(effect,
        GameKernelEffectV7::Presentation(_) | GameKernelEffectV7::UiChanged(_)
        | GameKernelEffectV7::Terminal(_))));
    assert!(guest.snapshot()?.current_proposal.is_none());
    let applied = guest.snapshot()?;
    assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(2)), &reply)?.effects.is_empty());
...
fn open_rebind_replaces_original_v1_reply_atomically_at_capacity_one() -> TestResult {
    ...
    let old_reply = wire(&host.ingest_network_frame(ConnectionGeneration::new(safe(1)), &old_proposal)?)?;
    ...
    let offer = begin(&mut host, &mut guest)?;
    handshake(&mut host, &mut guest, offer)?;
    assert_eq!(host.snapshot()?.current_coop_setup.as_ref().ok_or("setup")?
        .last_reply.as_ref().ok_or("original cache")?.canonical_bytes()?, old_reply);
    let (_, proposal) = actual_guest_proposal(&mut host, &mut guest, 2)?;
    let before = host.snapshot()?;
    let mut forged: serde_json::Value = serde_json::from_slice(&proposal)?;
    forged["unknown"] = serde_json::json!(true);
    assert!(host.ingest_network_frame(ConnectionGeneration::new(safe(2)), &er_canonical::canonical_bytes(&forged)?).is_err());
    assert_eq!(host.snapshot()?, before);
    let reply = wire(&host.ingest_network_frame(ConnectionGeneration::new(safe(2)), &proposal)?)?;
    ...
    assert_eq!(setup.last_reply_v2.as_ref().ok_or("V2 cache")?.canonical_bytes()?, reply);
    let fingerprints = &after.protocol.as_ref().ok_or("protocol")?.proposal_admission.as_ref().ok_or("admission")?.fingerprints;
    assert_eq!(fingerprints.len(), 1);
    assert_eq!(fingerprints[0].operation_id, receipt.evidence()?.proposal.proposal.context.operation_id);
    for generation in [1, 2] {
        let before_host = host.snapshot()?;
        let before_guest = guest.snapshot()?;
        assert!(host.ingest_network_frame(ConnectionGeneration::new(safe(generation)), &old_proposal).is_err());
        assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(generation)), &old_reply).is_err());
        assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(generation)), &old_material).is_err());
    ...
    assert!(GameKernelV7::from_snapshot(both, SeatId::new(safe(1)), GameKernelRoleV7::Authority, content()?).is_err());
...
fn open_rebind_receipt_and_owner_mutations_reject_with_complete_state_conservation() -> TestResult {
    ...
    assert!(receipt.rebind_transaction_id.bytes().all(|byte| byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte)));
    let mut different_transaction = receipt.rebind_transaction_id.clone();
    different_transaction.replace_range(..1, if receipt.rebind_transaction_id.starts_with('0') { "1" } else { "0" });
    ...
    for case in 0..9 {
        ...
        match case {
            ...
            4 => changed["material_fingerprint"] = serde_json::json!(format!("blake3-v1:{}", "0".repeat(64))),
            5 => changed["proposal_digest"] = serde_json::json!(format!("sha256-json-bytes-v1:{}", "0".repeat(64))),
            6 => changed["rebind_transaction_id"] = serde_json::json!(format!("blake3-v1:{}", receipt.rebind_transaction_id)),
            7 => changed["rebind_transaction_id"] = serde_json::json!(format!("sha256:{}", receipt.rebind_transaction_id)),
        ...
        assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(2)), &bytes).is_err(), "receipt case {case}");
    ...
    for case in 0..6 {
        ...
        match case {
            0 => changed.protocol.as_mut().ok_or("protocol")?.proposal_admission.as_mut().ok_or("admission")?.fingerprints.clear(),
            1 => changed.current_coop_setup.as_mut().ok_or("setup")?.rebind = None,
            2 => changed.current_coop_setup.as_mut().ok_or("setup")?.last_reply_v2.as_mut().ok_or("cache")?.rebind_transaction_id = different_transaction.clone(),
            3 => changed.current_coop_setup.as_mut().ok_or("setup")?.last_reply_v2.as_mut().ok_or("cache")?.authority_context.run_id = RunId::new("different-protocol-run")?,
            4 => {
                changed.current_coop_setup.as_mut().ok_or("setup")?.rebind.as_mut()
                    .ok_or("rebind")?.transcript.pop().ok_or("transcript")?;
            }
            _ => changed.current_coop_setup.as_mut().ok_or("setup")?.last_reply_v2.as_mut().ok_or("cache")?.material_hex.push_str("00"),
        }
        assert!(GameKernelV7::from_snapshot(changed, SeatId::new(safe(1)), GameKernelRoleV7::Authority, content()?).is_err(), "snapshot case {case}");
    ...
    let mut exhausted = GameKernelV7::from_snapshot(exhausted_snapshot, SeatId::new(safe(2)),
        GameKernelRoleV7::Replica, content()?)?;
    let before_overflow = exhausted.snapshot()?;
    assert!(exhausted.ingest_network_frame(ConnectionGeneration::new(safe(2)), &reply).is_err());
    assert_eq!(exhausted.snapshot()?, before_overflow);
    let before_oversize = guest.snapshot()?;
    assert!(guest.ingest_network_frame(ConnectionGeneration::new(safe(2)), &vec![0;
        er_kernel::current_proposal_v7::MAX_CURRENT_RECEIPT_BYTES_V1 + 1]).is_err());
