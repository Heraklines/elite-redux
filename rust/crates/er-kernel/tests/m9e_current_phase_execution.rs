...
fn navigate(kernel: &mut GameKernelV7, option: &str) -> Result<()> {
    ...
    for _ in 0..=count {
        ...
        if menu.selected_option_id.as_str() == option { return Ok(()); }
        let selected = menu.options.iter().position(|row| row.option_id == menu.selected_option_id)
            .ok_or("actual selected row absent")?;
        let wanted = menu.options.iter().position(|row| row.option_id.as_str() == option)
            .ok_or("actual requested row absent")?;
        press(kernel, if wanted < selected { PhysicalKey::ArrowUp } else { PhysicalKey::ArrowDown })?;
    }
...
fn natural(content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7> {
    ...
    let mut kernel = GameKernelV7::natural_start_with_fresh_friendship(FreshFriendshipStartV7 {
        profile, seed: "m9e-phase-execution-18".to_owned(), local_seat: seat()?,
...
fn raw_knockout_waits_for_xp_prompt_then_level_stats_with_exact_material_restore() -> Result<()> {
    ...
    for _ in 0..96 {
        ...
        if let Some(victory) = pending.and_then(|pending| pending.victory.as_ref()) {
            match &victory.descendant {
                ...
                CurrentVictoryDescendantV1::LevelUpStart { level_up } => {
                    ...
                    let GameKernelLifecycleSnapshotV7::Active(forged_state) = &mut forged.lifecycle else {
                        return Err("active level-start snapshot absent".into());
                    };
                    let retained = forged_state.current_battle_participation.as_mut()
                        .and_then(|owner| owner.experience.as_mut())
                        .and_then(|owner| owner.pending.iter_mut().find_map(|pending| pending.victory.as_mut()))
                        .ok_or("actual retained victory absent")?;
                    let CurrentVictoryDescendantV1::LevelUpStart { level_up } = &mut retained.descendant else {
                        return Err("actual level-start cursor absent".into());
                    };
                    level_up.previous_stats.attack = level_up.previous_stats.attack.checked_add(1).ok_or("stat overflow")?;
                    assert!(restore(forged, content.clone()).is_err(), "retained previous stats must match the captured Faint preimage");
                },
                CurrentVictoryDescendantV1::LevelUpPresentation { end, event_id } => {
                    ...
                    assert_eq!(state.current_friendship_profile.as_ref()
                        .and_then(|profile| profile.rewards.as_ref()).ok_or("actual fresh account rewards absent")?
                        .highest_level.get(), u64::from(pokemon.level), "highestLevel must be updated before the stat presentation");
        ...
        let snapshot = kernel.snapshot()?;
        kernel = restore(serde_json::from_slice(&canonical_bytes(&snapshot)?)?, content.clone())?;
        assert_eq!(canonical_bytes(&kernel.snapshot()?)?, canonical_bytes(&snapshot)?,
    ...
    pokemon.experience = Experience::new(safe(pokemon.experience.get().get().checked_add(1).ok_or("XP overflow")?)?);
    assert!(restore(wrong_xp, content.clone()).is_err(), "post-award XP must match the retained source arithmetic");
    ...
    assert!(pokemon.hp > 1);
    pokemon.hp -= 1;
    assert!(restore(wrong_hp, content.clone()).is_err(), "post-stat HP must match the captured pre-stat maximum");
