...
use er_kernel::game_kernel_v7::{FreshFriendshipStartV7, GameKernelEffectV7,
    GameKernelRoleV7, GameKernelStepV7, GameKernelV7};
...
fn seat() -> Result<SeatId> { Ok(SeatId::new(safe(1)?)) }
...
fn press(kernel: &mut GameKernelV7, key: PhysicalKey) -> Result<GameKernelStepV7> {
    let result = kernel.raw_input(RawInputEvent::KeyDown { code: key.clone(), printable: false,
...
fn navigate(kernel: &mut GameKernelV7, option: &str) -> Result<()> {
    let count = kernel.current_control().and_then(|control| control.menu.as_ref())
        .ok_or("actual menu absent")?.options.len();
    // Start at the actual top row, so clamped menus are navigable too.
    for _ in 0..count { press(kernel, PhysicalKey::ArrowUp)?; }
    for _ in 0..=count {
        if kernel.current_control().and_then(|control| control.menu.as_ref())
            .is_some_and(|menu| menu.selected_option_id.as_str() == option) { return Ok(()); }
...
fn active(snapshot: &CoreGameKernelSnapshotV7) -> Result<&GameStateV6> {
    match &snapshot.lifecycle { GameKernelLifecycleSnapshotV7::Active(state) => Ok(state),
        _ => Err("actual active lifecycle required".into()) }
}
fn natural(content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7> {
    let profile = ProfileStateV1 { schema_version: PROFILE_STATE_SCHEMA_VERSION_V1,
        unlocks: vec![], achievements: vec![], challenges: vec![], flags: Default::default(),
    ...
    let mut kernel = GameKernelV7::natural_start_with_fresh_friendship(FreshFriendshipStartV7 {
        profile, seed: "m9e-target-execution-source-0".to_owned(), local_seat: seat()?,
        save_slots: vec!["phase-source-slot".to_owned()], content: content.clone(),
        scheduler: KernelSchedulerSnapshotV2 { next_timer_id: Some(SafeU53::ZERO), timers: vec![],
            pauses: vec![], disposed: false },
    })?;
    press(&mut kernel, PhysicalKey::Space)?;
    let mode = content.bundle().bootstrap.modes.iter().find(|mode| mode.supported
        && !mode.cooperative && !mode.challenge_selection
        && content.world.mode(mode.mode).is_some_and(|definition| definition.key == "CLASSIC"))
    ...
    let clocks: Vec<_> = before_clock.pending_platform.iter().filter_map(|pending|
        matches!(pending.effect, GamePlatformEffectV2::StarterPokerusClock { .. })
            .then_some(pending.request_id)).collect();
    assert_eq!(clocks.len(), 1, "fresh raw entry must actually request source Date");
    let request = clocks[0];
    let mut resumed = restore(serde_json::from_slice(&canonical_bytes(&before_clock)?)?, content.clone())?;
    let original = canonical_bytes(&resumed.snapshot()?)?;
    assert!(resumed.apply_current_utc_clock_result(request, 8_640_000_000_000_001).is_err());
    ...
    assert_eq!(canonical_bytes(&kernel.snapshot()?)?, canonical_bytes(&resumed.snapshot()?)?);
    ...
    let starter = bootstrap.catalog.starters.iter().find(|row| row.species_id.get() == 1
        && row.form_index == 0 && row.ability_index == 0).ok_or("actual Bulbasaur selection absent")?;
    ...
    assert!(kernel.state().and_then(|state| state.current_battle_participation.as_ref())
        .and_then(|owner| owner.experience.as_ref()).is_some_and(|owner| owner.source_progression.is_some()));
    Ok(kernel)
}

fn accept_material(
    live: &mut Option<GameStateV6>, ledger: &mut AppliedGameMaterialLedgerV1,
    kernel: &GameKernelV7, content: &PreparedGameContentV2, step: &GameKernelStepV7,
) -> Result<GameMaterialV6> {
    let materials: Vec<_> = step.effects.iter().filter_map(|effect| match effect {
        GameKernelEffectV7::AuthorityMaterial { bytes, .. } => Some(bytes), _ => None,
    }).collect();
    assert_eq!(materials.len(), 1, "one source transaction per public phase entry");
    ...
    let presentations: Vec<_> = step.effects.iter().filter_map(|effect| match effect {
        GameKernelEffectV7::Presentation(effect) => Some(effect.clone()), _ => None,
    }).collect();
    assert_eq!(presentations, material.transition().presentation);
    assert_eq!(apply_game_material_v6(live, ledger, content, materials[0])?,
...
fn controlled_before_knockout(content: Arc<PreparedGameContentV2>, level: u16, moves: &[u64])
    -> Result<GameKernelV7>
{
    ...
    let definition = content.progression.species(pokemon.species_id, pokemon.form_index).ok_or("species absent")?;
    let growth = content.progression.growth_rate(definition.growth_rate).ok_or("growth absent")?;
    let threshold = er_progression::progression::current_growth_experience_for_level(growth, level + 1)?;
    pokemon.level = level;
    pokemon.experience = Experience::new(safe(threshold.get().get().checked_sub(1).ok_or("threshold underflow")?)?);
    let species = content.battle.species(pokemon.species_id)?;
    let form = content.battle.form(&er_types::FormId::parse(format!("{}:0", pokemon.species_id.get().get()))?)?;
    ...
    pokemon.stats = er_progression::current_stats::calculate_current_unmodified_stats(
        pokemon, form.stat_override.unwrap_or(species.base_stats), nature)?;
    ...
    tackle.move_id = MoveId::new(safe(33)?); tackle.pp_used = 0; tackle.pp_ups = 0; tackle.max_pp_override = None;
    pokemon.moves = [None, None, None, None];
    for (slot, id) in moves.iter().enumerate() {
        let mut entry = tackle.clone(); entry.move_id = MoveId::new(safe(*id)?);
    ...
    enemy.hp = 1; enemy.stats.speed = 500; enemy.stats.attack = 1;
...
fn admit_knockout(kernel: &mut GameKernelV7, content: &PreparedGameContentV2)
...
fn raw_knockout_waits_for_xp_prompt_then_level_stats_with_exact_material_restore() -> Result<()> {
    ...
    let pokemon = &kernel.state().ok_or("state absent")?.active_run.as_ref().ok_or("run absent")?.party[0];
    ...
    for _ in 0..96 {
        ...
        let pending = state.current_battle_participation.as_ref().and_then(|owner| owner.experience.as_ref())
            .and_then(|owner| owner.pending.iter().find(|pending| pending.victory.is_some()));
        if let Some(victory) = pending.and_then(|pending| pending.victory.as_ref()) {
            match &victory.descendant {
                CurrentVictoryDescendantV1::AwardPresentation { award, event_id } => {
                    assert!(!saw_award, "same award prompt repeated after its exact acknowledgement");
                    saw_award = true;
                    ...
                    assert!(checkpoint.pending_presentations.iter().any(|pending| pending.event_id == *event_id));
                ...
                CurrentVictoryDescendantV1::LevelUpStart { level_up } => {
                    ...
                    assert_eq!(pokemon.stats, before_stats, "stats must wait for actual LevelUp.start");
                    assert_eq!(pokemon.experience.get().get(), before_xp.get().get() + level_up.award.experience.get().get());
                },
        ...
        let resumed = restore(serde_json::from_slice(&canonical_bytes(&kernel.snapshot()?)?)?, content.clone())?;
        assert_eq!(canonical_bytes(&resumed.snapshot()?)?, canonical_bytes(&kernel.snapshot()?)?);
        kernel = resumed;
        if reached_stats { break; }
        ...
        let step = if let Some(clock) = snapshot.pending_platform.iter().find(|pending|
            matches!(pending.effect, GamePlatformEffectV2::CurrentFriendshipClock { .. })) {
    ...
    assert!(reached_stats, "actual XP and LevelUp presentation did not finish within the watchdog");
    assert_eq!(live.as_ref(), kernel.state());
    let state = kernel.state().ok_or("final state absent")?;
    let save = er_save::m9e_save_v2::GameSaveV2::new(state.content_identity.clone(), safe(1)?, state.clone())?;
    assert_eq!(er_save::m9e_save_v2::GameSaveV2::decode(&save.encode()?)?.state, *state);
    Ok(())
}

fn batch(kernel: &GameKernelV7) -> Result<er_state::current_experience_settlement::CurrentLearnMoveBatchV1> {
    kernel.state().and_then(|state| state.current_battle_participation.as_ref())
        .and_then(|owner| owner.experience.as_ref())
        .and_then(|owner| owner.pending.iter().find_map(|pending| pending.victory.as_ref()
            .and_then(|victory| match &victory.descendant {
                CurrentVictoryDescendantV1::LearnMoveBatch { batch } => Some(batch.clone()), _ => None,
...
fn raw_batch_option(
    kernel: &mut GameKernelV7, content: &PreparedGameContentV2, option: &str,
    live: &mut Option<GameStateV6>, ledger: &mut AppliedGameMaterialLedgerV1,
) -> Result<()> {
    ...
    assert!(matches!(material.transition().accepted_action,
...
fn raw_level_move_batch_retains_slot_cancel_undo_and_learning_stamp_across_restore() -> Result<()> {
    ...
    let definition = content.progression.species(species, 0).ok_or("actual Bulbasaur content absent")?;
    let initial_moves = [33_u64, 45, 230, 580];
    let level = (2_u16..=10).find(|level| {
        let novel: std::collections::BTreeSet<_> = definition.level_moves.iter()
            .filter(|row| i32::from(row.level) == i32::from(*level)
                && !initial_moves.contains(&row.move_id.get().get()))
            .map(|row| row.move_id).collect();
        novel.len() >= 2
    ...
    for _ in 0..96 {
        if batch(&kernel).is_ok() { reached_batch = true; break; }
        settle_presentations(&mut kernel, &live)?;
        kernel = restore(serde_json::from_slice(&canonical_bytes(&kernel.snapshot()?)?)?, content.clone())?;
        let checkpoint = kernel.snapshot()?;
    ...
    assert!(reached_batch, "actual KO/XP/LevelUp did not reach source learn batch");
    ...
    raw_batch_option(&mut kernel, content.as_ref(), "current/learn-batch/done", &mut live, &mut ledger)?;
    assert!(batch(&kernel)?.cancel_confirmation);
    assert_eq!(kernel.current_control().and_then(|control| control.menu.as_ref())
        .ok_or("actual confirmation absent")?.selected_option_id.as_str(), "current/learn-batch/cancel/no");
    let step = press(&mut kernel, PhysicalKey::Escape)?;
    accept_material(&mut live, &mut ledger, &kernel, content.as_ref(), &step)?;
    assert!(!batch(&kernel)?.cancel_confirmation);
    assert!(!batch(&kernel)?.complete);
    raw_batch_option(&mut kernel, content.as_ref(), &move_option, &mut live, &mut ledger)?;
    ...
    assert_eq!(kernel.state().ok_or("state absent")?.active_run.as_ref().ok_or("run absent")?.party[0].moves,
        initial.original_moves);
    raw_batch_option(&mut kernel, content.as_ref(), &move_option, &mut live, &mut ledger)?;
    raw_batch_option(&mut kernel, content.as_ref(), &format!("current/learn-batch/{}/slot/0", move_id.get()),
        &mut live, &mut ledger)?;
    ...
    let stamps = &state.current_achievement_tracker.as_ref().ok_or("real tracker absent")?
        .persistent.learned_move_stamps;
    assert_eq!(stamps.get(&move_id), Some(&wave));
    let stamp_image = stamps.clone();
    let checkpoint = kernel.snapshot()?;
    kernel = restore(serde_json::from_slice(&canonical_bytes(&checkpoint)?)?, content.clone())?;
    assert_eq!(canonical_bytes(&kernel.snapshot()?)?, canonical_bytes(&checkpoint)?);
    raw_batch_option(&mut kernel, content.as_ref(), "current/learn-batch/undo", &mut live, &mut ledger)?;
    let undone = batch(&kernel)?;
    assert!(!undone.complete && undone.assignments.is_empty());
    let state = kernel.state().ok_or("state absent")?;
    assert_eq!(state.active_run.as_ref().ok_or("run absent")?.party[0].moves, initial.original_moves);
    assert_eq!(state.current_achievement_tracker.as_ref().ok_or("real tracker absent")?
        .persistent.learned_move_stamps, stamp_image, "source Undo restores moves only, never tracker history");
    raw_batch_option(&mut kernel, content.as_ref(), &move_option, &mut live, &mut ledger)?;
    raw_batch_option(&mut kernel, content.as_ref(), &format!("current/learn-batch/{}/slot/1", move_id.get()),
        &mut live, &mut ledger)?;
    raw_batch_option(&mut kernel, content.as_ref(), "current/learn-batch/done", &mut live, &mut ledger)?;
    let state = kernel.state().ok_or("state absent")?;
    let pokemon = &state.active_run.as_ref().ok_or("run absent")?.party[0];
    let learned = pokemon.moves[1].as_ref().ok_or("actual learned slot absent")?;
    assert_eq!(learned.move_id, move_id);
    assert_eq!((learned.pp_used, learned.pp_ups, learned.max_pp_override), (0, 0, None));
    assert_eq!(state.current_achievement_tracker.as_ref().ok_or("real tracker absent")?
        .persistent.learned_move_stamps.get(&move_id), Some(&wave));
    let save = er_save::m9e_save_v2::GameSaveV2::new(state.content_identity.clone(), safe(1)?, state.clone())?;
    assert_eq!(er_save::m9e_save_v2::GameSaveV2::decode(&save.encode()?)?.state, *state);
