...
use er_battle::m7_resolver::{resolve_turn_v5_with_current_targets, TurnAuthorityContextV1};
use er_canonical::canonical_bytes;
use er_game::m9e_content_v2::{GameContentBundleV2, PreparedGameContentV2};
use er_game::m9e_material_v6::{apply_game_material_v6, game_state_digest, GameMaterialV6};
use er_kernel::game_kernel_v7::{FreshFriendshipStartV7, GameKernelEffectV7, GameKernelRoleV7, GameKernelStepV7, GameKernelV7};
...
use er_state::m7_state::{DexState, GameStateV5, ProfileStateV1, ProfileStatistics, RunStateV3, PROFILE_STATE_SCHEMA_VERSION_V1};
use er_state::m9e_state_v6::GameStateV6;
use er_types::battle_command::{AcceptedBattleCommand, BattleCommand, BattleCommandProposalV1, BattleTargetSelection, CommandSet, ScriptedEnemyBattleCommandV1, player_command_operation_id, scripted_enemy_command_operation_id};
use er_types::battle_ids::{AbilityId, BattleFormat, BattleSide, FieldSlot, MoveId, MoveSlotIndex, PokemonId, WaveIndex};
use er_types::input::{InputFocus, PhysicalKey, RawInputEvent};
use er_types::{BehaviorSourceId, GameActionV1, GameContentIdentity, GameControlKindV2, SafeU53, SeatId};
type Result<T> = std::result::Result<T, Box<dyn Error>>;
const BUNDLE: &[u8] = include_bytes!("../../../fixtures/m9/engineering/game-content-bundle-v2.json");
fn safe(value: u64) -> SafeU53 { SafeU53::new(value).expect("bounded test identity") }
fn seat() -> SeatId { SeatId::new(safe(1)) }
fn slot(side: BattleSide, position: u8) -> FieldSlot { FieldSlot { side, position } }
fn content() -> Result<Arc<PreparedGameContentV2>> {
    Ok(Arc::new(PreparedGameContentV2::prepare(Arc::new(serde_json::from_slice::<GameContentBundleV2>(BUNDLE)?))?))
}
fn profile() -> Result<ProfileStateV1> {
    Ok(ProfileStateV1 { schema_version: PROFILE_STATE_SCHEMA_VERSION_V1,
        unlocks: vec![], achievements: vec![], challenges: vec![], flags: Default::default(), dex: DexState::default(),
        statistics: ProfileStatistics { runs_started: SafeU53::ZERO, runs_won: SafeU53::ZERO,
            runs_lost: SafeU53::ZERO, battles_won: SafeU53::ZERO, pokemon_captured: SafeU53::ZERO,
...
fn navigate(kernel: &mut GameKernelV7, option: &str) -> Result<()> {
    let bound = kernel.current_control().and_then(|c| c.menu.as_ref()).ok_or("menu absent")?.options.len() + 1;
    for _ in 0..bound {
        if kernel.current_control().and_then(|c| c.menu.as_ref()).is_some_and(|m| m.selected_option_id.as_str() == option) { return Ok(()); }
...
fn natural(content: Arc<PreparedGameContentV2>, index: usize) -> Result<GameKernelV7> {
    let mut kernel = GameKernelV7::natural_start_with_fresh_friendship(FreshFriendshipStartV7 {
        profile: profile()?, seed: format!("m9e-target-execution-source-{index}"), local_seat: seat(),
        save_slots: vec!["target-source-slot".to_owned()], content: content.clone(),
        scheduler: KernelSchedulerSnapshotV2 { next_timer_id: Some(SafeU53::ZERO), timers: vec![], pauses: vec![], disposed: false },
    })?;
    press(&mut kernel, PhysicalKey::Space)?;
    let mode = content.bundle().bootstrap.modes.iter().find(|mode| mode.supported && !mode.cooperative
        && !mode.challenge_selection && content.world.mode(mode.mode).is_some_and(|m| m.key == "CLASSIC"))
        .ok_or("ordinary Classic mode absent")?;
    navigate(&mut kernel, &format!("bootstrap/mode/{}", mode.mode.get()))?;
    press(&mut kernel, PhysicalKey::Space)?;
    let snapshot = kernel.snapshot()?;
    let GameKernelLifecycleSnapshotV7::Bootstrap(bootstrap) = snapshot.lifecycle else { return Err("starter phase absent".into()); };
    let starter = content.bundle().bootstrap.starters.get(index).ok_or("source starter absent")?;
    let selected = bootstrap.catalog.starters.iter().find(|row| row.species_id == starter.species_id.get()
        && row.form_index == starter.form_index).ok_or("actual starter owner absent")?;
    navigate(&mut kernel, &format!("bootstrap/starter/{}", selected.pokemon_id.get()))?;
    press(&mut kernel, PhysicalKey::Space)?;
    navigate(&mut kernel, "bootstrap/starter/confirm")?;
    for _ in 0..4 { press(&mut kernel, PhysicalKey::Space)?; }
    assert_eq!(kernel.current_control().map(|c| c.kind), Some(GameControlKindV2::BattleCommand));
    assert!(kernel.state().ok_or("active state absent")?.current_targeting.is_some());
    Ok(kernel)
}
fn active(snapshot: &CoreGameKernelSnapshotV7) -> Result<&GameStateV6> {
    match &snapshot.lifecycle { GameKernelLifecycleSnapshotV7::Active(state) => Ok(state), _ => Err("active lifecycle required".into()) }
}
fn active_mut(snapshot: &mut CoreGameKernelSnapshotV7) -> Result<&mut GameStateV6> {
    match &mut snapshot.lifecycle { GameKernelLifecycleSnapshotV7::Active(state) => Ok(state), _ => Err("active lifecycle required".into()) }
}
fn active_run(state: &GameStateV6) -> Result<&RunStateV3> { state.active_run.as_ref().ok_or_else(|| "run absent".into()) }
fn active_run_mut(state: &mut GameStateV6) -> Result<&mut RunStateV3> { state.active_run.as_mut().ok_or_else(|| "run absent".into()) }
fn restore(snapshot: CoreGameKernelSnapshotV7, content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7> {
    Ok(GameKernelV7::from_snapshot(snapshot, seat(), GameKernelRoleV7::Authority, content)?)
}
fn two_enemies(content: Arc<PreparedGameContentV2>) -> Result<CoreGameKernelSnapshotV7> {
    ...
    second.hp = 200; second.max_hp = 200;
    battle.enemy_party[0].hp = 200; battle.enemy_party[0].max_hp = 200;
...
fn assign_move(state: &mut GameStateV6, id: u64) -> Result<()> {
    let player = active_run_mut(state)?.party.first_mut().ok_or("player absent")?;
    let first = player.moves[0].as_mut().ok_or("move absent")?;
    first.move_id = MoveId::new(safe(id)); first.pp_used = 0; first.pp_ups = 0; first.max_pp_override = None;
    Ok(())
}
fn material(step: &GameKernelStepV7) -> Result<GameMaterialV6> {
    let bytes = step.effects.iter().find_map(|effect| match effect {
        GameKernelEffectV7::AuthorityMaterial { bytes, .. } => Some(bytes), _ => None,
...
fn project(state: &GameStateV6) -> GameStateV5 {
    GameStateV5 { schema_version: er_state::m7_state::GAME_STATE_SCHEMA_VERSION_V5,
        content_identity: GameContentIdentity { oracle_sha: state.content_identity.oracle_sha.clone(),
            content_hash: state.content_identity.bundle_hash.clone(), battle_content_hash: state.content_identity.battle_hash.clone(),
...
fn commands(state: &GameStateV6, targets: &[FieldSlot], prepared: &PreparedGameContentV2) -> Result<CommandSet> {
    ...
    for (index, row) in battle.field.slots.iter().filter(|row| row.occupant.is_some()).enumerate() {
        ...
        let plan = CurrentTargetExecution::from_state(state)?.plan(run, actor, prepared.battle.move_definition(move_id)?)?;
        let selection = plan.selections()?.into_iter().find(|selection|
        ...
        if row.slot.side == BattleSide::Player {
            entries.push(AcceptedBattleCommand::human(BattleCommandProposalV1::new(
                player_command_operation_id(battle.battle_id, battle.wave, battle.turn, row.slot, seat())?,
                battle.battle_id, battle.wave, battle.turn, seat(), actor, row.slot, command,
                menu.instance_id, menu.control_id.clone(),
            )?));
        } else {
            let cursor = safe(u64::try_from(index)?);
            entries.push(AcceptedBattleCommand::scripted_enemy(ScriptedEnemyBattleCommandV1::new(
                scripted_enemy_command_operation_id(battle.battle_id, battle.wave, battle.turn, row.slot, cursor)?,
                battle.battle_id, battle.wave, battle.turn, cursor, actor, row.slot, command,
            )?));
        }
    }
...
fn natural_raw_turn_uses_current_targets_and_preserves_save_material() -> Result<()> {
    let content = content()?; let mut kernel = natural(content.clone(), 2)?;
    let before = kernel.snapshot()?; let state = active(&before)?;
    let actor = active_run(state)?.party[0].id;
    let sources = CurrentTargetExecution::from_state(state)?.ability_sources(active_run(state)?, &active_run(state)?.party[0])?;
    assert_eq!(sources.iter().filter(|s| matches!(s, BehaviorSourceId::PassiveAbility { .. })).count(), 0);
    let plan = CurrentTargetExecution::from_state(state)?.plan(active_run(state)?, actor,
        content.battle.move_definition(active_run(state)?.party[0].moves[0].as_ref().ok_or("move absent")?.move_id)?)?;
    assert_eq!(plan.selections()?, vec![BattleTargetSelection::implicit()]);
    assert!(plan.retain(&BattleTargetSelection::selected(vec![slot(BattleSide::Enemy,0)])?).is_err());
    let step = choose_first_move(&mut kernel)?;
    let actual = material(&step)?;
    assert_eq!(actual.transition().after_state.current_targeting, state.current_targeting);
    assert_eq!(actual.transition().after_state.current_friendship_profile, state.current_friendship_profile);
    let after = kernel.state().ok_or("after state absent")?;
    assert_eq!(active_run(after)?.party.iter().find(|p| p.id == actor).ok_or("actor absent")?.moves[0].as_ref().ok_or("move absent")?.pp_used, 1);
    let mut live = Some(state.clone()); let mut ledger = before.material_ledger.clone();
    apply_game_material_v6(&mut live, &mut ledger, content.as_ref(), &actual.canonical_bytes()?)?;
    ...
    assert_eq!(canonical_bytes(&restored.snapshot()?)?, canonical_bytes(&kernel.snapshot()?)?);
    Ok(())
}

#[test]
fn actual_target_menu_retains_move_restore_and_cancel_owner() -> Result<()> {
    let content = content()?; let snapshot = two_enemies(content.clone())?;
    let mut kernel = restore(snapshot, content.clone())?;
    choose_first_move(&mut kernel)?;
    assert_eq!(kernel.current_control().map(|c| c.kind), Some(GameControlKindV2::BattleTarget));
    let leaf = kernel.snapshot()?;
    let control = kernel.current_control().ok_or("target leaf absent")?;
    assert_eq!(control.menu.as_ref().ok_or("target menu absent")?.options.len(), 2);
    ...
    assert_eq!(canonical_bytes(&restored.snapshot()?)?, canonical_bytes(&leaf)?);
    let mut forged = leaf.clone();
    let forged_control = &mut active_run_mut(active_mut(&mut forged)?)?.control;
    let option = forged_control.menu.as_mut().ok_or("menu absent")?.options.first_mut().ok_or("option absent")?;
    if let GameActionV1::Battle { action: er_types::BattleUiActionV1::SelectMoveTarget { move_slot, .. } } = &mut option.action {
        *move_slot = MoveSlotIndex::new(3)?;
    } else { return Err("actual typed target option required".into()); }
    assert!(restore(forged, content).is_err());
    press(&mut restored, PhysicalKey::Escape)?;
    assert_eq!(restored.current_control().map(|c| c.kind), Some(GameControlKindV2::BattleCommand));
    navigate(&mut kernel, "battle/target/1")?;
    let step = press(&mut kernel, PhysicalKey::Space)?;
    let actual = material(&step)?;
    assert!(matches!(actual.transition().accepted_action, Some(GameActionV1::Battle {
...
fn source_spread_group_executes_all_opponents_and_multihit_exception() -> Result<()> {
    ...
    let plan = CurrentTargetExecution::from_state(&prior)?.plan(active_run(&prior)?, active_run(&prior)?.party[0].id,
        content.battle.move_definition(MoveId::new(safe(57)))?)?;
    assert!(plan.multiple); assert_eq!(plan.ordered, vec![slot(BattleSide::Enemy,0),slot(BattleSide::Enemy,1)]);
    let mut kernel = restore(snapshot.clone(), content.clone())?;
    let step = choose_first_move(&mut kernel)?;
    material(&step)?;
    let after = active_run(kernel.state().ok_or("state absent")?)?.battle.as_ref().ok_or("battle absent")?;
    assert!(after.enemy_party.iter().all(|p| p.hp < 200));
    let state = active_mut(&mut snapshot)?;
    let chatter = content.battle.move_definition(MoveId::new(safe(448)))?;
    assert!(chatter.flags.contains(&er_types::battle_model::MoveFlag::SoundBased));
    assign_move(state, 448)?;
    active_run_mut(state)?.party[0].abilities.active = AbilityId::new(safe(5115));
    let actor = active_run(state)?.party[0].id;
    let spread = CurrentTargetExecution::from_state(state)?.plan(active_run(state)?, actor,
        content.battle.move_definition(MoveId::new(safe(448)))?)?;
    assert!(spread.multiple); assert_eq!(spread.ordered, vec![slot(BattleSide::Enemy,0),slot(BattleSide::Enemy,1)]);
    let mut sound_kernel = restore(snapshot.clone(), content.clone())?;
    let sound_step = choose_first_move(&mut sound_kernel)?; material(&sound_step)?;
    assert!(active_run(sound_kernel.state().ok_or("state absent")?)?.battle.as_ref()
        .ok_or("battle absent")?.enemy_party.iter().all(|p| p.hp < 200));
    let state = active_mut(&mut snapshot)?;
    ...
    assert!(!plan.multiple); assert_eq!(plan.selections()?.len(), 2);
    Ok(())
}
...
fn poison_redirect_and_source_passive_gate_share_actual_owner() -> Result<()> {
    ...
    for (mask, expected) in [(0, 0), (1, 0), (2, 0), (3, 1), (12, 1), (48, 1), (63, 3)] {
        ...
        let profile = controlled.current_friendship_profile.as_mut().ok_or("profile absent")?;
        let account = profile.accounts.iter_mut().find(|a| a.species == species).ok_or("account absent")?;
        account.passive_attr = mask;
        let run = active_run(&controlled)?;
        let sources = CurrentTargetExecution::from_state(&controlled)?.ability_sources(run, &run.party[0])?;
        assert_eq!(sources.iter().filter(|s| matches!(s, BehaviorSourceId::PassiveAbility { .. })).count(), expected);
    }
    for (level, expected) in [(14, 1), (15, 2), (23, 2), (24, 3)] {
        let mut controlled = active(&snapshot)?.clone();
        active_run_mut(&mut controlled)?.battle.as_mut().ok_or("battle absent")?.enemy_party[0].level = level;
        let run = active_run(&controlled)?;
        let enemy = &run.battle.as_ref().ok_or("battle absent")?.enemy_party[0];
        let sources = CurrentTargetExecution::from_state(&controlled)?.ability_sources(run, enemy)?;
        assert_eq!(sources.iter().filter(|s| matches!(s, BehaviorSourceId::PassiveAbility { .. })).count(), expected);
    }
    let state = active_mut(&mut snapshot)?; assign_move(state, 40)?;
    let run = active_run_mut(state)?;
    run.battle.as_mut().ok_or("battle absent")?.enemy_party[0].abilities.active = AbilityId::new(safe(5082));
    let state = active(&snapshot)?; let run = active_run(state)?; let actor = run.party[0].id;
    let owner = CurrentTargetExecution::from_state(state)?;
    let definition = content.battle.move_definition(MoveId::new(safe(40)))?;
    assert_eq!(owner.execution_targets(run, actor, definition, &[slot(BattleSide::Enemy,1)])?, vec![slot(BattleSide::Enemy,0)]);
    let mut kernel = restore(snapshot, content)?;
    choose_first_move(&mut kernel)?; navigate(&mut kernel, "battle/target/1")?;
    let step = press(&mut kernel, PhysicalKey::Space)?; material(&step)?;
    let enemies = &active_run(kernel.state().ok_or("state absent")?)?.battle.as_ref().ok_or("battle absent")?.enemy_party;
    ...
    assert!(enemies[0].hp < 200); assert_eq!(enemies[1].hp, 200);
    Ok(())
}
...
fn queued_faint_retargets_opponents_but_preserves_same_side_cancellation() -> Result<()> {
    ...
    let accepted = commands(state, &[slot(BattleSide::Enemy,1),slot(BattleSide::Enemy,1),slot(BattleSide::Player,0)], content.as_ref())?;
    let before = canonical_bytes(state)?;
    let owner = CurrentTargetExecution::from_state(state)?;
    let result = resolve_turn_v5_with_current_targets(&project(state), &accepted, &content.battle,
        &TurnAuthorityContextV1 { authority_seat: seat(), revision: active_run(state)?.control.revision }, &owner)?;
    let after = result.after_state.active_run.as_ref().ok_or("run absent")?.battle.as_ref().ok_or("battle absent")?;
...
fn unsupported_selection_and_owner_stripping_fail_atomically() -> Result<()> {
    ...
    let GameMaterialV6::BattleTurn(transition) = &mut stripped else { return Err("actual turn material required".into()); };
    ...
    stripped.validate()?;
    let mut live = Some(active(&before)?.clone()); let mut ledger = before.material_ledger.clone();
    let unchanged = canonical_bytes(&(live.clone(), ledger.clone()))?;
    assert_eq!(apply_game_material_v6(&mut live, &mut ledger, content.as_ref(), &stripped.canonical_bytes()?), Err(er_game::m9e_material_v6::GameMaterialV6Error::CurrentTargetOwnership));
    ...
    let mut forged = before.clone(); active_mut(&mut forged)?.current_targeting.as_mut().ok_or("owner absent")?.run_id = er_types::run_ids::GameRunId::new(safe(999));
    assert!(restore(forged, content).is_err());
    Ok(())
}
