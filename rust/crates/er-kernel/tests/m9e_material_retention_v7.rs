...
use er_game::m9e_material_v6::{
    AppliedGameMaterialRecordV1, AppliedMaterialRetentionV1, GameMaterialV6, GameMaterialV6Error,
    GamePlatformEffectV2, GamePresentationEffectV2, MAX_APPLIED_MATERIAL_RECORDS_V1,
    game_state_digest,
};
...
use er_kernel::snapshot_v7::{
    CoreGameKernelSnapshotV7, GameKernelLifecycleSnapshotV7, PendingPresentationV3,
};
...
use er_types::{
    GameActionContextV1, GameActionV1, InputFocus, OperationId, PlatformRequestId,
...
fn reject_material(
    ...
    assert_eq!(
        message,
        GameRuntimeV6Error::Material(expected.to_string()).to_string()
    );
...
fn v7_material_rollover_restores_pending_effects_and_continues_exact_snapshots() -> TestResult {
    ...
    for revision in 1..=capacity + 2 {
        ...
        assert_eq!(
            material.platform_effects,
            vec![GamePlatformEffectV2::StorageDelete {
                request: PlatformRequestId::new(safe(revision)),
                slot: "v7-retention".to_owned(),
            }]
        );
        let expected_step = GameKernelStepV7 {
            effects: vec![GameKernelEffectV7::Presentation(presentation)],
        ...
        if let Some(restored) = &mut resumed {
            assert_eq!(
                restored.apply_authority_material(&material.material_bytes)?,
                expected_step
            );
        }
        ...
        expected
            .material_ledger
            .records
            .push(AppliedGameMaterialRecordV1 {
                operation_id: OperationId::new(format!("v7-retention/{revision}"))?,
                material_fingerprint: format!(
                    "blake3-v1:{}",
                    content_digest(&material.material_bytes)?
                ),
                authority_revision: safe(revision),
                after_digest: game_state_digest(&expected_state)?,
            });
        ...
        expected.pending_presentations = vec![PendingPresentationV3 {
            event_id,
            semantic,
            blocking: mapping.blocking,
            skip: mapping.skip,
        ...
        if revision == capacity {
            ...
            assert_eq!(
                resumed.as_ref().ok_or("restored kernel")?.snapshot()?,
                expected
            );
        ...
        if let Some(restored) = &mut resumed {
            restored.settle_presentation(event_id)?;
        }
...
fn v7_restore_rejects_historical_gapped_evidence_and_continues_a_valid_suffix() -> TestResult {
    ...
    assert_eq!(
        live.apply_authority_material(&material.material_bytes)?,
        resumed.apply_authority_material(&material.material_bytes)?
    );
