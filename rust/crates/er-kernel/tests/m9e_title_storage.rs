...
use er_kernel::game_kernel_v7::{GameKernelEffectV7, GameKernelRoleV7, GameKernelStepV7, GameKernelV7, KernelStorageResultV2};
...
use er_types::{BootstrapActionV1, GameActionV1, GameControlKindV2, GameMenuCancelV2, OperationId, PlatformRequestId, SaveActionV1, SafeU53, SeatId};
...
fn restore(snapshot: CoreGameKernelSnapshotV7, content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7, Box<dyn Error>> {
    Ok(GameKernelV7::from_snapshot(serde_json::from_slice(&serde_json::to_vec(&snapshot)?)?,
        SeatId::new(safe(1)), GameKernelRoleV7::Authority, content)?)
}
fn bootstrap(snapshot: &CoreGameKernelSnapshotV7) -> Result<&RunBootstrapMachineV1, Box<dyn Error>> {
    let GameKernelLifecycleSnapshotV7::Bootstrap(value) = &snapshot.lifecycle else {
        return Err("expected Bootstrap".into());
    };
    Ok(value)
}
fn bootstrap_mut(snapshot: &mut CoreGameKernelSnapshotV7) -> Result<&mut RunBootstrapMachineV1, Box<dyn Error>> {
...
fn list(kernel: &mut GameKernelV7) -> Result<PlatformRequestId, Box<dyn Error>> {
    ...
    let GameKernelEffectV7::Platform(GamePlatformEffectV2::StorageList { request }) = step.effects[1] else {
...
fn read(kernel: &mut GameKernelV7, slot: &str) -> Result<PlatformRequestId, Box<dyn Error>> {
    ...
    let GameKernelEffectV7::Platform(GamePlatformEffectV2::StorageRead { request, slot: actual }) = &step.effects[1] else {
...
fn saved_write(content: Arc<PreparedGameContentV2>) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    for pending in natural.snapshot()?.pending_presentations { natural.settle_presentation(pending.event_id)?; }
    ...
    let mut writer = controlled_read_save_menu(&natural.snapshot()?, content,
        SaveActionV1::Write { slot: "actual-slot".to_owned() }, 30, 10, 40)?;
    press(&mut writer, PhysicalKey::Space)?.effects.into_iter().find_map(|effect| match effect {
        GameKernelEffectV7::Platform(GamePlatformEffectV2::StorageWrite { bytes, generation, .. }) => {
            assert_eq!(generation, safe(1)); Some(bytes)
        }
        _ => None,
...
#[test]
fn title_list_read_normalizes_exact_saved_state_and_raw_write_generation_two() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    let bytes = saved_write(content.clone())?;
    let saved = GameSaveV2::decode(&bytes)?;
    for floor in [1, 90] {
        let mut initial = title(content.clone())?.snapshot()?;
        bootstrap_mut(&mut initial)?.current_storage.as_mut().ok_or("owner absent")?.next_platform_request_id = safe(floor);
        ...
        let outcome = KernelStorageResultV2::Slots { slots: vec!["actual-slot".to_owned()] };
        assert_eq!(reader.apply_storage_result(list_request, outcome.clone())?, clone.apply_storage_result(list_request, outcome)?);
        ...
        let outcome = KernelStorageResultV2::Read { bytes: Some(bytes.clone()) };
        assert_eq!(reader.apply_storage_result(request, outcome.clone())?, clone.apply_storage_result(request, outcome)?);
        let loaded = reader.snapshot()?;
        assert_eq!(loaded, clone.snapshot()?);
        let storage = bootstrap(&before)?.current_storage.as_ref().ok_or("owner absent")?;
        if floor == 1 { assert!(saved.state.identities.next_platform_request_id > storage.next_platform_request_id); }
        else { assert!(saved.state.identities.next_platform_request_id < storage.next_platform_request_id); }
        let mut state = saved.state.clone();
        state.identities.next_platform_request_id = state.identities.next_platform_request_id.max(storage.next_platform_request_id);
        let control = &mut state.active_run.as_mut().ok_or("saved run absent")?.control;
        let revision = safe(bootstrap(&before)?.control.revision.get().max(control.revision.get()) + 1);
        let instance = before.next_menu_instance_id.max(MenuInstanceId::new(safe(control.menu.as_ref().ok_or("saved menu absent")?.instance_id.get().get() + 1)));
        control.revision = revision;
        control.menu.as_mut().ok_or("saved menu absent")?.instance_id = instance;
        ...
        expected.storage_frontiers = vec![StorageFrontierSnapshotV1 { slot: "actual-slot".to_owned(), generation: safe(1) }];
        expected.replay_sequence = safe(before.replay_sequence.get() + 1);
        assert_eq!(loaded, expected, "all saved gameplay and unrelated owners stay exact");
        reader.raw_input(RawInputEvent::KeyUp { code: PhysicalKey::Space })?;
        clone.raw_input(RawInputEvent::KeyUp { code: PhysicalKey::Space })?;
        assert_eq!(reader.snapshot()?, loaded, "held Title submit cannot bleed into loaded control");
        let step = press(&mut reader, PhysicalKey::Space)?;
        assert_eq!(step, press(&mut clone, PhysicalKey::Space)?);
        assert_eq!(reader.snapshot()?, clone.snapshot()?);
        let (next, bytes) = step.effects.iter().find_map(|effect| match effect {
            GameKernelEffectV7::Platform(GamePlatformEffectV2::StorageWrite { request, slot, generation, bytes }) => {
                assert_eq!(slot, "actual-slot"); assert_eq!(*generation, safe(2)); Some((*request, bytes))
            }
            _ => None,
        ...
        let GameKernelLifecycleSnapshotV7::Active(loaded_state) = &loaded.lifecycle else { return Err("not active".into()); };
        expected_write.active_run.as_mut().ok_or("run absent")?.control = loaded_state.active_run.as_ref().ok_or("run absent")?.control.clone();
        ...
        assert_eq!(reader.apply_storage_result(next, KernelStorageResultV2::Written)?, clone.apply_storage_result(next, KernelStorageResultV2::Written)?);
        assert_eq!(reader.snapshot()?, clone.snapshot()?);
        assert_eq!(reader.snapshot()?.storage_frontiers[0].generation, safe(2));
        let settled = reader.snapshot()?;
        assert!(reader.apply_storage_result(request, KernelStorageResultV2::Read { bytes: Some(bytes.clone()) }).is_err());
...
#[test]
fn title_inventory_is_bounded_actual_and_missing_read_returns_selection() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    let mut reader = title(content.clone())?;
    let request = list(&mut reader)?;
    let pending = reader.snapshot()?;
    let invalid = [vec![String::new()], vec!["b".to_owned(), "a".to_owned()], vec!["a".to_owned(), "a".to_owned()],
        vec!["é".repeat(129)], (0..65).map(|index| format!("{index:04}")).collect()];
    for slots in invalid {
        assert!(reader.apply_storage_result(request, KernelStorageResultV2::Slots { slots }).is_err());
        assert_eq!(reader.snapshot()?, pending);
    }
    assert!(reader.apply_storage_result(request, KernelStorageResultV2::Read { bytes: None }).is_err());
    assert_eq!(reader.snapshot()?, pending);
    assert!(reader.settle_platform_request(request).is_err());
    assert_eq!(reader.snapshot()?, pending);
    reader.apply_storage_result(request, KernelStorageResultV2::Slots { slots: vec!["actual-slot".to_owned(), "é".repeat(128)] })?;
    let selected = reader.snapshot()?;
    assert_eq!(bootstrap(&selected)?.catalog.save_slots, vec!["preview-slot"]);
    assert!(!bootstrap(&selected)?.current_storage.as_ref().ok_or("owner absent")?.slots.contains(&"preview-slot".to_owned()));
    let request = read(&mut reader, "actual-slot")?;
    reader.apply_storage_result(request, KernelStorageResultV2::Read { bytes: None })?;
    let missing = reader.snapshot()?;
    assert_eq!(bootstrap(&missing)?.stage, RunBootstrapStageV1::ExistingSaveSelect);
    let owner = bootstrap(&missing)?.current_storage.as_ref().ok_or("owner absent")?;
    ...
    let menu = bootstrap(&empty)?.control.menu.as_ref().ok_or("empty inventory menu absent")?;
    assert_eq!(menu.options.len(), 1);
    assert_eq!(menu.selected_action(), Some(&GameActionV1::Bootstrap { action: BootstrapActionV1::Cancel }));
    press(&mut reader, PhysicalKey::Space)?;
    assert_eq!(bootstrap(&reader.snapshot()?)?.stage, RunBootstrapStageV1::Title);
    Ok(())
}

#[test]
fn title_cancel_retires_core_owner_without_reusing_ids_and_new_game_carries_floor() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    let mut reader = title(content.clone())?;
    ...
    for index in 0..20 {
        ...
        if index % 2 == 0 {
            reader.apply_storage_result(request, KernelStorageResultV2::Slots { slots: vec!["actual-slot".to_owned()] })?;
        ...
        for request in &retired {
            for result in [KernelStorageResultV2::Slots { slots: vec![] }, KernelStorageResultV2::Read { bytes: None }] {
    ...
    let before = reader.snapshot()?;
    let floor = bootstrap(&before)?.current_storage.as_ref().ok_or("owner absent")?.next_platform_request_id;
    ...
    assert!(!after.material_ledger.records.is_empty(), "real bootstrap material owns the carried floor");
...
fn title_default_bytes_strict_extension_and_overflow_are_atomic() -> Result<(), Box<dyn Error>> {
    ...
    assert!(old_bootstrap.apply_game_action(GameActionV1::Bootstrap { action: BootstrapActionV1::OpenExistingSaves }).is_err());
    ...
    bootstrap_mut(&mut forged)?.control.menu.as_mut().ok_or("menu absent")?.options[0].action = GameActionV1::Bootstrap { action: BootstrapActionV1::OpenExistingSaves };
    ...
    for field in ["platform", "replay"] {
        ...
        if field == "platform" {
            bootstrap_mut(&mut boundary)?.current_storage.as_mut().ok_or("owner absent")?.next_platform_request_id = safe(9_007_199_254_740_991);
        } else { boundary.replay_sequence = safe(9_007_199_254_740_991); }
    ...
    bootstrap_mut(&mut forged)?.current_storage.as_mut().ok_or("owner absent")?.pending.as_mut().ok_or("pending absent")?.kind = BootstrapStorageKindV1::Read { slot: "preview-slot".to_owned() };
    assert!(restore(forged, content.clone()).is_err());
    for field in ["stage", "owner", "platform", "ledger", "frontier", "unpaired"] {
        let mut forged = pending.clone();
        match field {
            "stage" => bootstrap_mut(&mut forged)?.stage = RunBootstrapStageV1::ExistingSaveSelect,
            "owner" => bootstrap_mut(&mut forged)?.current_storage.as_mut().ok_or("owner absent")?.owner_seat = SeatId::new(safe(2)),
            "platform" => forged.pending_platform[0].effect = GamePlatformEffectV2::StorageRead { request, slot: "actual-slot".to_owned() },
            "ledger" => forged.material_ledger = AppliedGameMaterialLedgerV1::new(safe(2))?,
            "frontier" => forged.storage_frontiers.push(StorageFrontierSnapshotV1 { slot: "foreign".to_owned(), generation: safe(1) }),
    ...
    assert!(reader.apply_storage_result(request, KernelStorageResultV2::Slots { slots: vec![] }).is_err());
...
#[test]
fn title_read_rejects_corrupt_nonlocal_private_and_inactive_saves_atomically() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    let bytes = saved_write(content.clone())?;
    let saved = GameSaveV2::decode(&bytes)?;
    let mut reader = title(content.clone())?;
    ...
    let request = read(&mut reader, "actual-slot")?;
    let before = reader.snapshot()?;
    let mut inactive = saved.state.clone(); inactive.active_run = None;
    ...
    control.action_context.as_mut().ok_or("context absent")?.authority_seat = SeatId::new(safe(2));
    let mut cooperative = saved.state.clone();
    cooperative.active_run.as_mut().ok_or("run absent")?.mode = bootstrap(&title(content.clone())?.snapshot()?)?.catalog.modes.iter().find(|mode| mode.cooperative).ok_or("cooperative policy absent")?.mode;
    let mut wrong_content = saved.state.clone();
    wrong_content.content_identity.oracle_sha = er_types::OracleSha::parse("0000000000000000000000000000000000000000")?;
    ...
    assert_eq!(natural.current_control().ok_or("control absent")?.kind, GameControlKindV2::BattleMove);
    ...
    for bytes in invalid {
        assert!(reader.apply_storage_result(request, KernelStorageResultV2::Read { bytes: Some(bytes) }).is_err());
        assert_eq!(reader.snapshot()?, before);
    }
    for result in [KernelStorageResultV2::Written, KernelStorageResultV2::Slots { slots: vec![] },
        KernelStorageResultV2::Failed { reason: "provider unavailable".to_owned() },
        KernelStorageResultV2::Uncertain { reason: "no terminal evidence".to_owned() }] {
        assert!(reader.apply_storage_result(request, result).is_err());
        assert_eq!(reader.snapshot()?, before);
    }
    ...
    let bytes = GameSaveV2::new(canonical.content_identity.clone(), safe(7), canonical)?.encode()?;
    ...
    assert_eq!(step, press(&mut clone, PhysicalKey::Space)?);
    assert_eq!(reader.snapshot()?, clone.snapshot()?);
    assert_eq!(reader.current_control().ok_or("control absent")?.kind, GameControlKindV2::BattleMove);
    Ok(())
}

#[test]
fn title_menu_revision_and_read_allocator_overflow_preserve_full_snapshot() -> Result<(), Box<dyn Error>> {
    let content = content()?;
    let mut reader = title(content.clone())?;
    navigate_down_to(&mut reader, "bootstrap/title/existing-saves")?;
    for field in ["menu", "revision"] {
        ...
        if field == "menu" {
            ...
            bootstrap.control.menu.as_mut().ok_or("menu absent")?.instance_id = instance;
            bootstrap.control.action_context.as_mut().ok_or("context absent")?.menu_instance = instance;
            ...
            bootstrap.control.menu.as_mut().ok_or("menu absent")?.control_id = id.to_owned();
            let context = bootstrap.control.action_context.as_mut().ok_or("context absent")?;
    ...
    let mut exhausted = restore(boundary.clone(), content.clone())?;
    assert!(exhausted.apply_storage_result(request, KernelStorageResultV2::Read { bytes: Some(valid_bytes.clone()) }).is_err());
    assert_eq!(exhausted.snapshot()?, boundary, "accepted READ normalization also rolls back on replay exhaustion");
    let saved = GameSaveV2::decode(&valid_bytes)?;
    for field in ["menu", "revision"] {
        let mut state = saved.state.clone();
        let control = &mut state.active_run.as_mut().ok_or("run absent")?.control;
        if field == "menu" {
            control.menu.as_mut().ok_or("menu absent")?.instance_id = MenuInstanceId::new(safe(9_007_199_254_740_991));
            control.action_context.as_mut().ok_or("context absent")?.menu_instance = MenuInstanceId::new(safe(9_007_199_254_740_991));
        } else {
            control.revision = safe(9_007_199_254_740_991);
            control.action_context.as_mut().ok_or("context absent")?.authority_revision = safe(9_007_199_254_740_991);
        }
