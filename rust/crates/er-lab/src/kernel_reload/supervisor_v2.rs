...
use er_kernel_worker::{
    KernelGenerationIdentityV2, KernelWorkerHealthV2, KernelWorkerInitializationV2,
};
use er_types::SeatId;
...
impl CurrentReloadTicketV2 {
    ...
    pub fn snapshot(&self) -> &CoreGameKernelSnapshotV7 {
        &self.snapshot
    }
}
...
impl CurrentKernelSupervisorV2 {
    ...
    pub fn oldest_retained_frontier(&self) -> u64 {
        self.oldest_frontier
    }
    ...
    pub fn process_id(&self) -> u32 {
        self.active.process_id()
    }
    pub fn is_fenced(&self) -> bool {
        self.active.is_fenced()
    }
    pub fn session_context(&self) -> Option<(SeatId, GameKernelRoleV7)> {
        self.active.session_context()
    }
    pub fn maximum_success_response_bytes(&self) -> usize {
        self.active.maximum_success_response_bytes()
    }
    ...
    pub fn dispatch(
        &mut self,
        event: CurrentExternalEvent,
    ) -> Result<CurrentDispatchV2, CurrentReloadErrorV2> {
        let position = self
            .frontier
            .checked_add(1)
        ...
        let encoded_bytes = serde_json::to_vec(&(position, &event, &evidence))
            .ok()
            .map(|bytes| bytes.len());
    ...
    pub fn prepare_reload(
        ...
        let mut candidate = ChildKernelGenerationV2::spawn_with_limits(
            artifact,
            self.active.deadlines(),
            self.active.maximum_success_response_bytes(),
        )?;
        let observation = candidate.initialize(
            content_bundle,
        ...
        for retained in self
            .tail
            .iter()
            .filter(|record| record.position > ticket.frontier)
        {
            expected_position = expected_position
                .checked_add(1)
                .ok_or(CurrentReloadErrorV2::PositionExhausted)?;
            if retained.position != expected_position {
                return Err(CurrentReloadErrorV2::TicketExpired {
                    ticket: ticket.frontier,
                    oldest: self.oldest_frontier,
                });
            }
        ...
        if candidate.snapshot()? != self.active.snapshot()?
            || candidate.observe()? != self.active.observe()?
        {
            return Err(CurrentReloadErrorV2::Divergence {
                position: self.frontier,
                kind: "final snapshot or observation",
            });
        }
    ...
    pub fn commit_reload(
        &mut self,
        prepared: PreparedCurrentReloadV2,
    ) -> Result<CurrentReloadAcceptedV2, CurrentReloadErrorV2> {
        if !Arc::ptr_eq(&prepared.owner, &self.owner)
            || prepared.source_identity != *self.identity()
            || prepared.frontier != self.frontier
            || self.active.is_fenced()
            || self.active.is_disposed()
        {
        ...
        Ok(CurrentReloadAcceptedV2 {
            ...
            active_identity,
            frontier: self.frontier,
            replayed_events: prepared.replayed_events,
            retirement_issue,
        })
    }

    pub fn snapshot(&mut self) -> Result<CoreGameKernelSnapshotV7, CurrentReloadErrorV2> {
        Ok(self.active.snapshot()?)
    }
    pub fn observe(&mut self) -> Result<CurrentGameObservation, CurrentReloadErrorV2> {
        Ok(self.active.observe()?)
    }
    pub fn health(&mut self) -> Result<KernelWorkerHealthV2, CurrentReloadErrorV2> {
        Ok(self.active.health()?)
    }
    pub fn dispose(&mut self) -> Result<(), CurrentReloadErrorV2> {
        Ok(self.active.dispose()?)
    }
    ...
    fn check_candidate(
        &self,
        candidate: &KernelGenerationIdentityV2,
    ) -> Result<(), CurrentReloadErrorV2> {
        let active = self.identity();
        if candidate.session_id != active.session_id
            || candidate.generation.0 <= active.generation.0
        {
            return Err(CurrentReloadErrorV2::Identity(
                "session must match and generation must increase",
            ));
        }
        if candidate.content_identity != active.content_identity
            ...
            || candidate.build_target != active.build_target
            || candidate.build_profile != active.build_profile
        {
            return Err(CurrentReloadErrorV2::Identity(
                "content, ABI, snapshot range or build context differs",
            ));
        }
        Ok(())
    }
