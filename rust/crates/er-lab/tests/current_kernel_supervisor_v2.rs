...
use er_lab::kernel_reload::{
    ...
    CurrentReloadErrorV2, CurrentTailLimitsV2, CurrentTraceRetentionV2,
    KernelEndpointErrorV2, KernelWorkerDeadlinesV2, VerifiedKernelExecutableV2,
};
...
fn safe(value: u64) -> SafeU53 { SafeU53::new(value).expect("safe test value") }
...
fn natural() -> Result<KernelWorkerInitializationV2, Box<dyn Error>> {
    Ok(KernelWorkerInitializationV2::Natural {
        profile: Box::new(profile()?), seed: SEED.to_owned(), local_seat: seat(),
...
fn fixture() -> Result<(GameContentBundleV2, VerifiedKernelExecutableV2), Box<dyn Error>> {
    ...
    let identity = KernelGenerationIdentityV2 {
        schema_version: 2, session_id: KernelSessionIdV1("current-supervisor".to_owned()),
        generation: KernelGenerationV1(1), artifact_sha256: digest.clone(), executable_sha256: digest,
...
fn next_artifact(artifact: &VerifiedKernelExecutableV2) -> Result<VerifiedKernelExecutableV2, Box<dyn Error>> {
    let mut identity = artifact.identity().clone();
    identity.generation = KernelGenerationV1(identity.generation.0 + 1);
    Ok(VerifiedKernelExecutableV2::verify(artifact.allowed_root(), artifact.executable(), identity)?)
}

fn supervisor(bundle: &GameContentBundleV2, artifact: &VerifiedKernelExecutableV2, limits: CurrentTailLimitsV2)
    -> Result<CurrentKernelSupervisorV2, Box<dyn Error>>
{
    ...
    assert_eq!(active.session_context(), Some((seat(), GameKernelRoleV7::Authority)));
...
fn reference(bundle: &GameContentBundleV2) -> Result<CurrentGameSession, Box<dyn Error>> {
    Ok(CurrentGameSession::natural_start(
        profile()?, SEED.to_owned(), seat(), vec!["preview-slot".to_owned()], true,
        Arc::new(PreparedGameContentV2::prepare(Arc::new(bundle.clone()))?), None,
    )?)
}
...
fn selected(reference: &CurrentGameSession) -> Result<String, Box<dyn Error>> {
    Ok(reference.observe()?.control.ok_or("missing control")?.menu
...
fn reach_active_battle(
    ...
    assert_eq!(reference.observe()?.control.ok_or("title control")?.kind, GameControlKindV2::Title);
    for _ in 0..3 { press_pair(supervisor, reference, PhysicalKey::Space)?; }
    let bound = reference.observe()?.control.ok_or("starter control")?.menu
        .ok_or("starter menu")?.options.len() + 1;
    for _ in 0..bound {
        if selected(reference)? == "bootstrap/starter/confirm" { break; }
        press_pair(supervisor, reference, PhysicalKey::ArrowDown)?;
    }
    assert_eq!(selected(reference)?, "bootstrap/starter/confirm");
    for _ in 0..4 { press_pair(supervisor, reference, PhysicalKey::Space)?; }
    for pending in reference.snapshot()?.pending_presentations {
        dispatch_pair(supervisor, reference, CurrentExternalEvent::PresentationOutcome {
...
#[test]
fn exact_current_reload_replays_full_tail_then_continues_in_new_process() -> Result<(), Box<dyn Error>> {
    ...
    for (index, event) in [key(PhysicalKey::ArrowDown, true), time(249), time(1), time(500)].into_iter().enumerate() {
    ...
    let invalid = supervisor.dispatch(CurrentExternalEvent::PresentationOutcome {
        event_id: PresentationEventId::new(safe(999)), outcome: KernelPresentationOutcomeV2::Settled,
    });
    assert!(matches!(invalid, Err(CurrentReloadErrorV2::Endpoint(KernelEndpointErrorV2::Fault(_)))));
    ...
    dispatch_pair(&mut supervisor, &mut reference, key(PhysicalKey::ArrowDown, false))?;
    assert!(supervisor.snapshot()?.scheduler.timers.is_empty());
    assert!(dispatch_pair(&mut supervisor, &mut reference, time(500))?.evidence.step.effects.is_empty());
    assert_eq!(supervisor.snapshot()?, reference.snapshot()?);
    supervisor.dispose()?;
    Ok(())
}
...
#[test]
fn prepared_reload_rejects_a_later_active_event_without_losing_progress() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut supervisor = supervisor(&bundle, &artifact, CurrentTailLimitsV2::default())?;
    let ticket = supervisor.begin_reload()?;
    ...
    assert!(matches!(supervisor.commit_reload(prepared), Err(CurrentReloadErrorV2::StalePrepared)));
    #[cfg(target_os = "linux")]
    assert!(!PathBuf::from(format!("/proc/{candidate_pid}")).exists(), "stale candidate was not reaped");
...
#[test]
fn effectful_success_budget_rejection_preserves_frontier_context_and_reload_limit() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut setup = supervisor(&bundle, &artifact, CurrentTailLimitsV2::default())?;
    let mut reference = reference(&bundle)?;
    reach_active_battle(&mut setup, &mut reference)?;
    dispatch_pair(&mut setup, &mut reference, key(PhysicalKey::ArrowDown, true))?;
    ...
    let encoded_len = |response| -> Result<usize, serde_json::Error> {
        Ok(serde_json::to_vec(&KernelWorkerResponseEnvelopeV2 {
            ...
            request_id: 100, accepted_sequence: Some(100),
            after_mechanical_digest: observation.mechanical_digest.clone(), response,
        })?.len())
    };
    ...
    let cap = [
        encoded_len(KernelWorkerResponseV2::Snapshot { snapshot: Box::new(held.clone()) })?,
        encoded_len(KernelWorkerResponseV2::Initialized { observation: Box::new(observation.clone()) })?,
        encoded_len(KernelWorkerResponseV2::Ready(Box::new(artifact.identity().clone())))?,
    ].into_iter().max().ok_or("missing response lengths")? + 16_384;
    ...
    for ticks in [8, 32, 128, 512, 1024] {
        ...
        let evidence = CurrentGenerationStepV2 {
            step: probe.apply(event.clone())?, observation: probe.observe()?,
        };
        let bytes = encoded_len(KernelWorkerResponseV2::Effects {
            step: evidence.step.clone(), observation: Box::new(evidence.observation.clone()),
        })?;
    ...
    let (large_event, expected_large) = oversized.ok_or("no bounded effectful response exceeded negotiated cap")?;
    // The identical event succeeds through the actual worker at its hard cap.
    assert_eq!(setup.dispatch(large_event.clone())?.evidence, expected_large);
    setup.dispose()?;

    let mut active = ChildKernelGenerationV2::spawn_with_limits(
        &artifact, KernelWorkerDeadlinesV2::default(), cap,
    )?;
    active.initialize(bundle.clone(), KernelWorkerInitializationV2::Snapshot {
        snapshot_bytes: serde_json::to_vec(&held)?, local_seat: seat(), role: GameKernelRoleV7::Authority,
    })?;
    ...
    assert_eq!(active.accepted_sequence(), accepted.map(|sequence| sequence + 1));
    ...
    assert_eq!(supervisor.session_context(), context);
    assert_eq!(dispatch_pair(&mut supervisor, &mut reference, time(250))?.position, 1);
    assert_eq!(supervisor.snapshot()?, reference.snapshot()?);
    supervisor.dispose()?;
    Ok(())
}
...
#[test]
fn tail_rotation_rejects_expired_ticket_and_replays_every_retained_event() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut supervisor = supervisor(&bundle, &artifact, CurrentTailLimitsV2 { maximum_events: 1, maximum_bytes: 16_777_216 })?;
    ...
    assert!(matches!(supervisor.prepare_reload(expired, &candidate, bundle.clone()),
        Err(CurrentReloadErrorV2::TicketExpired { ticket: 0, oldest: 1 })));
...
fn candidate_content_fault_and_generation_reuse_preserve_active() -> Result<(), Box<dyn Error>> {
    ...
    let expected = supervisor.snapshot()?;
    assert!(matches!(supervisor.prepare_reload(ticket.clone(), &artifact, bundle.clone()), Err(CurrentReloadErrorV2::Identity(_))));
    let mut invalid = serde_json::to_value(&bundle)?;
    invalid["schema_version"] = serde_json::json!(0);
    let candidate = next_artifact(&artifact)?;
    assert!(matches!(supervisor.prepare_reload(ticket.clone(), &candidate, serde_json::from_value(invalid)?),
        Err(CurrentReloadErrorV2::Endpoint(KernelEndpointErrorV2::Fault(fault)))
...
fn retention_gap_reports_accepted_event_and_expires_previous_ticket() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut supervisor = supervisor(&bundle, &artifact, CurrentTailLimitsV2 { maximum_events: 2, maximum_bytes: 1 })?;
    let ticket = supervisor.begin_reload()?;
    let before = supervisor.snapshot()?;
    ...
    let candidate = next_artifact(&artifact)?;
    assert!(matches!(supervisor.prepare_reload(ticket, &candidate, bundle.clone()),
        Err(CurrentReloadErrorV2::TicketExpired { ticket: 0, oldest: 1 })));
...
fn byte_budget_rotates_at_absolute_frontier() -> Result<(), Box<dyn Error>> {
    ...
    let mut reference = reference(&bundle)?;
    let event = time(1);
    let evidence = CurrentGenerationStepV2 { step: reference.apply(event.clone())?, observation: reference.observe()? };
    let one_record_bytes = serde_json::to_vec(&(1_u64, &event, &evidence))?.len();
    let mut supervisor = supervisor(&bundle, &artifact, CurrentTailLimitsV2 { maximum_events: 8, maximum_bytes: one_record_bytes })?;
    let ticket = supervisor.begin_reload()?;
    assert_eq!(supervisor.dispatch(event.clone())?.retention, CurrentTraceRetentionV2::Retained);
    assert_eq!(supervisor.retained_bytes(), one_record_bytes);
    assert_eq!(supervisor.dispatch(event)?.retention, CurrentTraceRetentionV2::Retained);
    assert_eq!(supervisor.retained_events(), 1);
    assert_eq!(supervisor.oldest_retained_frontier(), 1);
    assert!(supervisor.retained_bytes() <= one_record_bytes);
    assert!(matches!(supervisor.prepare_reload(ticket, &next_artifact(&artifact)?, bundle),
        Err(CurrentReloadErrorV2::TicketExpired { ticket: 0, oldest: 1 })));
    supervisor.dispose()?;
    Ok(())
}
...
#[test]
fn acknowledged_restore_context_survives_fault_and_is_preserved_by_reload() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut active = ChildKernelGenerationV2::spawn(&artifact)?;
    active.initialize(bundle.clone(), natural()?)?;
    ...
    active.restore(serde_json::to_vec(&replica_snapshot)?, seat(), GameKernelRoleV7::Replica)?;
...
#[cfg(target_os = "linux")]
#[test]
fn accepted_activation_reports_failed_retirement_without_rejecting_candidate() -> Result<(), Box<dyn Error>> {
    let (bundle, artifact) = fixture()?;
    let mut supervisor = supervisor(&bundle, &artifact, CurrentTailLimitsV2::default())?;
    let ticket = supervisor.begin_reload()?;
    ...
    let status = std::process::Command::new("/bin/kill")
        .arg("-KILL").arg(previous_pid.to_string())
