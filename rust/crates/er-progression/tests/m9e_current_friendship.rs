...
use er_progression::current_friendship::{
    CurrentFriendshipError, ExistingStarterAccount, FRIENDSHIP_ORACLE_SHA, FriendshipIntent,
    FriendshipSourceProvenance, PINNED_FRIENDSHIP_CAPS, ResolvedFriendshipMode,
    ResolvedFriendshipStarter, ResolvedPositiveFriendship, add_resolved_starter_candy,
    friendship_booster_step, plan_resolved_friendship, starter_friendship_cap,
};
...
fn negative_and_zero_friendship_short_circuit_without_resolution() -> Result<()> {
    ...
    assert_eq!(
        plan_resolved_friendship(3.0, 1.0, &accounts, None),
        Err(CurrentFriendshipError::Input)
    );
...
fn boosted_caps_and_max_friendship_preserve_full_candy_progress() -> Result<()> {
    ...
    assert_eq!(
        friendship_booster_step(0.5, 0)?.to_bits(),
        0.0_f64.to_bits()
    );
    ...
    for before in [254.0, 255.0] {
        ...
        assert_eq!(
            ...
            vec![
                ...
                FriendshipIntent::AwardFriendshipRibbonToSpeciesLine {
                    original_species: 1
                },
...
fn binary64_fusion_and_starter_cap_order_match_source_cases() -> Result<()> {
    ...
    for (cost, expected) in [
        ...
        assert_eq!(
            starter_friendship_cap(cost, &PINNED_FRIENDSHIP_CAPS)?,
            expected
        );
    }
    for (amount, classic, boosted, expected) in [
        (0.6, true, true, 1),
        (0.6, true, false, 0),
        (1.5, true, true, 3),
        (1.5, true, false, 2),
        ...
        resolved.mode = if classic {
            ResolvedFriendshipMode::Classic {
                candy_multiplier: 3.0,
            }
        } else {
            ResolvedFriendshipMode::NonClassic
        };
        resolved.event_boosts_fusions = boosted;
        resolved.fusion_starter = Some(ResolvedFriendshipStarter {
            source_root: 4,
            candy_account_root: 4,
            starter_cost: 3.0,
        });
        let plan = plan_resolved_friendship(
            50.0,
            amount,
            &[account(1, 0, 0), account(4, 0, 0)],
            Some(&resolved),
        )?;
        assert_eq!(
            plan.accounts,
            vec![account(1, expected, 0), account(4, expected, 0)]
        );
        assert!(plan.intents.is_empty());
    }
    ...
    event.mode = ResolvedFriendshipMode::Classic {
        candy_multiplier: 4.0,
    };
...
fn candy_saturation_zero_rate_egg_and_negative_requests_match_source() -> Result<()> {
    for (before, count, egg, show, debug, rate, expected, returned, bar) in [
        ...
        assert_eq!(
            (
                actual.candy_count,
                actual.returned_true,
                actual.candy_bar_count
            ),
            (expected, returned, bar)
        );
    }
    let mut resolved = resolution();
    ...
    assert_eq!(
        plan.intents,
        vec![FriendshipIntent::ShowStarterCandy {
            root: 1,
            scaled_count: 0,
            before_candy: 7,
            after_candy: 7
...
fn shared_fusion_progress_and_candy_roots_update_in_source_order() -> Result<()> {
    ...
    let plan = plan_resolved_friendship(
        50.0,
        1.0,
        &[account(1, 74, 9_998), account(4, 74, 27)],
        Some(&resolved),
    )?;
    assert_eq!(
        plan.accounts,
        vec![account(1, 0, 9_999), account(4, 74, 27)]
    );
    assert_eq!(
        plan.intents,
        vec![FriendshipIntent::ShowStarterCandy {
...
fn invalid_resolution_and_late_arithmetic_leave_inputs_unchanged() {
    ...
    assert_eq!(
        plan_resolved_friendship(50.0, 3.0, &duplicates, Some(&original)),
        Err(CurrentFriendshipError::Account)
    );
    let mut late = original.clone();
    late.fusion_starter = Some(ResolvedFriendshipStarter {
        source_root: 4,
        candy_account_root: 4,
        starter_cost: 3.0,
    });
    ...
    assert_eq!(
        plan_resolved_friendship(50.0, 3.0, &before, Some(&late)),
        Err(CurrentFriendshipError::Arithmetic)
    );
