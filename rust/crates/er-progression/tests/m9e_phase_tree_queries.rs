use er_progression::current_phase_tree::{CurrentPhaseTree, CurrentPhaseTreeSnapshot, CurrentPhaseTreeError, MAX_CURRENT_QUEUED_PHASES};
    ...
    for line in text.lines() {
        ...
        for (index, op) in fields[1].chars().enumerate() {
            ...
            match op {
                ...
                'n' => result = tree.next_phase().map_or_else(|| "-".to_owned(), |n| n.to_string()),
                'c' | 'k' => tree.clear(op == 'k'),
                'i' => tree.add_after_where(id, &mut predicate).expect("source insert after"),
                'f' => result = tree.find_where(&mut predicate).map_or_else(|| "-".to_owned(), |n| n.to_string()),
                'g' => { result = numbers(tree.find_all_where(&mut predicate).into_iter().copied()); if result.is_empty() { result = "_".to_owned(); } },
                'r' => result = if tree.remove_where(&mut predicate) { "T" } else { "F" }.to_owned(),
                'e' => result = if tree.exists_where(&mut predicate) { "T" } else { "F" }.to_owned(),
            ...
            let actual = format!("{result}:{}:{}:{}:{levels}:{}", visits.join(","), snapshot.current_level, u8::from(snapshot.deferred_active), numbers(tree.queued()));
            assert_eq!(actual, steps[index], "case {seed}, step {index}");
            let bytes = serde_json::to_vec(&snapshot).expect("snapshot serialization");
            let restored: CurrentPhaseTreeSnapshot<u32> = serde_json::from_slice(&bytes).expect("snapshot deserialization");
...
fn find_and_exists_preserve_opposite_source_search_orders() {
    ...
    assert_eq!(tree.find_where(|p| { calls.push(*p); true }), Some(&2));
    assert_eq!(calls, [2]);
    calls.clear();
    assert!(tree.exists_where(|p| { calls.push(*p); true }));
    assert_eq!(calls, [1]);
    calls.clear();
    assert_eq!(tree.find_all_where(|p| { calls.push(*p); true }), [&2, &1]);
    assert_eq!(calls, [2, 1]);
    calls.clear();
    tree.remove_all_where(|p| { calls.push(*p); false });
    assert_eq!(calls, [1, 2]);
}
...
fn insertion_and_removal_choose_first_deepest_match_and_source_fallback() {
    ...
    tree.add_after_where(4, |_| true).expect("first deepest match");
    assert_eq!(tree.queued(), [2, 4, 3, 1]);
    assert!(tree.remove_where(|p| *p == 4));
    assert!(!tree.remove_where(|p| *p == 99));
    tree.add_after_where(5, |_| false).expect("addPhase fallback");
...
fn query_admission_bounds_and_unknown_snapshot_fields_fail_closed() {
    let snapshot = CurrentPhaseTreeSnapshot { levels: vec![VecDeque::from(vec![1_u32; MAX_CURRENT_QUEUED_PHASES])], current_level: 0, deferred_active: false };
    let mut tree = CurrentPhaseTree::restore(snapshot).expect("bounded state");
    let before = tree.clone();
    assert_eq!(tree.add_after_where(2, |_| true), Err(CurrentPhaseTreeError::Capacity));
