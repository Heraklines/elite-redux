...
use er_types::{
    ...
    CommandFrontierControl, ControlAddress, InteractionControlAddress, NextControl, OperationId,
    SafeU53, SeatId,
};
...
pub fn validate_next_control(value: &Value) -> Result<NextControl, ControlValidationError> {
    ...
    if !issues.is_empty() {
        return Err(ControlValidationError { issues });
    }
...
pub fn partition_control_for_seat(control: &NextControl, seat: SeatId) -> Option<NextControl> {
    match control {
        NextControl::CommandFrontier(control) => {
            let commands = canonical_command_targets(&control.commands)
                .into_iter()
                .filter(|command| command.owner_seat_id == seat)
                .cloned()
                .collect::<Vec<_>>();
            Some(NextControl::CommandFrontier(CommandFrontierControl {
                epoch: control.epoch,
                wave: control.wave,
                turn: control.turn,
                commands,
        ...
        NextControl::Replacement(control) if control.owner_seat_id == seat => {
            Some(NextControl::Replacement(control.clone()))
        }
        NextControl::SharedInteraction(control) if control.owner_seat_id == seat => {
            Some(NextControl::SharedInteraction(control.clone()))
        }
        NextControl::AwaitSuccessor(_) | NextControl::Terminal(_) => Some(control.clone()),
        NextControl::Replacement(_) | NextControl::SharedInteraction(_) => None,
    }
}
...
fn shared_interaction_issues(control: &Map<String, Value>) -> Vec<String> {
    ...
    let Some(operation_kinds) =
        ...
        return operation_ids_issues(&mut issues, successor);
    };
...
fn operation_ids_issues(issues: &mut Vec<String>, successor: &Map<String, Value>) -> Vec<String> {
    let Some(operation_ids) = successor_field(successor, "operationIds") else {
        issues.push("successor.operationIds".to_owned());
        return std::mem::take(issues);
    };
...
fn js_number_string(value: &serde_json::Number) -> String {
    let Some(value_as_f64) = value.as_f64() else {
        return value.to_string();
    };
    if value_as_f64 == 0.0 {
        return "0".to_owned();
    }
    if value_as_f64.is_finite() && value_as_f64.fract() == 0.0 {
        return format!("{value_as_f64:.0}");
    }
    value.to_string()
}
