...
#[test]
fn schedule_batch_allocates_in_input_order_and_preserves_metadata() -> Result<(), SchedulerError> {
    let mut scheduler = KernelScheduler::new();
    let specs = vec![
        timer_spec(
            seat(2),
...
#[test]
fn schedule_batch_empty_does_not_consume_an_id() -> Result<(), SchedulerError> {
    let mut scheduler = KernelScheduler::new();
...
#[test]
fn schedule_batch_rejects_disposed_scheduler_without_mutation() -> Result<(), SchedulerError> {
    let mut scheduler = KernelScheduler::new();
...
fn scheduler_commands_keep_the_frozen_serde_shape() -> Result<(), String> {
    ...
    let encoded_spec = serde_json::to_value(&spec).map_err(|error| error.to_string())?;
    assert_eq!(encoded_spec, expected_spec);
    let decoded_spec =
        serde_json::from_value::<TimerSpec>(encoded_spec).map_err(|error| error.to_string())?;
