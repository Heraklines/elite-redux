...
impl Default for CurrentReproLimitsV1 {
    fn default() -> Self {
        Self { maximum_events: 256, maximum_bytes: 2 << 20 }
    }
}

impl CurrentReproLimitsV1 {
    pub fn validate(self) -> Result<(), CurrentReproErrorV1> {
        if self.maximum_events == 0 || self.maximum_events > MAXIMUM_CURRENT_REPRO_EVENTS_V1
            || self.maximum_bytes == 0 || self.maximum_bytes > MAXIMUM_CURRENT_REPRO_BYTES_V1
        {
...
pub enum CurrentReproRejectionKindV1 { Disposed, Kernel, Digest }
...
impl CurrentReproRejectionV1 {
    pub fn from_error(error: &CurrentSessionError) -> Self {
        ...
        Self { kind, message: bounded_text(&error.to_string(), MAXIMUM_ERROR_BYTES) }
    }
}
...
impl CurrentReproOutcomeV1 {
    fn evidence(&self) -> (&CurrentGameObservation, &str) {
        match self {
            Self::Applied { observation, snapshot_digest, .. }
            | Self::KernelRejected { observation, snapshot_digest, .. } => (observation, snapshot_digest),
        }
    }
}
...
impl CurrentReproCapsuleV1 {
    pub fn validate(&self, limits: CurrentReproLimitsV1) -> Result<(), CurrentReproErrorV1> {
        limits.validate()?;
        if self.schema_version != CURRENT_REPRO_SCHEMA_VERSION_V1 || self.checkpoint.schema_version != 7 {
            return Err(invalid("schema_version"));
        }
        if self.base_position > MAXIMUM_CURRENT_REPRO_POSITION_V1 || self.final_position > MAXIMUM_CURRENT_REPRO_POSITION_V1 {
        ...
        let mut browser_generation = self.browser_transport.map(|context| context.base_generation);
        if browser_generation == Some(SafeU53::ZERO) { return Err(invalid("browser generation")); }
        for attempt in &self.attempts {
            position = position.checked_add(1).ok_or_else(|| invalid("attempt position overflow"))?;
            if attempt.position != position { return Err(divergence(position, "position")); }
            if attempt.origin.as_ref().is_some_and(|origin| origin.len() > MAXIMUM_ORIGIN_BYTES) {
            ...
            match (browser_generation, attempt.browser_transport) {
                (None, None) => {}
                (Some(generation), Some(transition)) if generation == transition.before_generation
                    && valid_browser_transition(&attempt.event, matches!(&attempt.outcome, CurrentReproOutcomeV1::Applied { .. }), transition) => {
                    browser_generation = Some(transition.after_generation);
                }
                _ => return Err(divergence(position, "browser transport")),
            }
        }
        if position != self.final_position { return Err(divergence(position, "final_position")); }
        if browser_generation != self.browser_transport.map(|context| context.final_generation) {
...
pub enum CurrentCaptureStatusV1 {
    Available { base_position: u64, final_position: u64 },
...
impl CurrentReproRecorderV1 {
    ...
    pub fn from_capsule(
        ...
        let recorder = Self {
            content, local_seat: capsule.local_seat, role: capsule.role, limits,
            position: capsule.final_position, capsule: Some(capsule), unavailable_reason: String::new(),
    ...
    pub fn new(
        ...
        let mut recorder = Self {
            content, local_seat, role, limits, position: 0,
            browser_context_required: false,
            capsule: None, unavailable_reason: "initial checkpoint exceeds capture bound".to_owned(),
        };
        if fits(&checkpoint, limits.maximum_bytes) {
            ...
            if fits(&capsule, limits.maximum_bytes) { recorder.capsule = Some(capsule); }
        }
        Ok(recorder)
    }
    ...
    pub fn new_with_browser_transport(
        ...
        if generation == SafeU53::ZERO { return Err(invalid("browser generation")); }
        ...
        if let Some(capsule) = &mut recorder.capsule {
            capsule.browser_transport = Some(CurrentReproBrowserTransportV1 { base_generation: generation, final_generation: generation });
            if !fits(capsule, limits.maximum_bytes) { let _ = recorder.gap("initial checkpoint exceeds capture bound"); }
        }
        Ok(recorder)
    }
    ...
    pub fn record_with_browser_transport(
        ...
        self.record_attempt(before, event, outcome, after, observation, origin,
            Some(CurrentReproBrowserTransitionV1 { before_generation, after_generation }))
    }
    ...
    fn record_attempt(
        ...
        let Some(position) = self.position.checked_add(1).filter(|value| *value <= MAXIMUM_CURRENT_REPRO_POSITION_V1) else { return self.gap("attempt position exhausted"); };
        self.position = position;
        match self.record_checked(before, event, outcome, after, observation, origin, browser_transport) {
            Ok(capsule) => { self.capsule = Some(capsule); self.status() }
    ...
    pub fn status(&self) -> CurrentCaptureStatusV1 {
        match &self.capsule {
            ...
            None => CurrentCaptureStatusV1::Unavailable {
                position: self.position, reason: self.unavailable_reason.clone(),
            },
        }
    }

    pub fn export(&self) -> Result<CurrentReproCapsuleV1, CurrentReproErrorV1> {
        self.capsule.clone().ok_or_else(|| CurrentReproErrorV1::Unavailable {
            position: self.position, reason: self.unavailable_reason.clone(),
        })
    }
    ...
    fn record_checked(
        ...
        if self.browser_context_required != browser_transport.is_some() { return Err(invalid("browser transport context missing or unexpected")); }
        if let Some(transition) = browser_transport {
            if !valid_browser_transition(&event, outcome.is_ok(), transition) { return Err(invalid("browser transport admission or reduction")); }
            if self.capsule.as_ref().and_then(|capsule| capsule.browser_transport)
        ...
        if origin.is_some_and(|value| value.len() > MAXIMUM_ORIGIN_BYTES) { return Err(invalid("origin bound")); }
        if !fits(before, self.limits.maximum_bytes) || !fits(after, self.limits.maximum_bytes)
            || !fits(&event, self.limits.maximum_bytes)
            || outcome.as_ref().is_ok_and(|step| !fits(step, self.limits.maximum_bytes))
        { return Err(invalid("single attempt capture bound")); }
        let before_observation = self.verified_observation(before)?;
        if self.verified_observation(after)? != *observation { return Err(invalid("after observation")); }
        let before_digest = snapshot_digest(before)?;
        if self.capsule.as_ref().is_some_and(|capsule| capsule.final_snapshot_digest != before_digest) {
        ...
        let outcome = match outcome {
            Ok(step) => CurrentReproOutcomeV1::Applied {
                step: Box::new(step.clone()), observation: Box::new(observation.clone()), snapshot_digest: after_digest.clone(),
            },
            Err(error) => {
                if before != after || before_observation != *observation { return Err(invalid("rejection changed state")); }
                CurrentReproOutcomeV1::KernelRejected {
                    error: CurrentReproRejectionV1::from_error(error),
                    observation: Box::new(observation.clone()), snapshot_digest: after_digest.clone(),
                }
            }
        };
        let attempt = CurrentReproAttemptV1 { position: self.position, origin: origin.map(str::to_owned), browser_transport, event, outcome };
        let base_generation = browser_transport.map(|transition| transition.before_generation);
        let mut capsule = self.capsule.take().unwrap_or_else(|| self.empty_capsule(before.clone(), self.position - 1, before_digest.clone(), base_generation));
        capsule.attempts.push(attempt);
        capsule.final_position = self.position;
        capsule.final_snapshot_digest = after_digest.clone();
        if let (Some(context), Some(transition)) = (&mut capsule.browser_transport, browser_transport) { context.final_generation = transition.after_generation; }
        if capsule.attempts.len() > self.limits.maximum_events || !fits(&capsule, self.limits.maximum_bytes) {
            let attempt = capsule.attempts.pop().ok_or_else(|| invalid("missing current attempt"))?;
            capsule = self.empty_capsule(before.clone(), self.position - 1, before_digest, base_generation);
            capsule.attempts.push(attempt);
            capsule.final_position = self.position;
            capsule.final_snapshot_digest = after_digest;
            if let (Some(context), Some(transition)) = (&mut capsule.browser_transport, browser_transport) { context.final_generation = transition.after_generation; }
        }
        if !fits(&capsule, self.limits.maximum_bytes) { return Err(invalid("single attempt capture bound")); }
        Ok(capsule)
    }

    fn verified_observation(&self, snapshot: &CoreGameKernelSnapshotV7) -> Result<CurrentGameObservation, CurrentReproErrorV1> {
        restored(snapshot, self.local_seat, self.role, Arc::clone(&self.content))?
            .observe().map_err(|_| invalid("snapshot observation"))
    }

    fn empty_capsule(&self, checkpoint: CoreGameKernelSnapshotV7, position: u64, digest: String, generation: Option<SafeU53>) -> CurrentReproCapsuleV1 {
        CurrentReproCapsuleV1 {
            schema_version: CURRENT_REPRO_SCHEMA_VERSION_V1,
            content_identity: self.content.identity().clone(), local_seat: self.local_seat, role: self.role,
            browser_transport: generation.map(|generation| CurrentReproBrowserTransportV1 { base_generation: generation, final_generation: generation }),
            base_position: position, checkpoint: Box::new(checkpoint), attempts: Vec::new(),
            final_position: position, final_snapshot_digest: digest,
        }
    }
...
pub fn replay_current_capsule_v1(
    ...
    if capsule.content_identity != *content.identity() { return Err(invalid("content_identity")); }
    let mut session = restored(&capsule.checkpoint, capsule.local_seat, capsule.role, content)?;
    for attempt in &capsule.attempts {
        let before = session.snapshot().map_err(|_| divergence(attempt.position, "before snapshot"))?;
        let result = session.apply(attempt.event.clone());
        match (&attempt.outcome, result) {
            (CurrentReproOutcomeV1::Applied { step, .. }, Ok(actual)) => {
                if actual != **step { return Err(divergence(attempt.position, "step")); }
            }
            (CurrentReproOutcomeV1::KernelRejected { error, .. }, Err(actual)) => {
                if CurrentReproRejectionV1::from_error(&actual) != *error { return Err(divergence(attempt.position, "rejection")); }
                if session.snapshot().map_err(|_| divergence(attempt.position, "rejected snapshot"))? != before {
        ...
        if session.observe().map_err(|_| divergence(attempt.position, "observation"))? != *expected_observation {
            return Err(divergence(attempt.position, "observation"));
        }
        let snapshot = session.snapshot().map_err(|_| divergence(attempt.position, "snapshot"))?;
        if snapshot_digest(&snapshot)? != expected_digest { return Err(divergence(attempt.position, "snapshot_digest")); }
    }
    let final_snapshot = session.snapshot().map_err(|_| divergence(capsule.final_position, "final snapshot"))?;
...
fn restored(snapshot: &CoreGameKernelSnapshotV7, seat: SeatId, role: GameKernelRoleV7, content: Arc<PreparedGameContentV2>) -> Result<CurrentGameSession, CurrentReproErrorV1> {
    CurrentGameSession::from_snapshot(snapshot.clone(), seat, role, content).map_err(|_| invalid("checkpoint snapshot/context"))
}

fn snapshot_digest(snapshot: &CoreGameKernelSnapshotV7) -> Result<String, CurrentReproErrorV1> {
    content_digest(snapshot).map(|digest| format!("blake3-v1:{digest}")).map_err(|_| invalid("snapshot digest"))
}

fn invalid(field: &str) -> CurrentReproErrorV1 { CurrentReproErrorV1::Invalid { field: field.to_owned() } }
fn divergence(position: u64, field: &'static str) -> CurrentReproErrorV1 { CurrentReproErrorV1::Divergence { position, field } }
...
fn valid_browser_transition(event: &CurrentExternalEvent, applied: bool, transition: CurrentReproBrowserTransitionV1) -> bool {
    let before = transition.before_generation;
    let after = transition.after_generation;
    if before == SafeU53::ZERO || after == SafeU53::ZERO { return false; }
    match event {
        ...
        CurrentExternalEvent::TransportChanged { generation, .. } => {
            *generation >= ConnectionGeneration::new(before)
                && if applied { *generation == ConnectionGeneration::new(after) } else { after == before }
        }
        _ => after == before,
    }
}
...
#[derive(Debug)]
struct BoundedCounter { remaining: usize }
impl Write for BoundedCounter {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        if bytes.len() > self.remaining { return Err(io::Error::other("capture byte bound")); }
