...
use er_repro::current::{
    CurrentCaptureStatusV1, CurrentReproLimitsV1, CurrentReproOutcomeV1,
    CurrentReproRecorderV1, MAXIMUM_CURRENT_REPRO_BYTES_V1,
    MAXIMUM_CURRENT_REPRO_EVENTS_V1, replay_current_capsule_v1,
};
...
fn checkpoint(
    ...
    assert_eq!(session.observe()?.control.ok_or("control")?.kind, expected_kind);
    Ok(Checkpoint {
        name,
...
fn checkpoints(content: Arc<PreparedGameContentV2>) -> TestResult<Vec<Checkpoint>> {
    ...
    let mut result = vec![checkpoint(
        "title", &session, GameControlKindV2::Title, PhysicalKey::Space,
    )?];
    press(&mut session, PhysicalKey::Space)?;
    result.push(checkpoint(
        "mode", &session, GameControlKindV2::ModeSelect, PhysicalKey::Space,
    )?);
    press(&mut session, PhysicalKey::Space)?;
    result.push(checkpoint(
        "starter", &session, GameControlKindV2::StarterSelect, PhysicalKey::Space,
    )?);
    press(&mut session, PhysicalKey::Space)?;
    ...
    result.push(checkpoint(
        "active", &session, GameControlKindV2::BattleCommand, PhysicalKey::ArrowDown,
    )?);
    Ok(result)
}
...
fn probe_checkpoint(checkpoint: &Checkpoint) -> TestResult<Value> {
    ...
    assert!(!expected_step.effects.is_empty(), "{} event must be effectful", checkpoint.name);
    assert_ne!(snapshot, expected_snapshot, "{} event must change state", checkpoint.name);
    ...
    let phases = vec![measure("fork", || Ok(()), |_| Ok(black_box(session).fork()?), |_, result| {
        assert!(Arc::ptr_eq(result.content(), session.content()));
        assert_eq!(result.snapshot()?, snapshot);
        assert_eq!(result.observe()?, observation);
        Ok(())
    })?,
    measure("snapshot", || Ok(session.fork()?), |input| Ok(input.snapshot()?), |_, result| {
        assert_eq!(*result, snapshot);
        Ok(())
    })?,
    measure("validate", || Ok(session.fork()?), |input| {
        input.validate()?;
        Ok(())
    }, |input, _| {
    ...
    measure("observe", || Ok(session.fork()?), |input| Ok(input.observe()?), |_, result| {
        assert_eq!(*result, observation);
        Ok(())
    })?,
    measure("canonical_encode_snapshot", || Ok(()), |_| Ok(canonical_bytes(black_box(&snapshot))?), |_, result| {
        assert_eq!(*result, encoded);
        Ok(())
    })?,
    measure("canonical_digest_snapshot", || Ok(()), |_| Ok(content_digest(black_box(&snapshot))?), |_, result| {
        assert_eq!(*result, digest);
        Ok(())
    })?,
    measure("blake3_preencoded_snapshot", || Ok(()), |_| Ok(blake3::hash(black_box(&encoded))), |_, result| {
        assert_eq!(result.to_hex().to_string(), digest);
        Ok(())
    })?,
    measure("apply_effectful_raw_input", || Ok((session.fork()?, Some(checkpoint.event.clone()))), |input| {
    ...
    measure("recorder_append", || {
        let (seat, role) = session.session_context()?;
        let recorder = CurrentReproRecorderV1::new(
            snapshot.clone(), seat, role, Arc::clone(session.content()), capture_limits,
        )?;
        assert!(matches!(recorder.status(), CurrentCaptureStatusV1::Available { .. }));
        Ok((recorder, Some(checkpoint.event.clone())))
    }, |input| {
        let event = input.1.take().ok_or("event consumed twice")?;
        Ok(input.0.record(
            black_box(&snapshot), event, Ok(black_box(&expected_step)),
            black_box(&expected_snapshot), black_box(&expected_observation),
        ))
    }, |input, status| {
        assert_eq!(*status, CurrentCaptureStatusV1::Available {
            base_position: 0, final_position: 1,
        });
        ...
        match &capsule.attempts[0].outcome {
            CurrentReproOutcomeV1::Applied { step, observation, .. } => {
        ...
        let replayed = replay_current_capsule_v1(
            &capsule, Arc::clone(session.content()), capture_limits,
        )?;
...
fn current_native_phase_costs_preserve_semantics() -> TestResult {
    ...
    let decode = measure("content_decode", || Ok(()), |_| Ok(serde_json::from_slice::<GameContentBundleV2>(black_box(BUNDLE))?), |_, result| {
        assert_eq!(result, bundle.as_ref());
        Ok(())
    })?;
    let prepare = measure("content_prepare_and_arc", || Ok(Arc::clone(&bundle)), |input| Ok(Arc::new(PreparedGameContentV2::prepare(Arc::clone(input))?)), |_, result| {
        ...
        for checkpoint in &checkpoints {
            ...
            let mut restored = CurrentGameSession::from_snapshot(
                checkpoint.session.snapshot()?, seat, role, Arc::clone(result),
            )?;
            let mut reference = checkpoint.session.fork()?;
            assert_eq!(restored.apply(checkpoint.event.clone())?, reference.apply(checkpoint.event.clone())?);
    ...
    let output = format!("M9E_CURRENT_COST_PROBE {}\n", serde_json::to_string(&evidence)?);
