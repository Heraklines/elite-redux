...
use crate::capability::{
    ...
    RewardCapabilityAction, RouteRngDomain, RunCapabilityManifest, RunCapabilityStatus,
    RUN_CAPABILITY_MANIFEST_VERSION, RUN_ORACLE_GAME_SHA,
};
use crate::rng_audit::RunRngDraw;
use er_canonical::{canonical_bytes, CanonicalError};
...
impl RunContentPack {
    ...
    pub fn validate(&self) -> Result<(), RunContentError> {
        ...
        if actual != self.run_content_hash {
            return Err(RunContentError::HashMismatch { expected: self.run_content_hash.clone(), actual });
        }
        Ok(())
    }
    ...
    pub fn modifier_by_id(&self, id: ModifierId) -> Option<&ModifierDefinition> {
        self.modifiers.iter().flatten().find(|definition| definition.id == id)
    }
    pub fn modifier_by_registry_key(&self, key: &str) -> Option<&ModifierDefinition> {
        self.modifiers.iter().flatten().find(|definition| definition.oracle_registry_key == key)
    }
    pub fn classify_modifier(&self, id: ModifierId, key: &str) -> RunCapabilityStatus {
        match self.modifier_by_id(id) {
            Some(definition) if definition.oracle_registry_key == key => self.capability_manifest.classify_modifier(id, key),
            _ => RunCapabilityStatus::Unsupported(crate::capability::UnsupportedReasonCode::ModifierIdentityMismatch),
        }
    }
}
...
fn validate_growth_rates(values: &[Option<GrowthRateDefinition>]) -> Result<(), RunContentError> {
    ...
    for (index, definition) in values.iter().enumerate() {
        ...
        if usize::from(definition.id.get()) != index {
            return Err(invalid("growth rate", "numeric vector hole does not match ID"));
        }
        if definition.key.is_empty() { return Err(invalid("growth rate", "empty key")); }
        if !seen.insert(definition.id.to_string()) { return Err(duplicate("growth rate ID", definition.id.to_string())); }
    }
...
fn validate_natures(values: &[Option<NatureDefinition>]) -> Result<(), RunContentError> {
    ...
    for (index, definition) in values.iter().enumerate() {
        ...
        if definition.key.is_empty() { return Err(invalid("nature", "empty key")); }
        if definition.raised_stat == definition.lowered_stat && definition.raised_stat.is_some() { return Err(invalid("nature", "raised and lowered stats are equal")); }
        if !seen.insert(definition.id.to_string()) { return Err(duplicate("nature ID", definition.id.to_string())); }
    }
    Ok(())
}
fn validate_species(values: &[Option<SpeciesProgressionDefinition>], capabilities: &RunCapabilityManifest) -> Result<(), RunContentError> {
    let mut seen = BTreeSet::new();
    for (index, definition) in values.iter().enumerate() {
        let Some(definition) = definition else { continue };
...
fn validate_modifiers(values: &[Option<ModifierDefinition>], capabilities: &RunCapabilityManifest) -> Result<(), RunContentError> {
    ...
    {
        return Err(invalid("modifier", "numeric ID and registry-key lists are not bijective"));
    }
    ...
    for (index, definition) in values.iter().enumerate() {
        ...
        if !ids.insert(definition.id.to_string()) { return Err(duplicate("modifier ID", definition.id.to_string())); }
        if definition.oracle_registry_key.is_empty() || !keys.insert(definition.oracle_registry_key.clone()) { return Err(duplicate("modifier registry key", definition.oracle_registry_key.clone())); }
        if definition.maximum_stack == 0 { return Err(invalid("modifier", "zero maximum stack")); }
        if !capabilities.modifier_registry_keys.iter().any(|key| key == &definition.oracle_registry_key) { return Err(unsupported("modifier registry key", definition.oracle_registry_key.clone())); }
        if !capabilities.supported_modifier_ids.contains(&definition.id) { return Err(unsupported("modifier ID", definition.id.to_string())); }
...
fn validate_effect(effect: &ModifierEffectSpec) -> Result<(), RunContentError> {
    match effect {
        ModifierEffectSpec::MoneyMultiplier { percent } | ModifierEffectSpec::ExperienceMultiplier { percent } | ModifierEffectSpec::HealingMultiplier { percent } if *percent == 0 || *percent > 10_000 => Err(invalid("modifier effect", "percentage outside closed range")),
        ModifierEffectSpec::HpRestore { points, percent } if *points == 0 && *percent == 0 => Err(invalid("modifier effect", "empty healing effect")),
        ModifierEffectSpec::MoneyReward { multiplier_milli } if *multiplier_milli == 0 => Err(invalid("modifier effect", "zero money multiplier")),
        ModifierEffectSpec::InventoryItem { key } if key.is_empty() => Err(invalid("modifier effect", "empty inventory key")),
        _ => Ok(()),
    }
}
fn validate_biomes(values: &[Option<BiomeDefinition>], capabilities: &RunCapabilityManifest) -> Result<(), RunContentError> {
    let mut seen = BTreeSet::new();
    for (index, definition) in values.iter().enumerate() {
        let Some(definition) = definition else { continue };
        if u64::from(definition.id) != index as u64 {
        ...
        if definition.key.is_empty() { return Err(invalid("biome", "empty key")); }
        if !seen.insert(definition.id.to_string()) { return Err(duplicate("biome ID", definition.id.to_string())); }
        if !capabilities.biome_ids.contains(&definition.id) { return Err(unsupported("biome ID", definition.id.to_string())); }
        let mut routes = BTreeSet::new();
        for route in &definition.base_routes { if !routes.insert(route.to_string()) { return Err(duplicate("biome route", route.to_string())); } }
    }
    Ok(())
}
fn validate_encounters(values: &[EncounterPlanDefinition], biomes: &[Option<BiomeDefinition>]) -> Result<(), RunContentError> {
    let mut ids = BTreeSet::new();
    for definition in values {
        if definition.captured_vector_key.is_empty() { return Err(invalid("encounter", "empty captured vector key")); }
        if !ids.insert(definition.id.to_string()) { return Err(duplicate("encounter ID", definition.id.to_string())); }
        if !biomes.iter().flatten().any(|biome| biome.id == definition.biome_id) { return Err(unsupported("encounter biome", definition.biome_id.to_string())); }
    }
    Ok(())
}
fn validate_rules(reward: &RewardRuleSet, market: &MarketRuleSet, modifiers: &[Option<ModifierDefinition>]) -> Result<(), RunContentError> {
    let definitions = modifiers.iter().flatten().collect::<Vec<_>>();
    if reward.selected_modifier_keys.iter().any(|key| !definitions.iter().any(|definition| &definition.oracle_registry_key == key)) {
        return Err(invalid("reward rules", "selected modifier key is not in the bijection"));
    }
    if market.selected_modifier_keys.iter().any(|key| !definitions.iter().any(|definition| &definition.oracle_registry_key == key)) {
        return Err(invalid("market rules", "selected modifier key is not in the bijection"));
    }
    if !reward.supports_reroll || !reward.supports_locks || reward.lock_cost_tiers.is_empty() {
        return Err(invalid("reward rules", "selected regular reward surface lost reroll/lock support"));
    }
    if market.supports_reroll || market.supports_locks || market.maximum_stock_entries == 0 || market.maximum_stock_entries > 16 {
        return Err(invalid("market rules", "biome market reroll/lock or stock bound is invalid"));
    }
    Ok(())
}
fn duplicate(kind: &'static str, value: String) -> RunContentError { RunContentError::Duplicate { kind, value } }
fn invalid(kind: &'static str, detail: &'static str) -> RunContentError { RunContentError::InvalidDefinition { kind, detail: detail.to_owned() } }
fn unsupported(kind: &'static str, value: String) -> RunContentError { RunContentError::Unsupported { kind, value } }
