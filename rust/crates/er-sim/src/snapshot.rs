...
/// Shared native/wasm32 continuation evidence for the complete M3 pair
/// snapshot. A native builder records uninterrupted traces from the required
/// live boundaries; both targets then reconstruct fresh pairs from those same
/// canonical bytes and replay every later public operation.
pub mod m3_continuation {
    ...
    pub const M3_CONTINUATION_SUITE_ID: &str =
    ...
    impl fmt::Display for M3ContinuationError {
        fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                Self::Json(reason) => {
                    write!(formatter, "invalid M3 continuation JSON: {reason}")
                }
                ...
                Self::Canonical(reason) => write!(
                    formatter,
                    "could not canonicalize M3 continuation evidence: {reason}"
                ),
            }
        }
    }
    ...
    fn safe_len(length: usize, field: &str) -> Result<SafeU53, M3ContinuationError> {
        ...
        SafeU53::new(value).map_err(|error| {
            M3ContinuationError::InvalidSuite(format!(
                "{field} length is not JS-safe: {error}"
            ))
        })
    }
    ...
    pub fn parse_suite_json(
        input: &str,
    ) -> Result<M3ContinuationSuiteV1, M3ContinuationError> {
        let suite: M3ContinuationSuiteV1 = serde_json::from_str(input)
            .map_err(|error| M3ContinuationError::Json(error.to_string()))?;
        suite.validate()?;
        Ok(suite)
    }
        ...
        for scenario in &suite.scenarios {
            let replay = scenario
                ...
                .map_err(|error: SnapshotError| {
                    snapshot_error(&scenario.boundary_id, error)
                })?;
            ...
            scenarios.push(M3ContinuationScenarioReportV1 {
                ...
                .map_err(|error| snapshot_error(&scenario.boundary_id, error))?,
                operation_count: safe_len(
                    scenario.trace.entries.len(),
                    "scenario operations",
                )?,
