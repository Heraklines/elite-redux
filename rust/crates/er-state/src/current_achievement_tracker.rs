...
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CurrentAchievementSpreadMoveV1 {
    pub move_id: MoveId,
    pub user_id: PokemonId,
    pub turn: TurnIndex,
}
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CurrentAchievementKillerV1 {
    pub user_id: PokemonId,
    pub field_index: u8,
}
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
...
pub struct CurrentAchievementKoStintV1 {
    pub kos: u32,
    pub had_plus_six: bool,
    pub entered_low_hp: bool,
    pub healed: bool,
}
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CurrentAchievementNoSellV1 {
    pub attacker_id: PokemonId,
    pub survivor_id: PokemonId,
}
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CurrentAchievementBossDamageV1 {
    pub first_damage_turn: TurnIndex,
    pub full_hp_at_first_damage: bool,
    pub ohko_used: bool,
}
...
impl CurrentAchievementTrackerV1 {
    ...
    pub fn fresh(run_id: GameRunId) -> Self {
        Self {
            ...
            persistent: CurrentAchievementRunV1 {
                bargain_accepted: false,
                bargain_refused_pending_boss: false,
                black_market_credited: false,
                learned_move_stamps: BTreeMap::new(),
                parallel_play_ko_ids: BTreeSet::new(),
                last_credited_revive_wave: -1,
            },
        }
    }
    ...
    pub fn valid(&self, run: &crate::m7_state::RunStateV3) -> bool {
        if self.run_id != run.run_id
            ...
            || i128::from(self.persistent.last_credited_revive_wave)
                > i128::from(run.wave.get().get())
            || self
                .persistent
                .learned_move_stamps
                .iter()
                .any(|(move_id, wave)| move_id.get() == er_types::SafeU53::ZERO || *wave > run.wave)
            || self.absol_warning_failed.is_some() != self.absol_warning_wave.is_some()
        {
            return false;
        }
        let Some(owner) = &self.battle else {
            return true;
        };
        let Some(battle) = &run.battle else {
            return false;
        };
        owner.wave_index == run.wave
            && owner.damage_source_turn <= battle.turn
            ...
            && owner
                .faint_ledger_turn
                .is_none_or(|turn| turn <= battle.turn)
            && owner
                .longest_turn_number
                .is_none_or(|turn| turn <= battle.turn)
    }
    ...
    pub fn stamp_learned_move(
        &mut self,
        run: &crate::m7_state::RunStateV3,
        pokemon: PokemonId,
        move_id: MoveId,
    ) -> Result<(), crate::m9e_state_v6::GameStateV6Error> {
        if !self.valid(run)
            || !run.party.iter().any(|member| {
                member.id == pokemon
                    && member
                        .moves
                        .iter()
...
impl CurrentAchievementBattleV1 {
    ...
    pub fn fresh(wave_index: WaveIndex, turn: TurnIndex) -> Self {
        Self {
            wave_index,
            beam_spam_used: None,
            beam_spam_invalid: None,
            weave_streak: 0,
            switched_in_player_ids: BTreeSet::new(),
            turn_one_charged_moves: BTreeSet::new(),
            damage_source_turn: turn,
            damage_sources_by_target: BTreeMap::new(),
            last_spread_move: None,
            faint_ledger_turn: None,
            player_field_faints: BTreeSet::new(),
            enemy_field_faints: BTreeSet::new(),
            flash_turn: None,
            player_acted_this_turn: None,
            player_ever_acted: false,
            flash_failed: false,
            player_fainted_this_battle: false,
            enemy_ko_turns: BTreeMap::new(),
            enemy_ko_killers: BTreeMap::new(),
            player_dealt_direct_damage: false,
            relic_saved_mon_ids: BTreeSet::new(),
            last_enemy_killer_id: None,
            ko_stints: BTreeMap::new(),
            no_sell_token: None,
            charge_low_hp_user_ids: BTreeSet::new(),
            boss_damage_tracking: BTreeMap::new(),
            longest_turn_number: None,
            longest_turn_effects: None,
            last_mon_standing_armed: false,
            identity_theft_armed: false,
            spread_ko_key: None,
            spread_ko_count: None,
        }
    }
}
