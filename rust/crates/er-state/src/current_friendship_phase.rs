            ...
            .filter(|recipient| {
                recipient.hp > 0
                    && pending
                        .participants
                        .iter()
                        .any(|participant| participant.pokemon == recipient.pokemon)
            })
            .map(|recipient| recipient.pokemon)
            .collect()
    }

    pub fn validate(
        &self,
        pending: &CurrentPendingExperienceV1,
    ) -> Result<(), CurrentExperienceOwnerError> {
        let recipients = Self::recipients(pending);
        if recipients.len() > 6
            || self.awards.len() > recipients.len()
            || (self.complete
                && (self.awards.len() != recipients.len()
                    || self.clock.is_some()
                    || self.head.is_some()))
        ...
        for (index, (award, recipient)) in self.awards.iter().zip(&recipients).enumerate() {
            if award.request.pending != pending.id
                || award.request.recipient != *recipient
                ...
                || !(-8_640_000_000_000_000..=8_640_000_000_000_000)
                    .contains(&award.utc_milliseconds)
                || award.source_root.get() == SafeU53::ZERO
                || award.candy_root.get() == SafeU53::ZERO
                || award.pokemon_friendship_before > 255
                || award.pokemon_friendship_after > 255
                || self.awards[..index]
                    .iter()
                    .any(|prior| prior.request.request == award.request.request)
            {
                return Err(CurrentExperienceOwnerError::Invalid);
            }
            validate_max_clock(
                award.max_clock.as_ref(),
                award.max_utc_milliseconds,
                pending.id,
                *recipient,
                Some(award.request.request),
            )?;
        }
...
fn validate_max_clock(
    clock: Option<&CurrentFriendshipClockRequestV1>,
    utc: Option<i64>,
    pending: SafeU53,
    recipient: PokemonId,
    timed_request: Option<PlatformRequestId>,
) -> Result<(), CurrentExperienceOwnerError> {
    match (clock, utc) {
        (None, None) => Ok(()),
        (Some(clock), Some(utc))
            if clock.pending == pending
