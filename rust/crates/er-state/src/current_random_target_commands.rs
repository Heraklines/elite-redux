...
impl CurrentRandomTargetCommandsV1 {
    pub fn validate(
        ...
        for (index, entry) in self.entries.iter().enumerate() {
            ...
            if proposal.battle_id != self.battle
                || proposal.wave != self.wave
                || proposal.turn != self.turn
                || proposal.field_slot.side != BattleSide::Player
                || !run.party.iter().any(|pokemon| {
                    pokemon.id == proposal.actor && pokemon.owner_seat == Some(proposal.owner_seat)
                })
                || !matches!(
                    &proposal.command,
                    BattleCommand::Fight {
                ...
                || self.entries[..index].iter().any(|prior| {
                    prior.command.operation_id() == entry.command.operation_id()
                ...
                || entry.candidates.iter().enumerate().any(|(i, (slot, id))| {
                    slot.side != BattleSide::Enemy
                        || slot.position >= battle.format.enemy_capacity
                        || !battle.enemy_party.iter().any(|pokemon| pokemon.id == *id)
                        || entry.candidates[..i]
                            .iter()
                            .any(|(prior_slot, prior_id)| prior_slot == slot || prior_id == id)
                })
            {
                return Err(CurrentRandomTargetCommandError);
            }
            let expected_candidates = if let Some(owner) = turn {
                ...
                let action = owner
                    .actions
                    .iter()
                    .find(|action| action.accepted == entry.command)
                ...
                battle
                    ...
                    .iter()
                    .filter_map(|row| {
                        if row.slot.side != BattleSide::Enemy {
                            return None;
                        }
                        let id = row.occupant?;
                        battle
            ...
            let retained = turn
                .is_some_and(|owner| owner.accepted_commands.entries.contains(&entry.command))
                || battle
                    .command_state
                    .frontier
                    .iter()
                    .any(|row| match &row.status {
            ...
            entry
                ...
                .validate()
                .map_err(|_| CurrentRandomTargetCommandError)?;
            let before_battle = entry
                .draw
                .before_state
                .battle
                .as_ref()
                .ok_or(CurrentRandomTargetCommandError)?;
            if before_battle.battle_seed != battle.battle_rng.battle_seed
                || before_battle.turn != self.turn
                || entry.draw.before_state.run != run.run_rng.rdg
                || entry.draw.after_state.run != run.run_rng.rdg
                || entry.draw.sequence.get()
                    != u64::try_from(index).map_err(|_| CurrentRandomTargetCommandError)?
                || entry.draw.before_state.seed_offset.is_some()
                || entry.draw.callsite_id != RngCallsiteId::current_move_target()
                || entry.draw.cardinality.get()
                    != u64::try_from(entry.candidates.len())
                        .map_err(|_| CurrentRandomTargetCommandError)?
            ...
            let mut replay = RngRuntime::from_states_at_sequence(
                RunRngState {
                    rdg: entry.draw.before_state.run.clone(),
                },
                Some(before_battle.clone()),
                entry.draw.sequence,
            )
            .map_err(|_| CurrentRandomTargetCommandError)?;
            let selected = replay
                .battle_rand_seed_int(
                    entry.draw.cardinality,
                    SafeU53::ZERO,
            ...
            if replay.audit_entries() != std::slice::from_ref(&entry.draw)
                || entry
                    .candidates
                    .get(
                        usize::try_from(selected.get())
                            .map_err(|_| CurrentRandomTargetCommandError)?,
                    )
                    .map(|row| row.0)
                    != Some(entry.selected)
                || (index > 0
                    && self.entries[index - 1].draw.after_state != entry.draw.before_state)
            {
                return Err(CurrentRandomTargetCommandError);
            }
        }
        ...
        if turn.is_none()
            && self
                .entries
                .last()
                .and_then(|entry| entry.draw.after_state.battle.as_ref())
