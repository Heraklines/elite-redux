...
pub enum CurrentRewardRunError { Invalid }
impl CurrentRewardRunV1 {
    ...
    pub fn fresh_ordinary_with_startup_map() -> Self {
        Self { balls: [5,0,0,0,0], map_owned: true, lures: Vec::new() }
    }
    pub fn valid(&self) -> bool {
        self.balls.iter().all(|n| *n <= 99) && self.lures.len() <= 3
            && self.lures.iter().all(|l| matches!(l.duration,10|15|30)
                && l.remaining > 0 && l.remaining <= l.duration)
            && self.lures.iter().enumerate().all(|(i,l)|
                self.lures[..i].iter().all(|earlier| earlier.duration != l.duration))
    }
    ...
    pub fn add_ball_reward(&mut self, ball_type: u8) -> Result<(), CurrentRewardRunError> {
        if !self.valid() || ball_type > 4 { return Err(CurrentRewardRunError::Invalid); }
    ...
    pub fn add_lure(&mut self, duration: u8) -> Result<bool, CurrentRewardRunError> {
        if !self.valid() || !matches!(duration,10|15|30) { return Err(CurrentRewardRunError::Invalid); }
        if let Some(lure) = self.lures.iter_mut().find(|l| l.duration == duration) {
            if lure.remaining == lure.duration { return Ok(false); }
            lure.remaining = lure.duration;
        } else { self.lures.push(CurrentRewardLureV1 {duration,remaining:duration}); }
        Ok(true)
    }
    ...
    pub fn lapse_lures(&mut self) -> Result<(), CurrentRewardRunError> {
        if !self.valid() { return Err(CurrentRewardRunError::Invalid); }
        for lure in &mut self.lures { lure.remaining -= 1; }
