...
#[derive(Clone,Debug,Eq,PartialEq,Serialize,Deserialize)]
#[serde(tag="generator",rename_all="SCREAMING_SNAKE_CASE",deny_unknown_fields)]
pub enum CurrentRewardArgsV1 {
    Berry {kind:u8}, TemporaryStat {stat:u8}, BaseStat {stat:u8}, Mint {nature:u8},
    AttackType {kind:u8}, Tera {kind:u8}, EvolutionItem {item:u16},
    SpeciesItem {key:CurrentRewardSpeciesItemV1}, FormChangeItem {item:u16},
}
#[derive(Clone,Copy,Debug,Eq,PartialEq,Serialize,Deserialize)]
#[serde(rename_all="SCREAMING_SNAKE_CASE")]
pub enum CurrentRewardSpeciesItemV1 { LightBall,ThickClub,MetalPowder,QuickPowder,DeepSeaScale,DeepSeaTooth }
#[derive(Clone,Debug,Eq,PartialEq,Serialize,Deserialize)]
#[serde(tag="kind",rename_all="SCREAMING_SNAKE_CASE",deny_unknown_fields)]
pub enum CurrentRewardStageV1 {
    Choice,
    Holder {offer:u8},
    TmMove {offer:u8,holder:PokemonId},
    TmPending {offer:u8,holder:PokemonId},
    ...
    Applied {offer:u8,holder:Option<PokemonId>},
}
impl CurrentRewardSelectionV1 {
    pub fn structurally_valid(&self)->bool {
        (1..=3).contains(&self.offers.len()) && self.party_before.len()==1
            && self.run_before.valid()
            && self.rng_audit.len()<=4096 && self.rng_audit.iter().all(|draw|draw.validate().is_ok())
            && [&self.rng_before,&self.rng_regenerated,&self.rng_after].iter()
                .all(|s|s.starts_with("!rnd,")&&s.len()<=512)
            && self.offers.iter().all(|o|!o.source_id.is_empty()&&o.source_id.len()<=128
                && !o.name.is_empty()&&o.name.len()<=256&&o.tier<=4&&o.upgrade_count<=4096
                && o.group.as_ref().is_none_or(|g|g.len()<=128))
            && self.declined_tms.len()<=4096
            && self.declined_tms.iter().chain(self.tm.iter().map(Box::as_ref))
                .try_fold(0usize,|total,tm|total.checked_add(tm.messages.len())).is_some_and(|total|total<=4096)
            && self.tm.as_ref().is_none_or(|tm| tm.history_before.valid() && tm.slot<=4 && tm.movement.get()!=er_types::SafeU53::ZERO)
            && match &self.stage {
                CurrentRewardStageV1::Choice=>true,
                CurrentRewardStageV1::Holder{offer}|CurrentRewardStageV1::TmMove{offer,..}
                |CurrentRewardStageV1::TmPending{offer,..}|CurrentRewardStageV1::Applied{offer,..}
...
pub fn initial_inventory_valid(
    ...
    match (inventory,receipts) {
        ...
        (Some(live),[receipt])=>{
            live.valid() && receipt.structurally_valid() && receipt.run_before==fresh
                && (matches!(receipt.stage,CurrentRewardStageV1::Applied{..}) || live==&receipt.run_before)
        },
        _=>false,
    }
}