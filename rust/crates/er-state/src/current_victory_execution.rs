...
pub enum CurrentVictoryDescendantV1 {
    Ready,
    AwardPresentation { award: CurrentExperienceAwardV1, event_id: PresentationEventId },
    ...
    HidePartyBar { award: CurrentExperienceAwardV1 },
    HidePartyBarPresentation { award: CurrentExperienceAwardV1, event_id: PresentationEventId },
    LevelUpStart { level_up: CurrentLevelUpV1 },
    LevelUpPresentation { end: CurrentLevelUpEndV1, event_id: PresentationEventId },
    LevelUpChildren { children: CurrentLevelUpChildrenV1 },
    LearnMoveBatch { batch: CurrentLearnMoveBatchV1 },
    Evolution { children: CurrentLevelUpChildrenV1 },
    Complete,
}
...
impl CurrentVictoryExecutionV1 {
    pub fn valid(&self, pending_id: SafeU53) -> bool {
        ...
        if self.phases.len() > 6 || next > self.phases.len() || self.completed.len() != next
            || self.phases.iter().any(|phase| phase.pending_id != pending_id
                || phase.pokemon.get() == SafeU53::ZERO || phase.party_index >= 6)
            || self.phases.iter().enumerate().any(|(index, phase)| self.phases[..index].iter()
                .any(|prior| prior.pokemon == phase.pokemon || prior.party_index == phase.party_index))
            || self.completed.iter().zip(&self.phases).any(|(award, phase)|
                award.phase != *phase || !valid_award(award))
        { return false; }
        let retained = match &self.descendant {
            ...
            CurrentVictoryDescendantV1::AwardPresentation { award, event_id } => {
                if !award.phase.on_field || event_id.get() == SafeU53::ZERO { return false; }
                award
            }
            CurrentVictoryDescendantV1::PartyAwardPresentation { award, level_up, event_id } => {
            ...
            CurrentVictoryDescendantV1::HidePartyBar { award } => {
                if award.phase.on_field { return false; }
                award
            }
