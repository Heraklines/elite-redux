...
fn borrowed_projection_keeps_logical_identity_error_precedence() {
    ...
    assert_eq!(
        value.validate(),
        Err(GameMenuError::Logical(LogicalMenuError::ZeroInstanceId))
    );
    value.instance_id = MenuInstanceId::new(SafeU53::new(1).expect("nonzero instance"));
    assert_eq!(
        value.validate(),
        Err(GameMenuError::Logical(LogicalMenuError::EmptyControlId))
    );
    ...
    value
        .logical_menu()
        .expect("same accepted canonical projection");
}
