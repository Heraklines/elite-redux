...
fn main() -> Result<()> {
    ...
    let mut natural = GameKernelV7::natural_start(
        ProfileStateV1 {
            schema_version: 1,
            unlocks: Vec::new(),
        ...
        "m9e-storage-controlled".to_owned(),
        SeatId::new(safe(1)?),
        vec!["controlled-slot".to_owned()],
        true,
        content.clone(),
        scheduler(),
        None,
    ...
    if rewrite_case["before"] != load_case["continued"]
        || rewrite_case["request"]["request_id"].as_u64()
            <= load_case["request"]["request_id"].as_u64()
        || rewrite_case["presentation"]["event_id"].as_u64()
            <= load_case["presentation"]["event_id"].as_u64()
    {
    ...
    if encoded.len() > 32 * 1024 * 1024 {
        return Err("storage fixture exceeds bound".into());
    }
