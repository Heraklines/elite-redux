...
use er_game::m9e_material_v6::{
    AppliedGameMaterialLedgerV1, GamePlatformEffectV2, GamePresentationEffectV2,
};
...
use er_types::{
    GameActionV1, GameControlKindV2, GameMenuCancelV2, OperationId, PresentationEventId, SafeU53,
    SaveActionV1, SeatId,
};
...
fn navigate(kernel: &mut GameKernelV7, target: &str) -> Result<()> {
    ...
    for _ in 0..bound {
        if kernel
            .current_control()
            .and_then(|control| control.menu.as_ref())
            .is_some_and(|menu| menu.selected_option_id.as_str() == target)
        {
            return Ok(());
        }
...
fn checked_snapshot(
...
) -> Result<CoreGameKernelSnapshotV7> {
    ...
    if restored.snapshot()? != snapshot {
        return Err("fixture snapshot round trip differs".into());
    }
    Ok(snapshot)
}

fn controlled(
    natural: &GameKernelV7,
    action: SaveActionV1,
    name: &str,
    content: Arc<PreparedGameContentV2>,
) -> Result<GameKernelV7> {
    let reached = natural.snapshot()?;
    let mut state = natural
        .state()
        .cloned()
        .ok_or("natural active state missing")?;
    ...
    let control = generic_vertical_control_v2(
        reached.next_menu_instance_id,
        revision,
        seat,
        OperationId::new(format!("m9e/storage/{name}"))?,
        GameControlKindV2::Save,
        &format!("m9e/storage/{name}"),
        &[(format!("storage/{name}"), GameActionV1::Save { action })],
...
fn normalized_read(
    ...
    let GameKernelLifecycleSnapshotV7::Active(live) = &pending.lifecycle else {
        return Err("READ is not Active".into());
    };
    let mut state = save.state.clone();
    state.identities.next_platform_request_id = state
        .identities
        .next_platform_request_id
        .max(live.identities.next_platform_request_id);
    ...
    let revision = pending.pending_presentations.iter().try_fold(
        pending
            ...
            .max(safe(control.revision.get() + 1)?),
        |floor, presentation| -> Result<SafeU53> {
            Ok(floor.max(safe(presentation.event_id.get().get() + 1)?))
        },
    )?;
    let menu = control.menu.as_mut().ok_or("saved menu absent")?;
    let instance = pending
        .next_menu_instance_id
        .max(MenuInstanceId::new(safe(menu.instance_id.get().get() + 1)?));
    menu.instance_id = instance;
    control.revision = revision;
    let context = control
        .action_context
        .as_mut()
        .ok_or("saved action context absent")?;
    ...
    if !expected.input_router.repeats.is_empty() {
        return Err("controlled Space fixture unexpectedly owns repeats".into());
    }
...
fn case(
    ...
    let presentations: Vec<_> = step
        ...
        .filter_map(|effect| match effect {
            GameKernelEffectV7::Presentation(value) => Some(value.clone()),
            _ => None,
        })
        .collect();
    if presentations != vec![expected_presentation.clone()]
        || pending.pending_presentations.len() != 1
    {
        return Err(
            "controlled Save action did not produce the independently expected presentation".into(),
        );
    }
    let requests: Vec<_> = step
        ...
        .iter()
        .filter_map(|effect| match effect {
            GameKernelEffectV7::Platform(value) => Some(value.clone()),
            _ => None,
        })
        .collect();
    ...
    let (id, bytes, result, wire) = match (&requests[0], save_bytes) {
        ...
        (GamePlatformEffectV2::StorageRead { request, slot }, Some(bytes))
            if slot == "controlled-slot" =>
        {
            (
                *request,
                bytes.to_vec(),
    ...
    if kernel
        .apply_storage_result(id, KernelStorageResultV2::Deleted)
        .is_ok()
        || kernel.snapshot()? != pending
    {
        return Err("wrong callback did not conserve the whole snapshot".into());
    }
    ...
    if callback.pending_presentations != pending.pending_presentations
        || !callback.pending_platform.is_empty()
        || callback.storage_frontiers.len() != 1
    ...
    let expected = if let Some(bytes) = save_bytes {
        normalized_read(&pending, &GameSaveV2::decode(bytes)?)?
    } else {
        let mut expected = pending.clone();
        expected.pending_platform.clear();
        expected.storage_frontiers = vec![StorageFrontierSnapshotV1 {
            slot: "controlled-slot".to_owned(),
            generation: safe(expected_generation)?,
        }];
        expected.replay_sequence = safe(expected.replay_sequence.get() + 1)?;
        expected
    };
    if callback != expected {
        return Err("callback changed fields outside exact owned normalization".into());
    }
    ...
    Ok((
        json!({"before": before, "pending": pending, "callback": callback,
...
fn natural(content: Arc<PreparedGameContentV2>) -> Result<GameKernelV7> {
    Ok(GameKernelV7::natural_start(
        ...
        "m9e-title-storage-retirement".to_owned(),
        SeatId::new(safe(1)?),
        vec!["new-run-destination".to_owned()],
        true,
        content,
        scheduler(),
        None,
...
) -> Result<&er_game::m72_bootstrap::RunBootstrapMachineV1> {
    let GameKernelLifecycleSnapshotV7::Bootstrap(owner) = &snapshot.lifecycle else {
        return Err("Title Bootstrap absent".into());
    };
    Ok(owner)
}
fn list_case(
    reader: &mut GameKernelV7,
    content: Arc<PreparedGameContentV2>,
) -> Result<(Value, er_types::PlatformRequestId)> {
    ...
    let step = reader.raw_input(RawInputEvent::KeyDown {
        code: PhysicalKey::Space,
        printable: false,
        browser_repeat: false,
        focus: InputFocus::Game,
    ...
    Ok((
        json!({"before": before, "selected": selected, "pending": pending, "request_id": request}),
        *request,
    ))
}
fn cancelled(
    ...
    reader.raw_input(RawInputEvent::KeyDown {
    ...
    if released_owner.pressed_keys.len() != 1
        || !released_owner.pressed_keys.contains(&PhysicalKey::Escape)
    {
    ...
    if held.replay_sequence != safe(pending.replay_sequence.get() + 1)? || after != released {
        return Err(
            "Cancel key release changed unrelated state or lost a causal replay step".into(),
        );
    }
    let before_owner = title_owner(&pending)?;
    let after_owner = title_owner(&after)?;
    let storage = before_owner
        .current_storage
        .as_ref()
        .ok_or("pending Title extension absent")?;
    let next = after_owner
        .current_storage
        .as_ref()
        .ok_or("cancelled Title extension absent")?;
    if after_owner.stage != er_game::m72_bootstrap::RunBootstrapStageV1::Title
        || next.pending.is_some()
        || !next.slots.is_empty()
        || next.missing_slot.is_some()
        ...
        || after_owner.menu_instance_high_water.get().get()
            != before_owner.menu_instance_high_water.get().get() + 1
        || after.replay_sequence != safe(pending.replay_sequence.get() + 2)?
        || !after.pending_platform.is_empty()
    {
    ...
    if after != unrelated {
        return Err("Title Cancel changed unrelated core fields".into());
    }
    for result in [
        KernelStorageResultV2::Slots {
            slots: vec!["controlled-slot".to_owned()],
        },
...
fn read_case(
    reader: &mut GameKernelV7,
    content: Arc<PreparedGameContentV2>,
) -> Result<(CoreGameKernelSnapshotV7, er_types::PlatformRequestId)> {
    let step = reader.raw_input(RawInputEvent::KeyDown {
        code: PhysicalKey::Space,
        printable: false,
        browser_repeat: false,
    ...
    if slot != "controlled-slot" {
        return Err("READ selected a destination absent from inventory".into());
    }
    Ok((checked_snapshot(reader, content)?, *request))
}
fn normalized_title_read(
    pending: &CoreGameKernelSnapshotV7,
    save: &GameSaveV2,
) -> Result<CoreGameKernelSnapshotV7> {
...
fn main() -> Result<()> {
    let out = std::env::args_os()
        .nth(1)
        .map(PathBuf::from)
        .ok_or("output directory missing")?;
    fs::create_dir_all(&out)?;
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let bundle: GameContentBundleV2 = serde_json::from_slice(&fs::read(
        root.join("fixtures/m9/engineering/game-content-bundle-v2.json"),
    )?)?;
    let content = Arc::new(PreparedGameContentV2::prepare(Arc::new(bundle))?);
    let mut reached = natural(content.clone())?;
    for _ in 0..3 {
        press(&mut reached, PhysicalKey::Space)?;
    }
    ...
    if reached
        .current_control()
        .is_none_or(|control| control.kind != GameControlKindV2::BattleCommand)
    {
        return Err("natural producer did not reach BattleCommand".into());
    }
    for pending in reached.snapshot()?.pending_presentations {
        reached.settle_presentation(pending.event_id)?;
    }
    ...
    for index in 0..21 {
        ...
        reference["mode"] = json!(if index == 1 {
            "QUEUED_NOT_STARTED"
        } else {
            "ACTIVE_TRANSACTION"
        });
        reference["cancelled"] =
            serde_json::to_value(cancelled(&mut reader, request, content.clone())?)?;
    ...
    for mode in ["ACTIVE_TRANSACTION", "CALLBACK_READY"] {
        ...
        read_cancels.push(
            json!({"mode": mode, "listing": listing, "selected": selected,
    ...
    let selected = accept_list(&mut reader, list_request, content.clone())?;
    let (pending, request) = read_case(&mut reader, content.clone())?;
    if saved.state.identities.next_platform_request_id
        >= title_owner(&pending)?
            .current_storage
            .as_ref()
            .ok_or("Title owner absent")?
            .next_platform_request_id
    {
        return Err(
            "cancel sequence did not demonstrate live platform highwater beyond saved state".into(),
        );
    }
    ...
    if loaded != expected {
        return Err("Title READ changed fields outside exact saved-state normalization".into());
    }
    if reader
        .apply_storage_result(request, KernelStorageResultV2::Read { bytes: Some(bytes) })
        .is_ok()
        || reader.snapshot()? != loaded
    {
        return Err("duplicate Title READ changed full snapshot".into());
    }
    ...
    if encoded.len() > 32 * 1024 * 1024 {
        return Err("Title reference exceeds existing fixture bound".into());
    }
