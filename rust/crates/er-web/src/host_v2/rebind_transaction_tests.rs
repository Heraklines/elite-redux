...
use er_types::{ConnectionGeneration, FrameContext, InputFocus, MembershipRevision, PhysicalKey,
    RawInputEvent, RunId, SeatId, SessionId, TimeClass};
...
impl Peer {
    fn natural(authority: bool) -> TestResult<Self> {
        ...
        let config = if authority { authority_protocol(host, guest, generation(1))? }
            else { replica_protocol(host, guest, generation(1))? };
        let mut peer = Self { host: BrowserKernelHostV2::from_content(content()?) };
        let response = peer.send(BrowserRequestV2::Initialize {
            initialization: Box::new(BrowserSessionInitializationV2::NaturalCoop {
                context: BrowserSessionContextV2 {
                    local_seat: seat,
                    role: if authority { GameKernelRoleV7::Authority } else { GameKernelRoleV7::Replica },
                ...
                profile: profile()?, seed: "owned-startup".to_owned(),
                save_slots: vec!["owned-save".to_owned()], local_is_host: authority,
            }),
        })?;
        assert_eq!(response, BrowserResponseV2::Ready);
        assert!(peer.host.session()?.snapshot()?.current_coop_setup.is_some());
        Ok(peer)
    }
    ...
    fn envelope(&self, request: BrowserRequestV2) -> TestResult<Vec<u8>> {
        Ok(canonical_bytes(&BrowserRequestEnvelopeV2 {
            ...
            request_id: increment(self.host.next_sequence)?, sequence: self.host.next_sequence, request,
        })?)
    }
    ...
    fn rebind(&mut self, control: CurrentCoopRebindEventV1) -> TestResult<CurrentSessionRebindOutputV1> {
        ...
        let BrowserResponseV2::Rebind { output, observation } = self.send(BrowserRequestV2::CoopRebind { control })? else {
    ...
    fn press(&mut self, key: PhysicalKey) -> TestResult<Vec<Vec<u8>>> {
        let mut frames = self.frames(BrowserRequestV2::RawInput { event: RawInputEvent::KeyDown {
            code: key.clone(), printable: false, browser_repeat: false, focus: InputFocus::Game,
        } })?;
        frames.extend(self.frames(BrowserRequestV2::RawInput { event: RawInputEvent::KeyUp { code: key } })?);
        Ok(frames)
    }

    fn navigate(&mut self, id: &str) -> TestResult {
        let bound = self.host.session()?.observe()?.control.as_ref().and_then(|control| control.menu.as_ref())
            .ok_or("natural menu required")?.options.len() + 1;
        for _ in 0..bound {
            if self.host.session()?.observe()?.control.as_ref().and_then(|control| control.menu.as_ref())
                .is_some_and(|menu| menu.selected_option_id.as_str() == id) { return Ok(()); }
    ...
    fn choose(&mut self, authority: bool) -> TestResult<Vec<Vec<u8>>> {
        ...
        let mode = prepared.bundle().bootstrap.modes.iter().find(|mode| mode.cooperative && mode.supported)
        ...
        let GameKernelLifecycleSnapshotV7::Bootstrap(before) = self.host.session()?.snapshot()?.lifecycle else {
            return Err("actual starter stage required".into());
        };
        assert_eq!(before.stage, RunBootstrapStageV1::StarterSelect);
        let starter = before.catalog.starters.iter().find(|starter| starter.cost <= before.catalog.maximum_starter_cost)
        ...
        if authority {
            for _ in 0..4 {
                if self.host.session()?.kernel_ref()?.state().is_some()
                    || matches!(self.host.session()?.snapshot()?.lifecycle, GameKernelLifecycleSnapshotV7::Bootstrap(ref bootstrap) if bootstrap.stage == RunBootstrapStageV1::Complete) { break; }
                frames.extend(self.press(PhysicalKey::Space)?);
            }
        }
        Ok(frames)
    ...
    fn export(&mut self) -> TestResult<CurrentReproCapsuleV1> {
        ...
        assert_eq!(capsule.browser_transport.as_ref().ok_or("browser transport capture")?.final_generation, self.host.generation);
        Ok(capsule)
    }
    ...
    fn import(&mut self) -> TestResult {
        ...
        let mut restored = Self { host: BrowserKernelHostV2::from_content(content()?) };
        assert_eq!(restored.send(BrowserRequestV2::Initialize {
    ...
    fn settle(&mut self) -> TestResult<usize> {
        ...
        for _ in 0..16 {
            ...
            for presentation in pending {
                assert!(self.frames(BrowserRequestV2::PresentationSettled {
                    event_id: presentation.event_id, outcome: BrowserPresentationOutcomeV2::Settled,
                })?.is_empty());
                count += 1;
            }
        }
    ...
    fn next_frame(&mut self) -> TestResult<Vec<u8>> {
        for _ in 0..8 {
            ...
            if !frames.is_empty() { return one_frame(&frames); }
        }
...
fn one_frame(frames: &[Vec<u8>]) -> TestResult<Vec<u8>> {
    let [frame] = frames else { return Err("one actual frame required".into()); };
    Ok(frame.clone())
}
...
fn pair() -> TestResult<(Peer, Peer)> {
    ...
    let choice = one_frame(&guest.choose(false)?)?;
    assert!(host.choose(true)?.is_empty());
    let started = one_frame(&host.frames(BrowserRequestV2::NetworkFrame { generation: safe(1), bytes: choice })?)?;
    assert!(guest.frames(BrowserRequestV2::NetworkFrame { generation: safe(1), bytes: started })?.is_empty());
    assert!(host.host.session()?.kernel_ref()?.state().is_some());
    assert_eq!(host.host.session()?.kernel_ref()?.state(), guest.host.session()?.kernel_ref()?.state());
    host.export()?; guest.export()?;
    Ok((host, guest))
}

fn begin(host: &mut Peer, guest: &mut Peer) -> TestResult<Vec<u8>> {
    for peer in [&mut *host, &mut *guest] {
        assert!(peer.frames(BrowserRequestV2::TransportChanged { generation: safe(1), connected: false })?.is_empty());
        assert!(peer.rebind(CurrentCoopRebindEventV1::Begin)?.frames.is_empty());
        assert_eq!(peer.host.generation, safe(1));
        assert!(peer.frames(BrowserRequestV2::TransportChanged { generation: safe(2), connected: true })?.is_empty());
...
fn handshake(host: &mut Peer, guest: &mut Peer, mut frame: Vec<u8>) -> TestResult {
    for index in 0usize..8 {
        ...
        let output = peer.rebind(CurrentCoopRebindEventV1::Receive { generation: generation(2), bytes: frame })?;
        assert_eq!(output.generation, generation(2));
        if index < 7 { frame = one_frame(&output.frames)?; } else { assert!(output.frames.is_empty()); frame = Vec::new(); }
    ...
    for peer in [host, guest] {
        ...
        let owner = snapshot.current_coop_setup.as_ref().and_then(|setup| setup.rebind.as_deref()).ok_or("rebind owner required")?;
...
fn evidence(host: &BrowserKernelHostV2) -> TestResult<Vec<u8>> {
    let retained = host.retained.iter().map(|(id, entry)| (id, entry.accepted_sequence, &entry.fingerprint, &entry.response)).collect::<Vec<_>>();
    Ok(canonical_bytes(&(host.session()?.snapshot()?, host.next_sequence, host.generation, retained, host.retained_response_bytes, host.disposed))?)
}
...
fn browser_rebind_natural_controls_and_generation_two_gameplay_replay() -> TestResult {
    ...
    assert!(guest.frames(BrowserRequestV2::NetworkFrame { generation: safe(2), bytes: material })?.is_empty());
    host.settle()?; guest.settle()?;
    ...
    assert_eq!(one_frame(&guest.frames(BrowserRequestV2::RetryCoopSetup)?)?, proposal);
    assert_eq!(guest.host.session()?.snapshot()?, pending);
    let reply = one_frame(&host.frames(BrowserRequestV2::NetworkFrame { generation: safe(2), bytes: proposal.clone() })?)?;
    ...
    assert_eq!(one_frame(&host.frames(BrowserRequestV2::NetworkFrame { generation: safe(2), bytes: proposal })?)?, reply);
    assert_eq!(host.host.session()?.snapshot()?, after);
    guest.import()?;
    assert!(guest.frames(BrowserRequestV2::NetworkFrame { generation: safe(2), bytes: reply.clone() })?.is_empty());
    assert!(guest.host.session()?.snapshot()?.current_proposal.is_none());
    let after = guest.host.session()?.snapshot()?;
    assert!(guest.frames(BrowserRequestV2::NetworkFrame { generation: safe(2), bytes: reply })?.is_empty());
    ...
    for (index, peer) in [&mut host, &mut guest].into_iter().enumerate() {
        ...
        let controls = capsule.attempts.iter().filter(|attempt| matches!(attempt.event, CurrentExternalEvent::CoopRebind { .. })).collect::<Vec<_>>();
        assert_eq!(controls.len(), if index == 0 { 6 } else { 5 });
        assert_eq!(controls.iter().filter(|attempt| matches!(attempt.event, CurrentExternalEvent::CoopRebind { control: CurrentCoopRebindEventV1::Receive { .. } })).count(), 4);
        for attempt in controls {
            assert_eq!(attempt.origin.as_deref(), Some("browser.coop.REBIND"));
            let transport = attempt.browser_transport.ok_or("rebind transport evidence required")?;
            assert_eq!(transport.before_generation, transport.after_generation);
            assert!(matches!(attempt.outcome, CurrentReproOutcomeV1::RebindApplied { .. }));
        }
        peer.import()?;
    }
    Ok(())
...
fn browser_rebind_receive_response_and_cache_rejection_preserve_transaction() -> TestResult {
    let (mut host, mut guest) = pair()?;
    let offer = begin(&mut host, &mut guest)?;
    let request = guest.envelope(BrowserRequestV2::CoopRebind { control: CurrentCoopRebindEventV1::Receive { generation: generation(2), bytes: offer } })?;
    let before = evidence(&guest.host)?;
    let before_snapshot = guest.host.session()?.snapshot()?;
    let position = guest.host.repro.as_ref().ok_or("capture required")?.export()?.final_position;
    let mut reference = guest.host.clone();
    let expected = reference.process_bytes(&request)?;
    assert_ne!(reference.session()?.snapshot()?, before_snapshot, "genuine RECEIVE must change owned state before response admission");
    assert!(expected.len() > 1);
    for cache_bound in [false, true] {
        let mut rejected = guest.host.clone();
        let result = if cache_bound {
            rejected.process_bytes_with_limits(&request, MAXIMUM_BROWSER_RESPONSE_BYTES_V2, expected.len() - 1)
        } else {
        ...
        assert!(matches!(rejected.capture_status(), Some(CurrentCaptureStatusV1::Unavailable { position: actual, .. }) if actual == position + 1));
...
fn browser_rebind_duplicates_noops_and_wrong_generation_keep_capture_exact() -> TestResult {
    let (mut host, mut guest) = pair()?;
    for peer in [&mut host, &mut guest] {
        peer.frames(BrowserRequestV2::TransportChanged { generation: safe(1), connected: false })?;
        ...
        assert_eq!(peer.host.generation, safe(1));
        peer.frames(BrowserRequestV2::TransportChanged { generation: safe(2), connected: true })?;
    }
    let offer = one_frame(&host.rebind(CurrentCoopRebindEventV1::Retry)?.frames)?;
    let before = evidence(&guest.host)?;
    let position = guest.host.repro.as_ref().ok_or("capture required")?.export()?.final_position;
    let wrong = guest.envelope(BrowserRequestV2::CoopRebind { control: CurrentCoopRebindEventV1::Receive { generation: generation(1), bytes: offer.clone() } })?;
    assert!(matches!(guest.host.process_bytes(&wrong), Err(BrowserWebErrorV2::Kernel(_))));
    ...
    let last = rejected_capsule.attempts.last().ok_or("rejected attempt required")?;
    assert!(matches!(last.outcome, CurrentReproOutcomeV1::KernelRejected { .. }));
    assert_eq!(last.browser_transport.ok_or("transport evidence required")?.after_generation, safe(2));
    guest.import()?;
    let control = CurrentCoopRebindEventV1::Receive { generation: generation(2), bytes: offer };
    let bytes = guest.envelope(BrowserRequestV2::CoopRebind { control: control.clone() })?;
    ...
    assert_eq!(guest.host.capture_status(), capture, "same envelope retry must not record another control");
    let BrowserResponseEnvelopeV2 { response: BrowserResponseV2::Rebind { output, .. }, .. } = serde_json::from_slice(&response)? else {
        return Err("actual rebind response required".into());
    };
    let after = guest.host.session()?.snapshot()?;
    let position = guest.export()?.final_position;
    assert_eq!(guest.rebind(control)?, output, "new envelope duplicate returns original immediate response");
    assert_eq!(guest.host.session()?.snapshot()?, after);
...
fn browser_rebind_wire_decoding_rejects_unknown_control_fields() -> TestResult {
    ...
    for control in [CurrentCoopRebindEventV1::Begin, CurrentCoopRebindEventV1::Retry,
        CurrentCoopRebindEventV1::Receive { generation: generation(2), bytes: vec![1] }] {
    ...
    for bytes in cases {
        ...
        assert!(matches!(peer.host.process_bytes(&bytes), Err(BrowserWebErrorV2::Invalid)));
