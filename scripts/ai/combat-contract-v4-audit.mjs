...
function makeEpisode(envelope) {
  return {
    ...
    envelope: {
      ...
      gameModeId: envelope.gameModeId,
      startWave: envelope.startWave,
      startedAt: envelope.startedAt,
      build: envelope.build,
      erVersion: envelope.erVersion,
      mode: envelope.mode,
    },
  ...
  function finish(extra = {}) {
    ...
    for (const episode of episodes.values()) {
      ...
      onEpisodeFinished?.({
        ...
        auxiliaryDecisions: [...episode.auxiliaryDecisions.values()].map(entry => entry.record),
        transitions: [...episode.transitions.values()].map(entry => entry.record),
        battleTerminals: [...episode.battleTerminals.values()].map(entry => entry.record),
        runTerminals: [...episode.runTerminals.values()].map(entry => entry.record),
        result,
      });
    }
    const report = {
