"""Remote actual initialized achievement and Egg descendants; no battle or hatching qualification."""
import base64
import hashlib
import json
import os
...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-achievement-source"
FULL = REPORT / "diagnostics"
COMPACT = REPORT / "compact"
OUTPUT = FULL / "generated"
ORACLE = REPORT / "oracle"
STORE = ROOT / ".m9e-achievement-source-store"
...
HELPER = "test/kernel-fixtures/m9/observe-achievement-descendants.ts"
INJECTED = "test/kernel-fixtures/m9-observe-achievement-descendants.test.ts"
TITLE = "observe actual level and Flash achievement descendants"
VERIFIER = "scripts/ci/m9e_achievement_source_verify.mjs"
PRODUCER = "scripts/ci/m9e_achievement_source_diagnostic.py"
WORKFLOW = ".github/workflows/m9e-achievement-source.yml"
...
EXPORTER_SHA256 = "4a8abbc164755f167597acb721d28e1f45561fa80e5b3c4922b6b616422313ac"
...
def source_method_closure(summary):
    selections = {
        "src/battle-scene.ts": [("validateAchvs", 2), ("validateAchv", 2), ("validateVoucher", 2)],
        "src/data/elite-redux/er-achievement-rewards.ts": [("grantErAchievementReward", 0), ("applyRewardSpec", 0)],
        "src/data/egg.ts": [("constructor", 2), ("rollSpecies", 2), ("rollEggMoveIndex", 2), ("addEggToGameData", 2)],
        "src/system/game-data.ts": [("addStarterCandy", 2)],
        "src/utils/common.ts": [("randomString", 0), ("randInt", 0), ("randSeedInt", 0)],
    }
    excerpts = []
    methods = []
    ...
    require(0 < len(raw) <= 32768, "achievement source excerpt bound")
    (OUTPUT / "source-methods.txt").write_bytes(raw)
    summary["achievement_source_methods"] = {"methods": methods, "excerpt": file_fact(OUTPUT / "source-methods.txt", 32768)}


def main(summary):
    sha = os.environ["GITHUB_SHA"]
    require(os.environ.get("GITHUB_REF_NAME") == "codex/m9e-achievement-source-20260912" and os.environ.get("GITHUB_EVENT_NAME") == "push", "exact source branch/event")
    ...
    for ordinal in ("one", "two"):
        destination = OUTPUT / f"achievement-{ordinal}.json"
        ...
        run(["pnpm", "exec", "vitest", "run", INJECTED, "--pool=forks", "--isolate", "--no-file-parallelism",
             "--reporter=json", "--outputFile=" + str(report)], "fresh-achievement-" + ordinal,
            cwd=ORACLE, extra={"M9_ACHIEVEMENT_DESCENDANTS_OUTPUT": str(destination)})
    ...
    require(inventory(ORACLE, PIN, "oracle-after-achievement-verification") == pinned,
            "oracle changed during achievement verification")
...
def entry():
    ...
    summary = {"schema_version": 1, "source_sha": os.environ.get("GITHUB_SHA"), "run_id": os.environ.get("GITHUB_RUN_ID"),
        ...
        "scope": "actual initialized achievement level/Flash/MAX zero-date and Egg reward descendants; no battle trigger, hatching, or runtime payout qualification",
