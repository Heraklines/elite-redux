"""Remote F for the existing ownership stack regression and current co-op preservation."""
import hashlib
import json
import os
...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-coop-stack-focused"
FULL = REPORT / "diagnostics"
COMPACT = REPORT / "compact"
TARGET = REPORT / "target"
os.environ["CARGO_TARGET_DIR"] = str(TARGET)
DEADLINE = time.monotonic() + 1800
RUST_SOURCES = ["rust/crates/er-kernel/src/game_kernel_v7.rs", "rust/crates/er-kernel/src/snapshot_v7.rs",
                "rust/crates/er-kernel/src/current_coop_setup_v7.rs",
                "rust/crates/er-kernel/tests/m9e_current_proposal_v7.rs", "rust/crates/er-kernel/tests/m9e_coop_choices_v7.rs"]
TARGETS = [("m9e_current_proposal_v7", RUST_SOURCES[3], [
    ...
    ("m9e_coop_choices_v7", RUST_SOURCES[4], [
...
def main(summary):
    ...
    sources = [*RUST_SOURCES, "rust/crates/er-kernel/src/game_kernel_v7.rs", "rust/crates/er-kernel/src/snapshot_v7.rs",
               "rust/crates/er-game/src/m9e_new_run_v6.rs", "rust/crates/er-game/src/m72_bootstrap.rs", "rust/crates/er-types/src/m72_bootstrap.rs",
               ...
               "scripts/ci/m9e_current_cost.py", "scripts/ci/m9e_coop_stack_diagnostic.py",
               ".github/workflows/m9e-coop-stack-focused.yml",
    ...
    if os.environ.get("RUST_MIN_STACK") is not None:
        raise RuntimeError("stack regression must use the unchanged default test stack")
    summary["rust_min_stack"] = None
    summary["targets"] = []
    for TEST_TARGET, TEST_SOURCE, TEST_IDS in TARGETS:
        run(["cargo", "clippy", "--locked", "-p", "er-kernel", "--test", TEST_TARGET, "--no-deps", "--", "-D", "warnings"], "clippy-" + TEST_TARGET)
        build = run(["cargo", "test", "--locked", "-p", "er-kernel", "--test", TEST_TARGET,
                     "--no-run", "--message-format=json"], "build-" + TEST_TARGET)
        ...
        if (artifact.get("manifest_path") != str(ROOT / "rust/crates/er-kernel/Cargo.toml")
                ...
                or artifact["profile"].get("overflow_checks") is not True or artifact["profile"].get("opt_level") != "0"
        ...
        listing = run([str(binary), "--list", "--format", "terse"], "list-" + TEST_TARGET, seconds=30, bound=16384).read_text()
        if listing != "".join(name + ": test\n" for name in TEST_IDS):
            raise RuntimeError("exact target inventory differs")
        ...
        output = run([str(binary), "--format", "terse", "--nocapture", "--test-threads=1"], "execute-" + TEST_TARGET,
        ...
        if counts != [(str(len(TEST_IDS)), "0", "0", "0", "0")]:
            raise RuntimeError("exact target completion differs")
        ...
        summary["targets"].append({"target": TEST_TARGET, "artifact": test_artifact, "passed": len(TEST_IDS)})
...
if __name__ == "__main__":
    ...
    summary = {"status": "failed", "qualification": "focused default-stack owner and co-op regression; no integration or M9 qualification",
