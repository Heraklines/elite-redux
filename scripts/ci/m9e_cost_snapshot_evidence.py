"""Remote observation of difficulty-owned cost checkpoints; strict unaffected baseline checks."""
import hashlib
...
INSTRUMENTATION = b'''    assert_eq!(session.snapshot()?, snapshot);
    std::fs::write(
        std::path::Path::new(&std::env::var("M9E_COST_SNAPSHOT_DIR")?)
            .join(format!("{}.json", checkpoint.name)),
        &encoded,
    )?;
    Ok(json!({
'''
...
def main(summary):
    ...
    need(len(cohort["source_hashes"]) == 21, "complete original cost sources, four difficulty owners, and actual recoil resolver")
    ...
    snapshot = SNAPSHOTS / "active.json"
    ...
    try:
        ...
        os.environ.update(CARGO_TARGET_DIR=str(TARGET), CARGO_INCREMENTAL="0",
                          ...
                          RUSTUP_TOOLCHAIN="1.97.1", M9E_COST_SNAPSHOT_DIR=str(SNAPSHOTS))
        ...
        for expected, actual in zip(cohort["checkpoints"], measured["checkpoints"], strict=True):
            need(actual["checkpoint"] == expected["checkpoint"], "ordered original checkpoints")
            if expected["checkpoint"] != "active":
                need(all(actual[key] == value for key, value in expected.items() if key != "snapshot_digest"),
                     "unchanged title/mode/starter checkpoint sizes and metrics; actual digests captured")
        active = [row for row in measured["checkpoints"] if row["checkpoint"] == "active"][0]
        for key, expected in cohort["active"].items():
            if key not in ("phases", "snapshot_canonical_bytes", "snapshot_digest", "recorder_capsule_bytes"):
                need(active[key] == expected, "original active semantic metric: " + key)
        need(snapshot.is_file() and not snapshot.is_symlink() and 0 < snapshot.stat().st_size <= 16384
        ...
        summary.update(status="passed", tests=dict(passed=1, failed=0, skipped=0),
                       ...
                       baseline_source_sha=cohort["baseline_source_sha"], baseline_run_id=cohort["baseline_run_id"],
...
if __name__ == "__main__":
    ...
    result = dict(status="failed", source_sha=os.environ["GITHUB_SHA"], run_id=os.environ["GITHUB_RUN_ID"],
                  ...
                  qualification="paired published-drain checkpoint observation; byte-difference audit pending; no timing or full M9 qualification")
    try:
        main(result)
