        ...
        for expected, actual in zip(cohort["checkpoints"], measured["checkpoints"], strict=True):
            ...
            if expected["checkpoint"] != "active":
                need(all(actual[key] == value for key, value in expected.items()),
                     "unchanged title/mode/starter checkpoint digests and sizes")
...
if __name__ == "__main__":
    ...
    result = dict(status="failed", source_sha=os.environ["GITHUB_SHA"], run_id=os.environ["GITHUB_RUN_ID"],
                  ...
                  qualification="difficulty owner snapshot observation against qualified baseline; byte-difference audit pending; no timing or full M9 qualification")
    try:
        main(result)
