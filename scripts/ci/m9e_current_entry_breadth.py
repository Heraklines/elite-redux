...
def main():
    ...
    try:
        ...
        output, record = command("status", ["git", "status", "--porcelain"])
        require(record["returncode"] == 0 and not output.strip(), "clean source checkout")
        result["source_bindings"] = {
            "workflow_sha256": sha((ROOT / ".github/workflows/m9e-current-entry-breadth.yml").read_bytes()),
            "lockfile_sha256": sha((ROOT / "rust/Cargo.lock").read_bytes()),
            "toolchain_file_sha256": sha((ROOT / "rust/rust-toolchain.toml").read_bytes()),
        }
