...
def main():
    ...
    try:
        ...
        selector = ["-p", "er-battle", "-p", "er-game", "-p", "er-kernel", "-p", "er-cli", "-p", "er-repro", "-p", "er-web"] + [argument for name, row in TARGETS.items() if row.get("kind") != "lib" for argument in ("--test", name)]
        ...
        require(len(tests) == 22 and {row["target"]["name"] for row in tests} == set(TARGETS), "twenty-two complete native targets including both whole libraries, browser host and reward arithmetic")
        result["artifacts"] = []
        ...
        require(result["targets_attempted"] == 22, "every whole target must be attempted")
        ...
        require(result["tests_executed"] == 116, "all83 predecessor tests plus1 Flash phase,1 source constructor,14 whole state-library,14 whole browser-host and3 source reward arithmetic tests")
