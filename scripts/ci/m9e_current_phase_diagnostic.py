...
def main():
    ...
    try:
        ...
        # Keep both complete campaigns in this same qualification, after the
        # shorter whole targets so their shared-budget cost cannot hide them.
        deferred = {"m9e_natural_coop_campaign_v7": 1, "m9e_natural_campaign_replay": 2}
        for artifact in sorted(tests, key=lambda row: (deferred.get(row["target"]["name"], 0), row["target"]["name"])):
