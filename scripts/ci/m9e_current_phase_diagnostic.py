...
def run(name, argv, cwd=ROOT, maximum=8 << 20, allow_test_failure=False):
    ...
    require(reason is None and (process.returncode == 0 or allow_test_failure) and len(raw) <= maximum, reason or name + " failed")
...
def main():
    ...
    try:
        ...
        run("proposal-clippy", ["cargo", "clippy", "--locked", "--keep-going", "-p", "er-types", "-p", "er-battle", "-p", "er-state", "-p", "er-game", "-p", "er-kernel", "-p", "er-save", "-p", "er-env", "-p", "er-cli", "-p", "er-progression", "-p", "er-web", "--tests", "--no-deps", "--", "-D", "warnings"], ROOT/"rust")
        ...
        require(len(tests) == 23 and {row["target"]["name"] for row in tests} == set(TARGETS), "twenty-three complete native targets including both whole libraries, browser host, reward arithmetic and menu types")
        result["artifacts"] = []
        ...
        for artifact in sorted(tests, key=lambda row: row["target"]["name"]):
            ...
            output = run(name+"-execute", [str(binary), "--format", "terse"], ROOT/("rust/crates/"+crate), maximum=32768, allow_test_failure=True)
            ...
            result["artifacts"].append(dict(target=artifact["target"], profile=profile, manifest_path=artifact["manifest_path"], path=str(binary), bytes=binary.stat().st_size, sha256=digest, ids=ids, listing_bytes=len(listing), listing_sha256=sha(listing), passed=passed))
            if passed:
                result["tests_executed"] += len(ids)
        require(result["targets_attempted"] == 23, "every whole target must be attempted")
        ...
        require(result["tests_executed"] == 121, "all83 predecessor tests plus1 Flash phase,1 source constructor,14 whole state-library,14 whole browser-host and3 source reward arithmetic tests plus5 whole menu-type tests")
