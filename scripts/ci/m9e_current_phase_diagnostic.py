...
def main():
    ...
    try:
        ...
        require(len(tests) == 21 and {row["target"]["name"] for row in tests} == set(TARGETS), "twenty-one complete native targets including both whole libraries and browser host")
        result["artifacts"] = []
        ...
        require(result["targets_attempted"] == 21, "every whole target must be attempted")
        ...
        require(result["tests_executed"] == 113, "all83 predecessor tests plus1 Flash phase,1 source constructor,14 whole state-library and14 whole browser-host tests")
