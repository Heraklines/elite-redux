...
def main():
    ...
    try:
        ...
        selector = ["-p", "er-battle", "-p", "er-game", "-p", "er-kernel", "-p", "er-cli", "-p", "er-repro"] + [argument for name in TARGETS for argument in ("--test", name)]
        ...
        require(len(tests) == 18 and {row["target"]["name"] for row in tests} == set(TARGETS), "eighteen complete native targets")
        result["artifacts"] = []
        ...
        require(result["tests_executed"] == 68, "all58 inherited tests plus4 coop,2 phase,1 progression,1 campaign,1 coop-campaign and1 replay witnesses")
