...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-defender-ability-focused"
FULL = REPORT / "diagnostics"
COMPACT = REPORT / "compact"
OUTPUT = FULL / "generated"
ORACLE = REPORT / "oracle"
STORE = ROOT / ".m9e-defender-ability-source"
PIN = "399d5d368f0b5642ebf8f45bd8a5e73350fa4de7"
BASE = "40a59698400da73aa15fe883538b7135b69ee51e"
BASE_TREE = "11e62a7097f2b2acd9cc32d5c2f1547fc7394927"
...
HELPER = "test/kernel-fixtures/m9/observe-defender-ability.ts"
INJECTED = "test/kernel-fixtures/m9-observe-defender-ability.test.ts"
TITLE = "observe actual defender absorb phase and healing ownership"
VERIFIER = "scripts/ci/m9e_defender_ability_verify.mjs"
PRODUCER = "scripts/ci/m9e_defender_ability_diagnostic.py"
WORKFLOW = ".github/workflows/m9e-defender-ability-focused.yml"
...
EXPORTER_SHA256 = "32d9ca111df6ac827e0995ba3d01c05b4ca54cc7bfa211df2716dcc4c55f6c4b"
...
ORACLE_PINS.update({
  "src/enums/hit-check-result.ts": ["0f672742aa81a5614150f29e55abae976cd46ae04c362a702b48307c74d7e983", 846],
  "src/utils/common.ts": [
    "65a83f24675a5b56a426f6d2d49c1e2549fad641e3e187e6b031154cec286e48",
    14626
  ],
  "test/framework/phase-interceptor.ts": [
    "4646673b9e7648ff27bdf950368d1bd340b1e45e5adfbe1f0d9c6cad3c294b37",
    13494
  ],
  "test/helpers/move-helper.ts": [
    "d29def82f73491c4153a5072d3ef94bc1924c011d8c48febaba84747c7758d4d",
    16804
  ],
  "src/phases/move-effect-phase.ts": [
    "5e02a2c6f6fdb11ea6b30143ff881975e8d2f5b88f020c9c84f61c89d29a4382",
    57798
  ],
  "src/data/elite-redux/archetypes/immunity-with-absorb.ts": [
    "9948910f42e1190cc4d770eb534ba88e76313c2c8de51e919c96111d7b7bf9ae",
    9542
  ],
  "src/phase-tree.ts": [
    "2a152ebab21e2d54e72a0655896ece0d731c48aa82d61d8130d2a30f1e895b3c",
    9396
  ],
  "src/data/elite-redux/er-endless-rift-runtime.ts": [
    "b684761e27e0809ce154eaff0f9516fa36dfeb90b1f7142e18f2add5befd39c5",
    35712
  ],
  "src/data/elite-redux/moody/moody-scene-adapter.ts": [
    "66b388f4f7d8822449afec90d632e233bbbc4099b23b782e353d37b73cd7d3e1",
    23039
  ],
  "src/data/elite-redux/er-endless-continuation.ts": [
    "bda671c08adadfcf5dfa4dd91ccd03384262528b5748367959cad431c0add559",
    35310
  ],
  "src/phase-manager.ts": [
    "8905870d2c79b52032e8d7770b864d4511c77480fce3c04cd4d7c9595995547b",
    51511
  ],
  "src/data/elite-redux/abilities/soulmate.ts": [
    "b7a55e9cc96835490730d695543981214484976aad0c2982eb63b3d0e6e43c04",
    4096
  ],
  ...
  "src/phases/pokemon-heal-phase.ts": [
    "564676e3c243bb7c76caff251e26fed84caf8af77f16734b134e49b0f0f9e292",
    6730
  ],
  "src/phases/show-ability-phase.ts": [
    "a2ffe763f675bdfd6b16ec6ba05ce5552b4029953a4eec8660c27a8a6a5222a8",
    4898
  ],
  "src/data/elite-redux/moody/moody-coordinator-combat-state.ts": [
    "6c1381b7b70e7e61a6fbacb1f4405c26ce5b3c95962bdaff3ed13786f6c68c84",
    8509
  ]
})
...
def fetch_tackle():
    ...
    request = urllib.request.Request(ASSET_URL, headers={"Accept": "application/vnd.github+json",
                                                       "User-Agent": "m9e-defender-ability"})
...
def main(summary):
    ...
    require(os.environ.get("GITHUB_REF_NAME") == "codex/m9e-defender-ability-20260909" and os.environ.get("GITHUB_EVENT_NAME") == "push", "exact source branch/event")
    ...
    for ordinal in ("one", "two"):
        ...
        run(["pnpm", "exec", "vitest", "run", INJECTED, "--pool=forks", "--isolate", "--no-file-parallelism",
            ...
            cwd=ORACLE, extra={"M9_DEFENDER_ABILITY_OUTPUT": str(destination)})
...
def entry():
    ...
    summary = {"schema_version": 1, "source_sha": os.environ.get("GITHUB_SHA"), "run_id": os.environ.get("GITHUB_RUN_ID"),
        ...
        "scope": "two actual pinned PhaseTree absorb observations with explicit controlled Pokemon inputs; no Rust qualification",
