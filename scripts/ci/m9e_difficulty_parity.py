"""Actual paired difficulty traces; original native/Wasm goldens remain unchanged."""
import copy
import hashlib
import itertools
import json
...
OUT = RUNNER / "m9e-difficulty-parity"
TARGET = RUNNER / "m9e-difficulty-parity-target"
START = int(os.environ["M9E_STARTED_AT"])
DEADLINE = START + 1800
COMMANDS = []
BASE = "0b211cfbf0730d8228b6b16656b8c94fa3000790"
OLD = "4bb63a076b3a3662313be26668d59a445238fef1"
SOURCE = "rust/crates/er-wasm/tests/m9e_parity.rs"
ORIGINAL_BYTES = 29501
ORIGINAL_HASH = "95c09508d4ac63e3a401d2abd1a14725de8d85b1d5405f4de9d0e1999032ee6c"
DIAGNOSTIC_HASH = "a8caadde1b2222c189e5448ad7594c93849f891e4b6b1a1483dd111672af1ded"
CI = [".github/workflows/m9e-difficulty-parity-focused.yml", "scripts/ci/m9e_difficulty_parity.py"]
...
def main():
    ...
    try:
        ...
        require(TARGET.resolve().parent == RUNNER and TARGET.name == "m9e-difficulty-parity-target", "cleanup must remain in owned runner target")
...
def mechanical_comparison(left, right):
    if left is None or right is None:
        return left == right
    require(isinstance(left, dict) and isinstance(right, dict), "typed mechanical state required")
    require(left.get("content_identity") == right.get("content_identity"), "content must remain identical")
    require("current_run_difficulty" not in left and right.get("current_run_difficulty") == {"difficulty": "YOUNGSTER", "run_id": 1}, "exact captured difficulty addition required")
    normalized = copy.deepcopy(right)
    del normalized["current_run_difficulty"]
...
def compare_actual_preimages(summary):
    ...
    full = {"actual_events": comparisons, "decoded_authority_material_counts": decoded_material_counts, "mechanical_states_equal_after_only_difficulty_owner": mechanical_equal,
            "all_mechanical_states_equal_after_only_difficulty_owner": mechanical_equal == comparisons,
