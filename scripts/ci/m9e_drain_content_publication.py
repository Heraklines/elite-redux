...
SOURCE_BRANCH = "codex/m9e-drain-content-publication-focused-v3-20260908"
...
def validate_payloads(summary, archive, candidate, receipt):
    ...
    admitted_units = [row["behavior_unit"] for row in baseline["admitted_classifications"]]
    removed_units = []
    for entry in expected_bespoke["entries"]:
        removed_units.extend(unit for unit in entry["behavior_units"] if unit in admitted_units)
    ...
    require(len(removed_units) == 12 and sorted(map(canonical, removed_units)) == sorted(map(canonical, admitted_units)), "exact twelve admitted units leave bespoke dispatch")
    require(battle["bespoke"] == expected_bespoke, "bespoke dispatch changed beyond exact source-derived twelve removals")
    require(sha256(canonical(old_battle["bespoke"])) == "01d38c0fc304dbc9bf1056c9821c996ce1b1fc781cd297dbcd3536c09978736d"
            and sha256(canonical(battle["bespoke"])) == "242284f8c342399e6cce98f1b82cdb95007d0a722557d506b875be08c4637465", "observed bespoke preimages differ")
    receipt["bespoke_dispatch_delta"] = {"removed_units": admitted_units, "all_other_entries_equal": True,
        "compiler_rule": "remove admitted units, then remove empty entries", "removed_unit_count": 12}
