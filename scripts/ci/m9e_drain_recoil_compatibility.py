...
CI = [".github/workflows/m9e-drain-recoil-compatibility-focused.yml", "scripts/ci/m9e_drain_recoil_compatibility.py"]
DELTAS = json.loads(r'''{"after":{"rust/crates/er-game/tests/m9e_move_drains.rs":"de3246873b5c23d61e13121e5b7ed2051e244fac149b33b4c69f1e72ad7f4b10"},"before":{"rust/crates/er-game/tests/m9e_move_drains.rs":"95e29923db0f0237272516f30fef425a560acd9695204b2c812f4f1ee990a9d0"}}''')
...
OUT = RUNNER / "m9e-drain-recoil-compatibility"
TARGET = RUNNER / "m9e-drain-recoil-compatibility-target"
...
def main():
    ...
    result = {"status": "failed", "qualification": "five whole existing drain tests against published drain-plus-recoil content with all original drain and recoil baseline conservation; full M9 remains unqualified",
    ...
    try:
        ...
        pins = [*DELTAS["after"], *CI, "rust/crates/er-battle/src/m7_resolver.rs", "rust/crates/er-content-compiler/src/lib.rs", "rust/crates/er-content-compiler/src/m9e_full_content.rs", "rust/crates/er-content-compiler/src/m9e_move_recoil.rs", "rust/Cargo.toml", "rust/Cargo.lock", "rust/rust-toolchain.toml",
        ...
        require(not any(os.environ.get(key) for key in ("M9E_RECOIL_BATTLE_PACK", "M9E_RECOIL_BUNDLE", "M9E_RECOIL_EXPORT", "M9E_RECOIL_BASELINE_EXPORT")), "actual checked-in bundle default required")
        for path, expected in PUBLISHED.items():
            actual = (ROOT / path).read_bytes()
            require(len(actual) == expected["bytes"] and sha(actual) == expected["sha256"] and hashlib.sha1(b"blob " + str(len(actual)).encode() + b"\0" + actual).hexdigest() == expected["git_blob"], "actual published complete fixture binding")
        require(generated_bytes == (ROOT / "rust/fixtures/m9/engineering/battle-content-pack-v3.json").read_bytes(), "fresh complete source compiler byte equality to published battle pack")
        result["published_inputs"] = PUBLISHED
        result["runtime_bundle_override"] = False
        ...
        require(TARGET.resolve().parent == RUNNER and TARGET.name == "m9e-drain-recoil-compatibility-target",
