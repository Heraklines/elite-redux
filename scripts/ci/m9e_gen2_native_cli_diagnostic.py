"""Remote native CLI rebind and retained CLI/Worker regression qualification."""
import hashlib
import json
import os
...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-gen2-native-cli-focused"
...
TARGETS = [
  ...
  [
    "er-cli",
    "m9e_current_coop_rebind",
    "rust/crates/er-cli/tests/m9e_current_coop_rebind.rs",
    [
      "actual_native_cli_rebind_preserves_capture_admission_restore_and_gameplay"
    ]
  ],
  [
    "er-cli",
  ...
  [
    "er-cli",
    "m9e_current_coop_startup",
    "rust/crates/er-cli/tests/m9e_current_coop_startup.rs",
    [
      "current_native_cli_owned_coop_retry_replay_matches_browser_host",
...
REVERSE_TARGETS = {
  ...
  "er-cli:m9e_current_coop_rebind:test": "rust/crates/er-cli/tests/m9e_current_coop_rebind.rs",
...
PRODUCT_SOURCES = sorted(RUST_SOURCES)
CI_SOURCES = ["scripts/ci/m9e_gen2_native_cli_diagnostic.py", ".github/workflows/m9e-gen2-native-cli-focused.yml"]
CHANGED_SOURCES = sorted(PRODUCT_SOURCES + CI_SOURCES)
ADDED_SOURCES = sorted(["rust/crates/er-cli/tests/m9e_current_coop_rebind.rs", *CI_SOURCES])
FIXTURE_INPUTS = {
