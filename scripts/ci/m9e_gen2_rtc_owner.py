...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-gen2-rtc-owner-focused"
...
IDS = ["current V7 RTC owner automatically rebinds and continues natural gameplay across fresh connections"]
SOURCES = [EXAMPLE, SPEC, "scripts/ci/m9e_gen2_rtc_owner.py", ".github/workflows/m9e-gen2-rtc-owner-focused.yml",
...
def main(summary):
    ...
    changes = run(["git", "diff", "--name-only", "c82b22f033435e378d396f262d2f057dfc2567c9", "HEAD"], "source-delta", 30, 16384).read_text().splitlines()
    if sorted(changes) != sorted([SPEC, "scripts/ci/m9e_gen2_rtc_owner.py", ".github/workflows/m9e-gen2-rtc-owner-focused.yml", "src/rust-browser/routes/rust-current-rtc-rebind-entry.ts", "src/rust-browser/routes/rust-current-rtc-entry.ts", "scripts/build-kernel-m9e-v7-web.mjs"]):
        raise RuntimeError("only new generation-bound transport, export, builder inventory and focused proof files may change")
    summary["base_sha"] = "c82b22f033435e378d396f262d2f057dfc2567c9"
...
def execute_prepared(summary, *, install_chromium=True):
    ...
    if len(attachments) != 1 or attachments[0].get("name") != "m9e-current-rtc-owner-rebind" or attachments[0].get("contentType") != "application/json":
    ...
    expected = {"source_sha": sha, "manifest_sha256": digest(OUTPUT / "m9e-v7-worker-assets.json"),
                ...
                "schema_version": 1, "browser_worker_protocol_version": 2, "observed_worker_count": 6,
                "actual_workers": 6, "disposed_workers": 6, "generation": 2, "successful_signaling_pairs": 2,
                "automatic_rebind_controls": [4, 4], "connected_callbacks": [1, 1], "disconnected_callbacks": [1, 1]}
    ...
    for key in ("live_begin_rejected", "checkpoint_handoff_exact", "full_replay", "deleted_control_rejected",
        ...
        if evidence.get(key) is not True:
            raise RuntimeError(f"actual owner assertion missing: {key}")
    ...
    if len(evidence.get("raw_inputs", [])) != 4 or any(type(count) is not int or count <= 0 for count in evidence["raw_inputs"]):
        raise RuntimeError("actual raw inputs required for both natural and rebound owners")
...
if __name__ == "__main__":
    ...
    summary = {"status": "failed", "source_sha": os.environ["GITHUB_SHA"], "run_id": os.environ["GITHUB_RUN_ID"],
               "qualification": "focused actual RTC owner natural startup, automatic generation two rebind, continued gameplay and full capsule replay; not full M9 qualification"}
    try:
        main(summary)
