...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-gen2-rtc-transport-focused"
...
SPEC = "test/browser/rust-browser/m9e-v7-rebind-rtc-transport.spec.ts"
IDS = ["current V7 generation-bound RTC transport reconnects and replays natural Worker gameplay"]
SOURCES = [EXAMPLE, SPEC, "scripts/ci/m9e_gen2_rtc_transport.py", ".github/workflows/m9e-gen2-rtc-transport-focused.yml",
...
SOURCES += ["rust/crates/er-battle/src/m7_resolver.rs", "test/browser/rust-browser/m9e-v7-rebind.spec.ts", "rust/crates/er-kernel/src/current_coop_rebind_v7.rs", "src/rust-browser/adapters/current-rtc-transport-v2.ts", "test/browser/rust-browser/m9e-v7-rebind-rtc.spec.ts"]
logs = {}
failed_log = None
...
def main(summary):
    ...
    changes = run(["git", "diff", "--name-only", "abc8cd305cc858692c7997d9a35e4af671a49343", "HEAD"], "source-delta", 30, 16384).read_text().splitlines()
    if sorted(changes) != sorted([SPEC, "scripts/ci/m9e_gen2_rtc_transport.py", ".github/workflows/m9e-gen2-rtc-transport-focused.yml", "src/rust-browser/adapters/current-rtc-transport-v2.ts", "src/rust-browser/routes/rust-current-rtc-entry.ts", "scripts/build-kernel-m9e-v7-web.mjs"]):
        raise RuntimeError("only new generation-bound transport, export, builder inventory and focused proof files may change")
    summary["base_sha"] = "abc8cd305cc858692c7997d9a35e4af671a49343"
...
def execute_prepared(summary, *, install_chromium=True):
    ...
    if len(attachments) != 1 or attachments[0].get("name") != "m9e-current-browser-transport-rebind" or attachments[0].get("contentType") != "application/json":
    ...
    transports = evidence.get("transport_evidence", [])
    if len(transports) != 2:
        raise RuntimeError("two actual generation-bound transports required")
    for index, transport in enumerate(transports):
        count = (1, 6)[index]
        if transport.get("generation") != index + 1 or transport.get("connected") != [1, 1] or transport.get("disconnected") != [1, 1] or transport.get("wrong_generation_rejected") is not True or len(transport.get("endpoints", [])) != 2:
            raise RuntimeError("actual transport handshake/disconnect/rejection differs")
        for endpoint in transport["endpoints"]:
            if endpoint.get("closed") is not True or endpoint.get("connected") is not False:
                raise RuntimeError("actual transport must close")
            if any(endpoint.get(key) != 0 for key in ("sendPending", "receivePending", "sendBytes", "receiveBytes")):
                raise RuntimeError("actual transport queues must drain")
            if any(endpoint.get(key) != count for key in ("sentFrames", "receivedFrames", "kernelDeliveredFrames")):
                raise RuntimeError("actual transport packet counts differ")
...
if __name__ == "__main__":
    ...
    summary = {"status": "failed", "source_sha": os.environ["GITHUB_SHA"], "run_id": os.environ["GITHUB_RUN_ID"],
               "qualification": "focused physical RTC generation two carrier plus actual Worker rebind and replay; generation-bound transport with actual Worker callbacks; full reconnect route owner remains unqualified"}
    try:
        main(summary)
