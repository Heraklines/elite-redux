"""Remote source qualification of held XP distribution with original party and phase regressions."""
import difflib
import hashlib
import json
import os
...
BASE = "65aeebd47855e4cdf9bf0fe0d8ab52385d786b06"
MODULE = "rust/crates/er-progression/src/current_party_experience.rs"
TEST = "m9e_held_experience"
CI = [".github/workflows/m9e-held-xp-focused.yml", "scripts/ci/m9e_held_xp.py", "scripts/ci/m9e_held_xp_oracle.mjs"]
IDS = ["actual_whole_source_held_matrix_matches_every_fractional_phase_argument", "held_booster_receives_fractional_product_before_distribution_floor", "explicit_empty_held_context_preserves_unboosted_source_behavior", "malformed_held_context_and_unsafe_awards_are_rejected_atomically"]
ROOT = Path.cwd().resolve()
RUNNER = Path(os.environ["RUNNER_TEMP"]).resolve()
OUT = RUNNER / "m9e-held-xp"
TARGET = RUNNER / "m9e-held-xp-target"
START = float(os.environ["M9E_STARTED_AT"])
DEADLINE = START + 1800
COMMANDS = []
PRODUCTS = {"rust/crates/er-progression/src/current_party_experience.rs":"6b097726570b014fb12bcbcaa6470d761cc72bf17d9316fb6ca56b70d4debf83","rust/crates/er-progression/tests/m9e_held_experience.rs":"678003c4f842a31988e355d86efa5a6d728616389b451daac424e75a01d094fb"}
...
def main():
    ...
    result = {"status": "failed", "source_sha": os.environ["GITHUB_SHA"],
              ...
              "scope": "actual applyPartyExp held distribution plus original party and phase regressions; resolved ownership, no pending-owner settlement or gameplay activation"}
    original = None
    try:
        ...
        require(sha(git("show", BASE + ":" + MODULE)) == "26b3e17d7940314cae10ce1bc6d29e5f20dfe65928830c7263cfea819a6e3cb2", "original qualified whole party source")
        dependencies = ["src/battle-scene.ts", "src/data/balance/starters.ts", "src/modifier/modifier.ts", "pnpm-lock.yaml", "package.json",
                        ...
                        "rust/crates/er-progression/tests/m9e_party_experience.rs", "rust/crates/er-progression/src/lib.rs",
                        "rust/crates/er-progression/src/current_phase_experience.rs", "rust/crates/er-progression/tests/m9e_phase_experience.rs",
                        "scripts/ci/m9e_party_xp_oracle.mjs", "scripts/ci/m9e_phase_xp_oracle.mjs",
        ...
        require(oracle["cases"] == 288 and oracle["runtime"] == "v24.9.0" and oracle["phaser_version"] == "3.90.0", "actual pinned source matrix")
        ...
        os.environ["M9E_HELD_XP_ORACLE"] = str(oracle_path)
        for label, script, variable, cases, output_hash in [
            ("original-party", "scripts/ci/m9e_party_xp_oracle.mjs", "M9E_PARTY_XP_ORACLE", 144, "80658168c8fa7638b481a97e370ff5e8b7cb383342c6374bd31a7037e225b0da"),
            ("original-phase", "scripts/ci/m9e_phase_xp_oracle.mjs", "M9E_PHASE_XP_ORACLE", 336, "2ed825e0157992c7aab24bfe7552fa055e024a3410137bdbfa0e4d3f48c9edbf"),
        ]:
            folder = OUT / "diagnostics" / label
            folder.mkdir()
            path = folder / "source-cases.tsv"
            prior = json.loads(run(label + "-oracle", ["node", "--disable-warning=ExperimentalWarning", script, str(path)]))
            require(prior["runtime"] == "v24.9.0" and prior["cases"] == cases and prior["output_sha256"] == output_hash == sha(path.read_bytes()), "actual unchanged prior source observations")
            os.environ[variable] = str(path)
            result[label + "-oracle"] = prior
        run("clippy", ["cargo", "clippy", "--locked", "-p", "er-progression", "--test", TEST, "--test", "m9e_party_experience", "--test", "m9e_phase_experience", "--", "-D", "warnings"], ROOT / "rust")
        ...
        def regressions(label):
            observed = {}
            for target in ["m9e_party_experience", "m9e_phase_experience"]:
                name = label + "-" + target
                executable, binary = build(name, target)
                execute(name + "-execute", executable, 4, 0)
                observed[target] = binary
            return observed
        result["original_positive"] = regressions("original-positive")
        ...
        before = b"let mut amount = exp_value * multiplier;"
        after = b"let mut amount = (exp_value * multiplier).floor();"
        ...
        result["negative"] = {"passed": 2, "failed": 2, "mutation": "premature floor before actual held XP booster application"}
        result["original_negative"] = regressions("original-negative")
        ...
        result["original_restored"] = regressions("original-restored")
        require(result["original_positive"] == result["original_restored"], "both original four-test executables exactly restored")
