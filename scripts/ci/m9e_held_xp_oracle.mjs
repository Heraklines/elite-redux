...
class PokemonHeldItemModifier {
  constructor(type, pokemonId, stackCount) { this.type = type; this.pokemonId = pokemonId; this.stackCount = stackCount; }
  getStackCount() { return this.stackCount; }
  // Applicable ownership is an explicit input boundary, not qualified here.
  shouldApply(pokemon) { return pokemon.id === this.pokemonId; }
}
const modifierText = sources["src/modifier/modifier.ts"];
const heldStart = modifierText.indexOf("export class PokemonExpBoosterModifier extends PokemonHeldItemModifier {");
const heldEnd = modifierText.indexOf("export class ", heldStart + 20);
if (heldStart < 0 || heldEnd <= heldStart) throw new Error("Entire actual held modifier boundary");
const heldBody = modifierText.slice(heldStart, heldEnd).trimEnd();
const heldExecutable = stripTypeScriptTypes(heldBody.replace("export class ", "class "));
const PokemonExpBoosterModifier = vm.runInNewContext(heldExecutable + "\nPokemonExpBoosterModifier", { PokemonHeldItemModifier }, { timeout: 1000 });
...
const rows = [];
for (const pattern of patterns) for (const [variant, raw] of [7, 101.5, 4294967295].entries()) for (const mods of modifiers) for (const heldVariant of [0, 1]) {
  const trainer = variant !== 0;
  const defeated = variant !== 1;
  const friendCalls = [];
  const phases = [];
  const held = pattern.party.map((_, index) => heldVariant === 0 ? [[20, 1]] : index % 2 === 0 ? [[20, 1], [60, 2]] : [[60, 99], [100, 1]]);
  ...
  Object.assign(oracle, {
    ...
    applyModifiers: (Type, player, member, holder) => {
      if (Type !== PokemonExpBoosterModifier || player !== true || !party.includes(member) || !(holder instanceof NumberHolder)) throw new Error("Actual held modifier call");
      for (const [percent, stacks] of held[member.id]) {
        const booster = new Type({}, member.id, percent, stacks);
        if (stacks > booster.getMaxHeldItemCount(member) || !booster.shouldApply(member, holder) || booster.apply(member, holder) !== true) throw new Error("Actual held constructor/cap/application");
      }
    },
  ...
  rows.push([raw, +trainer, +defeated, 50, pattern.count, ...mods.map(optional),
    pattern.party.map(member => member.join(":")).join(","), held.map(items => items.map(pair => pair.join(":")).join(";")).join(","),
    friendCalls.length ? friendCalls.join(",") : "-",
    phases.length ? phases.map(phase => phase.join(":")).join(",") : "-"].join("\t"));
}
const output = Buffer.from(rows.join("\n") + "\n");
if (rows.length !== 288 || output.length > 65536) throw new Error("Exact bounded source matrix");
...
console.log(JSON.stringify({ source_hashes: pins, function_sha256: hash(original), function_bytes: Buffer.byteLength(original),
  ...
  friendship_argument: friendship, cases: rows.length, output_sha256: hash(output), output_bytes: output.length, held_class_bytes: Buffer.byteLength(heldBody), held_class_sha256: hash(heldBody),
  runtime: process.version, scope: "actual applyPartyExp and PokemonExpBoosterModifier; resolved held ownership, normal Classic without Macho Brace; no phase execution or pending-owner settlement" }));
