...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-natural-replay-focused"
...
def main(summary):
    ...
    sources = [*RUST_SOURCES, "rust/crates/er-kernel/src/game_kernel_v7.rs", "rust/crates/er-kernel/src/snapshot_v7.rs",
               ...
               "rust/crates/er-game/Cargo.toml", "rust/crates/er-kernel/Cargo.toml", "rust/crates/er-repro/Cargo.toml", "rust/crates/er-repro/src/current.rs", "rust/crates/er-env/src/current.rs",
               "scripts/ci/m9e_current_cost.py", "scripts/ci/m9e_natural_replay_diagnostic.py",
               ".github/workflows/m9e-natural-replay-focused.yml",
               "rust/fixtures/m9/engineering/game-content-bundle-v2-manifest.json"]
    sources.append("scripts/ci/m9e_campaign_replay.py")
    ...
    run(["cargo", "clippy", "--locked", "-p", "er-repro", "--test", TEST_TARGET, "--no-deps", "--", "-D", "warnings"], "clippy-test")
    build = run(["cargo", "test", "--locked", "-p", "er-repro", "--test", TEST_TARGET,
    ...
    output = run([str(binary), "--format", "terse", "--nocapture", "--test-threads=1"], "execute",
                 cwd=ROOT / "rust/crates/er-repro", seconds=600 if os.environ.get("M9E_PHASE") == "native" else 900, bound=16384).read_text()
