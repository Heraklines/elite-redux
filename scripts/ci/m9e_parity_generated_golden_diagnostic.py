"""Remote content-bound parity qualification; preserves both exact cohort goldens."""
import hashlib
import json
import os
...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-parity-generated-golden"
...
CHANGED_SOURCES = [TEST_SOURCE, "scripts/ci/m9e_parity_generated_golden_diagnostic.py", ".github/workflows/m9e-parity-generated-golden-focused.yml"]
...
def execute_cohort(name, summary):
    ...
    selected = [OLD_IDS[1]] if name == "old" else OLD_IDS
    execution, raw_digest, timer_digest = [], None, None
    execution_deadline = time.monotonic() + 600
    for ordinal, test_id in enumerate(selected):
        remaining = execution_deadline - time.monotonic()
        if remaining <= 0:
            raise RuntimeError("whole native cohort target execution exceeds600 seconds")
        ...
        if counts != [("1", "0", "0", "0", "1")]:
            raise RuntimeError("exact native selection/count differs")
        execution.append({"id": test_id, "passed": 1, "filtered_out": 1})
        if test_id == OLD_IDS[1]:
...
def cleanup_owned_targets():
    for name in ("old", "new", "wasm", "tools"):
...
def source_conservation(summary, phase):
    ...
    if sorted(changes) != expected:
        raise RuntimeError("only exact parity test repair and two new CI paths may differ from f501")
...
def main(summary):
    ...
    summary["qualified_invocations"] = {"old_native": 1, "new_native": 2, "new_wasm": 2}
...
if __name__ == "__main__":
    ...
    summary = {"status": "failed", "source_sha": os.environ["GITHUB_SHA"], "run_id": os.environ["GITHUB_RUN_ID"],
               ...
               "preimage_diagnostic": {"source_sha": "658274f301de3b0d68977afc63a2c9bdb58ea0fc", "run_id": "34144718100",
                   "summary_sha256": "2d91496199dfb3e267cd7dc1ee53f751c7d09c492dc8be7a1eb1b08db6eace6a",
                   "actual_events": 30, "decoded_materials_per_cohort": 6, "difference_paths": 26, "omitted_paths": 0},
               "qualification": "exact old native raw witness plus both existing new native/Wasm parity IDs; not full M9 qualification"}
    try:
        main(summary)
