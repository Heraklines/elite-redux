...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-progression-parity-focused"
FULL = REPORT / "diagnostics"
COMPACT = REPORT / "compact"
TARGET = REPORT / "target"
os.environ["CARGO_TARGET_DIR"] = str(TARGET)
DEADLINE = time.monotonic() + 1800
RUST_SOURCES = ["rust/crates/er-kernel/src/game_kernel_v7.rs", "rust/crates/er-wasm/tests/m9e_parity.rs",
                "rust/crates/er-game/src/m9e_runtime_v6.rs", "rust/crates/er-progression/src/progression.rs"]
TEST_TARGET = "m9e_parity"
TEST_IDS = ["native_replays_v7_held_timers_eventwise", "native_replays_v7_raw_inputs_eventwise"]
sequence = 0
logs = {}
failed_log = None
...
def run(args, name, *, cwd=None, seconds=900, bound=16 << 20, env=None):
    ...
    try:
        result = run_bounded(args, cwd=ROOT / "rust" if cwd is None else cwd,
                             environment=dict(os.environ) if env is None else env, output=output, seconds=seconds,
...
def main(summary):
    ...
    sources = [*RUST_SOURCES, "rust/crates/er-kernel/src/game_kernel_v7.rs", "rust/crates/er-kernel/src/snapshot_v7.rs",
               ...
               "rust/crates/er-wasm/Cargo.toml", "rust/crates/er-wasm/src/m9e_parity.rs",
               "scripts/ci/m9e_feedback.py", "scripts/ci/m9e-targets.json",
               "scripts/ci/m9e_current_cost.py", "scripts/ci/m9e_progression_parity_diagnostic.py",
               ".github/workflows/m9e-progression-parity-focused.yml",
    ...
    run(["cargo", "clippy", "--locked", "-p", "er-wasm", "--test", TEST_TARGET, "--no-deps", "--", "-D", "warnings"], "clippy-test")
    build = run(["cargo", "test", "--locked", "-p", "er-wasm", "--test", TEST_TARGET,
    ...
    output = run([str(binary), "--format", "terse", "--nocapture", "--test-threads=1"], "execute",
                 cwd=ROOT / "rust/crates/er-wasm", seconds=600, bound=16384).read_text()
    ...
    os.environ["M9E_REPORT_DIR"] = str(REPORT / "feedback")
    import m9e_feedback as feedback
    summary["native_timer_parity_digest"] = feedback.timer_parity_digest(output, "native")
    feedback.run = lambda args, name, cwd=ROOT / "rust", env=None: run(args, name, cwd=cwd, env=env)
    feedback.wasm_checks({"wasm_test": TEST_TARGET}, summary)
...
if __name__ == "__main__":
    ...
    summary = {"status": "failed", "qualification": "native and Wasm raw-input/held-timer report parity after qualified progression repair only; no full campaign or M9 qualification",
               "source_sha": os.environ["GITHUB_SHA"], "run_id": os.environ["GITHUB_RUN_ID"],
               "run_attempt": os.environ["GITHUB_RUN_ATTEMPT"], "base_sha": "b27ce85fee62067d10cb865d247213d54a4d8843"}
    try:
        main(summary)
        if TARGET.exists():
