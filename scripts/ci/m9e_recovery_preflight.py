...
def main():
    ...
    for row in owned_inventory:
        ...
        if row["crate"] not in plan["packages"] or not ("*" in scope or row["target"] in scope):
            raise RuntimeError("complete803 inventory has an unselected target: " + row["crate"] + ":" + row["target"])
    ...
    if (guard is None or guard.get("status") != "verified" or guard.get("baseline_sha") != QUALIFIED_BASELINE
        ...
        raise RuntimeError("exact five test dependencies require a complete verified guard")
    ...
    if len(product) != 92 or set(product) != set([*feedback.RECOVERY_PATHS, *generated.PATHS]):
        raise RuntimeError("combined product source must be exactly the92 reviewed paths")
    ...
    if (sum(map(len, plan["required_native_targets"].values())) != 61
    ...
    owned_receipt = {"qualification": "source and planned inventory conservation only; combined execution pending",
                     ...
                     "selected_tests": 803, "selected_targets": 105, "prior_preserved_tests": 759,
                     "required_targets": 61, "required_identity_maps": 55, "composition_paths": 92,
    ...
    if projected_test_count != 803 or len(owned_inventory) != 105:
    ...
    print("Passed: exact92-path source composition, all759 prior IDs/exclusions and44 additions across105 targets, qualified56 product sources, and retained co-op/platform/cost/rule/mutant obligations. No native/platform qualification.")
