...
BRANCH = "codex/m9e-phase-lint-fix-20260923"
...
def main():
    ...
    result = {"status": "failed", "source_sha": os.environ["GITHUB_SHA"],
              ...
              "scope": "test-only warning repair; strict Clippy and all four whole native phase tests; no aggregate M9 acceptance",
    ...
    try:
        ...
        _, code = run("toolchain", ["rustup", "toolchain", "install", "1.97.1", "--profile", "minimal", "--component", "rustfmt", "--component", "clippy"], 120)
        ...
        _, code = run("clippy", ["cargo", "clippy", "--locked", "-p", "er-kernel", "--test", "m9e_current_phase_execution", "--", "-D", "warnings"], 600, ROOT / "rust")
        require(code == 0, "strict phase-target Clippy")
