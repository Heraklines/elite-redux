"""Remote initialized reward generation; no post-victory or applied reward qualification."""
import base64
import hashlib
import json
import os
...
REPORT = Path(os.environ["RUNNER_TEMP"]) / "m9e-reward-selection-source"
FULL = REPORT / "diagnostics"
COMPACT = REPORT / "compact"
OUTPUT = FULL / "generated"
ORACLE = REPORT / "oracle"
STORE = ROOT / ".m9e-reward-selection-source-store"
...
HELPER = "test/kernel-fixtures/m9/observe-reward-selection.ts"
INJECTED = "test/kernel-fixtures/m9-observe-reward-selection.test.ts"
TITLE = "observe actual initialized reward selection"
VERIFIER = "scripts/ci/m9e_reward_selection_source_verify.mjs"
PRODUCER = "scripts/ci/m9e_reward_selection_source_diagnostic.py"
WORKFLOW = ".github/workflows/m9e-reward-selection-source.yml"
...
EXPORTER_SHA256 = "803bb1f3c0754101c81f2f795c23b2cbfa4b122a433d62d6ccd3ce56c40b3215"
...
def source_method_closure(summary):
    selections = {
        "src/phases/select-modifier-phase.ts": [("updateSeed", 2), ("getModifierCount", 2), ("getModifierTypeOptions", 2)],
        "src/modifier/modifier-type.ts": [("regenerateModifierPoolThresholds", 0), ("getPlayerModifierTypeOptions", 0), ("getNewModifierTypeOption", 0)],
        "src/data/elite-redux/coop/coop-reward-options.ts": [("serializeRewardOptions", 0)],
    }
    excerpts = []
    methods = []
    ...
    require(0 < len(raw) <= 32768, "reward source excerpt bound")
    (OUTPUT / "source-methods.txt").write_bytes(raw)
    summary["reward_source_methods"] = {"methods": methods, "excerpt": file_fact(OUTPUT / "source-methods.txt", 32768)}


def main(summary):
    sha = os.environ["GITHUB_SHA"]
    require(os.environ.get("GITHUB_REF_NAME") == "codex/m9e-reward-selection-source-20260912" and os.environ.get("GITHUB_EVENT_NAME") == "push", "exact source branch/event")
    ...
    for ordinal in ("one", "two"):
        destination = OUTPUT / f"reward-{ordinal}.json"
        ...
        run(["pnpm", "exec", "vitest", "run", INJECTED, "--pool=forks", "--isolate", "--no-file-parallelism",
             "--reporter=json", "--outputFile=" + str(report)], "fresh-reward-" + ordinal,
            cwd=ORACLE, extra={"M9_REWARD_SELECTION_OUTPUT": str(destination)})
    ...
    require((OUTPUT / "reward-one.json").read_bytes() == (OUTPUT / "reward-two.json").read_bytes(),
            "two actual fresh reward outputs are not byte-identical")
    run(["node", str(ROOT / VERIFIER), str(OUTPUT / "reward-one.json"),
         str(OUTPUT / "reward-two.json"), str(OUTPUT / "validation.json")],
        "independent-reward-verification", seconds=60, bound=65536)
    ...
    require(set(summary["generated"]) == {"reward-one.json", "reward-two.json", "source-methods.txt", "validation.json"}, "exact output inventory")
...
def bound_partial_outputs():
    ...
    for path in sorted(OUTPUT.iterdir()):
        ...
        if path.name not in {"reward-one.json", "reward-two.json", "source-methods.txt", "validation.json"} or path.stat().st_size > (8192 if path.name == "validation.json" else 32768):
...
def entry():
    ...
    summary = {"schema_version": 1, "source_sha": os.environ.get("GITHUB_SHA"), "run_id": os.environ.get("GITHUB_RUN_ID"),
        ...
        "scope": "actual initialized reward pool predicates and option generation; not a post-victory or runtime reward qualification",
