...
function validateMoveInputs(e){
 const m=e.move_inputs;shape(m,['closure','scope','battle_rng_calls','calls','tuning']);assert.equal(m.scope,'actual single wild enemy generator calls and complete returned level registry; no all-species or natural encounter closure');assert.equal(m.battle_rng_calls,0);
 ...
 assert(Array.isArray(m.closure)&&m.closure.length===1);const x=m.closure[0];shape(x,['species','signature','chance','types','rows']);assert.equal(x.species,c.species);integer(x.chance,0,100);
 shape(x.signature,['own','value']);assert.equal(typeof x.signature.own,'boolean');if(!x.signature.own)assert.equal(x.signature.value,null);else if(Array.isArray(x.signature.value)){assert(x.signature.value.length<=32);for(const id of x.signature.value)integer(id,1,100000);}else integer(x.signature.value,1,100000);
 assert(Array.isArray(x.types)&&x.types.length>0&&x.types.length<=32);for(const types of x.types){assert(Array.isArray(types)&&types.length>0&&types.length<=3);for(const t of types)integer(t,0,32);}
 assert(Array.isArray(x.rows));assert.deepEqual(x.rows.map(r=>r[0]),c.registry.map(r=>r[0]));for(const row of x.rows){assert(Array.isArray(row)&&row.length===6);integer(row[1],0,32);integer(row[2],0,1023);integer(row[3],0,64);assert(Array.isArray(row[4])&&row[4].length<=8);for(const stat of row[4])integer(stat,0,7);integer(row[5],0,8);}
 assert(Buffer.byteLength(JSON.stringify(m.closure))<=1800);
...
validate(data);
const mutations=[d=>d.direct_queued_encounter.move_inputs.closure[0].rows.pop(),d=>d.direct_queued_encounter.move_inputs.closure[0].chance=101,d=>d.direct_queued_encounter.move_inputs.closure[0].types=[],d=>d.direct_queued_encounter.move_inputs.closure[0].rows[0][2]=1024,d=>d.direct_queued_encounter.move_inputs.battle_rng_calls=1,d=>d.direct_queued_encounter.move_inputs.calls[0].registry.pop(),d=>d.direct_queued_encounter.move_inputs.calls[0].species+=1,d=>d.direct_queued_encounter.move_inputs.calls[0].moves=[],d=>d.direct_queued_encounter.move_inputs.calls[0].trainer=true,d=>d.direct_queued_encounter.pool.rows.pop(),d=>d.direct_queued_encounter.pool.rows[0][4]=!d.direct_queued_encounter.pool.rows[0][4],d=>d.direct_queued_encounter.pool.rows[0][1]=0,d=>d.direct_queued_encounter.pool.difficulty="unknown",d=>d.direct_queued_encounter.unwrapped=[],d=>d.direct_queued_encounter.unwrapped[0].frames=["C:/secret.ts:1:1"],d=>d.binder.party[0].tm.max_moves=0,d=>d.binder.party[0].tm.omniform="false",d=>d.binder.party[0].tm.available.push(0),d=>d.binder.rare_candy_friendship=-1,d=>d.direct_queued_encounter.before="!rnd,wrong",d=>d.direct_queued_encounter.constructed[0].level+=1,d=>d.direct_queued_encounter.restored_same_standby=false,d=>d.direct_queued_encounter.constructed=[],d=>d.direct_queued_encounter.turn=2,d=>d.binder.party[0].evolutions.rows[0].item=-1,d=>d.binder.party[0].evolutions.rows[0].item="0",d=>d.binder.party[0].level_cap=11,d=>d.binder.party[0].move_closure.pop(),d=>d.option_count=2,d=>d.free_picks=2,d=>d.catalog[0][1]=99,d=>d.catalog[0][6]=-1,
