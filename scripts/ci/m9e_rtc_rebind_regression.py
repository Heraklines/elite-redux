...
SOURCES = list(dict.fromkeys([*SOURCES, *SPECS]))
logs = {}
failed_log = None
...
def main(summary):
    ...
    changes = run(["git", "diff", "--name-only", "6b2cc1736c1f836a047980256f4a84306e024b9a", "HEAD"], "source-delta", 30, 16384).read_text().splitlines()
    if sorted(changes) != sorted(["scripts/ci/m9e_rtc_rebind_regression.py", ".github/workflows/m9e-rtc-rebind-regression-focused.yml"]):
        raise RuntimeError("only two focused regression CI sources may change")
    summary["base_sha"] = "6b2cc1736c1f836a047980256f4a84306e024b9a"
...
def execute_prepared(summary, *, install_chromium=True):
    ...
    run(["pnpm", "exec", "playwright", "test", "--config", "playwright.rust-browser.config.ts", "--project=chromium",
         *SPECS, "--workers=1", "--reporter=line,json"], "browser", 1100)
