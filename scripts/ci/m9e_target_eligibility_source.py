...
BRANCH = "codex/m9e-target-eligibility-source-20260909"
ROOT = Path(__file__).resolve().parents[2]
OUT = Path(os.environ["RUNNER_TEMP"]) / "m9e-target-eligibility-source"
FILES = {"apply-ab-attrs.ts": "eb44ec1ce4ce9c3afa743c19112fa007e9d0f6ec"}
PREFIX = "src/data/abilities/"
...
def main():
    ...
    try:
        ...
        for name, oid in FILES.items():
            ...
            request=urllib.request.Request(f"https://api.github.com/repos/Heraklines/elite-redux/git/blobs/{oid}",
                headers={"Accept":"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28",
                         "User-Agent":"m9e-target-eligibility-source","Authorization":"Bearer "+os.environ["GITHUB_TOKEN"]})
            try:
            ...
            methods=[]
            for wanted in ("getEnemyPassiveSlotLimit",):
                hits=[i for i,line in enumerate(lines) if re.match(r"^export function "+wanted+r"\b",line)]
        ...
        require(found=={"getEnemyPassiveSlotLimit"},"actual enemy passive eligibility function found")
        ...
        receipt["source_hashes"]={path:hashlib.sha256((ROOT/path).read_bytes()).hexdigest() for path in (
            "scripts/ci/m9e_target_eligibility_source.py",".github/workflows/m9e-target-eligibility-source.yml")}
