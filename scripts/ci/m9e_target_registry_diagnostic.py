...
EXPORTER_SHA256 = "64deb0e68e8450e0b500dbf873e1b8c0dc5c6401000b80a82e2850be594a2346"
...
TAIL_SOURCE_PINS = {
    "src/data/elite-redux/er-run-pacing.ts": ["c63162a9d3618b7a2605dc0e90ca9493747b2b98522660162dc0cdac4078edf6", 5091],
...
def main(summary):
    ...
    require(os.environ.get("GITHUB_REF_NAME") == "codex/m9e-victory-tail-source-20260912" and os.environ.get("GITHUB_EVENT_NAME") == "push", "exact source branch/event")
    ...
    for ordinal in ("one", "two"):
        ...
        run(["pnpm", "exec", "vitest", "run", INJECTED, "--pool=forks", "--isolate", "--no-file-parallelism",
            ...
            cwd=ORACLE, extra={"M9_TARGET_REGISTRY_OUTPUT": str(destination),
                ...
                "M9_VICTORY_TAIL_OUTPUT": str(OUTPUT / f"tail-{ordinal}.json")})
        summary["fresh_process_exports"].append({"ordinal": ordinal, "report": validate_vitest(report),
            ...
            "tail": file_fact(OUTPUT / f"tail-{ordinal}.json", 16384)})
    ...
    run(["node", str(ROOT / VERIFIER), str(OUTPUT / "export-one.json"), str(OUTPUT / "export-two.json"),
         ...
         str(OUTPUT / "tail-one.json"), str(OUTPUT / "tail-two.json")],
    ...
    summary["victory_tail_observation"] = json.loads((OUTPUT / "tail-one.json").read_bytes())
    ...
    require(set(summary["generated"]) == {"export-one.json", "export-two.json", "sidecar-one.json", "sidecar-two.json", "tail-one.json", "tail-two.json", "source-methods.txt", "validation.json"}, "exact output inventory")
...
def bound_partial_outputs():
    ...
    for path in sorted(OUTPUT.iterdir()):
        ...
        if path.name not in {"export-one.json", "export-two.json", "sidecar-one.json", "sidecar-two.json", "tail-one.json", "tail-two.json", "source-methods.txt", "validation.json"} or path.stat().st_size > (16384 if path.name.startswith("tail-") else 32768):
