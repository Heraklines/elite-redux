...
EXPORTER_SHA256 = "53e1bed7a825db5a151b3fc5f934c976183980a3dd67aa6e6a72426ae800962a"
...
def main(summary):
    ...
    require(names[:13] == ["species", "moves", "forms", "abilities-meta", "gender", "form-flags", "ability-slots", "form-types", "form-stats", "level-two-forms", "level-two-meta", "level-two-movegen", "movegen-stage"] and len(set(names)) == len(names)
            and all(re.fullmatch(r"abilities-(rows|functions|shapes)-[0-9]{2}", name) for name in names[13:]), "exact semantic page names")
    exports = {}
    for name, length, digest in parts:
        cap = 4096 if name in {"gender", "form-flags", "movegen-stage"} else 8192 if name in {"ability-slots", "form-types", "level-two-forms", "level-two-meta", "level-two-movegen"} else 32768 if name in {"species", "moves"} else 16384
...
def generated_cap(name):
    if name in {"form-counts.json", "digest-two.json", "gender-one.json", "form-flags-one.json", "movegen-stage-one.json"}:
        return 4096
    if name in {"ability-slots-one.json", "form-types-one.json", "level-two-forms-one.json", "level-two-meta-one.json", "level-two-movegen-one.json"}:
        return 8192
