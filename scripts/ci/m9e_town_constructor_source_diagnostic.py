...
EXPORTER_SHA256 = "18eb49be967baf194f3fff74277c87e75336d848d7a4a88b6f1878c38ed26eb2"
...
def main(summary):
    ...
    require(names[:14] == ["species", "moves", "forms", "abilities-meta", "gender", "form-flags", "ability-slots", "form-types", "form-stats", "level-two-forms", "level-two-meta", "level-two-movegen", "level-two-abilities", "movegen-stage"] and len(set(names)) == len(names)
            and all(re.fullmatch(r"abilities-(rows|functions|shapes)-[0-9]{2}", name) for name in names[14:]), "exact semantic page names")
    exports = {}
    for name, length, digest in parts:
        cap = 4096 if name in {"gender", "form-flags", "movegen-stage"} else 8192 if name in {"ability-slots", "form-types", "level-two-forms", "level-two-meta", "level-two-movegen", "level-two-abilities"} else 32768 if name in {"species", "moves"} else 16384
