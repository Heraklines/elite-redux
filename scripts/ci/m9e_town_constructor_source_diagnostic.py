...
EXPORTER_SHA256 = "179655c741d9be7c584587db2702db6168614f2feb590075badc09873a5b8f88"
...
def main(summary):
    ...
    require(names[:12] == ["species", "moves", "forms", "abilities-meta", "gender", "form-flags", "ability-slots", "form-types", "form-stats", "level-two-forms", "level-two-meta", "movegen-stage"] and len(set(names)) == len(names)
            and all(re.fullmatch(r"abilities-(rows|functions|shapes)-[0-9]{2}", name) for name in names[12:]), "exact semantic page names")
    exports = {}
    for name, length, digest in parts:
        cap = 4096 if name in {"gender", "form-flags", "movegen-stage"} else 8192 if name in {"ability-slots", "form-types", "level-two-forms", "level-two-meta"} else 32768 if name in {"species", "moves"} else 16384
...
def generated_cap(name):
    if name in {"form-counts.json", "digest-two.json", "gender-one.json", "form-flags-one.json", "movegen-stage-one.json"}:
        return 4096
    if name in {"ability-slots-one.json", "form-types-one.json", "level-two-forms-one.json", "level-two-meta-one.json"}:
        return 8192
