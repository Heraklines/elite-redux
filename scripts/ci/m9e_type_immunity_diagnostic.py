"""Remote-only qualification of ordinary type immunity in actual damage queries and turn resolution."""
import difflib
import hashlib
import json
import os
...
TARGET = RUNNER / "m9e-type-immunity-target"
...
def main():
    ...
    result = {"status": "failed", "qualification": "six whole damage-query tests: immunity, actual turn RNG, neutral/resisted damage and Struggle; not complete M9 qualification",
    ...
    try:
        ...
        selector = ["-p", "er-game", "--test", "m9e_damage_query"]
        run("proposal-clippy", ["cargo", "clippy", "--locked", "-p", "er-game", "-p", "er-battle", "--tests", "--no-deps", "--", "-D", "warnings"], ROOT / "rust")
        ...
        require(result["tests_executed"] == 6, "all six whole damage query tests required")
        result["tests"] = {"passed": 6, "failed": 0, "ignored": 0, "filtered": 0}
        ...
        require(TARGET.resolve().parent == RUNNER and TARGET.name == "m9e-type-immunity-target",
