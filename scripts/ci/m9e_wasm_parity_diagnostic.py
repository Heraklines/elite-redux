...
def main() -> None:
    ...
    try:
        ...
        if subprocess.check_output(["git", "status", "--porcelain"], cwd=ROOT).strip():
            raise RuntimeError("checked-out source has uncommitted changes")
        summary["source_bindings"] = {
            "workflow_sha256": hashlib.sha256((ROOT / ".github/workflows/m9e-wasm-parity-focused.yml").read_bytes()).hexdigest(),
            "harness_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            "lockfile_sha256": hashlib.sha256((RUST / "Cargo.lock").read_bytes()).hexdigest(),
            "toolchain_file_sha256": hashlib.sha256((RUST / "rust-toolchain.toml").read_bytes()).hexdigest(),
            "native_target": "x86_64-unknown-linux-gnu",
            "wasm_target": "wasm32-unknown-unknown",
            "profile": "test-default-debug0",
        }
        compiler = subprocess.check_output(["rustc", "-Vv"], cwd=RUST, text=True)
        if "release: 1.97.1\n" not in compiler or "host: x86_64-unknown-linux-gnu\n" not in compiler:
            raise RuntimeError("unexpected native compiler identity")
        summary["source_bindings"]["rustc_release"] = "1.97.1"
        if os.environ.get("CARGO_INCREMENTAL") != "0" or os.environ.get("CARGO_PROFILE_TEST_DEBUG") != "0":
            raise RuntimeError("unexpected test profile")
        ...
        if subprocess.check_output(["wasm-bindgen", "--version"], cwd=RUST, text=True).strip() != "wasm-bindgen 0.2.127":
            raise RuntimeError("unexpected Wasm binding tool identity")
        summary["source_bindings"]["wasm_bindgen_version"] = "0.2.127"
