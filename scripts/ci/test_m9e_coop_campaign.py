...
import m9e_coop_campaign as replay


class CooperativeCampaignEvidenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.evidence = {"status": "passed", "source_sha": "a" * 40, "run_id": "123", "run_attempt": "1",
                         ...
                         "natural_cooperative_campaign": {"decisions": 2188, "proposals": 258, "materials": 1675, "presentations": 3352, "rewards": 199, "progression": 950}}
        self.proof = {"identity": self.identity, "lane": "e", "natural_cooperative_campaign": self.evidence,
                      "plan": {"requires_natural_coop_campaign": True,
    ...
    def test_complete_evidence_is_conserved(self):
        ...
        other["lane"] = "b"
        del other["natural_cooperative_campaign"]
    ...
    def test_missing_extra_and_wrong_lane_proofs_are_rejected(self):
        for mutate in (lambda proof: proof.pop("natural_cooperative_campaign"),
                       lambda proof: proof.update(lane="b"),
                       lambda proof: proof["plan"].update(requires_natural_coop_campaign=False),
                       lambda proof: proof["plan"].update(requires_natural_coop_campaign="true"),
        ...
        with self.assertRaisesRegex(RuntimeError, "ownership"):
            replay.validate_lane(self.proof, self.root, lambda inventory: {"e": []})
    ...
    def test_incomplete_or_over_budget_execution_is_rejected(self):
        for key, value in (("bytes", 16385), ("bytes", True), ("sha256", "bad"), ("elapsed_seconds", 601), ("elapsed_seconds", 0)):
            ...
            with self.assertRaisesRegex(RuntimeError, "log"):
                replay.validate_evidence(evidence, self.identity, self.root)
        for key, value in (("decisions", 0), ("proposals", 199), ("materials", 399), ("presentations", 399), ("rewards", 198), ("progression", 198), ("decisions", True)):
            evidence = copy.deepcopy(self.evidence)
            evidence["natural_cooperative_campaign"][key] = value
            with self.assertRaisesRegex(RuntimeError, "coverage"):
                replay.validate_evidence(evidence, self.identity, self.root)

    def test_override_rejects_non_owner_before_launch(self):
        for lane, phase, ids in (("b", "native", replay.IDS), ("e", "platform", replay.IDS), ("e", "native", [])):
            with mock.patch.dict("os.environ", {"M9E_NATIVE_LANE": lane, "M9E_PHASE": phase}):
                with mock.patch("m9e_current_cost.run_bounded") as launch:
                    with self.assertRaisesRegex(RuntimeError, "native E"):
