...
// Network tracking for pokemon sprite atlases — answers the key question when
// stuck: did the HTTP request complete (=> Phaser processing bug) or hang
// (=> loader/network not flushing)?
const netPending = new Map(); // url -> startMs
const netDone = new Map(); // url -> {status, ms}
const netFailed = new Map();
const isSprite = u => /\/images\/pokemon\/.*\.(png|json)(\?|$)/.test(u) || /pkmn/.test(u);
p.on("request", r => {
  if (isSprite(r.url())) {
...
p.on("requestfinished", r => {
  if (netPending.has(r.url())) {
    const st = netPending.get(r.url());
    netPending.delete(r.url());
    netDone.set(r.url(), { ms: Date.now() - st });
  }
});
p.on("requestfailed", r => {
  if (netPending.has(r.url())) {
    netPending.delete(r.url());
    netFailed.set(r.url(), r.failure()?.errorText ?? "?");
  }
});
const netSnapshot = () => ({
  pending: [...netPending.keys()].map(u => u.split("/").slice(-2).join("/")),
  pendingCount: netPending.size,
  doneCount: netDone.size,
  failedCount: netFailed.size,
  failedSample: [...netFailed.entries()].slice(-3).map(([u, e]) => `${u.split("/").slice(-2).join("/")}:${e}`),
});
const tap = async k => {
...
try {
  ...
  for (let round = 0; round < 16; round++) {
    ...
    // The user's strongest repro is cycling DOWN through rows (each Down jumps
    // 9 species, so the preview must load a brand-new sprite every press — far
    // more load churn than L/R within a row). Alternate heavy vertical runs with
    // horizontal + diagonal motion to mimic real frantic browsing.
    for (let i = 0; i < len; i++) {
      const dir =
        round % 3 === 0
          ? i % 2
            ? "ArrowDown"
            : "ArrowUp" // pure vertical storm
          : round % 3 === 1
            ? ["ArrowDown", "ArrowRight", "ArrowDown", "ArrowLeft"][i % 4] // diagonal-ish
            : i % 2
              ? "ArrowLeft"
    ...
    if (quick.ms < 0) {
      ...
      if (long.ms < 0) {
        ...
        bad.push("NET " + JSON.stringify(netSnapshot()));
      } else {
        furiousFail++;
