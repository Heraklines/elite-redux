...
function fail(message) {
  console.error(`M8 oracle refresh overlay: ${message}`);
  process.exitCode = 1;
...
function parseArgs(argv) {
  if (argv.length !== 4 || argv[0] !== "--oracle-root" || argv[2] !== "--oracle-sha") {
    fail(
      "usage: node scripts/prepare-kernel-m8-oracle-refresh.mjs --oracle-root <absolute-directory> --oracle-sha <40-hex>",
    );
  }
...
console.log(`M8 oracle refresh overlay: added ${OVERLAY_PATHS.length} test-only paths at ${oracleSha}`);
