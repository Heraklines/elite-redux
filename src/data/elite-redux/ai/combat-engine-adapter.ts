...
import { PokemonFormChangeItemModifier, PokemonHeldItemModifier } from "#modifiers/modifier";
...
function snapshotItems(mon: Pokemon, knowledge: ErCombatMonObservation["knowledge"]): ErCombatItemObservation[] | null {
  ...
  if (knowledge === "battle-info") {
    ...
    if (!mon.waveData.heldItemKnowledgeComplete && revealed.size === 0) {
      return null;
    }
    return items.filter(item => mon.waveData.heldItemKnowledgeComplete || revealed.has(item.itemId));
  }
  return items;
}
...
function snapshotMon(
  ...
  const effectiveStats = selfKnowledge
    ...
    : null;
  const formChangeAvailable =
    scene.findModifier(
      modifier => modifier instanceof PokemonFormChangeItemModifier && modifier.pokemonId === mon.id,
      mon.isPlayer(),
    ) != null;
  ...
  const moves = selfKnowledge ? safe(() => mon.getMoveset().map(snapshotMove), []) : snapshotPublicOpponentMoves(mon);
  return {
    entityId: mon.id,
    knowledge,
    partyIndex,
    ...
    heldItems,
    revealState: {
      abilities: selfKnowledge ? "complete" : abilities.length > 0 ? "partial" : "unknown",
      items: selfKnowledge
        ? "complete"
        : mon.waveData.heldItemKnowledgeComplete
          ? "complete"
    ...
    transformation: {
      ...
      teraAvailable: selfKnowledge
        ...
        : null,
      formChangeAvailable: selfKnowledge ? formChangeAvailable : null,
      formChanged: currentSpecies.speciesId !== mon.species.speciesId || mon.summonData.speciesForm != null,
    },
    boss: {
...
function candidateDerivedFeatures(
  ...
  const previews =
    !tera && move.category !== MoveCategory.STATUS
      ? targetMons.map(target => getErDamagePreview(actor, target, move))
      : [];
  ...
  const chartMultiplier = baseTypeMultiplier(scene, moveId, targets, perspective);
  return {
    ...
    expectedHits: previews.length > 0 ? previewHitCount(previews[0].hits) : null,
    immunityReason:
      chartMultiplier === 0
        ? "type-chart"
        : expectedDamageMax === 0 && move.category !== MoveCategory.STATUS
          ? "engine-preview-zero"
          : null,
    ...
    createsMoveLock: actor.getHeldItems().some(item => item.type.id.includes("CHOICE")),
  };
}
  ...
  for (const { move, moveSlot } of moveRows) {
    ...
    for (const targets of sets) {
      const shared = {
        ...
        targetMode,
        targets,
        baseTypeMultiplier: baseTypeMultiplier(scene, move.moveId, targets, perspective),
