...
export const ER_COMBAT_FEATURE_SCHEMA_VERSION = 4;
...
function monSemanticTokens(mon: ErCombatMonObservation | undefined, observation?: ErCombatObservation): string[] {
  ...
  const tokens = [
    ...
    ...(mon.transformation.formTransition == null
      ? []
      : [
          `form-transition:${mon.transformation.formTransition.fromSpecies}:${mon.transformation.formTransition.fromForm}`
            + `>${mon.transformation.formTransition.toSpecies}:${mon.transformation.formTransition.toForm}`,
        ]),
