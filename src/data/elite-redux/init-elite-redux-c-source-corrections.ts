...
// `scripts/elite-redux/parse-er-battle-moves.py`) and OVERWRITES the move
...
// `python scripts/elite-redux/parse-er-battle-moves.py` and bundled here as
...
const C_SOURCE_OVERRIDES: ReadonlyMap<string, {
  power?: number;
  accuracy?: number;
  pp?: number;
  chance?: number;
  ...
  // then run scripts/elite-redux/generate-c-source-overrides.py
  ["MOVE_POUND", { pp: 35 }],
...
export function initEliteReduxCSourceCorrections(): CSourceCorrectionResult {
  ...
  for (const [moveConst, override] of C_SOURCE_OVERRIDES) {
    ...
    if (override.power !== undefined) target.power = override.power;
    if (override.accuracy !== undefined) target.accuracy = override.accuracy;
