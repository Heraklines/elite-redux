...
function buildCustomSpecies(draft: ErSpeciesDraft, speciesId: number): PokemonSpecies {
  ...
  return new ErCustomSpecies(
    ...
    type1,
    type2,
    1.0, // height (m) — TODO: extract from ROM dex.hw[0] (available; weight pass did hw[1] only)
    // weight (kg). Resolved 2026-07-22 weight audit: the ROM (gSpeciesInfo `dex.hw`) DOES
    // carry per-species weight — it lives in the upstream ER dump but the ErSpeciesDraft
    // builder dropped it. ER_CUSTOM_SPECIES_WEIGHTS (generated by
    // build-custom-species-weights.mjs) holds the extracted kg for all ~827 dump customs:
    // 802 ROM-authoritative, 25 canon/sprite estimates for ER-original fakemon whose ROM
    // hw is [0,0]. 30.0 remains only as a defensive fallback for an id missing from the table.
    ER_CUSTOM_SPECIES_WEIGHTS[speciesId] ?? 30.0,
