...
export const DEV_SCENARIOS: DevScenario[] = [
  ...
  // ===========================================================================
  {
    ...
    description:
      ...
      + "opens listing the new move(s). The panel looks exactly like normal, but an\n"
      + "evolution STRIP sits on its TOP EDGE (Eevee + the partner eeveelutions, an (F)\n"
      + "button badge, a gold underline on the selected one). Learn an offered move onto\n"
      + "Eevee. Then press F (keyboard) / LB (controller) / the on-screen cycle button\n"
      + "(mobile) to select Vaporeon (or another evolution) and learn the SAME move onto it.\n"
      + "EXPECT: the strip cycles the selected evolution (underline moves); the CURRENT\n"
      + "column switches to THAT evolution's own moveset; a move already learned on an\n"
      + "evolution shows dimmed for it but stays learnable on the others; you can teach one\n"
      + "offered move onto Eevee AND, independently, onto each evolution (never twice on one).\n"
      + "\n"
      + "TM CASE / LEARNER'S SHROOM:\n"
      + "DO: after the battle, in the first shop, take the Learner's Shroom or TM Case and\n"
      + "use it on Eevee, then pick a move. The SAME batch panel opens with that one move\n"
      + "offered (not a separate menu).\n"
      + "EXPECT: cycle the strip to an evolution (e.g. Flareon) and learn the move into THAT\n"
      + "evolution's stored moveset (a full moveset lets you pick which move to overwrite); a\n"
      + "move an evolution cannot learn shows dimmed for it. Base Eevee learns onto its own\n"
      + "moveset. The per-evolution movesets persist across the run and a save/reload.",
    setup: () => {
      resetDevOverrides();
      setOverrides({
  ...
  },
  {
    ...
    description:
      ...
      + "materialization, and win backend empty-slot CAS. The conflicting local and cloud bytes of the\n"
      + "QUARANTINED slot must remain unchanged during discovery.\n"
      + "\n"
      + "FOLLOW-UP (2026-07-17 maintainer directive): with ALL FIVE slots occupied, the fresh launch now\n"
      + "reclaims a slot AUTOMATICALLY like solo new-game: unreadable/conflicted co-op remnants first,\n"
      + "then the save with the OLDEST timestamp, via the full cloud-safe delete, then the normal\n"
      + "verified-empty claim + backend CAS. EXPECT: the new run starts with no manual deletion; the\n"
      + "console logs 'reclaiming least-recent save slot=N'. Only when reclamation itself cannot be\n"
      + "verified (e.g. cloud unavailable) do both clients still fail closed, now with the ACTIONABLE\n"
      + "message ('free a save slot, then reconnect') and one per-slot verdict line\n"
      + "('fresh co-op slot scan found NO verified slot [0:local-occupied ...]').\n"
