...
export interface CurrentRtcIdentityV2 {
  source_sha: string;
...
export interface CurrentRtcTransportOptionsV2 {
  channel: RTCDataChannel;
  identity: CurrentRtcIdentityV2;
  negotiatedMaximumMessageBytes(): number | undefined;
  connected(): Promise<void>;
  receive(generation: number, bytes: Uint8Array): Promise<void>;
...
/** Immutable-generation, manually bound development RTC transport. Create a new instance for each new physical connection; generation must come from the kernel owner. Successful send
 * means local RTC buffering only. It never acknowledges remote kernel acceptance.
 */
export class CurrentRtcTransportV2 {
  readonly #options: CurrentRtcTransportOptionsV2;
  readonly #identity: CurrentRtcIdentityV2;
  ...
  constructor(options: CurrentRtcTransportOptionsV2) {
    ...
    try {
      ...
      if (!Number.isSafeInteger(this.#deadlineMs) || this.#deadlineMs < 1 || this.#deadlineMs > 30_000
        ...
        || channel.label !== "er-current-development-rebind-v2" || channel.protocol !== "er-current-rebind-v2") {
  ...
  send(generation: number, input: Uint8Array): Promise<void> {
    if (this.#closed || generation !== this.#identity.generation || input.byteLength === 0
      ...
      return Promise.reject(new Error(`current RTC frame cannot enter bounded generation-bound queue (${input.byteLength} bytes)`));
    }
  ...
  readonly #onOpen = (): void => {
    ...
    try {
      ...
      const bytes = encodeCanonicalJsonV2({ kind: "CURRENT_RTC_HELLO", schema_version: 2,
      ...
      while (!this.#closed && this.#receiveQueue.length > 0) {
        ...
        try {
          if (!this.#helloReceived) {
            ...
            this.#received++;
            await this.#options.receive(this.#identity.generation, bytes);
            this.#delivered++;
...
function validateIdentity(identity: CurrentRtcIdentityV2): void {
  if (Object.keys(identity).sort().join(",") !== "authority_seat,content_sha256,generation,local_seat,membership_revision,peer_seat,run_id,seat_map_id,session_epoch,session_id,source_sha,worker_sha256"
    ...
    || !/^[0-9a-f]{64}$/u.test(identity.worker_sha256) || (!safeCurrentInteger(identity.generation) || identity.generation === 0)
    ...
    throw new Error("current RTC identity is not a bounded two-seat generation-bound binding");
  }
}
function assertPeerHello(bytes: Uint8Array, identity: CurrentRtcIdentityV2): void {
  if (bytes.byteLength > MAXIMUM_HELLO_BYTES) throw new Error("current RTC peer handshake exceeds bound");
  const value: unknown = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes));
  const expected = { kind: "CURRENT_RTC_HELLO", schema_version: 2, browser_worker_protocol: 2,
