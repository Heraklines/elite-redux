...
import {
  ...
  type RustPhaserStorageHandlerV1,
  startRustPhaserHostV1,
} from "../routes/rust-phaser-entry";
...
export function startRustProductionViewV1(
  host: BrowserKernelGenerationV1,
  storage: RustPhaserStorageHandlerV1,
): Promise<RustProductionViewV1> {
  return new Promise((resolve, reject) => {
    ...
    const game = new Phaser.Game({
      ...
      scene: {
        create() {
          startRustPhaserHostV1(host, this, storage)
            .then(value => {
              route = value;
              resolve({
                route: value,
