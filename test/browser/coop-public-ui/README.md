...
| `half-wipe` | Pair, visibly confirm a one-mon Memento team on the replica and one-mon Tackle team on the authority, submit ordinary commands, prove the replica's empty replacement closes, then prove the next turn has exactly one owner and the omitted replica receipts the complete retained presentation before control retires | Dedicated build+URL-gated fresh accounts; the workflow always assigns the wiped fixture to the non-requesting replica |
...
| `evolution-sync` | Pair, New Run, publicly confirm a visible point-legal team, clear wave 1, prove the authority's real EvolutionPhase prompt and the partner's retained CoopWaveProgressionReplayPhase prompt are independently actionable and advanced, compare the immutable evolution ledger, then reach wave 2 command UI | Dedicated build+URL-gated initial-save fixture starts both visible teams at level 6 and primes one Caterpie one EXP below its natural level-7 evolution without pausing evolution; no runtime healing or post-launch mutation |
