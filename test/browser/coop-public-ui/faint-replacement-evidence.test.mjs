...
test("the two-browser half-wipe journey proves replica passive presentation before one-owner continuation", async () => {
...
test("faint replacement waits for the owned actionable party surface before pressing", async () => {
  ...
  const [stream, replayTurn] = await Promise.all([
    readFile(new URL("../../../src/data/elite-redux/coop/coop-battle-stream.ts", import.meta.url), "utf8"),
...
test("the replacement picker drive gates its battle-prompt advancer to picker-open (dirty lane)", async () => {
  ...
  // require one - no shared command address exists during a staggered/double faint, so an address-required
  // advancer would remain inert. This one drives only local faint-replay narration by live address.
  assert.match(
    drive,
