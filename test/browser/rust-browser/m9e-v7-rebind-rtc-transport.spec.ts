...
test("current V7 generation-bound RTC transport reconnects and replays natural Worker gameplay", async ({ page }, testInfo) => {
  const observed: string[] = [];
  page.on("worker", worker => observed.push(worker.url()));
  await page.goto(address);
  const evidence = await page.evaluate(async ({ entry, transportEntry, transportBinding, assets, initializations }) => {
    const module = await import(entry);
    const transportModule = await import(transportEntry);
    ...
    const connect = async (generation: number) => {
      ...
      const outgoing = left.createDataChannel("er-current-development-rebind-v2", { ordered: true, protocol: "er-current-rebind-v2" });
      ...
      const deliveries: Array<((bytes: number[]) => void) | null> = [null, null];
      const connected = [0, 0];
      const disconnected = [0, 0];
      const transports = channels.map((channel, index) => {
        const context = peers[index].context;
        const frame = context.protocol.frame_context.context;
        return new transportModule.CurrentRtcTransportV2({ channel,
          identity: { ...transportBinding, content_sha256: assets.content_sha256,
            session_id: frame.sessionId, run_id: frame.runId, session_epoch: frame.sessionEpoch,
            seat_map_id: frame.seatMapId, membership_revision: frame.membershipRevision,
            authority_seat: frame.authoritySeatId, local_seat: context.local_seat,
            peer_seat: context.protocol.connections[0].peer_seat, generation },
          negotiatedMaximumMessageBytes: () => [left, right][index].sctp?.maxMessageSize,
          connected: async () => {
            connected[index]++;
            if (generation === 2) await peers[index].ordinary({ kind: "TRANSPORT_CHANGED", generation, connected: true });
          },
          receive: async (actualGeneration: number, bytes: Uint8Array) => {
            assert(actualGeneration === generation && deliveries[index] != null, "bound transport receive generation and pending delivery required");
            record.received[index]++;
            const deliver = deliveries[index]!; deliveries[index] = null; deliver(Array.from(bytes));
          },
          disconnected: async () => {
            disconnected[index]++;
            await peers[index].ordinary({ kind: "TRANSPORT_CHANGED", generation, connected: false });
          },
        });
      });
      await deadline(Promise.all(transports.map(transport => transport.ready())));
      equal(connected, [1, 1], "both physical transport handshakes must complete");
      let wrongGenerationRejected = false;
      try { await transports[0].send(generation + 1, Uint8Array.of(1)); }
      catch { wrongGenerationRejected = true; }
      assert(wrongGenerationRejected && transports[0].status.sentFrames === 0 && !transports[0].status.closed,
        "wrong-generation outbound packet must reject before send without closing valid transport");
      const transfer = async (sender: number, bytes: number[]) => {
        ...
        assert(deliveries[1 - sender] == null, "one bounded pending delivery per endpoint");
        const delivery = new Promise<number[]>(resolve => { deliveries[1 - sender] = resolve; });
        await transports[sender].send(generation, Uint8Array.from(bytes)); record.sent[sender]++;
      ...
      const close = async () => {
        ...
        await Promise.all(transports.map(transport => transport.disposed()));
        equal(disconnected, [1, 1], "both actual transport disconnect callbacks required");
        const endpoints = transports.map(transport => transport.status);
        for (let index = 0; index < 2; index++) {
          const status = endpoints[index];
          assert(status.closed && !status.connected && status.sendPending === 0 && status.receivePending === 0
            && status.sendBytes === 0 && status.receiveBytes === 0
            && status.sentFrames === record.sent[index] && status.receivedFrames === record.received[index]
            && status.kernelDeliveredFrames === record.received[index], "actual transport must drain exactly delivered frames");
        }
        transportEvidence.push({ generation, connected, disconnected, wrong_generation_rejected: wrongGenerationRejected, endpoints });
    ...
    try {
      ...
      await second.close();
      const final = await Promise.all(peers.map(peer => snapshot(peer.client)));
      ...
      return { physical_connections: connections.length, physical_carriers: carriers, transport_evidence: transportEvidence,
        actual_workers: clients.length, disposed_workers: disposed, generation: 2, transcript_controls: 8,
  ...
  }, { entry: `${address}/m9e-assets/${manifest.entry}`, transportEntry: `${address}/m9e-assets/${transportManifest.entry}`,
    transportBinding: { source_sha: manifest.source_sha, worker_sha256: manifest.assets[manifest.worker].sha256 }, assets: assets(),
  ...
  expect(evidence.physical_connections).toBe(4);
  expect(evidence.transport_evidence).toHaveLength(2);
  ...
  await testInfo.attach("m9e-current-browser-transport-rebind", { body: report, contentType: "application/json" });
});
