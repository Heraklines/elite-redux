...
test("current V7 Workers replay owned generation two rebind and continue natural gameplay", async ({ page }, testInfo) => {
  ...
  const evidence = await page.evaluate(async ({ entry, assets, initializations }) => {
    ...
    try {
      ...
      for (const peer of peers) {
        ...
        peer.generation = 2;
        await peer.ordinary({ kind: "TRANSPORT_CHANGED", generation: 2, connected: true });
      }
      ...
      for (let index = 0; index < 8; index++) {
        ...
        const output = await peer.rebind({ kind: "RECEIVE", generation: 2, bytes: wire });
      ...
      await guest.ordinary({ kind: "NETWORK_FRAME", generation: 2, bytes: material });
      await host.settle(); await guest.settle();
      const proposal = await nextFrame(guest);
      const receipt = one(await host.ordinary({ kind: "NETWORK_FRAME", generation: 2, bytes: proposal }));
      ...
      equal(one(await host.ordinary({ kind: "NETWORK_FRAME", generation: 2, bytes: proposal })), receipt,
      ...
      await guest.ordinary({ kind: "NETWORK_FRAME", generation: 2, bytes: receipt });
  ...
  await testInfo.attach("m9e-current-browser-rebind", { body: report, contentType: "application/json" });
});
