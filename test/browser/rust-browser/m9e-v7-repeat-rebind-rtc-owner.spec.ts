...
test("current V7 RTC owner automatically rebinds twice with natural gameplay across three connections", async ({ page }, testInfo) => {
  ...
  const evidence = await page.evaluate(async ({ entry, workerEntry, source, workerHash, assets, initializations }) => {
    ...
    const receipts = (item: any, direction: string, generation = 2) => item.record.frames.filter((frame: any) => {
      if (frame.direction !== direction || frame.generation !== generation) return false;
    ...
    try {
      ...
      for (const item of pair) {
        const result = await item.peer.beginRebind();
        assert(result.response.kind === "REBIND" && result.response.output.generation === 3
          && result.response.output.frames.length === 0 && item.peer.status.generation === 3,
          "same shipping RTC owner must begin the second rebind");
      }
      await connect(pair);
      await pair[0].peer.retryRebind();
      await wait(async () => (await Promise.all(pair.map(item => snapshot(item.peer))))
        .every(state => state.current_coop_setup.rebind.phase === "OPEN"
          && state.current_coop_setup.rebind.to_generation === 3), "automatic second eight-control handshake");
      const repeatedControlCounts = pair.map(item => sent(item).filter((frame: any) =>
        frame.generation === 3 && decoded(frame).kind === "CURRENT_COOP_REBIND").length);
      equal(repeatedControlCounts, [4, 4], "shipping owner automatically routes the complete second handshake");
      await nextFrame(pair[0]);
      await wait(async () => pair[1].record.frames.some(frame => frame.direction === "received" && frame.generation === 3
        && decoded(frame).kind !== "CURRENT_COOP_REBIND"), "third-generation automatically routed material");
      const thirdProposal = await nextFrame(pair[1]);
      await wait(async () => receipts(pair[0], "sent", 3).length === 1 && receipts(pair[1], "received", 3).length === 1
        && pair.every(item => item.peer.status.pending === 0), "automatic third-generation receipt");
      const thirdReceipt = receipts(pair[0], "sent", 3)[0];
      const thirdValue = decoded(thirdReceipt);
      assert(thirdValue.authority_context.connectionGeneration === 3, "new receipt generation must be three");
      equal(thirdValue.proposal_hex, thirdProposal.bytes.map((byte: number) => byte.toString(16).padStart(2, "0")).join(""),
        "new automatic receipt must bind the actual new proposal");
      const thirdHostAfter = await snapshot(pair[0].peer);
      assert(thirdHostAfter.current_coop_setup.retired_reply_rebind == null,
        "new automatically routed proposal must replace previous receipt proof");
      await pair[1].peer.sendFrame(3, Uint8Array.from(thirdProposal.bytes));
      await wait(async () => receipts(pair[0], "sent", 3).length === 2 && receipts(pair[1], "received", 3).length === 2
        && pair.every(item => item.peer.status.pending === 0), "automatic third-generation duplicate receipt");
      equal(receipts(pair[0], "sent", 3)[1].bytes, thirdReceipt.bytes, "new duplicate returns exact current receipt");
      equal(await snapshot(pair[0].peer), thirdHostAfter, "duplicate new proposal changed shipping owner state");
      await Promise.all(pair.map(item => item.peer.closeTransport()));
      ...
      for (let index = 0; index < 2; index++) {
        ...
        assert(capsule.base_position === 0 && capsule.attempts.filter((attempt: any) =>
          attempt.event.kind === "COOP_REBIND" && attempt.event.control.kind === "BEGIN").length === 2,
          "shipping owner capsule must retain both automatic handshakes from the reached checkpoint");
      ...
      return { actual_workers: owners.length + replayers.length, disposed_workers: 6, generation: 3,
        automatic_rebind_controls: controlCounts, repeated_rebind_controls: repeatedControlCounts, successful_signaling_pairs: 3,
        ...
        third_proposal_sha256: await hash(Uint8Array.from(thirdProposal.bytes)), third_receipt_sha256: await hash(Uint8Array.from(thirdReceipt.bytes)),
        repeated_rebind: true, capsule_sha256: replayHashes };
    } finally {
  ...
  expect(evidence.connected_callbacks).toEqual([2, 2]); expect(evidence.disconnected_callbacks).toEqual([2, 2]);
  expect(evidence.repeated_rebind_controls).toEqual([4, 4]);
  ...
  await testInfo.attach("m9e-current-rtc-owner-repeat-rebind", { body: report, contentType: "application/json" });
});
