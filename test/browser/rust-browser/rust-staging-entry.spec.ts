...
import { startPrivateRustStagingRoute } from "/src/rust-browser/routes/rust-staging-entry.ts";
const manifest = await (await fetch("/m8-assets/m8-web-assets.json")).json(); const release = await (await fetch("/m8-assets/browser-release-manifest.json")).json();
...
new Phaser.Game({ type: Phaser.CANVAS, parent: "game", width: 640, height: 360, banner: false, scene: { create() {
  ...
  startPrivateRustStagingRoute({ manifest: release, location: { pathname: release.private_route }, authorize: async () => true, workerUrl: worker, executionIdentityBytes: identity, sessionStartBytes: snapshot, scene: this }).then(session => {
    ...
    plugins: [
      {
        ...
        configureServer(devServer) {
          devServer.middlewares.use((request, response, next) => {
            ...
            if (request.url === "/private/rust-m8") {
...
test("private staging route runs only an authorized Rust Phaser release", async ({ page }, testInfo) => {
  const address = server.resolvedUrls?.local[0];
  if (address == null) {
    throw new Error("Vite did not publish Rust Phaser route");
  }
  await page.goto(new URL("private/rust-m8", address).href);
