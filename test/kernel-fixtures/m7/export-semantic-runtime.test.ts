...
function callbackHash(value: (...args: never[]) => unknown): string {
...
function reflectNumber(value: number): Json {
  if (Number.isNaN(value)) {
    return { kind: "NONFINITE_NUMBER", value: "NAN" };
  }
  if (value === Number.POSITIVE_INFINITY) {
    return { kind: "NONFINITE_NUMBER", value: "POSITIVE_INFINITY" };
  }
  if (value === Number.NEGATIVE_INFINITY) {
    return { kind: "NONFINITE_NUMBER", value: "NEGATIVE_INFINITY" };
  }
  return Object.is(value, -0) ? 0 : value;
}

function reflectObject(value: object, seen: WeakSet<object>, depth: number): Json {
  if (seen.has(value)) {
    return { kind: "CIRCULAR_REFERENCE", class: value.constructor?.name ?? "Object" };
  }
  if (depth >= 6) {
    return { kind: "DEPTH_LIMIT", class: value.constructor?.name ?? "Object" };
  }
  seen.add(value);
  if (Array.isArray(value)) {
    return value.map(entry => reflect(entry, seen, depth + 1));
  }
  ...
  if (value instanceof Set) {
    return [...value]
      .map(entry => reflect(entry, seen, depth + 1))
  ...
  for (const key of Object.keys(value as RecordValue).sort()) {
    ...
    if (entry !== undefined) {
      output[key] = reflect(entry, seen, depth + 1);
    }
  }
...
function reflect(value: unknown, seen: WeakSet<object>, depth = 0): Json {
  ...
  if (typeof value === "number") {
    return reflectNumber(value);
  }
  if (typeof value === "function") {
    return { kind: "CALLBACK_PROVENANCE", hash: callbackHash(value), name: value.name || "anonymous" };
  }
  if (typeof value !== "object") {
    return { kind: "UNREPRESENTABLE", type: typeof value };
  }
  return reflectObject(value, seen, depth);
}
...
function outputPath(): string {
  const value = process.env.M7_SEMANTIC_RUNTIME_OUTPUT;
  if (!value || !isAbsolute(value)) {
    throw new Error("M7_SEMANTIC_RUNTIME_OUTPUT must be absolute");
  }
...
function capture(): Json {
  ...
  Phaser.Math.RND.sow(["m7-semantic-runtime"]);
  if (allSpecies.length === 0 || allMoves.length === 0 || allAbilities.length === 0) {
    initializeGame();
  }
  ...
  const abilities = [...allAbilities]
    ...
    .map(entry => ({ id: Number((entry as unknown as RecordValue).id), definition: descriptor(entry) }))
    .sort((left, right) => left.id - right.id);
  const modifiers = Object.keys(modifierTypes)
    .sort()
    .map(key => {
      const builder = (modifierTypes as unknown as Record<string, unknown>)[key];
      if (typeof builder !== "function") {
        return { key, builder: descriptor(builder), instance: null };
      }
      try {
...
describe("M7 runtime semantic catalog", () => {
  it("captures final post-initialization definitions", () => {
    if (process.env.M7_ORACLE_SHA !== ORACLE_SHA) {
      throw new Error("M7 oracle SHA mismatch");
    }
