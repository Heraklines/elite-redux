...
function packSpecies(part:ReturnType<typeof extractTownStatic>["species"]){
 ...
 const {rows:unusedRows,form_data:unusedForms,level_sets:unusedLevels,...metadata}=part;return {...metadata,schema:3,rows_columns:packColumns(rows),form_columns:packColumns(part.form_data),level_columns:packColumns(levels),level_rows:levelRows};
}
test("observe complete initialized Town constructor closure",async()=>{
 ...
 for(const [name,cap] of [["species",32768],["moves",32768],["abilities",16384]] as const){const payload=name==="species"?packSpecies(captured!.species):captured![name];const raw=Buffer.from(JSON.stringify(payload)+"\n");const sizes=Object.entries(payload).map(([key,value])=>[key,Buffer.byteLength(JSON.stringify(value)),Array.isArray(value)?value.length:null]);const rowColumns=name==="species"?Array.from({length:10},(_,i)=>[i,Buffer.byteLength(JSON.stringify(captured!.species.rows.map((row:unknown)=>Array.isArray(row)?row[i]:null)))]):[];const sizing=JSON.stringify({part:name,bytes:raw.length,fields:sizes,row_columns:rowColumns});expect(Buffer.byteLength(sizing)).toBeLessThanOrEqual(2048);expect(raw.length,`Complete payload byte bound: ${sizing}`).toBeLessThanOrEqual(cap);writeFileSync(join(output!,`${name}-${ordinal}.json`),raw,{flag:"wx"});}
});
