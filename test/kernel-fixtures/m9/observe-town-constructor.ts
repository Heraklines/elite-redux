...
function extractTownStatic(tiers:readonly (readonly [number,readonly number[]])[]){
 ...
 const moveIds=new Set<number>(),abilityIds=new Set<number>();const speciesRows:unknown[]=[],genderRows:Array<[number,number|null]>=[],formFlags:Array<[number,number]>=[],abilitySlots:Array<[number,number[][]]>=[],formTypes:Array<[number,Array<[number,number|null,number[]]>]>=[];
 ...
 for(let i=0;i<queue.length;i++){
  ...
  const formActives:number[][]=[];const formTypeRows:Array<[number,number|null,number[]]>=[];const formRows=forms.map((form,index)=>{
  ...
  abilitySlots.push([id,formActives]);formTypes.push([id,formTypeRows]);speciesRows.push([id,evoOwn,se.encode(evolutions),preOwn,se.encode(preValue),se.encode(preLevels),formRows,se.encode(FORCED_SIGNATURE_MOVES[id]),Object.hasOwn(FORCED_SIGNATURE_MOVES,id),incoming]);
 }
...
test("observe complete initialized Town constructor closure",async()=>{
 ...
 const packed=packSpecies(captured!.species);const outputs:Array<{name:string,value:unknown,cap:number}>=[{name:"species",value:packed.species,cap:32768},{name:"moves",value:captured!.moves,cap:32768},{name:"forms",value:packed.forms,cap:16384},{name:"abilities-meta",value:{schema:2,source:PIN,runtime_slots:captured!.abilities.runtime_slots},cap:16384},{name:"gender",value:captured!.gender,cap:4096},{name:"form-flags",value:captured!.form_flags,cap:4096},{name:"ability-slots",value:captured!.ability_slots,cap:8192},{name:"form-types",value:captured!.form_types,cap:8192}];
