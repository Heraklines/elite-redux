...
function extractTownStatic(tiers:readonly (readonly [number,readonly number[]])[]){
 ...
 const moveIds=new Set<number>(),abilityIds=new Set<number>();const speciesRows:unknown[]=[],genderRows:Array<[number,number|null]>=[],formFlags:Array<[number,number]>=[];
