...
import type { BrowserRequestV2, CurrentPresentationSceneV1Wire } from "../../../../src/rust-browser/contracts/browser-contracts-v2";
import { CurrentSceneDomV1 } from "../../../../src/rust-browser/routes/current-scene-dom-v1";

const scene: CurrentPresentationSceneV1Wire = {
  schema_version: 1,
  control: {
    schema_version: 2, revision: 7, kind: "BATTLE_COMMAND", owner_seat: 1, action_context: {}, actionable: true,
    menu: { control_id: "battle/command", selected_option_id: "battle/command/fight", options: [
  ...
  actors: [
    { slot: { side: "PLAYER", position: 0 }, pokemon: 1, species: 1, form: 0, owner_seat: 1,
      status: "NONE", hp: { kind: "PLAYER_EXACT", hp: 20, max_hp: 30 } },
    { slot: { side: "ENEMY", position: 0 }, pokemon: 2, species: 915, form: 0, owner_seat: null,
      status: "POISON", hp: { kind: "ENEMY_BAR", ten_thousandths: 6250 } },
  ],
};
...
describe("CurrentSceneDomV1", () => {
  it("renders the player-safe scene and sends only the owning seat's raw keys", async () => {
    ...
    const onError = vi.fn();
    const view = new CurrentSceneDomV1(root, 1, async request => { sent.push(request); }, onError);
    view.render(scene);
    ...
    expect(sent).toEqual([
      { kind: "RAW_INPUT", event: { kind: "KEY_DOWN", data: { code: { kind: "SPACE" }, printable: false,
        browser_repeat: false, focus: "GAME" } } },
