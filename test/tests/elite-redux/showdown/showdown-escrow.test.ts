...
function freshMatch(): ShowdownMatchRecord {
  const res = registerMatch("m1", 10, 20, nonShiny(8), nonShiny(8), 1000);
  if (!res.ok) {
...
describe("registerMatch", () => {
  ...
  it("rejects mismatched-tier stakes", () => {
    const res = registerMatch("m1", 10, 20, nonShiny(8), shiny(0), 1000);
    expect(res.ok).toBe(false);
  });
  it("rejects same-uid host/guest", () => {
    const res = registerMatch("m1", 10, 10, nonShiny(8), nonShiny(8), 1000);
    expect(res.ok).toBe(false);
  });
  it("rejects malformed stakes", () => {
    const res = registerMatch("m1", 10, 20, {} as StakeRecord, nonShiny(8), 1000);
    expect(res.ok).toBe(false);
  });
});
...
describe("roleOf", () => {
  it("maps uids to roles", () => {
    const m = freshMatch();
    expect(roleOf(m, 10)).toBe("host");
    expect(roleOf(m, 20)).toBe("guest");
    expect(roleOf(m, 999)).toBeNull();
  });
});
...
describe("applyResultReport — dual attestation", () => {
  it("two agreeing reports settle with that winner", () => {
    ...
    const a = applyResultReport(m, 10, "host", "victory", 2000, SILENCE);
    expect(a.resolution).toBe("pending");
    m = a.match;
    const b = applyResultReport(m, 20, "host", "victory", 2001, SILENCE);
  ...
  it("conflicting reports void the match (holds released)", () => {
    let m = recordBattlePhaseEntered(freshMatch());
    m = applyResultReport(m, 10, "host", "victory", 2000, SILENCE).match;
    const b = applyResultReport(m, 20, "guest", "victory", 2001, SILENCE);
  ...
  it("rejects a non-participant report (unchanged, pending)", () => {
    ...
    const r = applyResultReport(m, 777, "host", "victory", 2000, SILENCE);
...
describe("applyResultReport — lone report rules", () => {
  it("does NOT settle a lone report before the silence timer elapses", () => {
    const m = recordBattlePhaseEntered(freshMatch());
    const r = applyResultReport(m, 10, "host", "timeout", 2000, SILENCE);
    expect(r.resolution).toBe("pending");
  });
  ...
  it("settles a lone report once battle-entered AND silence elapsed (re-report)", () => {
    ...
    m = applyResultReport(m, 10, "host", "timeout", 2000, SILENCE).match;
    // Survivor re-reports after silence (t = 2000 + 60_000): the report `at` stays 2000.
    const r = applyResultReport(m, 10, "host", "timeout", 2000 + SILENCE, SILENCE);
  ...
  it("never settles a lone report when the battle was NOT entered (pre-battle abandon)", () => {
    ...
    m = applyResultReport(m, 10, "host", "timeout", 2000, SILENCE).match;
    const r = applyResultReport(m, 10, "host", "timeout", 2000 + SILENCE * 10, SILENCE);
    expect(r.resolution).toBe("pending");
  ...
  it("a re-report cannot flip the canonical winner", () => {
    ...
    m = applyResultReport(m, 10, "guest", "victory", 2500, SILENCE).match;
...
describe("settlement idempotency + void", () => {
  it("a settled match is idempotent under further reports", () => {
    let m = recordBattlePhaseEntered(freshMatch());
    m = applyResultReport(m, 10, "guest", "victory", 2000, SILENCE).match;
    m = applyResultReport(m, 20, "guest", "victory", 2001, SILENCE).match;
    expect(m.state).toBe("settled");
    const again = applyResultReport(m, 10, "host", "victory", 3000, SILENCE);
...
describe("resolveSettlement — mutation records", () => {
  it("emits loser removeUnlock + winner grantUnlock of the loser's stake", () => {
    ...
    const reg = registerMatch("m2", 10, 20, nonShiny(8), nonShiny(8), 1000);
    if (!reg.ok) {
  ...
  it("transfers a shiny stake with its variant bits", () => {
    const reg = registerMatch("m3", 10, 20, shiny(2), shiny(2), 1000);
    if (!reg.ok) {
      throw new Error(reg.error);
    }
    let m = recordBattlePhaseEntered(reg.match);
    m = applyResultReport(m, 10, "host", "victory", 2000, SILENCE).match;
    m = applyResultReport(m, 20, "host", "victory", 2001, SILENCE).match;
    const muts = resolveSettlement(m);
